...
> **Sonuç:** iki faz — [Faz 110](../110-TUKETICI-BAGLANTI-DUZLEMI.md) ·
...
| Soru | Cevap |
|---|---|
| AgentPrism içeride EF Core kullansın mı? | **Hayır.** L16 · L29 geçerli, üstüne iki yeni kanıt eklendi |
...
| Kanıt | Sonuç |
|---|---|
| `grep -l "AotCompatible>false" src/*/*.csproj` çıktısında `AgentPrism.PostgreSql` **yok** | Paket AOT uyumlu. EF Core AOT uyumlu değildir; bir EF store paketi bayrak sağlayıcının duruşunu bozar |
| [`SqlStoreContext.cs:24`](../../src/AgentPrism.Sql.Shared/Internal/SqlStoreContext.cs) | Store'lar zaten yalnız `DbDataSource` · `DbConnection` · `DbTransaction` görüyor. EF'in getireceği soyutlama ADO.NET seviyesinde **mevcut** |

`AgentPrism.EntityFrameworkCore` paketi **önerilmedi**. 33 store arayüzü ve
[`AgentPrism.Testing.Contracts.Xunit`](../../src/AgentPrism.Testing.Contracts.Xunit/README.md)
...
| P5 | `agentprism migrate` ile `dotnet ef database update` sıralaması belgesiz | [CLI README](../../src/AgentPrism.Cli/README.md) tek başına duruyor | 110 |
| P6 | Gömme örneğinde veritabanı ve EF yok | `ls samples/AgentPrism.Embedded/` | 110 |

## P2'nin neden yetenek kalemi olduğu

`AgentPrismPostgreSqlOptions` beş alan taşır: `ConnectionString`, `SchemaName`,
...
- Süresi dolan token'ı yenileyen parola sağlayıcı geri çağrımı — Entra ID / IAM
  kimliğine geçmiş bir kurum `AgentPrism.PostgreSql`'i kural dışına çıkmadan
  kullanamaz
...
| EF global query filter'ın AgentPrism satırlarına işlemesi | İki veri düzlemi; kiracı izolasyonu AgentPrism'in kendi `tenant_id` sütunuyla |
| `dotnet ef migrations` ile AgentPrism şeması | L29. İki ayrı adım kalır, yalnız sırası belgelenir |
