...
| Kod | A2A agent card açıkça streaming ve push notification desteğinin olmadığını beyan ediyor (`src/AgentPrism.AspNetCore/A2A/AgentPrismA2AExtensions.cs:158`). `WorkflowNodeRetryPolicy` yalnız `AddWorkflowFunction` yüzeyine bağlı (`src/AgentPrism.Workflows/AgentPrismWorkflowFunctionExtensions.cs:88-123`). |
| Kod | `WebhookDeliveryStatus.Failed` ve teslimat geçmişi var; yönetim yüzeyinde test gönderme ve geçmiş listeleme var, başarısız teslimatı yeniden sürme ucu yok (`src/AgentPrism.Abstractions/Webhooks/WebhookTypes.cs:11-23`, `src/AgentPrism.AspNetCore/Endpoints/WebhookEndpoints.cs:74-96`). |
| Kod | `ModelDescriptor` streaming, tool, reasoning ve structured output yeteneklerini taşıyor; compiler ölçümle yalnız structured output kontrol ediyor (`src/AgentPrism.Abstractions/Models/ModelDescriptor.cs:4-28`, `src/AgentPrism.Core/Compilation/AgentDefinitionCompiler.cs:763-789`). |
| Kod | RAG sonucu `SourceId`, `ChunkIndex`, içerik ve distance taşıyor. Agent yanıtında source lineage/citation sözleşmesi yok (`src/AgentPrism.Abstractions/Knowledge/VectorSearchHit.cs:4-19`, `src/AgentPrism.Core/Knowledge/VectorSearchToolFactory.cs:68-70`). |
| Kod | Tool approval evaluator kararı `bool` olarak döndürüyor. Eşleşen policy/rule nedenini simüle eden bir yönetim ucu görünmüyor (`src/AgentPrism.Core/Approvals/ToolApprovalRuleEvaluator.cs:63-115`). |
...
|---|---|---|---|---|
| 1 | MCP Tasks ile uzun süren tool/agent çağrılarının poll, result ve cancel yaşam döngüsü | AgentPrism'i MCP server olarak tüketen ekip | MCP Tasks 2025-11-25'te tanımlandı; AgentPrism'de queued run ve approval altyapısı zaten var | ✅ |
| 2 | MCP elicitation → AgentPrism approval/input mailbox köprüsü | Harici MCP tool'ları insan girdisi isteyen ekipler | MCP server artık kullanıcıdan form veya URL tabanlı ek bilgi isteyebiliyor | ✅ |
...
| 4 | A2A için SQL-backed task store ve push notification/SSE desteği | AgentPrism agent'ını başka agent'lara açan ekipler | Mevcut Agent Card push ve streaming'i unsupported ilan ediyor; A2A hosting dokümanı in-memory task store'u production için uygun görmüyor | ✅ |
...
| 10 | GenAI OpenTelemetry semantic convention conformance modu | OTel/Langfuse/Jaeger/Datadog kullanan işletim ekipleri | MEAI artık chat, embedding, image, speech ve realtime için GenAI semantic-convention client'ları sunuyor; AgentPrism'in özel span sözleşmesiyle uyum katmanı gerekiyor | ✅ |
...
|---|---|---|---|
| [MCP Tasks specification](https://modelcontextprotocol.io/specification/2025-11-25/basic/utilities/tasks) | 2026-08-23 | `tools/call` için task augmentation, `tasks/get`, `tasks/result`, `tasks/cancel`, TTL ve optional status notification tanımlanıyor. Özellik experimental. | AgentPrism'in queued run/approval state'i MCP task state'e bağlanabilir; task ID tenant/auth context'e sıkı bağlanmalı. |
...
| [MAF Agent Skills](https://learn.microsoft.com/en-us/agent-framework/agents/skills) | 2026-08-23 | MCP server `skill://index.json` ile skill keşfi ve içeriklerin on-demand alınmasını destekliyor. API experimental; archive script'leri çalıştırılmıyor. | AgentPrism'in kendi skill-script güvenlik sınırını gevşetmeden read-only remote skill context desteği üretilebilir. |
| [MAF A2A hosting](https://learn.microsoft.com/en-us/agent-framework/hosting/agent-to-agent) | 2026-08-23 | A2A hosting'de `ITaskStore` ve session store replace edilebilir; varsayılan in-memory store production için uygun değil. | AgentPrism SQL run/job store'u durable A2A task store'a bağlamak doğal entegrasyondur. |
| [MAF durable extension](https://learn.microsoft.com/en-us/agent-framework/integrations/durable-extension) | 2026-08-23 | Durable agents/workflows için checkpoint, resume, human-in-the-loop ve distributed worker yetenekleri sunuluyor. | Workflow-wide retry/compensation ve A2A task adaylarında MAF'ın kapalı-kutu sınırları yeniden ölçülmelidir; AgentPrism MAF tiplerini paralel sarmalamamalı. |
| [Microsoft.Extensions.AI namespace](https://learn.microsoft.com/en-us/DOTNET/api/microsoft.extensions.ai?view=netstandard-2.0-pp) | 2026-08-23 | OpenTelemetry semantic-convention decorators chat, embedding, image, speech ve realtime client'ları için mevcut. | AgentPrism'in OTel yüzeyi özel span'larla standardı birlikte taşıyacak bir conformance seçeneği kazanabilir. |
...
ve task lifecycle kodu için `rg -n "task|tasks" src/AgentPrism.AspNetCore/McpServer
src/AgentPrism.Mcp` yeni bir AgentPrism task yüzeyi göstermedi. Ekosistem kanıtı
...
**Karşı görüş:** MCP Tasks experimental olduğu için AgentPrism bugün değişken bir
...
**Kanıt seviyesi:** Ölçüldü — AgentPrism'de MCP prompt/resource client'ları var;
...
**Karşı görüş:** AgentPrism'in mevcut code-defined skill modelini ikinci bir
skill kaynağıyla bölmek discovery, versioning ve tenant isolation maliyeti yaratır.
**Sonuç:** Kullanıcı seçimine bırakıldı; F numarası verilmedi.

### 4. A2A durable task store + push/SSE

**Kanıt seviyesi:** Ölçüldü — `AgentPrismA2AExtensions.cs:158-159` streaming ve
...
**Kanıt seviyesi:** Ölçüldü — AgentPrism kendi `AgentPrism` ActivitySource'unu
...
ölçülmelidir.
**Karşı görüş:** Standardın kendisi experimental olduğu için AgentPrism public
