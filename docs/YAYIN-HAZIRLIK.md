...
| 6 | ~~**Kod düzeltmesi (doküman değil)**~~ ✅ **KAPANDI — [Faz 122](122-KAYIT-API-SI-VE-SESSIZ-BOSLUKLAR.md)** | BL-018 tam, BL-033 tam, BL-039 kısmen (yalnız duplicate-name), BL-051 tam | Decorator hatası artık `AgentPrismAgentSourceException`'a normalize ediliyor (sınıf taraması: 4 çağrı yeri); duplicate workflow adı `AgentPrismException` veriyor; guard/retention kayıtsızken `SilentGapWarningService` Production'da bir kez uyarıyor |
