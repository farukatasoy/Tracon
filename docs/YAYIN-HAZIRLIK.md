...
| OP-003 | Package ID sahipliği/uygunluğu | **Ölçüldü — 20/20 müsait** | 2026-08-27, salt-okunur `registration5-semver1` GET × 20: hepsi `404`. npm `@tracon/client` de `404`. **`tracon` npm scope'u YOK** (`/-/org/tracon` → `404`) | Kimlik çakışması yok. `Tracon.*` ID prefix reservation başvurusu ayrı bir kalem — karar verilmedi |
...
| OP-005 | **Tamamlandı (2026-08-28, KN-022)** | Trusted Publishing hesaba açıktı; kişisel owner `farukatasoy`, repo `farukatasoy/Tracon`, workflow `ci.yml`, environment `nuget`, pattern `Tracon*`, push-only policy oluşturuldu | CI publish job `id-token: write` + `NuGet/login@v1` kullanır; private repo policy ilk başarılı publish tamamlanana kadar yedi günlük geçici aktivasyondadır |
