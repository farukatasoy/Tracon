...
# Tracon — Uygulanabilirlik ve Geçiş Raporu

> **Tarih:** 2026-08-21
> **Soru:** ProdigyEnabler'ın AI altyapısı Tracon'e devredilebilir mi? Nerede oturur, nerede oturmaz, ne eksik?
> **İncelenen sürüm:** Tracon `0.0.0-preview.0.291` (13 paket, `ProdigyEnabler.Application`'a referanslı)
> **Kanıt tabanı:** Paketlerin XML dokümanları (~6.100 public member), `Tracon.AgentMap.md` yetenek haritası, `tracon.json` (OpenAPI 3.1.1, 123 path), nuspec bağımlılıkları · `docs/AI-MIMARISI.md` · `claudedocs/ai-altyapi-elestiri-2026-07-27.md` (B1–B28) · `claudedocs/ai-altyapi-hedef-mimari-2026-07-27.md` (Faz 0–7) · `claudedocs/ms-agent-framework-karsilastirma-2026-08-01.md` · `docs/AI-OPS-PANELI.md` · `docs/KARARLAR.md` · `ProdigyEnabler/src/` kodu
...
2. [Tracon ne yapar — mimari konumlandırma](#2-tracon-ne-yapar--mimari-konumlandırma)
...
**Tez:** Tracon, `ai-altyapi-hedef-mimari-2026-07-27.md` dokümanının yazdığı platform katmanının büyük bölümünü hazır getirir. Devir teknik olarak mümkündür ve yüksek getirilidir. Ancak devir **"AI altyapısını teslim etmek" değil, "AI altyapısının bir yarısını teslim etmek"** olarak planlanmalıdır.

2026-08-01 tarihli MS Agent Framework karşılaştırması sistemi ikiye ayırmıştı:

| Yarı | O günkü karar | Tracon'le bugünkü durum |
|---|---|---|
| **Agent yarısı** — tool şeması, tool-call formatı, onay, iterasyon kontrolü, gözlemlenebilirlik | "Tekrar ettik, framework'te hazır" | **Devredilebilir.** Tracon bunların hepsini kapatır. |
| **Orkestrasyon yarısı** — transaction sınırları, ABP tenant/yetki, Hangfire + 3 katmanlı kurtarma, SignalR kontratı, ConvAI | "Haklı özel geliştirme, framework'te karşılığı yok" | **Kısmen değişti.** Tracon tenant, kota, kuyruk, koşu mutabakatı getirir — ama ABP'ye bağlanmaz, dayanıklı fan-in yapmaz, SignalR bilmez. |

Kaba ölçüm:

- **28 mimari bulgunun (B1–B28) 20'si tam kapanır**, 6'sı kısmen kapanır, 1'i değişmez, 1'i (B22 — kuyruk çokluğu) **kötüleşir**.
- **7 fazlık hedef mimarinin 5'i büyük ölçüde Tracon'e devredilir.** Faz 5 (streaming protokolü) ve Faz 6'nın bir kısmı bizde kalır.
- **AI Ops Console'un 13 modülünün 9'u** Tracon'in gömülü konsoluyla karşılanır. 76 sayfalık gereksinim ciddi biçimde daralır.
...
**En büyük üç engel:** (1) ABP/EF Core ile Tracon iki ayrı veri düzlemidir, ortak transaction yoktur; (2) agent döngüsü süreç-içidir, bizim job-tabanlı dayanıklı döngümüzün karşılığı yoktur; (3) prompt'a çalışma-anı parametre enjeksiyonu (Scriban) Tracon'de yoktur.
...
## 2. Tracon Ne Yapar — Mimari Konumlandırma

Tracon, Microsoft Agent Framework (MAF) üstüne oturan bir **kontrol düzlemidir**. MAF tiplerini (`AIAgent`, `AgentSession`, `ChatMessage`, `AIFunction`) sarmalamaz, doğrudan kullanır. Kendi katkısı MAF'ın bilinçli olarak boş bıraktığı yerdir: kalıcılık, kimlik, para, denetim, operasyon.
...
graph TB
    ...
    subgraph ap["Tracon — kontrol düzlemi"]
        ...
        HTTP["MapTracon()<br/>123 path · SSE · OpenAI uyumlu"]
    end
...
| **Sürpriz yok.** `AddTracon()` tek başına hiçbir guard, worker, kota veya sağlayıcı açmaz. | Kademeli devir mümkündür; her yetenek tek tek açılır. |
...
|---|---|---|
| `Tracon.Core` + `.Abstractions` | Çalışma zamanı + sözleşmeler (4.014 member) | Zorunlu |
| `Tracon.PostgreSql` | Kalıcılık — ham Npgsql, kendi şeması, kendi migration'ları | Zorunlu (kalıcı kayıt isteniyorsa) |
| `Tracon.OpenAI` / `.Anthropic` / `.Google` | Sağlayıcı adaptörleri | 4 provider'ımızın 4'ü de karşılanır |
| `Tracon.AspNetCore` | 123 path'lik yönetim API'si, SSE, OpenAI uyumluluk | Konsol/panel için |
| `Tracon.UI` | Gömülü operatör konsolu + `embed.js` | AI Ops Console kapsamını daraltır |
| `Tracon.Workflows` | Çok-agent'lı workflow motoru (süreç-içi) | Sınırlı fayda — bkz. §7.6 |
| `Tracon.Voice` | ElevenLabs istemcisi + gerçek zamanlı sesli konuşma | ConvAI ile **rakip** mimari — bkz. §6.2 |
| `Tracon.Mcp` | MCP istemcisi | Bugün ihtiyaç yok |
| `Tracon.Testing` | `FakeModelProvider`, `TraconTestHost`, `RunAssertions` | Test yazımında doğrudan değer |
...
Sistemde dört AI altyapısı var. Tracon'in her birine değme derecesi farklıdır.
...
| Bizdeki kavram | Tracon karşılığı | Eşleme kalitesi |
|---|---|---|
...
| `BackendToolHandlerBase<TIn,TOut>` | `AIFunction` + `IToolRegistry` + `[TraconTool]` | ✅ Şema gücü artar (enum/dizi/nested) |
...
| `PipelineErrorClassifier` | Devre kesici + tipli `TraconProviderUnavailableException` | ✅ Metin eşleşmesi biter |
| `PipelineWatchdogScheduler` | `ClaimOrphanedRunsAsync` + `Tracon:RunReconciliation` | 🟡 Koşu seviyesinde var, adım seviyesinde yok |
...
`ai-altyapi-elestiri-2026-07-27.md` 28 bulgu listeler. Aşağıdaki tablo her birinin Tracon'deki karşılığını verir. Kanıt sütunu paketin kendi dokümanına dayanır.

| # | Bulgu | Şiddet | Tracon karşılığı | Durum |
|---|---|---|---|---|
...
| B7 | Pipeline token kaydı yok | 🔴 | Her çalıştırma bir `runs` satırı — pipeline çağrıları Tracon'e taşınırsa otomatik | ✅ Tam (koşullu) |
| B8 | Sıfır OpenTelemetry | 🟠 | `ActivitySource` + `Meter`; koşu/tool/model span'leri; `tracon.run` kök span'i | ✅ Tam |
...
| B11 | Tool kaydı tek assembly'ye kilitli | 🟠 | `TraconToolRegistration` DI kaydı; `AddToolsFrom`, MCP, client tool, skill | ✅ Tam |
...
| B17 | Provider hata string'i orkestrasyonda | 🟠 | Tipli `TraconProviderUnavailableException` + devre kesici + sağlayıcı sağlığı | ✅ Tam |
...
| B21 | Tool çıktısı sınırsız bağlamda | 🟡 | `CompactionSettings` + `Tracon:UtilityModel` (ucuz özetleyici model) | ✅ Tam |
| B22 | İki farklı kuyruk API'si | 🟡 | **Üçüncü kuyruk gelir** (Tracon job queue + scheduling) | ❌ Kötüleşir |
...
Kritik gözlem: **kırmızı (🔴) 7 bulgunun 6'sı tam kapanır.** Kalan biri (B4) yalnızca yarım kapanır ve o yarım bizim işimizdir — çünkü yetkilendirme ABP izin sistemine bağlıdır, Tracon onu bilemez.

---

## 5. Hedef Mimari Fazlarının Karşılığı

`ai-altyapi-hedef-mimari-2026-07-27.md` 7 faz tanımlar (~7 hafta iş). Tracon bu planın ne kadarını gereksiz kılar?

| Faz | Planlanan iş | Tracon'de | Kalan iş |
|---|---|---|---|
...
| **Faz 1** — Gateway + görünürlük | `IAiGateway`, `AiCallLog`, `AiPrice`, OTel, çağrı gezgini | **Tamamı hazır** — `runs`, `run_events`, `RunCost`, `ActivitySource`, konsol | Yalnızca çağrı yollarını Tracon'e bağlamak |
...
| **Faz 7** — Kalite + sertleştirme | Prompt versiyonlama, eval, egress, saklama, ConvAI sertleştirme | Sürüm + eval + `Tracon:Egress` + retention + veri sahibi silme hazır | Türkçe guard'lar, ConvAI kararı |
...
Bu, KARARLAR.md'nin kanıt eşiği açısından da önemlidir: hedef mimari zaten kabul edilmiş bir plandı. Tracon o planı **iptal etmiyor, uyguluyor**.
...
| Konu | Neden sorun |
|---|---|
| **SignalR `IChatClient` (14 metod)** | Tracon SSE konuşur. FE'nin 14 metotluk sözleşmesi bizde kalır; `RunEvent` → hub köprüsü yazılmalıdır. |
| **`ChatSession` / `ChatMessage` domain entity'leri** | Tracon oturum state'ini **opak** tutar (MAF `SerializeSession` çıktısı). Bizim "son 6 mesaja bak", "bayat tool tespiti", `RetrySessionAsync`, `ResumeSessionAsync` sorgularımız bu opak state üzerinde çalışamaz. Entity'ler kalmalı veya bu özellikler yeniden tasarlanmalıdır. |
...
| **`ContentRegen` önizleme akışı** | `PreviewVersionAdded` + `integrityToken` + `InMemoryContentPreviewStore` tamamen domain'e özgüdür. Tracon'de karşılığı yoktur ve olmamalıdır — tool gövdesinde kalır. |
...
| FE istemcisi | ElevenLabs SDK | Tracon WebSocket protokolü (`Sec-WebSocket-Protocol` ile token) |
...
> Not: Değerlendirme (evaluation) tarafı ayrıdır ve ses mimarisinden bağımsız devredilebilir. `ConvAIEvaluationJob`'ın her kriter için yaptığı AI çağrısı bugün bile Tracon agent'ına taşınabilir — `EvaluationCriterion.AgentConfigurationId` zaten agent başına ayrışmış durumdadır.

### 6.3 Content Generation Pipeline — orta uyum, sınırlı devir

**Devredilebilen:** Beş adımın AI çağrıları (`QAGeneration`, `PodcastScript`, `Assessment`). Her çağrı bir Tracon koşusu olur. Kazanç: B7 kapanır, pipeline harcaması görünür olur, structured output ile `EvaluationResponseJson` alias karmaşası biter, replay ile bir adım tekrar oynatılabilir.

**Devredilemeyen — ve devredilmemeli:**

| Parça | Neden kalmalı |
|---|---|
| Hangfire orkestrasyonu | Tracon job queue'su **kendi iş türlerini** çalıştırır (`JobKind` = `AgentRun`, `AgentBatch`, `Workflow`, `Eval`, `OnlineEval`, `ApprovalResume`, `Retention`, `WebhookDelivery`). Genel amaçlı iş kuyruğu değildir; `ImageGenerationJob` gibi AI-dışı bir adımı barındıramaz. |
| 3 katmanlı kurtarma | Tracon `ClaimOrphanedRunsAsync` ile **koşu** seviyesinde mutabakat yapar. Bizim öksüz **adım** tespiti, watchdog taraması ve orchestrator kurtarması bundan daha geniştir. |
| `ContentPipelineCompletionJob` fan-in | Tracon'de dağıtık fan-in yoktur. `Workflows` süreç-içi çalışır. |
...
| `ImageGenerationJob` | Tracon'de görsel üretme yeteneği **yok** (§10.6). |

**Verdikt:** "AI çağrısı katmanı devredilir, orkestrasyon kalır." Bu, MS Agent Framework karşılaştırmasının 4.3'teki sonucuyla birebir aynıdır ve Tracon o sonucu değiştirmemiştir.
...
1. İki `AgentConfiguration` kaydını Tracon `AgentDefinition`'ına taşı.
...
`Tracon.PostgreSql` EF Core kullanmaz. Ham `Npgsql` üzerinden çalışır, kendi `NpgsqlDataSource`'unu singleton olarak tutar, kendi migration runner'ıyla kendi şemasını kurar. Dokümanı açıkça yazıyor: *"Tüketicinin public şeması hiçbir koşulda ellenmez."*

```mermaid
graph TB
    subgraph app["Uygulama süreci"]
        UOW["ABP UnitOfWork<br/>EF Core · DbContext"]
        APDS["Tracon NpgsqlDataSource<br/>(singleton, ayrı havuz)"]
    end

    subgraph db["PostgreSQL"]
        PUB[("public şeması<br/>ChatSession · Pipeline · ConvAI")]
        APS[("tracon şeması<br/>runs · run_events · quotas · jobs")]
    end
...
| **ABP soft delete / tenant filtresi geçmez** | Tracon satırlarına ABP global query filter'ları uygulanmaz. Kiracı izolasyonu Tracon'in kendi `tenant_id` sütunuyla sağlanır. |
| **Migration ikili olur** | `db-migration` skill'i yalnızca EF tarafını kapsar. Tracon şeması uygulama açılışında kendi kendine migrate olur (`AutoApplyMigrations`). |
| **`DbMigrator` container'ı yetmez** | CI/CD'de migration adımı Tracon şemasını kapsamaz — ilk uygulama açılışı kapsar. Bu davranış `AutoApplyMigrations=false` ile kapatılıp ayrı adım yapılabilir. |
...
| Rol politikaları | Reader / Operator / Admin | ABP permission → ASP.NET policy | `TraconPolicies` isimleriyle kaydedilir. |
...
2. **Kiracı kaydı senkronize edilmelidir.** Tracon'in kendi `tenants` tablosu vardır (`ITenantStore`). ABP'de yeni kiracı açıldığında Tracon'e de kaydedilmelidir. `MigrationHostedService` yalnızca varsayılan kiracıyı garanti eder.
...
**Tracon'de bu gerekçe geçerliliğini yitiriyor** — çünkü Tracon ABP UoW'una hiç girmez. Kendi bağlantısını kullanır. `ChatSession` satır kilidi, Tracon koşusu boyunca **zaten tutulmaz**.
...
sequenceDiagram
    ...
    participant AP as Tracon koşusu
    ...
    Note over AP: tool döngüsü Tracon içinde döner<br/>job yeniden kuyruğa GİRMEZ
    end
...
Yani faz 1 ve faz 3 aynen korunur; yalnızca faz 2'nin **içi** Tracon'e devredilir. Bu, mevcut prod bug düzeltmesini bozmaz. Aksine sadeleştirir: her tool turunda job'ı yeniden kuyruğa alma ihtiyacı ortadan kalkar.
...
Deploy sırasında akan bir chat turu bugün hayatta kalır, Tracon ile kalmaz. Tek instance'lı Hetzner deploy'unda bu, her deploy'da aktif chat turlarının kesilmesi demektir.
...
Tracon'in taşıması SSE'dir (`RunEndpoints.RunEventStream`, `SseWriter`). Bizim FE'miz SignalR konuşur ve `IChatClient`'ta 14 metot vardır. FE'de merkezî SignalR hook'u yoktur; her component kendi bağlantısını kurar.

İki seçenek vardır:

| Seçenek | İş | FE etkisi | Değerlendirme |
|---|---|---|---|
| **A — Köprü yaz** | `RunEvent` akışını tüketip mevcut `IChatClient` metotlarına dönüştür | **Sıfır** | Önerilen. Mevcut FE hiç değişmeden Tracon'e geçilir. |
| **B — SSE'ye geç** | FE'yi Tracon'in SSE sözleşmesine taşı | **Yüksek** | Hedef mimari Faz 5'in işi. Ayrı proje, ayrı FE koordinasyonu. |
...
Bugün iki kuyruk var: chat için ABP BackgroundJobs, pipeline'lar için Hangfire. Tracon kendi kuyruğunu getirir (`IJobStore`, `FOR UPDATE SKIP LOCKED` ile lease, `MaxAttempts`, `ScheduledFor`).

Ancak Tracon kuyruğu genel amaçlı değildir. `JobKind` kapalı bir listedir ve yalnızca Tracon'in kendi iş türlerini taşır: `AgentRun`, `AgentBatch`, `Workflow`, `Eval`, `OnlineEval`, `ApprovalResume`, `Retention`, `WebhookDelivery`. Yani `ImageGenerationJob` gibi bir domain adımı oraya konamaz.
...
Tracon'de karşılığı `AIFunction`'dır. İki yol vardır:
...
| **Yeniden yazma** | `[TraconTool]` işaretli metotlara taşı | Daha temiz, ama 22 tool × test = büyük diff. Kazanç marjinal. |

**Öneri: sarmalama.** `new-chat-tool` skill'i güncellenir, `BackendToolHandlerBase` kalıbı korunur, kayıt noktası `BackendToolRegistry` yerine `TraconToolRegistration` olur. Şema üretimi B10'u kapatır.

**Yetkilendirme burada çözülür:** `TraconToolRegistration` `RequiredPermission` (opak string) ve `ToolEffect` (`Read`/`Write`/`Destructive`/`External`) taşır. `IToolAuthorizationHandler`'ı ABP `IPermissionChecker`'a bağlayınca 22 tool'un tamamı ABP izin ağacıyla korunur. Yetkilendirme onaydan **önce** koşar ve handler hata verirse çağrı reddedilir. Bu, B4'ün asıl çözümüdür.

Dikkat: tool gövdesi artık Tracon koşusu içinde çalışır. `ChatToolCallCoordinator.EnrichWithSessionContext`'in yaptığı `_sessionId` + `DynamicParametersJson` enjeksiyonunun yeni karşılığı planlanmalıdır — muhtemelen `AmbientRunAttributionScope` benzeri bir kapsam veya DI'dan okunan bir bağlam nesnesi.

### 7.8 Güvenlik yüzeyi — net kazanç, tek dikkat noktası

**Kazanç:**

- `MapTracon()` üç katmanlı korumayla gelir: loopback kısıtı (varsayılan), sabit bearer token, ASP.NET Core policy. Varsayılan **kapalıdır** — yanlışlıkla dışarı açılmaz.
- `Tracon:Egress` tek ayarla webhook, MCP ve sağlayıcı çağrılarının hedefini kısıtlar. Dokümanı bulut metadata adresini (`169.254.169.254`) açıkça örnek veriyor. Bizde egress kontrolü hiç yoktu.
...
Tracon'in gömülü konsolu (`Tracon.UI` + 123 path'lik API) bu kapsamın büyük kısmını karşılar:

| Modül | Tracon karşılığı | Durum |
|---|---|---|
...
**Sonuç:** 13 modülün 8'i tamamen, 2'si kısmen karşılanır. 3'ü bizde kalır ve bunların hepsi **domain operasyonudur** (Hangfire pipeline'ları, chat domain durumu, ConvAI oturumları) — zaten Tracon'in işi değildir.

**Öneri:** AI Ops Console'u iptal etme, **daralt**. Yeni kapsam: Tracon konsolunu ABP izniyle koru, üzerine yalnızca M9 + M10 + M11'i kapsayan bir "domain operasyon" ekranı yaz. 76 sayfa ~20 sayfaya iner ve §9'daki ~25 AppService'in çoğu gereksizleşir.
...
**Amaç:** Tracon'i ayağa kaldır, en izole akışta doğrula.

1. `AddTracon()` + `UsePostgreSql()` (ayrı şema, aynı veritabanı) + `UseOpenAI/UseAnthropic/UseGoogle` + `UseOpenAICompatible`.
2. `ITenantContext` ve `IRunAttributionContext` köprüleri (ABP `ICurrentTenant` / `ICurrentUser`).
3. ABP kiracı oluşturma olayına Tracon kiracı kaydı ekle.
4. Roleplay pipeline'ının 2 AI çağrısını Tracon agent'ına taşı. Structured output kullan.
...
6. `QAGenerationJob`, `AssessmentGenerationJob`, `PodcastScriptJob`, `ConvAIEvaluationJob` çağrılarını Tracon koşusuna taşı.
...
9. `Tracon:Egress` politikasını yaz.
...
10. 22 backend tool'u `TraconToolRegistration` ile sarmala. `BackendToolHandlerBase` korunur.
11. Frontend tool'ları `AddClientTool()`'a taşı.
12. `ChatMessageProcessor` faz 2'sini Tracon koşusuna devret. Faz 1 ve 3 korunur.
...
17. `AgentConfiguration` kayıtlarını Tracon agent tanımına taşı. **Tek kaynak kararı burada verilir** (§12.1).
...
22. `MapTracon()` → ABP izin politikasıyla (`AiOps.Reader/Operator/Admin`).
...
Bu bölüm, "Tracon şu yeteneğe de sahip olsaydı bu projede işimiz belirgin biçimde kolaylaşırdı" listesidir. Sıra, bu projedeki somut değere göredir. Her kalem bir gözlemden türetilmiştir, temenniden değil.
...
> **Not:** İlk taslak bunu bir `Tracon.Abp` paketi olarak istiyordu. Tracon'in karar defteri bunu iki yerden kesiyor: EF Core bilinçli olarak kullanılmıyor (L16) ve "genişleme noktası varsa somut uygulama tüketicinin işidir" emsali (S3/Azure Blob kalemi) mevcut. Paket doğru istek değil; **örnek + doküman** doğru istek.

**Neden değerli:** ABP dünyada yaygın bir .NET kurumsal çerçevesidir. Bu köprü yalnızca bize değil, Tracon'in benimsenme hızına da yarar. Bizim açımızdan: entegrasyonun en sıkıcı ve en hataya açık kısmını siler.
...
**Etki:** Bu yetenek olmadan **agent tanımlarını Tracon'e taşımak (Faz D) yarım kalır.** Değerlendirmesi en yüksek eksik budur.
...
**Bugün:** `Workflows` süreç-içidir. Tracon job queue'su her işte tek bir hedef çalıştırır; graf semantiği taşımaz.
...
**Bugün:** Tracon'de görsel üretme yoktur. MAF'ın `ImageGenerationToolCallContent` tipleri JSON bağlamında geçer, ama bir sağlayıcı yeteneği olarak sunulmaz.
...
**İstenen:** Ses için yapılanın aynısı — birinci sınıf görsel üretim aracı, ölçümlü (görsel başına/çözünürlük başına fiyat), koşu kaydına bağlı. `Tracon.Voice` için `SpeakTool`/`TranscribeTool` nasıl duruyorsa, `Tracon.Images` için de `GenerateImageTool` öyle durmalıdır.
...
**Bu, Tracon'in bilinçli kararıdır** (*Bilerek Önerilmeyenler*, 2026-08-18): dönem, kur dönüşümü, mark-up ve fatura satırı bir iş katmanıdır ve muhasebe sistemine göre değişir. Katılıyoruz — hedef mimarideki `AiInvoiceLine` bizim tarafımızda yazılır. Kırılım boyutları (kiracı · kullanıcı · etiket) geldikten sonra toplama katmanı ucuzdur.

### 10.10 🟡 Bildirim köprüsü

Webhook vardır (imzalı, teslimat geçmişli). Bizim `Notifications` modülümüz olay tabanlıdır ve kullanıcı tercihleri/çok dillilik taşır. Bütçe eşiği veya bekleyen onay gibi olayların bu modüle akması için köprü yazılmalıdır. Paket tarafında `ITraconEventSink` benzeri süreç-içi bir olay kancası (webhook'un HTTP'siz kardeşi) bu işi kolaylaştırırdı.
...
- **Agent isim çözümlemesi (B19):** Tracon'de de isim string'iyle çözülüyor. `POST /agents/validate` ve katalog tekliği durumu iyileştiriyor ama tipli bir agent kimliği yok. Düşük öncelik.
...
|---|---|---|---|---|
| R1 | Preview sürüm; API yüzeyi hareket halinde (287 → 291 arasında build-time kuralı eklendi) | Yüksek | Orta | Sürüm tek yerden pinli; `tracon-update` skill'i var; yükseltmeyi planlı yap |
...
| R4 | İki veri düzlemi arasında tutarsızlık kovalamak | Orta | Orta | Tek yön kuralı: Tracon koşu için tek kaynak, domain yalnızca `RunId` tutar |
...
`AgentConfiguration` bugün ABP tablosudur; admin ekranları, `ChatSessionType → AgentNameMap` eşlemesi, `EvaluationCriterion.AgentConfigurationId` FK'sı ve setting'ler ona bağlıdır. Tracon'e taşınırsa sürüm/diff/rollback/eval kazanılır ama bu FK'lar ve ekranlar yeniden düşünülmelidir. İki kaynağı senkron tutmak **önerilmez**.

### 12.2 `ChatSession` / `ChatMessage` korunacak mı?

Tracon oturum state'ini opak tutar. `RetrySessionAsync`, `ResumeSessionAsync`, bayat tool tespiti, son-6-mesaj heuristiği ve FE'nin mesaj listesi bu entity'lere bağlıdır. Öneri: korunsun, `RunId` ile bağlansın. Ama kabul edilirse "iki yerde mesaj" durumu doğar — bunun bilinçli kabulü gerekir.
...
Tracon bu proje için **doğru kategoride bir araçtır**. Eleştiri dokümanının teşhisi şuydu: *"Sistem 'LLM çağıran bir uygulama' olarak yazılmış; 'LLM'i çalıştıran bir platform' olarak değil."* Tracon tam olarak o eksik platform katmanıdır ve hedef mimarinin yazdığı sözleşmelerin çoğunu isim isim karşılar.
...
|---|---|
| `Tracon.AgentMap.md` (Core `buildTransitive/`) | Yetenek haritası, kural cümleleri |
| 12 paketin XML dokümanı (~6.100 member) | Tip sözleşmeleri, tasarım gerekçeleri |
| `tracon.json` (OpenAPI 3.1.1, 123 path) | HTTP yüzeyi, run endpoint davranışı |
