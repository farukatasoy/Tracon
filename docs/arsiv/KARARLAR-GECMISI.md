...
`[Obsolete]` biçimi (Açık Soru 4) ölçüldü: `DiagnosticId = "TRC9001"` ile Tracon.Core'un kendi `JsonSerializerContext`'i 30 hatayla kırıldı — System.Text.Json kaynak üreteci ürettiği koda yalnız `#pragma warning disable CS0612, CS0618` yazar. Aynı tip bir tüketicinin context'ine girerse o da kırılırdı. Bu yüzden yalnız mesaj + `UrlFormat` (`production.md#upgrading-credential-headers`); `ObsoleteMessagesTests` bağlantı başlığını ve `DiagnosticId` yokluğunu kilitler. OpenAPI `deprecated` üretmiyor (ölçüldü) → özet cümlesi "Deprecated:" der (Açık Soru 6 = B). Faz 188/189 `[Obsolete]`'suz kaldırıyordu; bu üye farklıdır: kalıcı sütun, form yazar, HTTP istemcisi gönderir. `Authorization` hem eski alanda hem yeni haritada veya OAuth ile yeni haritada → `400`, **birleşik** (korunan) harita üzerinde ölçülür. GA kaldırma + veri taşıma + form geçişi: F-290.
