...
> ✅ **TUR KAPANDI — 2026-09-19.** Bu dosya turun kapanış kaydıdır ve artık
> **arşivdedir**; açık bir işin kontrol düzlemi değildir. Baştan sona okunmaz,
> grep'lenir. Sonraki turun kendi kapanış planı olur.
>
...
> **26 açık case'in 26'sı da** [`00-INDEKS.md` §7.2](../../manuel-test/00-INDEKS.md)'ye gerekçesiyle yazıldı — **bitti tanımının 1. maddesi karşılandı**.
...
> **§6 BİTTİ:** damıtma (2.009.685 → 1.486.534 B, -%27) · arşiv (bu dizin) · `MEMORY.md` açık iş bloğu silindi · `YAYIN-HAZIRLIK.md` §4'te Adım 1 ve 2 ✅.
> **Sekiz kapı + doküman kapısı yeşil** (§3.12). `dokuman-bakim.py --denetle` bu turda **ilk kez çıkış 0** — koşum kaydı bütçesi arşivlemeyle düştü.
> **Kalan iş — yalnız 👤 kullanıcı:** §6.5 (beş sağlayıcı anahtarı + `CLAUDE_CODE_MESSAGING_TOKEN` döndürme, `~/tracon-manuel/` ve container temizliği).
> **Son güncelleme:** 2026-09-19 (kapanış tamamlandı)
...
### 3.12 Sekiz kapının tam ölçümü — 2026-09-19 (TURUN SON ÖLÇÜMÜ)

`MT-SEC-181`'in demo kancası dahil; damıtma ve arşivden **sonra** koşuldu.
...
| `dotnet pack --no-build` | ✅ çıkış 0 |
| `dotnet format --verify-no-changes` | ✅ çıkış 0 |
| `docs-site && npm run check` | ✅ çıkış 0 · 1152 sayfa bütçede |
| `dokuman-bakim.py --denetle` | ✅ **çıkış 0 — turda İLK KEZ**; 19 kalem DAR (uyarı) |

🚨 **İlk koşumda 83 test düştü ve sebebi kod DEĞİLDİ.** `dotnet test` sürerken
aynı makinede `unittest discover -s scripts` başlatıldı; o paket `dotnet pack`
ve bir yayın provası koşar. `SqlServer.IntegrationTests` **80** düştü (hepsi
aynı belirti: `Migration '0037_run_score_message_key' … Execution Timeout`) ve
`Ui.E2ETests` **3** düştü. Ölçüm ayrıştırdı: ikisi de **tek başına** `822/822`
ve `81/81` geçti, süre `6 dk 59 sn → 1 dk 41 sn` düştü. Tek başına yeniden
koşulan tam paket `7853/7853` verdi. Ders
[`hafiza/test-yalitimi-vakalari.md`](../../hafiza/test-yalitimi-vakalari.md)'ye
yazıldı: **dört kapı yalnız başına koşar.**
...
**26 açık case'in tamamı** [`00-INDEKS.md` §7.2](../../manuel-test/00-INDEKS.md)'ye dört
...
## 6. Kapanış bitince — ✅ 1–4 ve 6 BİTTİ (2026-09-19), 5 kullanıcıda

| Madde | Durum |
|---|---|
| 1. Bitti tanımı (skill §7) | ✅ On maddenin onu da kapandı. Kapılar §3.12'de; `hafiza/` · `ADAYLAR.md` (F-250, F-251) · `00-INDEKS.md` §3.2/§7/§7.2 §3.6'nın tablosunda |
| 2. Damıtma | ✅ 39 dosya · 1.028 case daraltıldı · 521 AYNEN korundu · 2.009.685 → 1.486.534 B (-%27) |
| 3. Arşiv | ✅ Kayıt `docs/arsiv/manuel-test-kosum-2026-09/` altındadır (bu dizin). `docs/arsiv` bütçesi ölçümle 5.496.000 → 7.400.000 B'ye kalibre edildi (ölçülen 6.289.937 / 0,85 — kullanıcı kararı) |
| 4. `MEMORY.md` açık iş bloğu | ✅ Silindi (930 B) |
| 5. Anahtarlar ve tur aparatı | 👤 **KULLANICIDA** — aşağıdaki liste |
| 6. Yayın hattı | ✅ `YAYIN-HAZIRLIK.md` §4'te Adım 1 ve 2 ✅; sıradaki iş **Adım 3** (RK-014, tam `git` geçmişinde secret taraması) |

Aşağısı kapanış sırasında yazılmış özgün metindir; 5. madde hâlâ açıktır.

---

### 6.0 Özgün kapanış listesi
