...
| 26 | [26-ANTHROPIC-VE-GEMINI.md](../26-ANTHROPIC-VE-GEMINI.md) | F-01 · F-02 | ✅ **Tamamlandı (2026-08-05)** — iki **resmî** SDK (K-204); `ModelBinding.ProviderSettings` sözleşmeye eklendi (K-208); içerik filtresi tespiti Core'da ortak dekoratör, devre kesicinin dışında (K-206) | `AgentPrism.Anthropic` · `AgentPrism.Google` | — |
| 27 | [27-AZURE-FOUNDRY.md](../27-AZURE-FOUNDRY.md) | F-04 | ✅ **Tamamlandı (2026-08-05)** — Azure OpenAI sağlayıcısı; deployment tabanlı çözüm, Entra kimliği (`Azure.Identity` alınmadı, K-210). **Foundry ertelendi** (37 geçişli paket, K-212); ayar yüzeyi ve Responses desteklenmiyor (K-211, K-213) | `AgentPrism.Azure` | — |
| 28 | [28-SES-TOOLLARI.md](../28-SES-TOOLLARI.md) | F-13 (1/2) | ✅ **Tamamlandı (2026-08-05)** — üç tool, **sıfır NuGet bağımlılığı** (K-216); sözleşmeler `Abstractions`'ta (K-215). Üç ölçüm planı düzeltti: `AgentRunScope.SessionId` yoksa ses eki saklama tarafından siliniyordu (K-217), MAF tool'a **boş** servis sağlayıcı geçiriyor (K-218), `tool_invocations` ölçüm sütunu taşımıyordu (K-219) | `AgentPrism.Voice` | 0015 (Postgres) / 0003 (SqlServer, Sqlite) |
| 29 | [29-KONUSMA-KATMANI.md](../29-KONUSMA-KATMANI.md) | F-13 (2/2) | ✅ **Tamamlandı (2026-08-05)** — WebSocket konuşma ucu, Seçenek A boru hattı (her tur normal bir `runs` satırı), kesinti geçmişe yazılır, token alt protokolde (K-224). Katman `AgentPrism.Voice`'ta değil **`Core`**'da (K-222); artımlı transkript yok (K-226) | `.Core` + `.AspNetCore` + `.UI` | 0016 |
| 30 | [30-ARAYUZ-CILASI.md](../30-ARAYUZ-CILASI.md) | F-25 · F-26 | ✅ **Tamamlandı (2026-08-05)** — arayüzün tamamı iki dilli (794 anahtar; eksik anahtar **derleme hatası**, K-228), komut paleti + kısayollar, dil başına ses eşlemesi istemcide (K-234, sunucuda değişiklik yok), WCAG AA kontrast hatası ölçülüp düzeltildi (K-236). Bundle 151,3 KB / 250 KB | `.UI` | — |
