...
| 15 | [15-WORKFLOWS-YURUTME.md](fazlar/15-WORKFLOWS-YURUTME.md) | F-27 (1/2) | ✅ **Tamamlandı** (2026-08-03) — beş desen, checkpoint, sürdürme | `AgentPrism.Workflows` | 0007 |
...
| 26 | [26-ANTHROPIC-VE-GEMINI.md](fazlar/26-ANTHROPIC-VE-GEMINI.md) | F-01 · F-02 | ✅ **Tamamlandı (2026-08-05)** — iki **resmî** SDK (K-204); `ModelBinding.ProviderSettings` sözleşmeye eklendi (K-208); içerik filtresi tespiti Core'da ortak dekoratör, devre kesicinin dışında (K-206) | `AgentPrism.Anthropic` · `AgentPrism.Google` | — |
| 27 | [27-AZURE-FOUNDRY.md](fazlar/27-AZURE-FOUNDRY.md) | F-04 | ✅ **Tamamlandı (2026-08-05)** — Azure OpenAI sağlayıcısı; deployment tabanlı çözüm, Entra kimliği (`Azure.Identity` alınmadı, K-210). **Foundry ertelendi** (37 geçişli paket, K-212); ayar yüzeyi ve Responses desteklenmiyor (K-211, K-213) | `AgentPrism.Azure` | — |
| 28 | [28-SES-TOOLLARI.md](fazlar/28-SES-TOOLLARI.md) | F-13 (1/2) | ✅ **Tamamlandı (2026-08-05)** — üç tool, **sıfır NuGet bağımlılığı** (K-216); sözleşmeler `Abstractions`'ta (K-215). Üç ölçüm planı düzeltti: `AgentRunScope.SessionId` yoksa ses eki saklama tarafından siliniyordu (K-217), MAF tool'a **boş** servis sağlayıcı geçiriyor (K-218), `tool_invocations` ölçüm sütunu taşımıyordu (K-219) | `AgentPrism.Voice` | 0015 (Postgres) / 0003 (SqlServer, Sqlite) |
| 29 | [29-KONUSMA-KATMANI.md](fazlar/29-KONUSMA-KATMANI.md) | F-13 (2/2) | ✅ **Tamamlandı (2026-08-05)** — WebSocket konuşma ucu, Seçenek A boru hattı (her tur normal bir `runs` satırı), kesinti geçmişe yazılır, token alt protokolde (K-224). Katman `AgentPrism.Voice`'ta değil **`Core`**'da (K-222); artımlı transkript yok (K-226) | `.Core` + `.AspNetCore` + `.UI` | 0016 |
