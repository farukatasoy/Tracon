...
> **Paketler:** `Tracon.Abstractions`, `.Testing.Contracts.Xunit`
...
- [x] En az bir **dış sample** (`PackageReference`, `ProjectReference` yok) contract'ı koşuyor — `Tracon.Samples.CustomJobHandler(.Tests)`
...
- [x] `samples/Tracon.Api` ile gerçek `run` yapıldı, çıktı belgeye yazıldı — bkz. § *Gerçekleşen Public API* altındaki koşum notu
...
- **`Tracon.Testing` paket referansı planın taslak sample kurulumunda yoktu, gerçekte de eklenmedi** — `NightlyReportJobHandler`in `TraconTestHost` gibi bir gerçek-run altyapısına ihtiyacı yok (bkz. aşağıdaki JobKind bulgusu: gerçek bir host üzerinden dispatch edilemiyor, bu yüzden öyle bir test yanlış yolu doğrulardı).
...
| 4 | 🟢 | Rehberin önerdiği "AddTracon()'den önce kaydet" çözümü hiçbir testte kanıtlanmıyor | Devredilmedi — rehber zaten kendi iddiasını "until this ordering limitation is resolved" diyerek sınırlıyor, yanıltıcı değil |
