...
> **Paketler:** `Tracon.Abstractions`, `Tracon.Core`, `Tracon.AspNetCore`, `Tracon.Sql.Shared` (üç SQL paketine linked-source, K-176), `Tracon.UI`
...
> · sevk edilen: yeni tiplerin XML dokümanı ve `<example>`'ları, `src/Tracon.Abstractions/README.md`. `api/` ve `http-api/` **üretilir**
...
Tracon'in agent tanımı bugün **parametresizdir**. `Instructions` düz metindir; `InstructionsByCulture` yalnız **dile** göre varyant verir. Koşu isteği de bir parametre taşımaz. Sonuç: parametreli bir agent Tracon tanımına **taşınamaz**. Bu bir ergonomi eksiği değildir.

## Bitiş Ölçütleri (DoD)

- [x] `POST /api/agents/{name}/run` eksik zorunlu parametrede `400` döner ve eksik parametrenin **adını** söyler — `samples/Tracon.Api`'ye karşı ölçüldü: `{"detail":"Missing required parameter 'musteri'.","missingParameters":["musteri"]}`
...
- [x] Belge içeriğindeki sınırlayıcı dizisi kaçırılır — canlı ölçüldü: sahte `-----END TRACON DOCUMENT-----` içeren bir belge gönderildi, model talimatı bozulmadı, kayıt yalnız gerçek belge adını/boyutunu gösterdi (bkz. denetim 🔴 bulgusu — Name kaçışı da eklendi)
...
- [x] `samples/Tracon.Api` ile gerçek `run` yapıldı, çıktı belgeye yazıldı — bkz. "Doğrulama komutları" altındaki gerçek çıktı
...
### Doğrulama komutları — gerçekleşen çıktı (2026-08-22, `samples/Tracon.Api`, gerçek OpenAI çağrısı)
...
curl -s -X POST http://localhost:5081/tracon/api/agents/parametreli/run \
...
curl -s -X POST http://localhost:5081/tracon/api/agents/parametreli/estimate \
  -H 'content-type: application/json' -d '{"message":"selam"}'
# → aynı gövde, aynı 400
...
curl -s http://localhost:5081/tracon/api/runs/01a02ade-.../events
...
ls -la src/Tracon.UI/wwwroot/assets/*.js | awk '{s+=$5} END {print s/1024" KB (raw)"}'
...
2. **Parametre değeri boyut sınırı eklendi** (Açık Soru 2, öneri A benimsendi):
   `TraconOptions.MaxParameterValueLength` (varsayılan 4096 bayt UTF-8,
...
5. **SQL-tabanlı `AgentDefinitionPayload` jsonb izdüşümü yeni alanları
   taşımıyordu** — ikinci 🔴 bulgu, yalnız gerçek bir `samples/Tracon.Api`
   ...
   yazılan ARA tip (`AgentDefinitionPayload`, `Tracon.Sql.Shared`) unutulmuştu
...
| 2 | `TraconOptions.MaxParameterValueLength` hiç yoktu; planın kendi "Beş soru" tablosu bir sınırın "gerekir" dediği hâlde ilk uygulamada eklenmemişti | 🟡 | **Düzeltildi** — K-582, bkz. "Bu Fazda Verilen Kararlar" |
...
doğrulama koşusunda bulundu** — `samples/Tracon.Api`'ye karşı gerçek bir
...
- `AgentDefinitionPayload` (`Tracon.Sql.Shared/Internal/`), `AgentDefinition`'ın
...
- `faz-tamamlama` Adım 2'nin ("örnek uygulamayı GERÇEKTEN çalıştır") gerekliliği
  ...
  kusur, yalnız `samples/Tracon.Api`'nin gerçek PostgreSQL'ine karşı elle
