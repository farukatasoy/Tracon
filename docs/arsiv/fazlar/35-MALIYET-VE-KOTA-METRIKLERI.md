...
`AgentPrismMetrics` beş enstrüman taşır ve **hiçbiri maliyet değildir**. Faz 20 maliyeti hesaplıyor ama yalnız `runs` tablosuna yazıyor. Grafana veya Azure Monitor'da maliyet panosu kurmanın yolu bugün veritabanını sorgulamaktır ve bu, tüketicinin APM'ine girmez. - **F-70** — `agentprism.run.cost` sayacı (para birimi etiketiyle) ve `agentprism.quota.usage` gözlemlenen ölçeri.
