# Faz 39 — `Tracon.Testing` Paketi

> **Durum:** ✅ Tamamlandı (2026-08-06)
> **Kaynak:** [ADAYLAR.md](../../ADAYLAR.md) · **F-46**
> **Önkoşul:** Yok
> **Paketler:** `Tracon.Testing` (**yeni**), `Tracon.Abstractions`, `.Core`, `.AspNetCore`
> **Yeni paket:** **`Tracon.Testing`** — K-007 gerekçesi [39.1](#391--k-007-gerekçesi-neden-ayrı-bir-paket) · **Migration:** Yok
...
- [x] Depo dışından bir tüketici senaryosu çalıştı: `samples/` altında veya
      geçici bir projede, Tracon'e bağlı bir agent **gerçek model
...
```bash
$ dotnet pack src/Tracon.Testing -c Release --no-build
$ unzip -p artifacts/package/release/Tracon.Testing.0.0.0-preview.0.41.nupkg \
  Tracon.Testing.nuspec | grep -A5 "<dependencies>"
    <dependencies>
      <group targetFramework="net10.0">
        <dependency id="Tracon.AspNetCore" version="0.0.0-preview.0.41" exclude="Build,Analyzers" />
        <dependency id="Tracon.Core" version="0.0.0-preview.0.41" exclude="Build,Analyzers" />
...
$ grep -c "Tracon.Testing" src/Tracon/Tracon.csproj
0
...
tests/Tracon.Core.UnitTests/Fakes/FakeModelProvider.cs
...
$ find src/Tracon.Testing -name "* 2.*"
# (bos)
...
| Proje | Sonuç | Not |
|---|---|---|
| `Tracon.Testing.UnitTests` (**yeni**) | 32/32 ✅ | Paketin kendi testleri |
| `Tracon.Core.UnitTests` | 576/576 ✅ | `DependencyDirectionTests` dahil |
| `Tracon.AspNetCore.FunctionalTests` | 319/319 ✅ | `EchoModelProvider`+`RoutingModelProvider` göçü |
| `Tracon.PostgreSql.IntegrationTests` | 505/505 ✅ | Gerçek Postgres container (Testcontainers) |
| `Tracon.Sqlite.IntegrationTests` | 255/255 ✅ | |
| `Tracon.Ui.E2ETests` (Playwright) | 41/41 ✅ | `ScriptedModelProvider` göçü |
| `Tracon.SqlServer.IntegrationTests` | ⚠️ atlandı | Bu makinede `mssql/server` ARM64 Docker'da hazır olmuyor (önceden bilinen ortam kısıtı, bu fazla ilgisiz) |
| `Anthropic`/`Google`/`Azure`/`OpenAI`/`Voice`/`Workflows`/`Mcp`.UnitTests | 331/331 ✅ | Faz 39'a dokunulmadı, regresyon sıfır |
| `Tracon.Templates.Tests` | 10/10 ✅ | Bu oturumun ayrı bir düzeltmesiyle ilgili (bkz. altta) |

**Depo dışı tüketici senaryosu** (gerçek çıktı):

```bash
$ mkdir /tmp/faz39-consumer-check && cd /tmp/faz39-consumer-check
# NuGet.config: tracon-local -> artifacts/package/release
$ dotnet test
...
+ `TraconTestHost.RunAsync("support", "ORD-7 nerede?")` →
...
**`samples/Tracon.Api` gerçek çalıştırma:**

```bash
$ curl -s http://localhost:5081/tracon/api/meta
{"version":"0.0.0-preview.0.41","prefix":"/tracon", ...}
$ curl -s -o /dev/null -w "%{http_code}\n" http://localhost:5081/tracon/api/agents
200
```
...
(`Tracon.AspNetCore.FunctionalTests.Infrastructure` vs
`Tracon.PostgreSql.IntegrationTests.Infrastructure`). Davranış birebir
aynıydı; Seçenek A (diff okunup bilinçli seçim yapılır) mekanik olarak
uygulandı, davranış farkı yoktu.

**Açık Soru 2, 3, 5 plandaki önerilerle aynen kapandı:** `TraconTestHost`
...
`TraconTestHost` JSON (de)serileştirmesi kaynak üreteci bağlamı taşımaz).
...
**Kaldırılan kopyalar listesi bir istisnayla uygulandı — `tests/Tracon.Core.UnitTests/Fakes/FakeModelProvider.cs`
...
`TraconTestHost.RunAsync` HTTP `/run` (SSE, akışlı) kullandığı için
...
- `Tracon.Testing` yalnız `net10.0` hedefler (K-270) — `net8.0`/`net9.0`
...
- `Tracon.Testing.TraconTestHost` ile
  `Tracon.AspNetCore.FunctionalTests.Infrastructure.TraconTestHost`
  (TestServer tabanlı, iç) **aynı adı taşır**; `using Tracon.Testing;`
  FunctionalTests dosyalarında `CS0104` verir — `using FakeModelProvider =
  Tracon.Testing.FakeModelProvider;` takma adıyla alınmalıdır.
- `Microsoft.AspNetCore.TestHost` merkezi sürümü barındırma framework'üyle
  ...
  bu paketin sürümü de güncellenmeli, aksi hâlde `Tracon.Testing` eski
...
- `Tracon.src.slnf` (K-268) yalnız `src/` projelerini listeler; yeni bir
  paket eklendiğinde bu dosyaya da satır eklenmesi gerekir — `dotnet pack
  Tracon.slnx` DoD kontrolü paket SAYISINI doğrular ama `.slnf`'in
