...
> **Paketler:** `Tracon.Core`, `.Abstractions`, `.AspNetCore`, `.Mcp`, `.OpenAI`, `.Anthropic`, `.Azure`, `.Google`, `.Voice`, `.PostgreSql`, `.SqlServer`, `.Sqlite`
...
- [x] `src/Tracon.Core/Properties/AssemblyInfo.cs` üç yeni `InternalsVisibleTo` satırını taşıyor ve her biri yorumla gerekçelendirilmiş — planlanan üç (`PostgreSql`/`SqlServer`/`Sqlite`, K-176 `Auditing*` + `InMemoryRunStore`) eklendi; uygulama sırasında **altı** ek satır daha gerekti (test projeleri, bkz. Plandan Sapmalar)
...
- [x] Tüm test metotları yeşil (`dotnet test Tracon.slnx -c Release --no-build` → 0 failed); hiçbir test **davranış** değişikliği için düzeltilmedi (yalnız `internal` erişimi için `InternalsVisibleTo` veya taşıma) — DoD taslağındaki "2.897" bayat bir sayıydı, ölçülmedi, düzeltilmedi
...
# yeni kapilar
python3 scripts/kapi.py test --proje Tracon.Core.UnitTests --sinif PublicSurfaceBaselineTests
python3 scripts/kapi.py test --proje Tracon.Core.UnitTests --sinif PublicApiTrackingDeclarationTests
python3 scripts/kapi.py test --proje Tracon.Package.Tests  --sinif ConsumerSurfaceTests
...
1. **Açık Soru 2'nin öncülü yanlıştı — ölçülüp düşürüldü.** Plan
   `OpenAIProviderOptionsValidator`'ın `Tracon.Azure` tarafından gerçek
   kodda kullanıldığını iddia ediyordu (kanıt olarak
   `AzureOpenAIProviderOptionsValidator.cs` gösteriliyordu) ve buna göre bir
   `InternalsVisibleTo("Tracon.Azure")` öngörüyordu. Uygulama anında
   ...
   yapıldıktan sonra) `Tracon.Azure`'da sıfır hata verdi. Ek
...
2. **`InternalsVisibleTo` deseni dışında kalan tip sayısı plandan farklı
   ...
   `InternalsVisibleTo` gerektirdi: `Tracon.Mcp.UnitTests`,
   `Tracon.Workflows.UnitTests`, `Tracon.AspNetCore.FunctionalTests`,
   `Tracon.PostgreSql.IntegrationTests`, `Tracon.SqlServer.IntegrationTests`,
   `Tracon.Sqlite.IntegrationTests` (`Tracon.Core.UnitTests` desenin
...
4. **DoD taslağındaki "2.897 test metodu" sayısı hiç ölçülmedi, bir önceki
   ...
   ölçüt `dotnet test Tracon.slnx -c Release --no-build`'in `0 failed`
...
| # | Bulgu | Sonuç |
|---|---|---|
| 1 | DoD'nin "gerçek `run`" satırı için kanıt yoktu | **Düzeltildi** — `samples/Tracon.Api` çalıştırıldı (bkz. aşağıdaki gerçek çıktı) |
...
yorumu (`src/Tracon.Core/Storage/InMemoryApprovalAndMcpStores.cs:197`,
...
`samples/Tracon.Api` bağımsız değişkensiz (bağlantı dizisi yok →
`InMemoryRunStore`, API anahtarı yok → `EchoModelProvider`) `http://localhost:5080`
üzerinde ayağa kalktı. `GET /tracon/api/diagnostics`:
...
`POST /tracon/api/agents/support/run` `{"message":"What is the status of
order ORD-1?"}` gerçek bir SSE akışı üretti (`run` olayı, `update` olayları,
`Echo: What is the status...` metni); `GET /tracon/api/runs/{runId}`
...
- `PublicSurfaceBaselineTests` ve `public-surface-baseline.txt` artık
  dördüncü mandal — tazeleme: `TRACON_PUBLIC_SURFACE_REFRESH=1 dotnet
  test tests/Tracon.Core.UnitTests -c Release`. Kapı **tip** sayar, girdi
  değil.
- `PublicApiTrackingDeclarationTests` her yeni packable paketin
  `PublicAPI.{Shipped,Unshipped}.txt` **ya da** açık
  `TraconPublicApiTrackingEnabled=false` taşımasını zorunlu kılar.
- `SurfaceProbeProject` (`tests/Tracon.Package.Tests/Infrastructure/`) bir
...
- 🚨 `docfx metadata` artımlıdır ve `internal` olan bir tipin eski sayfasını
  ...
  gerekmez.
- 🚨 `~/.nuget/packages/tracon*` temizlenmeden yapılan manuel bir
