...
> **Önkoşul:** Yok
> **Paketler:** `Tracon.AspNetCore` · kapı testi `Tracon.Core.UnitTests`
...
> **Tüketici yüzeyi:** `docs-site/`: `concepts/runs.md`, `http-api.md` (+ üretilen `api/`, `http-api/`, `llms-full.txt`) · sevk edilen: `TraconStreamRunEvents` uç açıklaması, `docs/openapi/tracon.json`, `packages/tracon-client/src/schema.ts`, `src/Tracon.Client/Generated/*.g.cs` (hepsi yeniden üretilir — gerçekleşen kapsam plandan geniş, bkz. "Plandan Sapmalar")
...
Tracon iki SSE akışı sevk ediyor ve **ikisi de sözleşmesini eksik ilan ediyor**. Kayıtlı olay akışı (`GET /api/runs/{id}/events`) 31 olay tipinin 21'ini `unknown` adıyla gönderiyor; oysa aynı metodun yorumu adları *"a **stable** contract"* ilan ediyor.
...
- [x] `curl -s .../openapi/v1.json | jq '.paths["/api/runs/{runId}/events"].get.responses."200".content'` `text/event-stream` döner — `samples/Tracon.Api`'de canlı doğrulandı, çıktı: `{"text/event-stream":{"schema":{"type":"string"}}}`
- [x] `docs/openapi/tracon.json` yeniden üretildi; SSE bildiren işlem sayısı **6 → 7** oldu — ölçüldü, `sorted(n)` yedi operationId listeler
- [x] `packages/tracon-client/src/schema.ts` yeniden üretildi ve diff'te yeni `text/event-stream` girdisi görünür
- [x] `TraconStreamRunEvents` `404`'ü de bildirir — `samples/Tracon.Api`'de canlı doğrulandı
...
- [x] `samples/Tracon.Api` ile gerçek `run` yapıldı, akış çıktısı belgeye yazıldı — bkz. "Gerçekleşen Public API" altındaki canlı çıktı
- [x] `secret` taraması boş döndü — `python3 scripts/kapi.py tarama`
- [x] Manuel kabul case'leri `docs/manuel-test/11-ARAYUZ-RUN-SESSION-SSE.md` içine eklendi (MT-UIRUN-056..061); otomatikleştirilebilen beşi `samples/Tracon.Api`'de canlı koşuldu, altıncısı (`child.timed-out`, MT-UIRUN-061) gerçek HTTP `TestServer` üzerinden `SubAgentTimeoutTests`'in fonksiyonel testiyle kanıtlandı (canlı sağlayıcıya karşı bir alt-agent askıya alma senaryosu bu oturumun bütçesinde kurulmadı)
...
python3 -c "
import json
s=json.load(open('docs/openapi/tracon.json'))
...
3. **`src/Tracon.Client/Generated/TraconApiClient.g.cs` planın
...
4. **Bağımsız denetim `TraconStreamRunEventsAsync`'in (ve ailenin
   diğer 4 saf-SSE üyesinin) her çağrıldığında `TraconApiException`
   ...
   ÖNCE `TraconRunAgentAsync`'te zaten vardı (NSwag'in `ReadObjectResponseAsync
   ...
   Kanıt: `tests/Tracon.AspNetCore.FunctionalTests/GeneratedClientSseTests.cs`
   (YENİ, plan dışı) gerçek bir `TestServer`'a karşı hem `TraconRunAgentAsync`
   hem `TraconStreamRunEventsAsync`'i çağırır ve SSE gövdesinin ARTIK
...
6. **Manuel kabul case'lerinin (`MT-UIRUN-056..061`) resmi bir `kosumlar/`
   ...
   `samples/Tracon.Api`'de gerçek bir OpenAI çağrısıyla canlı koşuldu ve
...
| 2 | 🔴 | `TraconStreamRunEventsAsync` (ve ailenin diğer 4 saf-SSE üyesi) gerçek bir SSE gövdesiyle çağrıldığında HER ZAMAN `TraconApiException` fırlatıyor — NSwag'in JSON-deserialize eden yardımcısı ham SSE metnini JSON sanıyor | **Düzeltildi** — `nswag-postprocess-client.py`'a dördüncü dönüştürme adımı eklendi (yapısal eşleşme: `status_ == 200` + `ReadObjectResponseAsync<string>`, tam 5 eşleşme). Kanıt: `GeneratedClientSseTests` (YENİ) gerçek `TestServer`'a karşı iki metodu da çağırıp SSE gövdesinin düz metin döndüğünü kanıtlıyor |
| 3 | 🟡 | `TraconApiClient.g.cs`'in yeniden üretilmesi plandan sapma olarak `Plandan Sapmalar`'a yazılmamıştı | **Kapandı** — bkz. "Plandan Sapmalar" #3/#4 |
...
- **🚨 `generate-client-json-context.py`'nin ikinci argümanı
  `src/Tracon.Client/Generated/TraconClientJsonContext.g.cs`'dir,
  `src/Tracon.Client/TraconApiClient.JsonContext.cs` DEĞİL.** Bu
...
- **`Tracon.Client`'ın generated DTO'larının koleksiyon alanları `null`
