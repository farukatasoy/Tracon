...
> **Paketler:** `Tracon.Abstractions` (yeni sözleşme), `Tracon.Core` (`Models/`, `Runs/`)
...
Tracon sağlayıcı arızasını iki ayrı yerde ve iki ayrı amaçla sınıflandırır: **yedek zincirine geçilsin mi** (retry) ve **run hangi hata sınıfına yazılsın** (taksonomi). İkisi de exception tipinin **adı** ve mesajdaki **HTTP metni** üzerinden regex ile karar verir.
...
- [x] `IProviderRetryClassifier` kaydeden tüketicinin kararı yedek zincirini yönetir; `Unknown` yerleşiğe düşer (`ProviderRetryClassifierSeamTests`; gerçek `samples/Tracon.Api` koşumunda da doğrulandı — MT-MYU-015/016)
...
- [x] `Tracon.Core` yeni bir sağlayıcı SDK referansı **almadı** — `grep -c PackageReference src/Tracon.Core/Tracon.Core.csproj` faz öncesiyle **aynı** (14)
- [x] Dört doğrulama kapısı sıfır uyarı verir (`python3 scripts/kapi.py kapanis --taban 386c386`)
- [x] `samples/Tracon.Api` ile gerçek `run` yapıldı, çıktı belgeye yazıldı (§ Gerçek Koşum Kanıtı)
...
- [x] `docs-site/` güncellendi; `npm run build` + `check-links.mjs` temiz (yeni public tiplerin API referans sayfaları üretildi: `Tracon.IProviderRetryClassifier.md`, `Tracon.ProviderRetryDecision.md`, `Tracon.DefaultProviderRetryClassifier.md`, `Tracon.RunErrorFingerprint.md`)

### Doğrulama komutları

```bash
# Core'a sağlayıcı SDK sızmadı
grep -n "PackageReference" src/Tracon.Core/Tracon.Core.csproj
...
1. **`DefaultProviderRetryClassifier.cs`'in konumu planın dosya listesinden
   sapıyor.** Plan `src/Tracon.Core/Runs/DefaultProviderRetryClassifier.cs`
   ...
   aittir. Kapanışta taşınmadı çünkü namespace `Tracon` her iki klasörde de
...
4. **Açık Soru 4 (`WorkflowNodeRetry`) kod değişikliği GEREKTİRMEDİ.** Plan
   "ölçülmeli" diyordu; ölçüm `TraconWorkflowFunctionExtensions.cs:118`'in
...
- **`FallbackChatClient`'ın hiçbir fonksiyonel (DI+HTTP+akış sınırı) testi yok**
  ...
  (MT-MYU-015/016) kanıtlıyor. `tests/Tracon.AspNetCore.FunctionalTests/`
