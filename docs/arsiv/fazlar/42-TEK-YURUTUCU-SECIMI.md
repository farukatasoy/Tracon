...
> **Önkoşul:** Yok
> **Paketler:** `Tracon.Abstractions`, `.Core`, `.Sql.Shared`, `.PostgreSql`, `.SqlServer`, `.Sqlite`, `.Mcp`
...
Tracon bugün **tek örnekli** çalışmayı varsayıyor. İki veya üç replika ile dağıtıldığında bazı işler sessizce N katına çıkıyor: her replika kendi MCP keşfini yapıyor, kendi sağlık yoklamasını gönderiyor ve kendi bellek içi durumunu tutuyor. Bu faz, "bu işi kümede yalnızca **bir** örnek yapsın" diyebilmeyi getirir.
...
- [x] Migration üç sette de uygulandı; numaralar sağlayıcı başına bağımsız
      ...
      `samples/Tracon.Api` çalıştırıldığında SQLite üzerinde "Tracon 7
...
İki `samples/Tracon.Api` süreci aynı SQLite dosyasına (`Data Source=demo3.db`)
bağlandı, `Tracon:SingletonExecution:Enabled=true`,
...
```
$ sqlite3 demo3.db "SELECT name, owner_id, expires_at, updated_at FROM tracon_singleton_leases;"
...
`tracon_model_provider_health`'i (sağlık önbelleği DB'ye yazmaz, ama
...
TRACON__SQLITE__CONNECTIONSTRING="Data Source=demo.db" \
  TRACON__SINGLETONEXECUTION__ENABLED=true \
  TRACON__SINGLETONEXECUTION__LEASEDURATION=00:01:00 \
  dotnet artifacts/bin/Tracon.Api/release/Tracon.Api.dll --urls http://localhost:5081 &
TRACON__SQLITE__CONNECTIONSTRING="Data Source=demo.db" \
  TRACON__SINGLETONEXECUTION__ENABLED=true \
  TRACON__SINGLETONEXECUTION__LEASEDURATION=00:01:00 \
  dotnet artifacts/bin/Tracon.Api/release/Tracon.Api.dll --urls http://localhost:5082 &

# Kira kimde
sqlite3 demo.db "SELECT name, owner_id, expires_at FROM tracon_singleton_leases;"

# Birinci sureci durdur (PID'i yukaridaki & ciktisindan al), devralma suresini olc
kill <PIDA>
watch -n 2 'sqlite3 demo.db "SELECT owner_id, expires_at FROM tracon_singleton_leases;"'

# Is kuyrugu degismedi mi
git diff --stat src/Tracon.Core/Scheduling/JobWorkerBackgroundService.cs
```
...
çalıştırır, `dotnet run` değil. Ayrıca `TraconMcpOptions` (MCP
...
1. **`SingletonGuard` `internal` değil `public` oldu** (Açık Soru 4'ün önerisi
   ...
   `Tracon.Mcp` derlemesinde yaşıyor, `SingletonGuard` ise
   `Tracon.Core`'da. `InternalsVisibleTo` yalnız
...
2. **`SingletonExecutionOptionsValidator` plana yazılı değildi**, ama her
   `Tracon*Options` sınıfının `IValidateOptions<T>` alma deseni
...
3. **K-278 ("çağıranın verdiği metin tek başına birincil anahtar olamaz")
   ...
   (bir kiracıdan) değil, Tracon'in **kendi kodundan** gelir (`"mcp-discovery"`,
...
4. **MCP keşif aralığının (`TraconMcpOptions.RefreshInterval`)
   ...
   (`TraconMcpBuilderExtensions.UseMcp` yalnız kod-taraflı `configure`
...
- `SingletonGuard` **public**'tir (`Tracon.Core`); herhangi bir paket
...
1. **`TraconMcpOptions` (dolayısıyla `RefreshInterval`)
   `IConfiguration`'dan bağlanmaz** — yalnız `.UseMcp(o => ...)` delegesiyle
   koddan verilir. `TRACON__MCP__REFRESHINTERVAL` gibi bir ortam
...
4. **`TraconMcpOptions`'ı `IConfiguration`'a bağlamak** — yukarıdaki
