...
> **Önkoşul:** Yok
> **Paketler:** `src/` **değişmiyor**. İş `tests/Tracon.Package.Tests` (bugünkü `Tracon.Templates.Tests`) içindedir.
...
Tracon bir NuGet paket ailesidir, ama bugün hiçbir kapı **bir paket tüketicisinin gerçek bir `run` koşturabildiğini** kanıtlamıyor. Bu faz o tek boşluğu kapatır: `.nupkg`'lerden `PackageReference` ile beslenen bir tüketici projesi üretilir, içinde ağa çıkmayan bir model ile gerçek bir `run` koşulur ve `run` kaydı doğrulanır.

## Bitiş Ölçütleri (DoD)

- [x] `dotnet test tests/Tracon.Package.Tests` yeşil; `ConsumerRunTests` alt sürecin `stdout`'unda `OK run=` satırını doğruluyor
...
- [x] `Tracon.Templates.Tests` → `Tracon.Package.Tests` yeniden adlandırması tamam; `Tracon.slnx` ve `Tracon.no-docker.slnf` güncel; `docs/arsiv/` **değişmedi**
...
dotnet add package Tracon --version <packed> --source <repo>/artifacts/package/release
...
grep -rln "Tracon\.Templates\.Tests" --exclude-dir=.git --exclude-dir=arsiv . 
```
...
- **95.3'ün taban çizgisi elle yazılmadı, ölçüldü.** Plan `Google.GenAI`'ın
  ...
  SDK 10.0.100) `Tracon.Google` şekli için **39** paket kimliği verdi (meta
  `Tracon`: 71, `Tracon.Core`: 30). Fark bir kusur değil — paket
...
- **`packages.md`'ye eklenen "What enters your graph if you opt in" bölümü
  ...
  taşıyordu; `Baselines/transitive-dependencies.txt`'in `# Tracon`
  bölümü `OpenAI` paket kimliğini (SDK'nın kendisi) taşıdığı için cümle
  yanıltıcıydı. Cümle kaldırıldı; yalnız ölçülen `Tracon.Google` kalemi
  kaldı.
...
- **🟡 1 — `packages.md`'nin yeni bölümü ölçülmeyen bir iddia taşıyordu**
  ...
  fazın ölçtüğü taban çizgisi dosyası bunu doğrulamıyordu, çünkü `Tracon`
  meta şeklinin listesi `OpenAI` SDK'sının kendisini de içeriyordu).
  **Sonuç: düzeltildi** — iddia kaldırıldı, yalnız ölçülen `Tracon.Google`
  kalemi bırakıldı.
...
- `tests/Tracon.Package.Tests` artık paket tüketimi ve şablon testlerinin
...
- 🚨 Tam çözüm test koşumunda (`dotnet test Tracon.slnx`) tek bir
...
- 🚨 Bu makinedeki `ap-pg` konteynerinin `tracon` şeması hâlâ eski bir
  migration checksum'ı taşıyor (Faz 94'ten devralınan durum, bu fazda da
  gözlemlendi): `samples/Tracon.Api`'yi varsayılan `Tracon:PostgreSql:SchemaName`
  ile başlatmak `0032_tenant_provider_bindings` checksum hatasıyla çöker.
  Bu fazda `Tracon__PostgreSql__SchemaName` ortam değişkeniyle geçici bir
  şema (`tracon_faz95_probe`) kullanılarak atlatıldı. Konteyner henüz
  tazelenmedi.
