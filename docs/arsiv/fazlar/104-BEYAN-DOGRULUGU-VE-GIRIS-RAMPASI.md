...
> **Önkoşul:** Yok
> **Paketler:** `Tracon.Core` (uyarı servisi + kalıcılık yargısının tek kaynağı) · `Tracon.AspNetCore` (yalnız `/api/meta` o tek kaynağa bağlanır)
> **Yeni paket:** Yok — `Microsoft.Extensions.Hosting.Abstractions` `Core`'un mevcut bağımlılığıdır ([`Tracon.Core.csproj:93`](../../../src/Tracon.Core/Tracon.Core.csproj)) · **Migration:** Yok
...
> **Tüketici yüzeyi:** site — [`guides/production.md`](../../../docs-site/src/content/docs/guides/production.md) (ölçekleme bölümüne hız sınırı kapsamı), [`concepts/governance.md`](../../../docs-site/src/content/docs/concepts/governance.md) (kiracı yalıtımının nerede zorlandığı), `api/Tracon.TraconRateLimitOptions.md` (**üretilir** — iş XML dokümanındadır) · sevk edilen — `TraconRateLimitOptions` XML dokümanı. Kök `CONTRIBUTING.md` ve `ARCHITECTURE.md` **pakete girmez**, depoya gelen katkıcıya dönüktür
...
- [x] ✅ `Production` + varsayılan store ile kalkışta **tam bir** uyarı düşer;
  `Development`'ta ve Sqlite bağlıyken **düşmez**. Üç koşum, `samples/Tracon.Api`:
...
- [x] ✅ `samples/Tracon.Api` ile gerçek `run` yapıldı: `POST /api/agents/support/run` SSE ile akıttı, üç `run` kaydı `Completed` olarak listelendi
...
ASPNETCORE_ENVIRONMENT=Production dotnet run --project samples/Tracon.Api 2>&1 | grep -i "not persistent"
ASPNETCORE_ENVIRONMENT=Development dotnet run --project samples/Tracon.Api 2>&1 | grep -ci "not persistent"   # 0

# Uc ile log ayni yargiyi verir
curl -s http://localhost:5081/tracon/api/meta | python3 -c "import sys,json; print(json.load(sys.stdin)['storage']['persistent'])"

# Kopya ifade kalmadi
grep -n "InMemoryRunStore\|InMemorySessionStore" src/Tracon.AspNetCore/Endpoints/MetaEndpoints.cs
...
iki gerçek boşluk girdi (`TraconRateLimitOptions` XML'i ve
...
kaydeder, ve `AddTracon()` çıplak bir `ServiceCollection` üzerinde de
...
düştü. Çözüm `TraconDrainService`'in emsalidir: opsiyonel bağımlılık +
...
| **K-624** 👤 | `Production` + kalıcı olmayan store **uyarır**, hata fırlatmaz, susturma seçeneği yoktur. `TraconDiagnosticsReport`'a alan bilinçli olarak **eklenmedi** (`required` alan yayından sonra kırıcıdır) |
...
| 6 | 🟡 | Log **seviyesi** ölçülmüyordu; `AllText.ShouldContain("Warning")` başka bir satırla da yeşil kalırdı | **Düzeltildi.** Eşleşen tek girdi üzerinde `ShouldStartWith("Warning Tracon.NonPersistentStorageWarningService")` |
...
- `StorePersistence` (internal, `Core`) kalıcılık yargısının **tek kaynağıdır**.
  ...
  `grep -rn "InMemoryRunStore" src/Tracon.AspNetCore/` → **sıfır**.
...
- 🚨 **`Core` içinde host-only bir servise bağımlı `IHostedService` yazma.**
  ...
  `AddTracon()` çıplak `ServiceCollection` üzerinde de geçerlidir. Parametreyi
  ...
  (`TraconDrainService`, `NonPersistentStorageWarningService`).
