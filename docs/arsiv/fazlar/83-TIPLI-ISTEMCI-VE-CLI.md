...
> **Paketler:** `Tracon.Client` (**yeni**), `Tracon.Cli` (**yeni**)
...
> · sevk edilen: `src/Tracon.Client/README.md` ve `src/Tracon.Cli/README.md` (**yeni**, `PackageReadmeFile` zorunlu), `TraconClientOptions` XML dokümanı, kök `README.md` paket tablosu. `api/` ve `http-api/` **üretilir**
...
Tracon'in yönetim API'sini bugün yalnız elle yazılmış HTTP çağrısıyla kullanabilirsin. Tipli bir yol yoktur. Ayrıca migration **yalnız uygulama açılırken** uygulanır; bir CI/CD hattı şemayı ayrı bir adımda hazırlamak isterse çalıştıracak araç yoktur — `MigrationHostedService`'in kendi XML dokümanı bu yolu **vaat ediyor** ama aracı yok.

## Bitiş Ölçütleri (DoD)

- [x] `tracon migrate --provider sqlite --connection "Data Source=<gerçek dosya>"` boş bir veritabanında şemayı kurar (**22 applied**, ölçüldü); `postgres` yerine `sqlite` ile doğrulandı — gerekçe: Devir Notu. `AutoApplyMigrations=false` ile açılan `samples/Tracon.Api` senaryosu **koşulmadı** (bkz. aşağıdaki "gerçek run" satırı)
- [x] `tracon migrate` ikinci kez koştuğunda `0 applied` der ve çıkış kodu `0`'dır — `MigrateCommandTests.Migrate_run_a_second_time_is_idempotent`
- [x] `tracon health --url <adres>` ayakta bir sunucudan sağlık durumu okur (gerçek Kestrel dinleyicisine karşı, `HealthCommandTests`); sunucu kapalıyken **sıfırdan farklı** çıkış kodu döner ve asılı kalmaz — `A_server_that_is_not_running_fails_fast_instead_of_hanging` (port `1`, anında ret)
...
- [x] 🚨 AOT iddiası **ölçülmüştür**: `Tracon.Client`'ın AOT sözünden **vazgeçildi** — `TraconAotCompatible=false`, gerekçe csproj yorumunda ve K-NNN'de. `ClientAotPublishTests` bu yüzden **yazılmadı**
- [x] 🚨 Özel önekle (`MapTracon("control")`) istemci çalışır — `HealthCommandTests.Reads_health_through_a_custom_MapTracon_prefix` (ayrı bir `ClientPrefixTests` sınıfı yerine burada, gerçek host'a karşı)
- [x] `DependencyDirectionTests` iki yeni anahtarla yeşil; iki paket meta pakette **değil**
- [x] İki paketin `README.md`'si vardır, `Tracon.slnx`'e eklenmiştir, `dotnet pack` ikisini de üretir (`Tracon.Client.*.nupkg`, `Tracon.Cli.*.nupkg` — ölçüldü)
- [x] `dotnet tool install -g` ile kurulan tool `tracon --help` çıktısını verir — gerçekten kuruldu, doğrulandı, sonra kaldırıldı
...
2. **`samples/Tracon.Api` ile "gerçek run" koşulmadı.** Uygulama
   ...
   Doğrulama bunun yerine `tests/Tracon.Cli.FunctionalTests` (gerçek
   Kestrel dinleyicisi, 15 test) ve `tests/Tracon.AspNetCore.FunctionalTests/ClientTenantScopeTests.cs`
...
tracon migrate --provider sqlite --connection "Data Source=/tmp/ap.db"
tracon migrate status --provider sqlite --connection "Data Source=/tmp/ap.db"

# Istemci ayakta bir sunucuyu okuyor mu
cd samples/Tracon.Api && dotnet run &        # http://localhost:5080/tracon
tracon health --url http://localhost:5080/tracon --json
...
1. **Manuel test dosya numarası `33` değil `34`.** Plan "bugün son sıra 32"
   ...
   olarak açıldı.
2. **`AddTraconClient` `IHttpClientBuilder` değil `IServiceCollection`
...
3. **AOT sözünden vazgeçildi.** §83.6'nın flagladığı risk gerçekleşti:
   `-p:TraconAotCompatible=true` 146 IL2026/IL3050/IL2075 tanısı üretti
   (42 çağrı noktası × 3 TFM). `TraconAotCompatible=false`. K-567.
...
5. **Enum dönüştürme iki aşamalı tasarlandı, tek aşamaya indi.** İlk tasarım
   ...
   "düz" enum için `TraconClientJsonContext`'in global `Converters`
...
7. **`samples/Tracon.Api` ile "gerçek run" koşulmadı — bkz. DoD tablosunun
...
1. **`http-api` kuralı** (`CatalogEndpoints.cs` değişti → `http-api.md`
   ...
   göremez. Değişikliğin gerçek kaynağı olan `docs/openapi/tracon.json`
...
2. **`cekirdek-kavram` kuralı** (yeni `IMigrationApplier.cs` → `concepts/`
   ...
   çözümdür — `tracon` CLI'sinin `MigrationRunner`'ın linked-source
   ...
   çağırmaz (`internal` değildir ama `Tracon.Cli`'nin DI çözümünün
...
> kurduğu üç şey oraya devredilir: (1) `docs/openapi/tracon.json`'ın
> üretim kaynağı olması, (2) `/tracon` önekini soyan dönüşüm adımı
> (§83.3), (3)
> `operationId` kapsama kapısının şekli (§83.2).
>
> 🚨 **Faz 84 için 2026-08-21'de ölçülen kanıt, burada saklanıyor ki
> kaybolmasın:** `src/Tracon.UI/frontend/src/lib/types.ts` **1 882 satır ·
...
> brotli** (`src/Tracon.UI/wwwroot/assets/`, ölçüldü) ve bütçe 250 KB
