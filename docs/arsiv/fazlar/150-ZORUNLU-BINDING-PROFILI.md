...
> **Önkoşul:** Yok
> **Paketler:** `Tracon.Core` · kanıt testi `Tracon.Core.UnitTests`
> **Yeni paket:** Yok · **Migration:** Yok
> **Public API:** Büyüyor — `ITraconBuilder`'a bir metot + bir istisna tipi (Açık Soru 2). `wc -l src/*/PublicAPI.Shipped.txt` → 17 satır / 17 dosya (yalnız başlık), **shipped giriş sıfır**: bugün eklemek bedava, Faz 7'den sonra bir sürüm kararı
> **Tüketici yüzeyi:** `docs-site/`: `guides/embedding.md`, `guides/production.md`, `concepts/governance.md`, `capabilities.md` · sevk edilen: `ITraconBuilder` XML `<example>`, `src/Tracon.Core/README.md`
...
Tracon yedi genişleme noktasını `TryAdd` ile kaydeder: tüketici bir şey kaydetmezse yerleşik varsayılan çalışır ve kurulum **sessizce** açılır. Bu K1'in ("sıfır sürpriz") doğru sonucudur — ama bir güvenlik profili için yanlış varsayılandır.
...
- [x] Kayıt `AddTracon`'den sonra yapılırsa host **başlamaz** (`TryAdd` senaryosu)
- [x] Hata mesajı üç bilgiyi taşır: hangi sözleşme · hangi tip çözüldü · nasıl düzeltilir
- [x] Kontrol HTTP'siz host'ta da çalışır — `MapTracon` çağırmayan bir host'la kanıtlandı
...
dotnet run --project samples/Tracon.Api 2>&1 | head -5
...
### 1 — 🚨 Planın `TryAdd` iddiası YANLIŞTI: `AddTracon`'den SONRA yapılan bir `Add*` kaydı KAZANIR

Plan (Hata Modları tablosu ve Manuel Case 4) şunu iddia ediyordu: *"Tüketici
kaydı `AddTracon`'den sonra yapılır ve `TryAdd` onu ezmez → host
...
kaydını `TryAdd` ile yapar, Tracon'in varsayılanı slotu zaten tutuyordur ve
...
- İki fonksiyonel test bunu iki yönden kilitler:
  `A_TryAdd_registration_made_after_AddTracon_still_stops_the_host` ve
  `An_Add_registration_made_after_AddTracon_wins_and_the_host_starts`.
- Aynı yanlış cümle **sevk edilmiş iki yerde** de yazılıydı ve düzeltildi:
  ...
  ignored"*) ve `samples/Tracon.Embedded/Program.cs` yorumu. `ITraconBuilder`'ın
...
### 2 — Yedi sözleşme tablosu TEK KAYNAĞA alındı (`TraconExtensionPoints`)

Plan yalnız doğrulayıcıyı istiyordu; ama "yerleşik varsayılan mı" sorusunu
`TraconDiagnosticsCollector` de yanıtlıyordu. İki yerde elle tekrarlanan
...
bayatlar. Yeni `TraconExtensionPoints` tablosu yediyi bir kez tanımlar;
...
`provider.GetServices(Type)` `RequiresDynamicCode`'dur ve `Tracon.Core` AOT
...
### 5 — Kapı `IHost` GEREKTİRİR; `Tracon.Cli` kapsam dışıdır

Planın risk satırı `AddTracon` çağıran giriş noktalarının sayılmasını
istiyordu. Sayıldı: kütüphane içinde tek bir yer bir host olmadan kap kurar —
`src/Tracon.Cli/Commands/SqlProviderSelector.cs:42`, `AddTracon()` +
...
### 6 — `samples/Tracon.Embedded` dört zorunlu binding ilan ediyor
...
| Karar | Gerekçe |
|---|---|
| **K-698** — `RequireCustomBinding<T>()` serbest generic'tir; yedi sözleşmenin kapalı kümesi **çalışma anında** zorlanır, derleme anında değil *(kullanıcı kararı)* | İşaretçi arayüz (`ITraconExtensionPoint`) kümeyi zaten KAPATMAZ: tüketicinin kendi sınıfı da onu uygularsa generic kısıttan geçer. Buna karşılık yedi public sözleşmeyi bir iç taksonomi için değiştirmeyi ve `IAttachmentStorage` gibi bağımsız bir depolama sözleşmesini Tracon'in sınıflandırmasına bağlamayı gerektirir. Tanınmayan tip host başlangıcında yediyi listeleyen açık bir hata verir. |
| **K-699** — İhlal `InvalidOperationException` atar; `TraconException` ailesine yeni tip eklenmez *(kullanıcı kararı)* | Emsal aynı repoda ve aynı hata sınıfıdır (`TraconRolePolicies.cs:84`, yanlış kompozisyon). Tüketicinin yakalayacağı bir şey değildir — host zaten başlamaz — ve yeni bir istisna tipi public yüzeyi bedelsiz büyütür. |
...
|---|---|---|
| MT-DIAG-058 | `samples/Tracon.Api`, değiştirilmemiş | `Application started` 1 · `required custom binding` **0** |
...
| MT-DIAG-060 | `samples/Tracon.Embedded`, dört zorunlu binding | `Application started` 1 · hata **0** |
...
| MT-DIAG-064 | `MapTracon` **ve** tool handler yorumda | `exit=134` · `AllowAllToolAuthorizationHandler` — HTTP yüzeyi olmadan |
...
Tracon registers IRunAuthorizationHandler with TryAdd, so a TryAdd registration
made after AddTracon() is dropped and the built-in default stays bound.
```

Gerçek `run`: `samples/Tracon.Api` üzerinde `faz150` agent'ı oluşturuldu ve
...
| 🔴 3 | `Tracon.AgentMap.md` 10297 B, tavan 10240 B; üretilen üç dosya `capabilities.md` ile eşleşmiyordu | Bulgu 1 kapanınca harita yeniden üretildi: **10239 B**, `check-content.mjs` temiz |
...
| 🟡 5 | Yokluk mesajı "Tracon has no built-in default" diyordu; `IAttachmentStorage` için bu, teşhis raporunun `"(database)"` ve `capabilities.md`'nin "content stays in the database" ifadeleriyle çelişiyordu | İddia **düşürüldü**; mesaj yalnız olguyu söylüyor: `nothing is registered for it` |
| 🟡 6 | Planın "giriş noktalarını say" risk satırının sonucu hiçbir yere yazılmamıştı | Sapma 5 olarak yazıldı: `Tracon.Cli/Commands/SqlProviderSelector.cs` `IHost` kurmaz, kapı orada koşmaz |
| 🟡 7 | "Örnek uygulamayla gerçek `run`" DoD satırının kanıtı yoktu | Yukarıdaki "Örnek Uygulama Koşumu" bölümü eklendi |
| 🟢 8 | Kayıt yorumu "ahead of every other `IHostedService`" diyordu; bu yalnız Tracon'in kendi hosted service'leri için doğru | Yorum daraltıldı: tüketicinin `AddTracon`'den önce kaydettiği hosted service önce başlar ve o sıra host'un kendi tercihidir |
...
- 🚨 **`AddTracon`'den SONRA yapılan `Add*` kaydı KAZANIR; yalnız `TryAdd*`
...
- **Yedi genişleme noktası artık TEK bir tabloda yaşıyor**
  (`src/Tracon.Core/Diagnostics/TraconExtensionPoints.cs`). Sekizinci
...
- **Kapı `IHost` gerektirir.** `IHostedService`'tir; `AddTracon()` çağırıp
  `BuildServiceProvider()` ile duran bir giriş noktası (bugün yalnız
  `Tracon.Cli/Commands/SqlProviderSelector.cs`) kapıdan geçmez. Yeni bir
...
- **Kapı bir kompozisyon kapısıdır, bir güvenlik kanıtı değildir.** XML, site ve
  bu doküman bunu üç kez yazıyor; "Tracon yetkilendirmeyi garanti ediyor"
