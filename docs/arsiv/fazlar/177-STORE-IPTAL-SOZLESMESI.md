...
| Yazma fırlatıyor ama **yarım satır bırakıyor** | Sözleşme | `Canceled_token_throws_on_write_and_leaves_no_trace` |
| SQL store beklenenden farklı bir istisna fırlatıyor | Sözleşme | Aynı case, üç sağlayıcı koşumunda |
| `ThrowIfCancellationRequested` yanlış yere konuyor — iş yapıldıktan **sonra** | Sözleşme | Yukarıdaki case yakalar (fırlatma yarısı yakalamaz); `StoreCancellationContractSelfProofTests` bunu kalıcı kanıtlar |
| 🚨 **Geri okuma, yazılanı hiç göremiyor** — "iz bırakmadı" iddiası tautolojiye dönüyor | Sözleşme | `The_write_this_contract_checks_can_be_read_back` (sapma 11; denetim bunu **iki** sözleşmede buldu) |
...
public abstract class StoreCancellationContract<TStore> : IAsyncLifetime
{
    ...
    [Fact] public Task Canceled_token_throws_on_read();
    [Fact] public Task Canceled_token_throws_on_write_and_leaves_no_trace();

    // Sapma 11 — iptal iddiası DEĞİL, onun kapısı: canlı token'la yazıp
    // WroteAnythingAsync()'in yazılanı gerçekten gördüğünü ister.
    [Fact] public Task The_write_this_contract_checks_can_be_read_back();
}

// DEĞİŞTİ — taban sınıf ve iki hook imzası
public abstract class TenantIsolationContract<TStore> : StoreCancellationContract<TStore>
{
    ...
    // ExistsAsync / TryDeleteAsync / TryOverwriteAsync değişmedi
}

// YENİ case — akış yüzeyi (K-794)
public abstract class RunStoreContract : TenantIsolationContract<IRunStore>
{
    [Fact] public Task Canceled_token_throws_on_the_first_step_of_the_event_stream();
}
```

`IdempotencyStoreContract`, `RetentionStoreContract`, `RunInputStoreContract` ve
`SingletonLeaseStoreContract` artık `StoreCancellationContract<T>`'ten türer;
kendi `Store` / `CreateStoreAsync` / `InitializeAsync` / `DisposeAsync` /
`OnDisposeAsync` üyeleri **kaldırıldı** (imzaları aynı, sahibi değişti).

Public tip sayısı: 48 → **49** (`Tracon.Testing.Contracts.Xunit`).
`PublicAPI.Shipped.txt` **ölçüldü: 17 bayt** (`#nullable enable` satırından ibaret)
— on yedi paketin hepsinde. K-603 gereği preview hattı boyunca boş kalıyor, yani
bu genişleme bugün **uyumluluk sözü kırmıyor**; yayından sonra aynı iş kırıcı
olurdu.
...
├── Contracts/StoreCancellationContract.cs   (YENİ — ortak taban, 2 case, 3 hook)
├── Contracts/TenantIsolationContract.cs     (taban değişti; Seed/Count token aldı)
├── Contracts/RunStoreContract.cs            (+ akış case'i)
├── Contracts/TraceStoreContract.cs          (+ kendi okuma VE yazma/geri-okuma hook'u — 🔴 1)
├── Contracts/WorkflowCheckpointStoreContract.cs (+ kendi geri-okuma hook'u — 🔴 2)
├── Contracts/ToolInvocationContract.cs      (+ kendi yazma hook'u — 🟡 4)
├── Contracts/{Idempotency,Retention,RunInput,SingletonLease}StoreContract.cs
│                                            (taban değişti; 3 hook + lifecycle kaldırıldı)
├── Contracts/EvalStoreContract.cs           (emsal case KALDIRILDI — tabana taşındı)
├── Contracts/RunScoreStoreContract.cs       (emsal case KALDIRILDI — tabana taşındı)
├── Contracts/<24 sözleşme daha>             (Seed/Count token aldı ve store'a geçirdi)
└── PublicAPI.Unshipped.txt

src/Tracon.Core/**/InMemory*.cs              (30 dosya · 177 iptal kontrolü)
src/Tracon.Core/Retention/NullRetentionStore.cs (4 kontrol)
src/Tracon.Core/Privacy/NullDataSubjectStore.cs (3 kontrol — 🟡 5)

samples/Tracon.Samples.FileRunStore/         (2 dosya · 18 kontrol)

tests/Tracon.Core.UnitTests/Contracts/StoreCancellationContractSelfProofTests.cs (YENİ)
tests/Tracon.Core.UnitTests/Architecture/public-surface-baseline.txt

docs-site/src/content/docs/guides/write-your-own-store.md  (yeni bölüm)
docs/manuel-test/21-DAYANIKLILIK-VE-IPTAL.md               (MT-RES-091, 092)
