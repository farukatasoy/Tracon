...
- **`kalicilik`** ve **`paket-tanimi`** (`AgentPrism.Sqlite.csproj` değişti → `persistence.md`/`packages.md` beklenir): tek değişiklik `AgentPrism.Benchmarks` için bir `InternalsVisibleTo` girdisidir — `.UnitTests`/`.IntegrationTests`/`.FunctionalTests` konvansiyonunun beşinci üyesi. Paketin bağımlılık grafiği, sürümü veya davranışı değişmedi.
