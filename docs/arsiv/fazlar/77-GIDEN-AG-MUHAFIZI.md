...
> **Paketler:** `Tracon.Abstractions`, `Tracon.Core`, `Tracon.Mcp`, `Tracon.AspNetCore`
...
> **Tüketici yüzeyi:** `docs-site/src/content/docs/getting-started/security.md` (giden ağ bölümü) · `docs-site/src/content/docs/guides/production.md` (yeni ayar) · sevk edilen: `TraconEgressOptions` XML dokümanı, `capabilities.md` satırı
...
Tracon bugün **üç** yerden dışarı ağ isteği atar: webhook teslimi, MCP sunucu bağlantısı ve model sağlayıcı çağrısı. SSRF koruması yalnız **birincide** vardır. Bu faz korumayı üçüne de taşır ve `secret`'ın yanlış hedefe gitmesini engelleyen yapılandırma anahtarı kısıtını eksik iki yüzeye yayar.
...
curl -s -X PUT http://localhost:5081/tracon/api/mcp-servers/probe \
...
| 2 | `EgressSocketGuard(IOptionsMonitor<TraconEgressOptions>)` tek kurucu | İkinci kurucu eklendi: `EgressSocketGuard(Func<EgressAddressPolicy>)`, ve yeni bir public tip doğdu: `EgressAddressPolicy` | Webhook'un politikası **iki boyutludur** — `AllowInsecureHttp` yalnız loopback açar, `AllowPrivateNetworkTargets` tüm özel ağı. Tek `bool` bunu modelleyemezdi; iki `bool` parametresi ise çağrı yerinde okunmuyordu |
| 3 | MCP önek ayarı adı geçmiyordu; doğal yer `TraconMcpOptions` | Ayar `Tracon.Abstractions`'ta yeni bir tipe kondu: `TraconMcpSecurityOptions` | `Tracon.AspNetCore` `Tracon.Mcp`'yi **görmez** (yalnız `Core`'a bağlıdır). Kural iki pakette birden zorlanır; ayarı Mcp'de bırakmak iki kaynak üretirdi. `SectionName` aynı (`Tracon:Mcp`). K-534 |
...
| **K-530** | `Tracon:Egress:AllowPrivateNetworkTargets` varsayılanı KAPALI; üç yüzeyde de özel ağ reddedilir 👤 |
...
| **K-534** | MCP önek ayarı `Abstractions`'ta yaşar (`TraconMcpSecurityOptions`) |
| **K-535** | Webhook ek başlıkları Tracon'in kendi başlık adlarını taşıyamaz |
...
**`samples/Tracon.Api` ile gerçek koşum** (2026-08-20, DoD gereği):

| Komut | Sonuç |
|---|---|
| `PUT /api/mcp-servers/probe` ← `http://169.254.169.254/` | `400` · `The target resolves to a private network address (169.254.169.254); set 'Tracon:Egress:AllowPrivateNetworkTargets' to true to allow it.` |
| Aynısı, `Tracon__Egress__AllowPrivateNetworkTargets=true` ile | `200` — **ayarın gerçekten bağlandığını** kanıtlar (K-406 sınıfı) |
| `PUT /api/mcp-servers/probe` ← `authorizationConfigurationKey: ConnectionStrings:Default` | `400` · `... may only reference a configuration key under 'Tracon:McpSecrets:'.` |
| Aynısı, `Tracon:McpSecrets:Token` ile | `200` |
| `PUT /api/tenants/acme/providers/anthropic` ← `http://10.0.0.5/` | `400` · ayar adını içerir |
| `PUT /api/webhooks/orders` ← `secretConfigurationKey: ConnectionStrings:Default` | `400` · `... under 'Tracon:WebhookSecrets:'.` |
...
| 9 | 🟢 | İki test Tracon hakkında hiçbir şey iddia etmiyordu (`Non_ip_families_are_not_private` ölçüldü: `new IPAddress(new byte[]{1,2,3,4})` zaten `InterNetwork`; `Address_family_of_a_parsed_literal_is_preserved` BCL'i doğruluyordu) | **Silindi.** Kapsam iddiasını şişiriyorlardı |
...
4. **`docs/hafiza/aspnetcore-di.md` %4 boş (15.297 / 16.000).** Bütçe içindedir
   ...
   kullanıcı kararıdır.
5. **`Tracon:Egress` bugün tek ayar taşıyor.** İkinci ayar adayı hazır ve
