...
> **Paketler:** `Tracon.Core` (yalnız `Evaluation/`)
...
- [x] Yeni tablo **yok**, migration **yok** (`ls src/Tracon.*/Migrations/` değişmedi) — doğrulandı
...
- Bu ortamda `Tracon:OnlineEvaluation` `dotnet user-secrets` ile açılmadığı sürece varsayılan kapalıdır (K1) — canlı bir retry senaryosunu uçtan uca gözlemlemek isteyen sonraki oturum önce bunu açmalı.
