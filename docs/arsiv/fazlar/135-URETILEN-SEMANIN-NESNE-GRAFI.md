...
> **Public API:** büyümüyor — üretilen şema metni ve derleme anı davranışı değişir. İki yeni analyzer diagnostic kodu eklenir (`TRC0011`, `TRC0012`)
> **Tüketici yüzeyi:** `docs-site/`: `guides/write-your-own-tool.md`, `concepts/tools.md`, `capabilities.md` (diagnostic bağlantısının hedef bölümü) · sevk edilen: `TRC0003` metni, yeni `TRC0011`/`TRC0012` metinleri, `AnalyzerReleases.Unshipped.md`
...
- [x] `[JsonSerializable]` eksikse `TRC0011` ile derleme **durur** ve eksik tipin adı yazılır (case 4)
- [x] Derinlik aşımı ve cycle `TRC0012` üretir; generator asılmaz (case 5–6)
- [x] `TRC0003` metni güncellendi; "never expresses a nested object" iddiası kalktı
...
2. **Üye taşımayan bir nesne (`{}` üretmesi gereken) desteklenmiyor — bu, planın
   ...
   davranıştan (TRC0003 ile red) daha kötü bir kusur sınıfı. Karar: ikisi de
   `TRC0003` ile reddedilir; "Kapsam dışı - bilerek" tablosuna bu kalem
...
3. **Nesne/nesne-dizisi şeklindeki bir parametrede (veya üyede) HERHANGİ bir
   `DataAnnotations` kısıt attribute'u her zaman `TRC0010` ile "uygulanmaz"
...
| 4 | Plan'ın hata modları tablosu üye taşımayan bir nesnenin `{}` ürettiğini vaat ediyordu; kod bunu reddediyor (`TRC0003`) | 🟡 | **Gerekçelendi** — bkz. "Plandan Sapmalar" #2; kod DEĞİŞTİRİLMEDİ, çünkü düzeltme sapma #1'deki henüz-desteklenmeyen ikinci şekille çakışıp gerçek property'leri sessizce düşürme riski taşırdı |
...
kayması yok), 3.6 (plan dışı public API yok), TRC0011'in graf hatası
