...
> **Paketler:** `Tracon.Sql.Shared` (bağlı kaynak, K-176), `Tracon.PostgreSql`, `Tracon.SqlServer`, `Tracon.Sqlite`
...
- [x] Faz başında üç dialect'in 199 sorgusu için anlık görüntü taban çizgisi üretildi (`tests/Tracon.Sql.Shared.UnitTests/Baselines/sql-text-baseline.*.txt`, 200 satır/dosya = 199 sorgu + `Schema`); commit henüz yapılmadı (kullanıcı istemedikçe commit edilmiyor, `AGENTS.md` kuralı)
...
- [x] Sözleşme testleri (`tests/Shared/Contracts/`) üç sağlayıcıda da yeşil — `Tracon.PostgreSql.IntegrationTests` 638/638, `Tracon.SqlServer.IntegrationTests` 574/574, `Tracon.Sqlite.IntegrationTests` 592/592 (hepsi gerçek veritabanına/dosyaya karşı, bu oturumda 94.2/94.3/94.4/94.4.3'ün her adımından sonra tekrar koşuldu)
- [x] SQLite tablo nitelendirmesi **noktasız** kalır — `sql-text-baseline.sqlite.txt` elle kontrol edildi (`tracon_mcp_servers`, nokta yok); `Tracon.Sqlite.IntegrationTests` yeşil
- [x] Dört doğrulama kapısı sıfır uyarı verir — `python3 scripts/kapi.py kapanis --taban 7f8ee9b` TAM koşuldu (tarama, dokuman-bakim, scripts unittest, agent-map, denetim-paketi, `dotnet build`, `dotnet test Tracon.slnx` [tüm proje ağacı], `dotnet pack`, `dotnet format --verify-no-changes`, `docs-site npm run check`) — hepsi ✅, çıkış kodu 0
- [x] `samples/Tracon.Api` ile gerçek `run` yapıldı, çıktı belgeye yazıldı — PostgreSQL (`ap-pg`, taze `tracon_p94` şeması) ile, bkz. `docs/manuel-test/03-KALICILIK-POSTGRESQL.md` MT-PG-067
...
wc -l src/Tracon.SqlServer/Internal/SqlServerQueries.cs \
      src/Tracon.PostgreSql/Internal/PostgresQueries.cs \
      src/Tracon.Sqlite/Internal/SqliteQueries.cs
...
grep -n "reader, [0-9]\+\|IsDBNull([0-9]\+)" src/Tracon.Sql.Shared/Stores/SqlRunStore.cs | sed -n '1,40p'
...
dotnet test tests/Tracon.Sqlite.IntegrationTests -c Release
```
...
5. **`SqlQueryCompletenessTests` ve `SqlTextSnapshotTests` planın öngördüğü
   ...
   (`tests/Tracon.Sql.Shared.UnitTests`) toplandı — Açık Soru 3'ün ölçülen
   cevabı.** Ölçüldü: `Tracon.PostgreSql.IntegrationTests` gibi bir
   ...
   `Tracon.Sql.Shared.UnitTests`'e özel bir `InternalsVisibleTo` eklendi
...
- `tests/Tracon.Sql.Shared.UnitTests` Docker'sız SQL doğruluk testleri
...
- 🚨 Bu makinedeki `ap-pg` konteynerinin `tracon` şeması ESKİ bir migration
