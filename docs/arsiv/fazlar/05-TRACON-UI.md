...
> ```
>
> Yol eski adı taşır **bilerek**: bu bir git komutudur ve `7f1833e`
> commit'inde dosya o adla durur. Faz 162 adı değiştirdi; komutu
> çevirmek onu çalışmaz hâle getirirdi.
>
...
├── Tracon.UI.Frontend.targets     npm zinciri + EmbeddedResource toplama
├── TraconUiBuilderExtensions.cs   UseUI()
├── Internal/EmbeddedUiProvider.cs     ITraconUiProvider uygulamasi
...
4. Çıktı `EmbeddedResource` olarak `Tracon.UI.wwwroot/<yol>` mantıksal adıyla gömülür
...
Varlık yoksa `TRACON0002` uyarısı verilir; `dotnet pack` sırasında aynı durum
`TRACON0003` **hatasıdır** — içi boş bir arayüz paketi yayınlanamaz.

### Base path

`MapTracon` herhangi bir prefix'e bağlanabilir. Vite `base: './'` ile derlenir;
`postbuild.mjs` `<head>` içine `<base href="__TRACON_BASE__">` yazar ve
`EmbeddedUiProvider` bunu çalışma anında gerçek prefix ile değiştirir. Mutlak varlık
yolu kullanılmaz.

### Sunum

`EmbeddedUiProvider` (`ITraconUiProvider` uygulaması):
...
flowchart TD
    ...
    RES -->|hayır| PROV["ITraconUiProvider.TryServeAsync"]
...
flowchart TD
    ...
    NODE -->|hayır| WARN["TRACON0002 uyarısı<br/>pack'te TRACON0003 hatası"]
    ...
    BROTLI --> INNER
    INNER --> EMBED["EmbeddedResource<br/>Tracon.UI.wwwroot/&lt;yol&gt;"]
...
`tests/Tracon.Ui.E2ETests` — **8 test**, Playwright + gerçek Kestrel.

| Test | Neyi doğrular |
|------|---------------|
| `Arayuz_acilir_ve_ana_ekran_cizilir` | `/tracon` açılır, başlık ve yedi gezinme bağlantısı çizilir |
...
**Çözüm:** `TraconRunOptions.RunId` eklendi (K-044). Uç kimliği kendisi üretir,
...
EXEC : error : ENOENT: no such file or directory, unlink
       .../src/Tracon.UI/wwwroot/assets/index-B9ruDlq1.js
```
...
### 🚨 S4 — `Tracon.UI.csproj` açık `Import` biçimine geçti

`Sdk="Microsoft.NET.Sdk"` niteliği kullanıldığında SDK hedefleri projenin **en
sonuna** yerleştirilir. `Tracon.UI.Frontend.targets` ondan önce yüklenir ve
...
Plan `TraconUiMiddleware` diyordu. Bunun yerine üçüncü bir `MapGroup` ve
yakalayıcı (`{**path}`) rota kullanıldı. Sebep: middleware sırası tüketicinin
`app.Use...` çağrılarına duyarlıdır ve `MapTracon` tek giriş noktası olma
...
| Ölçüt | Durum | Kanıt |
|-------|-------|-------|
| `dotnet run` → `http://localhost:5080/tracon` açılır | ✅ | Aşağıda gerçek çıktı |
...
```bash
$ curl -si localhost:5080/tracon | head -11
HTTP/1.1 200 OK
Content-Length: 844
...
$ curl -H "If-None-Match: <etag>" .../tracon        -> 304 Not Modified
$ curl .../tracon/runs/abc123                        -> 200 text/html  (SPA geri donusu)
$ curl .../tracon/assets/missing.js                  -> 404
$ curl .../tracon/api/agentz                         -> 404 application/problem+json
```
...
Tracon.UI.nupkg : 288 KB (uc hedef cerceve x gomulu varliklar)
nuspec dogrudan bagimlilik : 1  (Tracon.AspNetCore)
```
...
**1. `TraconRunOptions` genişletilebilir.** Faz 6 örnekleme (sampling) oranı veya
...
**8. `ITraconUiProvider` tek metotlu kalmalıdır.** Arayüz paketi kendi paketleme
