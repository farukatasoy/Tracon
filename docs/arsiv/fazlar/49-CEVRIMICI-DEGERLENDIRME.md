...
| Durum | Ne olur |
|---|---|
| Guard, puanlanan çalıştırmanın metnini **engellerse** | Yargıç çağrısı `TraconContentBlockedException` ile düşer. Puanlanan çalıştırma başarılıydı; puanlama başarısız olur |
...
[`RunScoreKind.cs`](../../../src/Tracon.Abstractions/Runs/RunScoreKind.cs).
...
- [x] 🚨 Varsayılan ayarlarla (`Enabled = false`, `SampleRate = 0`) yargıç
      ...
      hem `samples/Tracon.Api`'nin varsayılan appsettings'iyle (aşağıda)
      doğrulandı
...
- [x] `tracon.judge.cost` ve `tracon.judge.score` metrikleri yazılır —
      kod yolu `tracon.run.cost`/`RecordRun` ile birebir aynı desende
      (`TraconMetrics.RecordJudgeCost`/`RecordJudgeScore`); bu örnek
...
- [x] 🚨 `Tracon.Core` AOT uyarısı üretmez — `dotnet build` üç TFM'de de
...
- [x] `samples/Tracon.Api` ile gerçek `run` yapıldı, **gerçek bir yargıç
...
### Doğrulama komutları — gerçek koşum (2026-08-07, `samples/Tracon.Api`, port 5081)
...
RUN=$(curl -s -X POST http://localhost:5081/tracon/api/agents/support/run \
  -H "content-type: application/json" -d '{"message":"varsayilan ayarla test"}' \
  | grep -m1 '"runId"' | sed -E 's/.*"runId":"([^"]+)".*/\1/')
curl -s "http://localhost:5081/tracon/api/runs/$RUN/feedback" | jq 'length'
# -> 0

curl -s "http://localhost:5081/tracon/api/runs?kind=Eval" | jq 'length'
# -> 6 (ONCEKI acik-ayarli kosumdan kalan, bu YENI calistirmadan artmadi)

# --- yeniden baslatildi: Tracon__OnlineEvaluation__Enabled=true SampleRate=1.0 ---

# 2) Puanlandi mi
RUN2=$(curl -s -X POST http://localhost:5081/tracon/api/agents/support/run \
  -H "content-type: application/json" -d '{"message":"istanbul nerede, kisa cevap ver"}' \
  | grep -m1 '"runId"' | sed -E 's/.*"runId":"([^"]+)".*/\1/')
curl -s "http://localhost:5081/tracon/api/runs/$RUN2/feedback" | jq '.[] | {kind, value, source, comment}'
...
curl -s "http://localhost:5081/tracon/api/stats?agentName=support" | jq '{totalRuns, totalCost}'
# -> {"totalRuns":3,"totalCost":null}   (fiyat bu ortamda tanimsiz — null, 0 DEGIL)
curl -s "http://localhost:5081/tracon/api/runs?kind=Eval" | jq '.[] | {agentName, kind, cost}'
# -> uc satirin ucu de agentName:"judge:model", kind:"Eval" — support'un ustune YAZILMADI

# 4) Ozet ucu
curl -s http://localhost:5081/tracon/api/evaluation/online | jq
...
# 5) Elle puanlama
curl -s -X POST "http://localhost:5081/tracon/api/runs/$RUN2/judge" | jq
...
dotnet test   Tracon.slnx -c Release --no-build    → Core.UnitTests 712/712, AspNetCore.FunctionalTests
...
dotnet pack   Tracon.slnx -c Release --no-build    → Başarılı, paket sayısı değişmedi (yeni paket yok)
dotnet format Tracon.slnx --verify-no-changes      → exit 0, değişiklik yok
...
- **`docs/openapi/tracon.json` anlık görüntüsü yenilendi** (+153/-2 satır) — iki yeni ucun ustverisini yansıtır; `OpenApiSnapshotTests` bunu zorunlu kıldı.
...
- Yargıç modeli için fiyat tanımlı değilse (bu ortamda `gpt-5.4-mini` öyleydi) hem `tracon.judge.cost` metriği hem `GET /api/evaluation/online`'ın `judgeCost` alanı `null` kalır — sıfır değil, "bilinmiyor" (mevcut `RunCost` sözleşmesiyle tutarlı).
