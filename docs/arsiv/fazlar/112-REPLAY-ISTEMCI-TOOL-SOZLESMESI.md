...
> **Paketler:** `Tracon.Core` (`Replay/`), `Tracon.AspNetCore` (`Endpoints/RunEndpoints.cs`)
...
- [x] `samples/Tracon.Api` ile gerçek `run` yapıldı, çıktı belgeye yazıldı — bkz. altında
...
- [x] OpenAPI belgesi yeniden üretildi ve TS şeması + NSwag istemcisi + `@tracon/client` `dist`'i zinciri koşuldu (K-627'nin dört adımlı zinciri) — dördü de tek satırlık tutarlı diff üretti

### Doğrulama komutları — gerçek çıktı (`samples/Tracon.Api`, echo sağlayıcı, 2026-08-26)
...
- **🚨 Yeni bir replay hazırlık kontrolü eklerken İKİ kolu da kapat.** `RunReplayService.PrepareAsync`'in kalıcı-tanım kolu (`definition.ToolNames`) ile katalog/kod-agent kolu (`descriptor.ToolNames`, `PrepareFromCatalogAsync`) AYRI kod yollarıdır; Faz 47 ve bu faz ikisinde de yalnız birini kapatıp ikincisini unutma riskiyle karşılaştı. `grep -n "FindApprovalTool\|FindClientTool" src/Tracon.Core/Replay/RunReplayService.cs` her ikisinin de İKİ çağrı yeri olduğunu doğrular.
- **Bilinen sınır:** Örnek uygulamanın (`samples/Tracon.Api`) tek istemci-tool'lu kod agent'ı (`support`) aynı zamanda onay gerektiren bir tool taşıyor; katalog kolunun istemci-tool mesajını İZOLE gösteren bir manuel senaryo yoktur (yalnız otomatik testte var). Yeni bir kod agent'ı gerekirse ekle, gerekmiyorsa MT-IST-015'in notunu koru.
