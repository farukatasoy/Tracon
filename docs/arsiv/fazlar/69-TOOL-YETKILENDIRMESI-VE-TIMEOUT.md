...
> **Paketler:** `Tracon.Abstractions`, `Tracon.Core`, `Tracon.Generators`, `Tracon.AspNetCore`, `Tracon.UI`
...
- [x] `GET /api/tools` yeni üç alanı döner — `docs/openapi/tracon.json`
...
- `samples/Tracon.Api` ile gerçek `run` yapıldı, çıktı belgeye yazıldı —
      **yapılamadı**: bu oturumun sandbox'ında model sağlayıcı API anahtarı/ağ
      ...
      tool `get_slow_report` (`samples/Tracon.Api/OrderTools.cs`) bir sonraki
...
- [x] Manuel kabul case'leri
      ...
      `samples/Tracon.Api`'ye karşı gerçek model anahtarıyla curl ile
...
- [x] `docs-site/` güncellendi; `npm run build` + `check-links.mjs` temiz —
      ...
      **Ekran görüntüsü güncellenmedi** — `TRACON_UI_SCREENSHOTS=1` E2E
...
curl -s http://localhost:5081/tracon/api/tools | jq '.[0]'

# Timeout gerçekten kesiyor mu — sure olculur
time curl -s -X POST http://localhost:5081/tracon/api/agents/slow/run \
...
3. **`TraconToolAttribute.TimeoutSeconds` `int`, planın taslak `ToolDescriptor.Timeout`
...
4. **Gerçek `samples/Tracon.Api` çalıştırması ve `docs-site` ekran görüntüsü
   ...
   kanıt taşıyor. Sonraki oturumun `samples/Tracon.Api`'yi gerçek bir anahtarla
   çalıştırıp DoD'nin ilgili satırını ve `docs-site` ekran görüntüsünü
   (`TRACON_UI_SCREENSHOTS=1`) tamamlaması gerekiyor.
...
| 5 | 🟡 | `TraconToolAttribute.TimeoutSeconds` (`int`) planın taslak `TimeSpan?` imzasından farklı | **Dokümante edildi**, bkz. Plandan Sapmalar #3 — bulgu değil, gerekçeli bir tasarım zorunluluğu (attribute parametresi derleme-zamanı sabiti olmalı) |

Düzeltmelerden sonra dört doğrulama kapısı yeniden koşuldu (build/test/pack/format,
arayüz dahil) — hepsi temiz. `Tracon.SqlServer.IntegrationTests`'te tüm paket
...
1. **`samples/Tracon.Api` gerçek anahtarla koşulmadı.** `get_slow_report` demo
...
2. **`docs-site` ekran görüntüsü güncellenmedi.** `screenshots/tools.png` hâlâ
   ...
   `TRACON_UI_SCREENSHOTS=1` ile E2E koşumu bunu üretir.
...
5. **`TraconOptions.Tools.DefaultTimeout` (30 sn) ölçülmedi** (K-491). İlk
