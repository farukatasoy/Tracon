...
> sözleşme sınıfı kuralı (K-611), `TraconJudgeException` hata normalizasyon
> deseni ve yalnız-NuGet sample emsali (test projesi kısıtı dahil, bkz. Risk R4).
> **Paketler:** `Tracon.Abstractions`, `Tracon.Core`,
> `Tracon.AspNetCore`, `Tracon.Testing.Contracts.Xunit`, `samples/`
...
`IAgentSource`, Tracon'in dört genişleme noktasından dördüncüsüdür. Faz 98 depolamayı, Faz 99 sağlayıcıyı, Faz 100 yargıcı sevk edilen yüzeye taşıdı. Kaynak taşınmadı.
...
- [x] Sözleşme paketinin bağımlılık grafiğine `Tracon.Core` **inmez** (`project.assets.json` ölçümü — üç TFM'de de `[]`)
- [x] Sözleşme paketinin public yüzeyine `Shouldly` tipi sızmaz
- [x] `samples/Tracon.Samples.CustomAgentSource` yalnız `PackageReference` kullanır; gerçek `<ProjectReference` öğesi yok (`grep -c "<ProjectReference"` → `0`; ham `grep -c ProjectReference` yorum metnindeki kelimeyi de sayıp `2` döndüğü için denetimde yanlış pozitif tespit edildi, yorum metni düzeltildi — bkz. Denetim Bulguları #2)
- [x] Sample `AddAgentSource<T>()` ile kaydedilir; `TryAddEnumerable` sample kodunda **geçmez** (`grep -c TryAddEnumerable samples/Tracon.Samples.CustomAgentSource*/*.cs` → `0`)
...
- [x] Dört doğrulama kapısı sıfır uyarı verir — `dotnet build` · tam `dotnet test Tracon.slnx -maxcpucount:1` (23 proje, 5258/5258) · `dotnet pack --no-build` · `dotnet format --verify-no-changes` (restore'suz format'ın yerel feed paketleriyle çalışmadığı ölçüldü — bkz. Sonraki Faza Devir Notu)
- [x] `samples/Tracon.Api` ile gerçek `run` yapıldı; sample kaynak kaydedilip agent çalıştırıldı, çıktı belgeye yazıldı — **kısmi**: `samples/Tracon.Api` PostgreSQL'e bağımlı olduğu ve bu ortamda Docker kurulmadığı için doğrudan koşulamadı; gerçek run kanıtı `samples/Tracon.Samples.CustomAgentSource.Tests` (yalnız local NuGet feed'den `PackageReference`, 15/15 yeşil, `AddAgentSource<T>()` ve factory kayıt yollarının ikisiyle de gerçek `agent.RunAsync()`) ve `TraconTestHost` üzerinden fonksiyonel testlerle (`AgentSourceCustomOriginTests`, `AgentSourceFaultIsolationHttpTests`, `AgentSourceValidationHttpTests`, `DiagnosticsEndpointTests.Registered_agent_sources_are_reported_in_priority_order`) sağlandı
- [x] `secret` taraması boş döndü (`python3 scripts/kapi.py tarama` → ✅ temiz)
- [x] Manuel kabul case'leri `docs/manuel-test/02-CEKIRDEK-VE-KATALOG.md` içine eklendi (MT-CORE-087..096); otomatikleştirilebilenlerden MT-CORE-096 koşuldu (15/15) — 087-094 `samples/Tracon.Api`'nin Postgres bağımlılığı yüzünden, 095 kiracıya duyarlı örnek kaynak bu depoda olmadığı için (👤 gerekir) koşulmadı
...
grep -c "CombineFingerprints" src/Tracon.Core/Catalog/*.cs
...
python3 -c "import json;d=json.load(open('$F'));[print(t,[k for k in libs if 'Tracon.Core' in k]) for t,libs in d['targets'].items()]"
...
grep -c "<ProjectReference" samples/Tracon.Samples.CustomAgentSource/*.csproj || true   # 0
grep -c TryAddEnumerable samples/Tracon.Samples.CustomAgentSource*/*.cs || true          # 0
...
rm -rf ~/.nuget/packages/tracon* artifacts/obj/Tracon.Samples.CustomAgentSource.Tests

# İzolasyon ve tutarlılık regresyonları (ayrı filtrelerle — bu test host'u | ile birleşik deseni desteklemiyor)
B=./artifacts/bin/Tracon.Core.UnitTests/release/Tracon.Core.UnitTests
...
- **`ResolveDependenciesAsync` / `AgentCompilationDependencies` planlanandan
  ...
  gerçek bir sample (`Tracon.Samples.CustomAgentSource`) yazıldıktan sonra
...
- **`CompositeAgentCatalog`'un istisna normalizasyonu ikinci oturumda
  ...
  HER istisnayı `TraconAgentSourceException`'a sarıyordu — bu, kaynağın
  `CompileCachedAsync` üzerinden fırlattığı **normal** `TraconCompilationException`
  ...
  (`Tracon.Samples.CustomModelProvider.Tests.An_unknown_provider_name_fails_with_a_message_naming_the_registered_ones`)
  yakalandı. Düzeltme: `HandleSourceFailure` artık yalnız `TraconException`
  **olmayan** (yani ham, beklenmeyen) istisnaları sarar; herhangi bir
  `TraconException` alt tipi (kaynağın kendi fırlattığı dahil) tipini
  koruyarak yayılır. `AgentSourceValidationService`'in
  `catch (TraconException) when (source is DefinitionStoreAgentSource)`
  özel-durum kodu bu genel kuralla gereksiz hâle geldi ve kaldırıldı.
- **`tests/Tracon.Generators.UnitTests/Examples/ExamplePrelude.cs`'e
  `GitAgentSource` stub'ı eklendi.** `ITraconBuilder.AddAgentSource<T>()`'a
...
- **`samples/Tracon.Api` ile gerçek run DoD'si kısmen karşılandı.** Bu
  sample PostgreSQL'e bağımlı; bu ortamda Docker kurulu değildi. Gerçek run
  kanıtı bunun yerine `samples/Tracon.Samples.CustomAgentSource.Tests`
  (yalnız local NuGet feed'den `PackageReference`, gerçek `agent.RunAsync()`)
  ve `TraconTestHost` üzerinden fonksiyonel testlerle sağlandı — ikisi de
...
değeri, `TraconAgentSourceException`, `AgentSourcePriority`,
...
| `TraconException` alt tipleri (kaynağın kendi fırlattığı dahil) tipini koruyarak yayılır; yalnız ham istisna sarılır | `AgentSourceFaultIsolationTests`, `CompositeAgentCatalog.HandleSourceFailure` |
...
| `AddAgentSource<T>()` tek instance, `AddAgentSource(instance/factory)` çoklu | `TraconBuilderAgentSourceTests` |
...
- **NuGet global paket önbelleği (`~/.nuget/packages/tracon*`) `VersionOverride="*-*"` ile en yüksek sürümü seçer, local feed'i değil.** Bu makinede `1.0.0-preview.2` adında eski bir sürüm önbellekte kalmıştı (muhtemelen daha önceki bir `--surum` denemesinden); taze `dotnet pack` sonrası bile samples projeleri o eski sürümü çözüyordu ve `AgentDefinitionOrigin.Custom` gibi yeni tipler "bulunamadı" hatası veriyordu. **Kural: yeni bir sample projesi eklerken veya `Tracon.*` public API'sini değiştirdikten sonra samples'ı derlemeden önce `rm -rf ~/.nuget/packages/tracon*` çalıştır.**
- **`artifacts/package/release`'te birikmiş eski `.nupkg`'lar aynı sorunu ham `dotnet pack` ile üretir.** `python3 scripts/kapi.py yayin --kuru` (`_clean_stale_packages` çağırır) bunu otomatik temizler; ham `dotnet pack Tracon.src.slnf` **temizlemez** — bu fazda tam olarak bu yüzden `1.0.0-preview.2` sürümü tekrar seçildi. Faz 91'in hafızası bunu zaten "prior --surum run outlives an incremental pack" diye yazmıştı; bu fazda ham `dotnet pack` kullanılınca yeniden yaşandı.
- **`docfx metadata`'nın `references` globu (`*/release{,_net10.0}/*.dll`), `artifacts/bin`'de çok sayıda TEST projesi birikince (tam çözüm build'i + defalarca kısmi build) `CS1704` (aynı basit adlı derleme) fırtınasına giriyor.** Faz 98'in kaydettiği "yeni tek-TFM proje" senaryosundan farklı bir tetikleyici: burada suçlu YENİ proje değildi (yeni `Tracon.Samples.CustomAgentSource*` zaten `references.exclude`'da), suçlu birikmiş `artifacts/bin` idi. **Kural: `docs-site npm run check` çalıştırmadan önce `rm -rf artifacts/bin artifacts/obj && dotnet build Tracon.slnx -c Release` ile TEK, temiz bir build yap.**
- **`dotnet format --verify-no-changes --no-restore`, `samples/`'ın `VersionOverride="*-*"` + yerel `NuGet.config` kombinasyonuyla workspace'i yükleyemiyor** (`CS0246` — tip bulunamadı, `dotnet build`in kendisi başarılıyken). `--no-restore` bayrağını düşürmek (`dotnet format Tracon.slnx --verify-no-changes`) sorunu çözüyor; kök neden izole edilmedi ama örnekleri (`FileRunStore.Tests` gibi hiç dokunulmamış bir proje de aynı hatayı veriyor) bunun samples-genelinde bir ortam kısıtı olduğunu, benim değişikliklerimle ilgisiz olduğunu gösteriyor. `scripts/kapi.py`'nin `closing_commands` listesindeki format komutu hâlâ `--no-restore` taşıyor — bu depoda bir sonraki `dotnet format` çağrısı samples'a dokunan bir faz için tekrar kırmızı görünebilir.
...
- **`cekirdek-kavram → concepts/`**: `TraconException.cs`'in tek
...
**Yarım kalan iş.** `samples/Tracon.Api` ile gerçek run yapılamadı
...
gerektiriyor (👤). `tracon.agent_source.name`/`tracon.agent_source.operation`
