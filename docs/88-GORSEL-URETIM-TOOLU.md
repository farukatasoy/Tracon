...
| `Hosted = 2` | `HostedFileContent` | Sağlayıcıda yaşar; ek deposuna **kimlik** yazılır, içerik değil |
...
| 2 | `Hosted` biçim için ek deposuna ne yazılır? | A: sağlayıcı kimliği + medya tipi, içerik yok · B: içerik indirilip yazılır | **A** — B'nin indirme yolu her sağlayıcıda ayrıdır ve `HostedFileContent`'in varlık sebebini yok eder. **Önce ölç**: hangi sağlayıcı bu biçimi gerçekten döndürüyor |
...
- [ ] Ayar **kapalıyken** tool kayıtlı değildir ve uç bağlı değildir (K1)
- [ ] Ayar açıkken `generate_image` OpenAI · Azure · Google üzerinden çalışır
- [ ] Tool sonucu **yalnız ek kimliği** taşır; base64 taşımaz
- [ ] Üretilen ek doğru `TenantId` **ve** `SessionId` ile yazılır
- [ ] `ResponseFormat` `Data` ve `Uri` biçimlerinin ikisi de ek deposuna doğru girer; `Uri` indirmesi giden ağ muhafızından geçer
- [ ] `tool_invocations` satırı `ToolCallUsage` taşır; birim `images` **veya** token
- [ ] 🚨 Fiyat kaydı yoksa `cost` **`null`** döner — hiçbir koşulda uydurulmaz
- [ ] Hem `PerImage` hem token fiyatı doluysa başlangıçta yapılandırma hatası verilir
- [ ] **Yeni NuGet paketi alınmadı** — `dotnet list package --include-transitive` farkı sıfır
- [ ] `AgentPrism.OpenAI` AOT uyumlu kaldı
- [ ] Dört doğrulama kapısı sıfır uyarı verir
- [ ] `samples/AgentPrism.Api` ile gerçek `run` yapıldı, çıktı belgeye yazıldı
- [ ] `secret` taraması boş döndü
- [ ] Manuel kabul case'leri `docs/manuel-test/19-*` ve `12-*` içine eklendi; otomatikleştirilebilenler koşuldu
