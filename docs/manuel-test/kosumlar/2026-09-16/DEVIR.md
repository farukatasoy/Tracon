...
| `ap-s4` | 5084 | `mt_s4` | 18+ | 31 · 16 · 30 · 22 · 09 · 27 · 26 · 28 · 06 | ✅ **aile 31, 16, 30, 22, 09 ve 27 KAPANDI** (35/35, 98/98 — 1 Kaldı → `HATA-S4-002`, 27/27 — 25 Geçti/2 fiziksel, 56/56 — 53 Geçti/1 Kaldı → `HATA-S4-003`/2 fiziksel, 57/57 — 56 Geçti/1 fiziksel, 17/17 — yeni kusur yok). Aile 27: `samples/Tracon.Api/Program.cs`'e HİÇ dokunulmadı — dinamik `POST /api/agents` kaydı + izole `~/tracon-manuel/model-yedek-s4/` `TraconTestHost` betiği kullanıldı; OpenAI kataloğunda `ContextWindowTokens` olmadığı için 005/006/008 `anthropic`'e kaydırıldı (sapma notu dosyada). Sıradaki: `26-ISTEMCI-TOOLLARI-VE-GOMULEBILIR.md`, henüz açılmadı. Ana uygulama port 5084'te temel durumuna (Preflight/Tenancy kapalı, gerçek OpenAI/Anthropic/Google/OpenRouter hepsi `Healthy`) döndürülüp açık bırakıldı |
