...
`{"title":"Gecersiz istek govdesi","detail":"The JSON value could not be converted to Tracon.McpTransportMode. Path: $.transport..."}`.
...
genişletti: OpenAPI belgesindeki (`docs/openapi/tracon.json`)
...
`TraconRunOptions.SuspendOnApproval` (yalnız kuyruk yolunun ayarladığı,
...
**Gerçek sonuç**
**KALDI - HATA-S2-008 (Yuksek).** /tracon/tools ekraninda test-sunucu_* tool'lari dogru sekilde sari 'mcp: test-sunucu' rozeti gosterdi. AMA ayni ekranda get_order_status, cancel_order, list_recent_orders (UCU DE KOD-TANIMLI, [TraconTool] ozniteligiyle isaretli, MCP ile hicbir ilgisi yok) da YANLIS bir sekilde 'mcp: generated' rozeti gosteriyor - case'in kendi beklentisi ('Kod-tanimli tool'larda bu rozet HIC YOKTUR') ihlal edildi. Kok neden bulundu: src/Tracon.Generators/SourceWriter.cs:105-107, [TraconTool] kaynak ureteci HER kod-tanimli tool kaydi icin KOSULSUZ `source: "generated"` literal string'i geciriyor (TraconToolRegistration constructor'inin varsayilani null'dir, XML belgesi de 'Kodda tanimli tool'larda null' diyor - SourceWriter bu sozlesmeyi ihlal ediyor). ToolDescriptor.Source (ToolDescriptor.cs:29-37) kendi belgesinde bu alanin yalniz 'uzak MCP sunucusundan gelen tool'larda' dolu olmasi gerektigini soyluyor. tools.tsx:71 (`{tool.source != null && <Badge>mcp: {tool.source}</Badge>}`) bu degeri kosulsuz MCP rozeti olarak yorumluyor. **Kapsam genis**: [TraconTool] ozniteligi Tracon'in ONERILEN, kaynak-ureteci-tabanli (AOT uyumlu) tool tanimlama yontemidir - resmi `dotnet new` sablonu da (Tracon.Templates/content/Tracon.Starter/Tools/OrderTools.cs) ayni ozniteligi kullaniyor. Bu, [TraconTool] kullanan HER projede, HER kod-tanimli tool'un arayuzde yaniltici bir 'mcp: generated' rozetiyle gosterilecegi anlamina geliyor - yalniz bu ornek uygulamaya ozgu degil, framework genelinde.

---

**2026-08-14 yeniden koşum (KAPANIS-PLANI Aile I).** Kök neden doğrulandığı
gibi `src/Tracon.Generators/SourceWriter.cs`'nin `WriteAggregator`
...
`TraconToolRegistration` constructor'ının `source: null` varsayılanını
...
gelen tool'larda") tarif ettiği sözleşme gibi. Canlı doğrulama (`samples/Tracon.Api`,
gerçek Postgres şeması, `ProjectReference` ile tazelenmiş üreteç):
`GET /tracon/api/tools` artık `get_order_status`, `cancel_order`,
`list_recent_orders` için `"source": null` döndürüyor — eskiden `"generated"`.
Regresyon testi: `tests/Tracon.Generators.UnitTests/GeneratedOutputTests.cs`
