...
| 24 | [`24-TEST-PAKETI-VE-SABLON.md`](24-TEST-PAKETI-VE-SABLON.md) | `TEST` | 37, 39, 95, 98, 99, 143, 185 | `src/Tracon.Testing` · `src/Tracon.Templates` · `src/Tracon.Testing.Contracts.Xunit` · `samples/Tracon.Samples.FileRunStore(.Tests)` · `tests/Tracon.Package.Tests` · `samples/Tracon.Samples.CustomModelProvider(.Tests)` | **69** | ✅ | ✅ 64/66 · 2 ⬜ (`MT-TEST-077` · `083` — 👤, kayıt bloğu yok) · Faz 185: `MT-TEST-095`…`097` ✅ |
