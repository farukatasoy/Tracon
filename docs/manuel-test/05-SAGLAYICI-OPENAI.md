...
1. Ekle: `agentPrism.UseOpenAICompatible("a234567890123456789012345678901x", o => o.Endpoint = new Uri("http://localhost:9"));` (33 karakter — `a` + 30 rakam + `1x`, tam olarak sayılmış 33 karakter)
2. `dotnet run`, konsolu oku, satırı kaldır.
...
- **Hesap bakiyesine bağlıdır** — bu yüzden değişmez bir metin değil, bir
  ...
  **ya da** SSE `error` çerçevesinde `402` veya `insufficient credits`
  dizgisi görünür. İkinci durum bir kusur değildir; "Gerçek sonuç"e
...
yapmaz, önceki case'lerin "Gerçek sonuç" alanlarını bu açıdan yeniden okur.
