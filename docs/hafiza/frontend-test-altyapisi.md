...
- **🚨 Sorgu dönünce TİPİ değişen alan, `data-testid` aynı kaldığı için eski tipte eşleşir** (2026-09-24, Faz 189 kapanışı): deney formunda `variant-version-N` sürüm sorgusu dönene kadar `<input type=number>`, sonra `<select>`'tir. Yük altındaki tam koşumda `SelectOptionAsync` sorgudan önce koştu ve "Element is not a <select> element" verdi; izole koşum geçiyordu. Test önce `ToHaveJSPropertyAsync("tagName", "SELECT")` bekler. Aynı test id'yi iki tipte kullanan her alanda hedef tipi bekle.
- **🚨 Yalnız bir E2E'nin istediği Tracon özelliği `UiHost.StartAsync(configureTracon:)` ile açılır** (2026-09-24, Faz 186). Grant ekranı `UseSkillScripts` ister (kapalıyken uç `409` döner); paylaşılan kurulum zincirine eklemek her testin host'unu değiştirirdi. `configureServices` `AddTracon()`'dan ÖNCE koşar ve `ITraconBuilder`'a ulaşmaz — builder uzantısı için bu kanca gerekir.
- **Bundle payının tabanı WORKTREE'de ölçülür, `git stash` ile değil** (2026-09-24, Faz 186 denetimi 🔴). `git worktree add <depo dışı dizin> HEAD`; orada `packages/tracon-client` içinde `npm ci && npm run build` (`@tracon/client` bir `file:` bağımlılığıdır, `dist/` ister), sonra `src/Tracon.UI/frontend` içinde `npm ci && npx vite build && node scripts/postbuild.mjs`; `index-*.js.br` boyutu okunur, worktree `git worktree remove` ile kaldırılır. Faz 186'nın ölçümü `git stash push -- <yol>` ile alınmıştı; [`kurtarma.md`](../../.agents/ortak/kurtarma.md) bunu yasaklar (boş bir `push`'tan sonraki `pop` ilgisiz ilk kaydı açar). Değerler doğru kaldı: taban `index-BKpu3tFH.js.br` 162.542 B (planın 2026-09-20 ölçümüyle aynı dosya), faz sonrası 164.676 B.
