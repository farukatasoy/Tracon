...
- **🚨 Eval calistirmalari `RunKind.Eval` ile isaretlenmezse `/api/stats`'i kirletir** (2026-08-03, Faz 18): birim testleri yakalamadi; ornek uygulamada gercek bir OpenAI cagrisiyla olculdu (`totalRuns:1` → duzeltmeden sonra `0`). `TraconRunOptions.Kind` → `RunStart.Kind` → `RunStartInfo.Kind`; `IRunStore.GetStatisticsAsync` bu turu filtreler (K-141). Yeni bir `RunKind` degeri tasiyan bir ozellik eklerken bu zincirin tamamini guncelle.
...
- **🚨 Bir sayacin "bir kez artar" iddiasini SIRALI bir test KANITLAMAZ** (2026-09-15, Faz 173, K-782): `RunEventWriter.Disable`'a `Interlocked.Exchange` korumasi kondu (iki es zamanli yazim tek `run`'i iki kez saymasin diye) ve ilk test sirayla uc yazim deniyordu. **Koruma kaldirildi, 12 testin hicbiri dusmedi** — cunku `IsDisabled` ilk hatadan sonra diger ikisini zaten engelliyor, yani test "bir kez"i korumadan DEGIL `IsDisabled`'dan goruyordu; koruma testten ayirt edilemeyen olu koddu. Cozum yarisi ZORLAMAKTIR: `GatedFailingRunStore` sekiz cagirani depo icinde tutar, hepsi `IsDisabled` kontrolunu gectikten SONRA birlikte duser. Kural: **bir es zamanlilik korumasi eklerken, o korumanin kaldirilmasiyla dusen bir test yaz** — yoksa ne korumanin gerekli oldugunu ne de calistigini bilirsin. Yaris varsayimsal degildi: alt `run` kok yaziciya yazar.
- **🚨 `dotnet-counters` `dotnet run`'in PID'ine baglanirsa SESSIZCE bos doner** (2026-09-15, Faz 173): oturum acilir, "Status: Running" yazar ve **basligi disinda bos bir CSV** birakir — hicbir hata yok. Gercek uygulama ayri bir alt surectir; PID `dotnet-counters ps` ciktisindaki `Tracon.Api` satirindan alinir. `collect` icin `--duration 00:00:00:30` kullan; `pkill` ile durdurulan bir oturum dosyayi HIC yazmaz. Bir metrigi gercek surecte dogrulayan her faz bu tuzaga girer.
- **Etiket degerini bir sabite cevirmek yetmez — LOG cumlesi de ayni sabitten uretilmelidir** (2026-09-15, Faz 173): `Disable`'in gerekce parametresi dort ayri serbest metindi (K-483 sinifi) ve metrik etiketi olarak dogrudan kardinalite riskiydi. Sabit kumesi ve cumle eslemesi AYNI tipte yasar (`RunRecordingStages.All` + `Describe`); ayri tutmak "etiketi olan ama cumlesi olmayan" bir asamaya izin verirdi — operator filtreleyebildigi bir etiketin yaninda hicbir sey anlatmayan bir log satiri gorurdu. Bir test her `All` uyesinin KENDINE AIT bir cumlesi oldugunu iddia eder.
