...
- **🚨 Onay kuralı tool'un KAYNAĞINA bağlı değildir; MCP sunucusunun endpoint'i ve `RequiresApproval`'ı başka bir rolün kaydıdır** (2026-09-24, Faz 186 Karar 11, ölçüldü — F-283, F-284). `SecurityAdmin`'in `toolName`-yalnız kuralı, `AgentsAdmin` endpoint'i başka sunucuya çevirince yeni sunucunun aynı adlı tool'unu onayladı (ad `{server}_{tool}`, endpoint değişince aynı kalır); `AgentsAdmin` `requiresApproval: false` ile onayı kuralsız kapattı. Kanıt iki GERÇEK MCP sunucusuyla alınır: `ModelContextProtocol.AspNetCore` `Tracon.AspNetCore` üzerinden geçişli gelir (`AddMcpServer().WithHttpTransport().WithTools([McpServerTool.Create(...)])` + `MapMcp`), Tracon tarafı `UseMcp()` + `AllowPrivateNetworkTargets` + `POST /api/mcp-servers/refresh` ister (`McpApprovalRuleSourceTests`). Test bugünkü davranışı ölçer; düzeltmede ters çevrilir.
