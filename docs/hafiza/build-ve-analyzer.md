...
- **`Guid.CreateVersion7()` net9+** (2026-08-02): `net8.0` da hedeflediğimiz için `TraconId.NewId()` yazıldı (RFC 9562). Birincil anahtarlarda `Guid.NewGuid()` **kullanma** — index parçalanır.
- **`dotnet pack` kodsuz uyarı üretir** (2026-08-01): `IsPackable=false` projeler için NuGet **kodsuz** bir uyarı verir; `NoWarn` susturmaz. Çözüm `<WarnOnPackingNonPackableProject>false</...>`. Boş `<Target Name="Pack" />` **çalışmaz** — props SDK hedeflerinden önce yüklenir.
- **`CentralPackageTransitivePinningEnabled` kütüphanede zararlı** (2026-08-01): geçişli bağımlılıkları üretilen `.nuspec` içine **doğrudan** bağımlılık olarak yazar. Ölçüldü: `Tracon.PostgreSql` 13 → 2 doğrudan bağımlılık.
...
- **🚨 Tek `$` ile açılan raw/ham dizede `{{` KAÇIŞ DEĞİLDİR** (2026-08-02 · 2026-08-19, Faz 68): tek `{` her zaman interpolasyon deliği açar — `'{}'::jsonb` `CS9006`, `ISNULL(labels, N'{}')` `CS9006`/`CS1733` verir. `$$"""` + `{{` ile çözmek yerine deseni değiştir: sütunu INSERT listesinden çıkarıp şema varsayılanına bırak, ya da `labels IS NOT NULL AND EXISTS (...)` guard'ı yaz — brace istemez ve NULL davranışını AÇIK yazar.
...
- **🚨 `IsAotCompatible` `src/Directory.Build.props` içinde türetilemez** (2026-08-02): o dosya csproj gövdesinden **önce** yüklenir; csproj'daki `TraconAotCompatible=false` görülmez ve bayrak geri alınamaz biçimde `true` kalır. Türetme `Directory.Build.targets` içindedir. Aynı tuzak csproj'a bakan her türetilmiş özellik için geçerli.
- **🚨 MSBuild hedef `Condition`'ı `DependsOnTargets`'tan ÖNCE değerlendirilir** (2026-08-02): zincir hedeflerin kendi üzerinde kurulursa (`A` → `B` → `C`) ve `B`'nin koşulu `C`'nin ürettiği bir özelliğe bakıyorsa `C` **hiç çalışmaz**. Zinciri en dıştaki hedefin `DependsOnTargets` listesinde sırayla kur.
- **🚨 `Sdk="..."` niteliğiyle yüklenen SDK hedefleri projenin EN SONUNA gelir** (2026-08-02, K-051): csproj gövdesindeki `<Import Project="...targets" />` daha önce yüklenir ve içindeki `BeforeTargets="AssignTargetPaths"` **sessizce** atılır. Kanca atan `.targets` için açık `<Import ... Sdk="Microsoft.NET.Sdk" />` kullan.
- **🚨 Çok hedefli projede iç derlemeler paralel koşar** (2026-08-02, K-050): tek bir çıktı dizinine yazan dış araç (Vite) üç kez aynı anda çalışır. 🚨 **`BeforeTargets="DispatchToInnerBuilds"` tek başına YETMEZ** (2026-09-09, K-744): iç derlemeleri o projeye bağlanan BAŞKA projeler de kendi `ProjectReference`'larıyla sıraya koyar. Doğru tekleme adımı TFM taşımayan tek örneğe devretmektir — `<MSBuild Projects="$(MSBuildProjectFullPath)" Targets="..." RemoveProperties="TargetFramework" />`; MSBuild bir hedefi (proje, global özellik) çifti başına bir kez koşturur, yani dışlama zamanlamaya değil MOTORA dayanır. Ölçüm: [`HAFIZA-GECMISI.md`](../arsiv/HAFIZA-GECMISI.md).
- **🚨 Damganın PAYLAŞIMLI mı TFM'e özgü mü olacağını adımın işi belirler** (2026-09-09, K-744): işi build başına olan adım (npm, Vite) `BaseIntermediateOutputPath`'te paylaşımlı damga alır; işi ASSEMBLY başına olan adım TFM'e özgü `IntermediateOutputPath` damgası alır. Karıştırılırsa ilk biten iç derleme damgayı tazeler, diğerleri adımı ATLAR ve içi boş bir assembly üretilir. Kapı: `EveryFrameworkOfTheUiPackageEmbedsTheUi`.
- **AOT analyzer'ı jenerik tip argümanını da denetler** (2026-08-07, Faz 48): kendi jenerik parametrenizi `ServiceDescriptor.Singleton<TService, TImplementation>()`'a geçirmek `IL2091` verir; parametre `[DynamicallyAccessedMembers(PublicConstructors)]` ile işaretlenmelidir (`AddContentGuard<TGuard>` örnektir). Tanı derlemeyi kırar — analyzer'ın `Core` üzerinde canlı olduğunun kanıtı.
...
- **🚨 Faz 11 `dotnet format` calistirmadan kapanmis** (2026-08-02, Faz 12): `main` uzerinde 276 `IDE0055` hatasi vardi ve `dotnet build` tek basina yesil gorunmuyordu bile. Faz kapanisinda dort kapinin da gercekten calistirildigini dogrulayin.
- **Bash komutlarında `cd` kalıcıdır** (2026-08-01): bir komutta `cd artifacts/...` yapıldıysa sonraki komut oradan başlar. `rm -rf artifacts && dotnet build Tracon.slnx` sessizce yanlış dizinde çalıştı ve eski paketler doğru sanıldı. Doğrulama komutlarında mutlak yol kullan.
...
- **`Tracon.Core` — Evet.** Yansimaya dayanan tek yol `AddToolsFrom` ve
...
- **`Tracon.SqlServer` — Hayir (vaat ertelendi).** Olculdu: sifir IL2/IL3
  uyarisi. Canli bir sorgu AOT altinda dogrulanmadigi icin vaat verilmedi (K-181).
- **`Tracon.Sqlite` — Hayir (olculmedi).** Faz 24 kapanisinda olcum YAPILMADI;
  `SQLitePCLRaw` yerel kutuphane tasir (K-196).
- **`Tracon.OpenAI` — Evet.** Faz 3'te olculdu: `IsAotCompatible=true` ile
  sifir uyari. `OPENAI001` / `MAAI001` **deneysel API** tanilaridir, AOT tanisi
  degildir — karistirilmamalidir.
- **`Tracon.AspNetCore` — Hayir.** Minimal API delege yonlendirmesi reflection
...
- **🚨 `git diff`/`git show --stat` bir `.cs` dosyasını "Bin X -> Y bytes" gösterirse
  ...
  `CreateKey` ayırıcısı, `TraconResponseCachingChatClient.cs`'nin tool-adı
