...
- **Modal katmanı, doğrulama adımı ve `window.confirm` yasağı AYRI dosyadadır:**
  [`frontend-modal-katmani.md`](frontend-modal-katmani.md). Faz 175'te ayrıldı —
  bu dosya %4 boşluğa düşmüştü ve modal katmanı kendi başına tutarlı bir eksen
  (K-214 merdiveni: gerçek bölünme).
