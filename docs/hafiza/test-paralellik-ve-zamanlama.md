...
- **🚨 Negatif iddia önündeki sabit uyku bir KANIT değildir** — yük altında pencerede hiçbir şey olmayabilir ve test yeşil kalır. Pozitif sinyal ara: sürecin okuma sayacı (drain), sonraki geçişte kapanan sessionsız "nöbetçi" `run` (uzlaştırıcı), kendi kulvarındaki iş (lane), sayılan tur + kapanan soket (ses). 🚨 Sinyal, yanlış etkinin oluşabileceği **son noktadan SONRA** gelmeli: etkiden önce yazılan bir log satırı yalnız kararın verildiğini kanıtlar (geç tool'da log + kısa `negative` pencere kaldı). Ve iddia **her kanalı** kapsamalı: engellenen ses append testi yalnız `commentary`'ye bakıyordu, sızan tek append `thinking`'deydi — mutasyonla ölçüldü, test yeşil kalıyordu.
...
- **Tam koşum süresi E2E kazancını göstermez**: 884 → 863 → 879 sn (`-maxcpucount:1`); koşumdan koşuma `Package.Tests` tek başına 2:30–3:40 oynar. Proje süresine bak, toplam gürültüdür. Kazanç `-maxcpucount:2`'dedir (524–658 sn); orada `AspNetCore.FunctionalTests` 3:20 → 5:30–7:30 uzar, duvar saati yine kısalır.
