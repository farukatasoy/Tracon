...
- [ ] `IAgentSource` · `IVersionedAgentSource` · `AgentDescriptor` XML'i 101.1'deki **on üç maddenin hepsini** taşır
- [ ] Yanlış metinler gitti: `grep -rn "MAF hosting source\|from onwards" src/ docs-site/src/content/docs/api/` **boş döner**
- [ ] `AgentDefinitionOrigin.Custom` eklendi; `GET /api/agents/{name}` `Custom` için `isEditable: false` döner
- [ ] `Custom` kaynağa ait ada `PUT`/`DELETE` → `409` (bugün `404`)
- [ ] Bir kaynak `ListAsync`'te patlarken `GET /api/agents` `200` döner ve kalan kaynakların agent'larını listeler
- [ ] Bir kaynak `ResolveAsync`'te patlarken çağrı **durur**; daha düşük öncelikli kaynağın agent'ı çalışmaz
- [ ] Hata gövdesi ham üçüncü taraf metni taşımaz; yalnız `{ad} ({kod})`
- [ ] Gerçek `OperationCanceledException` normalize **edilmez**, yayılır
- [ ] Her kaynak hatası bir `LogError` + `AgentSourceFailures` sayacı + diagnostics kaydı üretir
- [ ] `AgentSourceValidationService` host başlarken çözülür; yinelenen kaynak adı host'u **başlatmaz**
- [ ] Startup doğrulaması hiçbir kaynağın `ListAsync`'ini çağırmaz (çağrı sayacı `0` ile kanıtlanır)
- [ ] Katalog artık descriptor **uydurmaz**; tutarsızlık uyarı + sayaç üretir, hata **atmaz**
- [ ] Descriptor ve koleksiyonları sınırda dondurulur; kaynak sonradan değiştirse de katalog çıktısı ve run kaydı değişmez
- [ ] `AddAgentSource<T>()` · `AddAgentSource(instance)` · `AddAgentSource(factory)` çalışır; lifetime **singleton**
- [ ] `CompileCachedAsync` fingerprint'i skill + alt agent + paylaşılan talimattan birleştirir ve BYOK'ta cache'i **atlar**; iki ayrı test
- [ ] Üç kopya blok tekile indi: `grep -c "CombineFingerprints" src/AgentPrism.Core/Catalog/` → `0`
- [ ] `AgentSourceContract` `AgentPrism.Testing.Contracts.AgentSources` ad alanında yayınlandı; on üç senaryo koşuyor
- [ ] `ContractCoverage.AgentSourceContracts` sabiti eklendi; kapsam testi yeşil
- [ ] `CodeAgentSource` ve `DefinitionStoreAgentSource` suite'i geçiyor; ikincisi iki opt-in sınıfı da geçiyor
- [ ] Mevcut üç ailenin kapsam testleri (dört depolama koşumu + sağlayıcı + yargıç) **yeşil kaldı**
- [ ] Sözleşme paketinin bağımlılık grafiğine `AgentPrism.Core` **inmez** (`project.assets.json` ölçümü)
- [ ] Sözleşme paketinin public yüzeyine `Shouldly` tipi sızmaz
- [ ] `samples/AgentPrism.Samples.CustomAgentSource` yalnız `PackageReference` kullanır; `grep -c ProjectReference` → `0`
- [ ] Sample `AddAgentSource<T>()` ile kaydedilir; `TryAddEnumerable` sample kodunda **geçmez**
- [ ] Arayüz bundle payı ölçüldü ve yazıldı; `en.ts`/`tr.ts` eksiksiz
- [ ] Dört doğrulama kapısı sıfır uyarı verir
- [ ] `samples/AgentPrism.Api` ile gerçek `run` yapıldı; sample kaynak kaydedilip agent çalıştırıldı, çıktı belgeye yazıldı
...
- [ ] `docs-site/` güncellendi (`guides/write-your-own-agent-source.md` + `concepts/agents.md` düzeltmesi dahil); `npm run build` + `check-links.mjs` temiz

### Doğrulama komutları

```bash
# Dört kapı — taban, bu fazdan ÖNCEKİ commit
python3 scripts/kapi.py kapanis --taban 88669f6
...
# Sample yalnız NuGet, ergonomik kayıt
...
# İzolasyon ve tutarlılık regresyonları
./artifacts/bin/AgentPrism.Core.UnitTests/release/AgentPrism.Core.UnitTests \
  --filter-method "*AgentSourceFaultIsolation*|*AgentSourceConsistency*|*AgentSourceMutation*|*AgentSourceValidation*"

# Mevcut aileler kırılmadı
./artifacts/bin/AgentPrism.Core.UnitTests/release/AgentPrism.Core.UnitTests \
  --filter-method "*ContractCoverage*"
```
