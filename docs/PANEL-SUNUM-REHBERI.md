...
> envanterdeki 13 kategori ve 182 özelliğin tamamı için sunum kararı verir.
...
| Sınıf | Sayı | Yorum |
| --- | ---: | --- |
| Doğrudan panel demosu | 73 | Console veya embedded UI üzerinden tetikleme ve görünür sonuç vardır. |
| Panel destekli demo | 65 | Hazırlık panel dışındadır; sonuç veya operasyon kanıtı paneldedir. |
| Panel dışı teknik demo | 32 | API, CLI, SDK, build, test veya altyapı kanıtı gerekir. |
| Gösterilmemeli | 12 | Canlı tüketici sunumunda değer/risk dengesi zayıftır. |
| Belirsiz | 0 | Envanter iddiaları kod ve ürün dokümantasyonunda karşılık buldu. |
| **Toplam** | **182** | Değerlendirilmeden kalan özellik yoktur. |
...
| C11 Dashboard/UI | `frontend/src/app.tsx` içindeki 29 route ve ilgili screen/component'ler | `ui.md` ve build ile üretilen gerçek-browser screenshots |
...
- **Reset/tekrar koşum:** R1 ile definition başlangıç version'ına döner. **Risk ve notlar:** Tool kodu panelde yazılmaz. **İlişkili özellikler:** 3, 6–13, 153–154.
...
- **Reset/tekrar koşum:** R1 run temizliği. **Risk ve notlar:** Factory'nin kendisi panelden değiştirilemez. **İlişkili özellikler:** 4, 150, 166.
...
- **Reset/tekrar koşum:** R1 seed revision'ı restore eder. **Risk ve notlar:** Rollback geçmişi silmez; yeni version üretir. **İlişkili özellikler:** 89–90, 101, 153.
...
- **Reset/tekrar koşum:** R1. **Risk ve notlar:** Source configuration ve refresh panel action'ı yoktur. **İlişkili özellikler:** 2, 5, 173.
...
- **Reset/tekrar koşum:** Failure flag kapatılır; R1. **Risk ve notlar:** Hata mesajı için ayrı panel yoktur. **İlişkili özellikler:** 4, 132, 136.
...
- **Reset/tekrar koşum:** Draft kapatılır; R1 gerekmez. **Risk ve notlar:** Gerçek provider çağrısı yapılmaması özellikle belirtilir. **İlişkili özellikler:** 7–12, 153.
...
- **Reset/tekrar koşum:** R1. **Risk ve notlar:** Provider-specific setting desteği modelden modele değişir. **İlişkili özellikler:** 8, 17, 21–27.
...
- **Reset/tekrar koşum:** R1. **Risk ve notlar:** JSON'un semantic doğruluğu ayrı eval ister. **İlişkili özellikler:** 6, 27, 84.

#### 9. Parameterized instructions
...
- **Reset/tekrar koşum:** Playground reset. **Risk ve notlar:** Parameter secret taşımamalıdır. **İlişkili özellikler:** 1, 6, 154.

#### 10. Culture-aware instructions
...
- **Reset/tekrar koşum:** R1. **Risk ve notlar:** Model dil kalitesi nondeterministic olabilir; fake fixture hazırdır. **İlişkili özellikler:** 3, 9, 160.

#### 11. Shared instructions
...
- **Reset/tekrar koşum:** R1. **Risk ve notlar:** Compiled instructions'ın tamamı secret/policy sızıntısı nedeniyle panelde gösterilmez. **İlişkili özellikler:** 20, 39–40.

#### 12. Agent-to-agent çağrı grafiği
...
- **Reset/tekrar koşum:** R1. **Risk ve notlar:** Gerçek model delegation'ı değişken olabilir; forced fake tool-call fallback'i bulunur. **İlişkili özellikler:** 53–54, 66.
...
- **Reset/tekrar koşum:** R1 ve file-memory tenant prefix temizliği. **Risk ve notlar:** Web search için local fixture kullanılır. **İlişkili özellikler:** 14, 48.
...
- **Reset/tekrar koşum:** R1 session/run temizliği. **Risk ve notlar:** Gerçek token eşiği zaman alır; düşük demo limiti yalnız showcase config'indedir. **İlişkili özellikler:** 19, 61, 124–125.
...
- **Reset/tekrar koşum:** Redis demo namespace + R1. **Risk ve notlar:** Model/provider cache token'ı ile AgentPrism response cache'i karıştırılmamalıdır. **İlişkili özellikler:** 126–128.

#### 16. Concurrent tool calls
...
- **Reset/tekrar koşum:** R1. **Risk ve notlar:** Side-effect tools paralel seçilmemelidir. **İlişkili özellikler:** 33, 124, 134.

#### 17. Model fallback zinciri
...
- **Reset/tekrar koşum:** Failure toggle kapatılır; circuit state ve R1 temizlenir. **Risk ve notlar:** Canlı provider outage'ı beklenmez; controlled proxy gerekir. **İlişkili özellikler:** 59, 129–130.

#### 18. Provider concurrency limiti
...
- **Reset/tekrar koşum:** R1. **Risk ve notlar:** Bekleme nedeni panelde özel queue adıyla görünmeyebilir; proxy metriği ek kanıttır. **İlişkili özellikler:** 126, 130.

#### 19. Context preflight
...
- **Reset/tekrar koşum:** Playground attachment temizlenir; R1. **Risk ve notlar:** Exact token tahmini model catalog doğruluğuna bağlıdır. **İlişkili özellikler:** 14, 27, 63.

#### 20. Compiled agent cache
...
- **Reset/tekrar koşum:** R4. **Risk ve notlar:** İç cache davranışını ürün özelliği gibi sunmak yanlış beklenti üretir. **İlişkili özellikler:** 3, 11, 41.
...
- **Reset/tekrar koşum:** R1. **Risk ve notlar:** Token maliyeti ve rate limit vardır. **İlişkili özellikler:** 7, 139–140.

#### 22. OpenAI-compatible provider
...
- **Reset/tekrar koşum:** R1. **Risk ve notlar:** Compatible endpoint'lerin capability parity'si değişir; structured output/tool desteği catalog'da doğru işaretlenir. **İlişkili özellikler:** 27, 129.
...
- **Reset/tekrar koşum:** R1. **Risk ve notlar:** Extended thinking maliyetlidir ve model desteği gerekir. **İlişkili özellikler:** 7, 126–127.
...
- **Reset/tekrar koşum:** R2. **Risk ve notlar:** Safety kararları model version'ına göre değişebilir; deterministic fallback gerekir. **İlişkili özellikler:** 97–98, 129.

#### 25. Azure OpenAI provider
...
- **Reset/tekrar koşum:** R1. **Risk ve notlar:** Entra demo ortamı ve role assignment hazırlığı pahalıdır; API key fallback'i bulunur. **İlişkili özellikler:** 97, 107, 132.

#### 26. Custom model provider
...
- **Reset/tekrar koşum:** R1. **Risk ve notlar:** Registration panelden yapılamaz. **İlişkili özellikler:** 129, 165, 173.

#### 27. Configuration tabanlı model catalog
...
- **Reset/tekrar koşum:** Config restart + R1. **Risk ve notlar:** Catalog permission sınırı değildir. **İlişkili özellikler:** 19, 127–128.
...
- **Reset/tekrar koşum:** R1 + attachment namespace. **Risk ve notlar:** Maliyet ve latency yüksektir; prerecorded valid attachment fallback'i kullanılır. **İlişkili özellikler:** 63–64, 122, 127.
...
- **Reset/tekrar koşum:** R1 + media izinleri. **Risk ve notlar:** Sessiz ortam, browser codec ve ücret gerekir; prerecorded audio fallback'i bulunur. **İlişkili özellikler:** 30, 63–64, 149.

#### 30. Live voice conversation
...
- **Reset/tekrar koşum:** R1. **Risk ve notlar:** Browser permission/network en kırılgan canlı adımdır; recorded video ve text fallback'i hazırdır. **İlişkili özellikler:** 49, 61, 149, 154.

### Tool, skill, MCP ve context yönetimi

#### 31. Tool kayıt seçenekleri
...
- **Reset/tekrar koşum:** R1. **Risk ve notlar:** Tool body panelden oluşturulamaz. **İlişkili özellikler:** 32–34, 159.

#### 32. Build-time tool generation
...
- **Reset/tekrar koşum:** R4. **Risk ve notlar:** Teknik ek sunuma alınır. **İlişkili özellikler:** 164, 174.

#### 33. Tool governance metadata
...
- **Reset/tekrar koşum:** R1. **Risk ve notlar:** `SafeToRepeat` her durumda idempotency garantisi değildir. **İlişkili özellikler:** 34–38.
...
- **Reset/tekrar koşum:** R1. **Risk ve notlar:** UI nav gizleme authorization kanıtı değildir; server response esas alınır. **İlişkili özellikler:** 35, 93–94.

#### 35. Tool approval akışı
...
- **Reset/tekrar koşum:** Pending approvals expire/delete; R1. **Risk ve notlar:** Kısa ama sunumdan uzun olmayan expiry seçilir. **İlişkili özellikler:** 36, 70, 101.

#### 36. Standing approval rules
...
- **Reset/tekrar koşum:** Rule seed'i restore; R1. **Risk ve notlar:** Exact-argument ile conditional rule karıştırılmamalıdır. **İlişkili özellikler:** 35, 101, 158.

#### 37. Tool timeout ve output sınırı
...
- **Reset/tekrar koşum:** R1. **Risk ve notlar:** Büyük payload panel/log'u şişirmemelidir; sentetik veri kullanılır. **İlişkili özellikler:** 33, 59.
...
- **Reset/tekrar koşum:** Playground reset; R1. **Risk ve notlar:** Client sonucu güvenilir input sayılmamalıdır. **İlişkili özellikler:** 148, 154.

#### 39. Custom content guards
...
- **Reset/tekrar koşum:** R1. **Risk ve notlar:** Guard configuration için panel yoktur. **İlişkili özellikler:** 40, 59, 101.

#### 40. Pattern content guard
...
- **Reset/tekrar koşum:** R1. **Risk ve notlar:** Gerçek kişisel veri kullanılmaz; regex guard semantic güvenlik değildir. **İlişkili özellikler:** 39, 108, 110.
...
- **Reset/tekrar koşum:** R1 skill/agent version restore. **Risk ve notlar:** Markdown source gösterilir, rendered HTML beklenmez. **İlişkili özellikler:** 42, 47–48, 153.

#### 42. Skill script execution
...
- **Reset/tekrar koşum:** Grant seed'i restore; R1. **Risk ve notlar:** OS sandbox sağlanmaz; yalnız güvenilir script kullanılır. **İlişkili özellikler:** 113, 158.

#### 43. Remote MCP tool discovery
...
- **Reset/tekrar koşum:** MCP cache ve R2. **Risk ve notlar:** Local deterministic MCP ana canlı default'udur. **İlişkili özellikler:** 44–46, 142.

#### 44. MCP authentication ve OAuth
...
- **Reset/tekrar koşum:** OAuth token cache + R2. **Risk ve notlar:** Gerçek OAuth redirect kırılgandır; local fixture önerilir. **İlişkili özellikler:** 43, 107.
...
- **Reset/tekrar koşum:** MCP cache + R1. **Risk ve notlar:** Ana canlı akışta kısa dipnot, protocol ekinde tam demo. **İlişkili özellikler:** 43–46.
...
- **Reset/tekrar koşum:** MCP cache + R1. **Risk ve notlar:** Resource içeriği için ayrı viewer yoktur. **İlişkili özellikler:** 41, 43, 47.

#### 47. Knowledge ingestion ve search
...
- **Reset/tekrar koşum:** Knowledge rows + R2. **Risk ve notlar:** Embedding maliyeti için precomputed synthetic vectors kullanılabilir. **İlişkili özellikler:** 48, 115, 123.
...
- **Reset/tekrar koşum:** File/vector memory namespace + R2. **Risk ve notlar:** File memory genel amaçlı filesystem değildir. **İlişkili özellikler:** 13–14, 47.
...
- **Reset/tekrar koşum:** R1. **Risk ve notlar:** Network buffering için fake slow stream fallback'i gerekir. **İlişkili özellikler:** 52, 137, 154.

#### 50. Non-streaming run
...
- **Reset/tekrar koşum:** R1. **Risk ve notlar:** Ana canlı akışta streaming ile kısa karşılaştırma yeterlidir. **İlişkili özellikler:** 49, 137.
...
- **Reset/tekrar koşum:** R1. **Risk ve notlar:** Input recording production'da privacy kararı ister. **İlişkili özellikler:** 52–60, 133.

#### 52. Ordered event stream
...
- **Reset/tekrar koşum:** R1. **Risk ve notlar:** Browser network throttling kontrollü yapılır. **İlişkili özellikler:** 49, 136.
...
- **Reset/tekrar koşum:** R1. **Risk ve notlar:** Çok büyük tree yerine üç node kullanılır. **İlişkili özellikler:** 12, 54–55, 127.
...
- **Reset/tekrar koşum:** R1. **Risk ve notlar:** Gerçek maliyet budget'ı yerine deterministic token/depth limiti canlıda güvenilirdir. **İlişkili özellikler:** 12, 53, 99.
...
- **Reset/tekrar koşum:** R1. **Risk ve notlar:** Cancel request anlık completion garantisi değildir. **İlişkili özellikler:** 53, 72, 82.
...
- **Reset/tekrar koşum:** R1. **Risk ve notlar:** Destructive tool için `LiveTools` canlı sunumda kullanılmaz. **İlişkili özellikler:** 33, 57, 81.

#### 57. Run karşılaştırma
...
- **Reset/tekrar koşum:** Seeded comparison runs restore edilir. **Risk ve notlar:** Comparison kazanan seçmez. **İlişkili özellikler:** 3, 56, 89.

#### 58. Run feedback ve score
...
- **Reset/tekrar koşum:** R1 score seed'i. **Risk ve notlar:** Human feedback ground truth olarak sunulmaz. **İlişkili özellikler:** 86, 88, 133.

#### 59. Run error classification
...
- **Reset/tekrar koşum:** R1. **Risk ve notlar:** Safe UI message debug detail yerine geçmez. **İlişkili özellikler:** 17, 71–72, 133.
...
- **Reset/tekrar koşum:** R1. **Risk ve notlar:** Correlation id UI'da her breakdown için ayrı chart olmayabilir; API ek kanıttır. **İlişkili özellikler:** 96, 99, 133.
...
- **Reset/tekrar koşum:** R1. **Risk ve notlar:** In-memory profile bu garantiyi vermez. **İlişkili özellikler:** 30, 62–65.

#### 62. Session branching
...
- **Reset/tekrar koşum:** Branched session silinir; R1. **Risk ve notlar:** In-memory store ile demo yapılmaz. **İlişkili özellikler:** 56, 61, 155.

#### 63. Attachment yönetimi
...
- **Reset/tekrar koşum:** Attachment namespace + R1. **Risk ve notlar:** Küçük sentetik dosyalar kullanılır. **İlişkili özellikler:** 28–30, 64–65, 122.

#### 64. Multimodal messages
...
- **Reset/tekrar koşum:** R1. **Risk ve notlar:** Tek model tüm modality'leri desteklemeyebilir; capability-driven agent seçilir. **İlişkili özellikler:** 27–30, 63, 149.
...
- **Reset/tekrar koşum:** R1. **Risk ve notlar:** Ayrım panelde tek başına kanıtlanamaz; technical trace eklenir. **İlişkili özellikler:** 40, 63–64.
...
- **Reset/tekrar koşum:** R1. **Risk ve notlar:** Beşini canlı çalıştırmak yerine yalnız ikisini çalıştır. **İlişkili özellikler:** 12, 67–70.

#### 67. Workflow tanımları
...
- **Reset/tekrar koşum:** R1. **Risk ve notlar:** Function/agent node registration yine kod gerektirir. **İlişkili özellikler:** 68, 153, 156.

#### 68. Workflow functions
...
- **Reset/tekrar koşum:** R1. **Risk ve notlar:** Function body panelde yazılmaz. **İlişkili özellikler:** 66–67, 71.

#### 69. Durable checkpoints
...
- **Reset/tekrar koşum:** R1. **Risk ve notlar:** Restart prova edilir; live failure olursa prerecorded evidence kullanılır. **İlişkili özellikler:** 70, 80–81.
...
- **Reset/tekrar koşum:** R1. **Risk ve notlar:** Tool approval ile workflow request aynı şey değildir. **İlişkili özellikler:** 35, 69.

#### 71. Workflow node retry
...
- **Reset/tekrar koşum:** Failure counter + R1. **Risk ve notlar:** Backoff demo için kısa tutulur. **İlişkili özellikler:** 59, 72.
...
- **Reset/tekrar koşum:** R1. **Risk ve notlar:** At-least-once handler idempotent olmalıdır. **İlişkili özellikler:** 73–76.

#### 73. Custom job handler
...
- **Reset/tekrar koşum:** R1. **Risk ve notlar:** Handler registration panelde yoktur. **İlişkili özellikler:** 72, 75, 173.

#### 74. Agent ve workflow jobs
...
- **Reset/tekrar koşum:** R1. **Risk ve notlar:** Batch küçük tutulur. **İlişkili özellikler:** 72, 77.
...
- **Reset/tekrar koşum:** Schedule seed + R1. **Risk ve notlar:** Günlük cron'u canlı bekleme; manual trigger kullan. **İlişkili özellikler:** 72, 76, 88.
...
- **Reset/tekrar koşum:** R1. **Risk ve notlar:** İki terminal/process sunum yükü getirir. **İlişkili özellikler:** 72, 79.
...
- **Reset/tekrar koşum:** R1. **Risk ve notlar:** Main story'de kısa terminal kullanımı gerekir. **İlişkili özellikler:** 72, 137.

#### 78. Idempotent HTTP işlemleri
...
- **Reset/tekrar koşum:** Idempotency row + R1. **Risk ve notlar:** Farklı tenant veya operation aynı key'i paylaşabilir. **İlişkili özellikler:** 77, 105.

#### 79. Singleton execution
...
- **Reset/tekrar koşum:** Lease expiry + R4. **Risk ve notlar:** Timing-sensitive ve failure injection gerektirir. **İlişkili özellikler:** 76, 80.

#### 80. Run reconciliation
...
- **Reset/tekrar koşum:** R4/R1. **Risk ve notlar:** Process kill diğer demoları bozar. **İlişkili özellikler:** 69, 81.
...
- **Reset/tekrar koşum:** R1/R4. **Risk ve notlar:** Ana canlıda önceden hazırlanmış kayıt önerilir. **İlişkili özellikler:** 56, 80.
...
- **Reset/tekrar koşum:** Process restart + R4. **Risk ve notlar:** Live showcase host'unu kapatır. **İlişkili özellikler:** 55, 76.

### Evaluation ve controlled change

#### 83. Eval suite ve case yönetimi
...
- **Reset/tekrar koşum:** Suite seed + R1. **Risk ve notlar:** Live için 3-case subset kullan. **İlişkili özellikler:** 84–88.

#### 84. Built-in eval checks
...
- **Reset/tekrar koşum:** R1. **Risk ve notlar:** Semantic kaliteyi tek başına kanıtlamaz. **İlişkili özellikler:** 8, 85–86.

#### 85. Custom eval checks
...
- **Reset/tekrar koşum:** R1. **Risk ve notlar:** Check implementation panelden yazılmaz. **İlişkili özellikler:** 84, 173.
...
- **Reset/tekrar koşum:** R1. **Risk ve notlar:** Model judge nondeterministic/maliyetlidir; fake judge fallback'i vardır. **İlişkili özellikler:** 58, 88.

#### 87. Run-to-case promotion
...
- **Reset/tekrar koşum:** Promoted case silinir; R1. **Risk ve notlar:** Sensitive recorded input promote edilmemelidir. **İlişkili özellikler:** 51, 83.

#### 88. Online evaluation
...
- **Reset/tekrar koşum:** Online-eval window + R1. **Risk ve notlar:** Clock kontrolü için fake time önerilir. **İlişkili özellikler:** 58, 75, 86.
...
- **Reset/tekrar koşum:** Experiment/results + R1. **Risk ve notlar:** UI winner iddiası yapmaz. **İlişkili özellikler:** 3, 57, 90.
...
- **Reset/tekrar koşum:** Agent/experiment seed restore. **Risk ve notlar:** Gerçek traffic beklenmez. **İlişkili özellikler:** 3, 89.

### Güvenlik, governance ve multi-tenancy

#### 91. Loopback varsayılanı
...
- **Reset/tekrar koşum:** Profile restart. **Risk ve notlar:** Ana demo remote profile gerektirirse auth zorunlu tutulur. **İlişkili özellikler:** 92–94, 112.

#### 92. Static bearer token
...
- **Reset/tekrar koşum:** Browser token forget. **Risk ve notlar:** Enterprise demo için consumer AuthN tercih edilir. **İlişkili özellikler:** 93–95.

#### 93. ASP.NET Core authorization
...
- **Reset/tekrar koşum:** Session/logout. **Risk ve notlar:** Gerçek corporate IdP ana sunuma bağlanmaz. **İlişkili özellikler:** 34, 94.
...
- **Reset/tekrar koşum:** Login switch. **Risk ve notlar:** Nav hiding güvenlik sınırı değildir. **İlişkili özellikler:** 93, 101.

#### 95. Scoped API key'ler
...
- **Reset/tekrar koşum:** Demo keys revoke/delete; seed metadata. **Risk ve notlar:** Ekran kaydında raw key maskelenir. **İlişkili özellikler:** 96, 112, 142–144.
...
- **Reset/tekrar koşum:** R2. **Risk ve notlar:** Header yalnız izinli profile'da kullanılır. **İlişkili özellikler:** 97–100, 123.
...
- **Reset/tekrar koşum:** Binding seed + R2. **Risk ve notlar:** Secret değeri asla ekrana yazılmaz. **İlişkili özellikler:** 24–25, 98, 107, 130.

#### 98. Tenant provider egress policy
...
- **Reset/tekrar koşum:** Policy seed + R2. **Risk ve notlar:** Catalog presence izin anlamına gelmez. **İlişkili özellikler:** 6, 27, 97.
...
- **Reset/tekrar koşum:** Quota usage window + R2. **Risk ve notlar:** Cost quota catalog pricing doğruluğuna bağlıdır. **İlişkili özellikler:** 54, 60, 126–127.

#### 100. HTTP rate limiting
...
- **Reset/tekrar koşum:** Fake clock/window. **Risk ve notlar:** Ana panel akışında kısa dipnot. **İlişkili özellikler:** 95–96, 105.
...
- **Reset/tekrar koşum:** R2; demo audit seed'i yeniden üretilir. **Risk ve notlar:** Secret-like test value bile sentetik olmalıdır. **İlişkili özellikler:** 3, 35–36, 94–99.

#### 102. Tamper-evident audit chain
...
- **Reset/tekrar koşum:** R2/R4. **Risk ve notlar:** Canlı database'i değiştirme; invalid kanıt prerecorded olmalıdır. **İlişkili özellikler:** 101, 108.

#### 103. Signed outbound webhooks
...
- **Reset/tekrar koşum:** Delivery/job + receiver counter. **Risk ve notlar:** Local HTTPS receiver kullan. **İlişkili özellikler:** 72, 101, 107.

#### 104. Signed inbound triggers
...
- **Reset/tekrar koşum:** Trigger/job + R2. **Risk ve notlar:** Signature helper terminalde çalışır; raw secret görünmez. **İlişkili özellikler:** 105, 107.

#### 105. Trigger replay ve flood koruması
...
- **Reset/tekrar koşum:** Replay window/fake clock + R1. **Risk ve notlar:** Security appendix'te göster. **İlişkili özellikler:** 78, 100, 104.

#### 106. Outbound network guard
...
- **Reset/tekrar koşum:** R4. **Risk ve notlar:** Production/private endpoint taraması yapılmaz. **İlişkili özellikler:** 43, 103, 112.

#### 107. Secret reference sınırı
...
- **Reset/tekrar koşum:** R2. **Risk ve notlar:** Placeholder bile secret değeri gibi persist edilmemelidir. **İlişkili özellikler:** 44, 97, 103–104.

#### 108. At-rest content protection
...
- **Reset/tekrar koşum:** R3/R4. **Risk ve notlar:** Key asla belgelenmez/gösterilmez; technical security appendix. **İlişkili özellikler:** 40, 63, 102.

#### 109. Retention ve archive
...
- **Reset/tekrar koşum:** Seed snapshot restore. **Risk ve notlar:** Demo tenant dışı veri hedeflenmez. **İlişkili özellikler:** 110, 120.

#### 110. Data subject hakları
...
- **Reset/tekrar koşum:** R2. **Risk ve notlar:** Technical privacy appendix; gerçek kişi verisi yok. **İlişkili özellikler:** 96, 108–109.
...
- **Reset/tekrar koşum:** Profile restart. **Risk ve notlar:** Ana canlıya yalnız widget kullanılıyorsa ekle. **İlişkili özellikler:** 148.

#### 112. External protocol guard
...
- **Reset/tekrar koşum:** Keys/profile + R4. **Risk ve notlar:** Security appendix. **İlişkili özellikler:** 91, 95, 142–144.

#### 113. Skill script güvenlik sınırı
...
- **Reset/tekrar koşum:** R1/R4. **Risk ve notlar:** Untrusted script çalıştırmak güvenli değildir. **İlişkili özellikler:** 42.

### Persistence ve storage

#### 114. In-memory varsayılanlar
...
- **Reset/tekrar koşum:** Restart seed'i tekrarlar. **Risk ve notlar:** Ana showcase persistence olarak kullanılmaz. **İlişkili özellikler:** 115–118, 150.

#### 115. PostgreSQL persistence
...
- **Reset/tekrar koşum:** R2/R3. **Risk ve notlar:** Ana önerilen provider. **İlişkili özellikler:** 47, 118, 123.

#### 116. SQL Server persistence
...
- **Reset/tekrar koşum:** R3. **Risk ve notlar:** Ana canlıda profile switch maliyetlidir; recorded evidence yeterlidir. **İlişkili özellikler:** 118–123.

#### 117. SQLite persistence
...
- **Reset/tekrar koşum:** Demo DB file'ı kontrollü yeniden üret; R3. **Risk ve notlar:** Multi-instance showcase için kullanılmaz. **İlişkili özellikler:** 118, 161.

#### 118. Migration yönetimi
...
- **Reset/tekrar koşum:** Snapshot restore/R3. **Risk ve notlar:** Shared DB'de canlı migration yapılmaz. **İlişkili özellikler:** 115–117, 132, 147.

#### 119. Consumer data source kullanımı
...
- **Reset/tekrar koşum:** R4. **Risk ve notlar:** Technical persistence appendix. **İlişkili özellikler:** 115–117, 121.

#### 120. Read contract view'ları
...
- **Reset/tekrar koşum:** R2/R4. **Risk ve notlar:** View tenant security boundary değildir. **İlişkili özellikler:** 127, 133.

#### 121. Replaceable store'lar
...
- **Reset/tekrar koşum:** R4. **Risk ve notlar:** Main live flow'a alınmaz. **İlişkili özellikler:** 119, 165, 173.

#### 122. External attachment storage
...
- **Reset/tekrar koşum:** Object prefix + R2. **Risk ve notlar:** Presigned production URL kullanılmaz. **İlişkili özellikler:** 63, 108.

#### 123. Tenant-isolated durable contracts
...
- **Reset/tekrar koşum:** R2. **Risk ve notlar:** Yalnız header switching'e güvenme; authenticated tenant resolver kullan. **İlişkili özellikler:** 96, 115–122.

### Gözlemlenebilirlik ve operasyon

#### 124. OpenTelemetry traces
...
- **Reset/tekrar koşum:** Collector + R4. **Risk ve notlar:** Persisted trace ile exported OTel trace aynı contract değildir. **İlişkili özellikler:** 125–126.

#### 125. Persisted trace sample
...
- **Reset/tekrar koşum:** R1. **Risk ve notlar:** Production sample oranı daha düşük olmalıdır. **İlişkili özellikler:** 16, 124.
...
- **Reset/tekrar koşum:** Exporter + R4. **Risk ve notlar:** Dashboard değerleri metric export'tan değil store analytics'ten gelebilir. **İlişkili özellikler:** 99, 133–134.

#### 127. Cost attribution
...
- **Reset/tekrar koşum:** R1. **Risk ve notlar:** Fiyatlar billing invoice garantisi değildir. **İlişkili özellikler:** 15, 28–29, 53, 128.

#### 128. Cost recalculation
...
- **Reset/tekrar koşum:** Price seed + R1. **Risk ve notlar:** Panelde doğrudan Recalculate action doğrulanmadı. **İlişkili özellikler:** 27, 127.
...
- **Reset/tekrar koşum:** Health cache/failure flag. **Risk ve notlar:** Credential'ı canlıda deliberately revoke etme. **İlişkili özellikler:** 17, 130–132.
...
- **Reset/tekrar koşum:** Breaker state/fake clock. **Risk ve notlar:** Özel breaker ekranı yoktur. **İlişkili özellikler:** 17–18, 97, 129.

#### 131. ASP.NET Core health check
...
- **Reset/tekrar koşum:** Profile restart. **Risk ve notlar:** Kubernetes probe appendix. **İlişkili özellikler:** 129, 132.

#### 132. Diagnostics report
...
- **Reset/tekrar koşum:** Profile restart. **Risk ve notlar:** Sensitive setup metadata nedeniyle production'da opt-in'dir. **İlişkili özellikler:** 25, 118–121, 159.
...
- **Reset/tekrar koşum:** Analytics seed restore. **Risk ve notlar:** Live few runs chart için yetersizdir; seeded history gerekir. **İlişkili özellikler:** 51, 59–60, 126–128.

#### 134. Tool usage analytics
...
- **Reset/tekrar koşum:** R1. **Risk ve notlar:** Tool business outcome'u başarı status'undan ayrı olabilir. **İlişkili özellikler:** 31–38, 126.

#### 135. Run event bridge
...
- **Reset/tekrar koşum:** Queue + R4. **Risk ve notlar:** Delivery guarantee consumer implementation'ına bağlıdır. **İlişkili özellikler:** 52, 103.

#### 136. Silent-gap diagnostics
...
- **Reset/tekrar koşum:** R4. **Risk ve notlar:** Live store'u bozma. **İlişkili özellikler:** 52, 125.

### ASP.NET Core, protokoller ve client araçları

#### 137. Management HTTP API
...
- **Reset/tekrar koşum:** R1. **Risk ve notlar:** Endpoint listesi için published OpenAPI kullan. **İlişkili özellikler:** 138, 145–147, 150–151.

#### 138. OpenAPI metadata
...
- **Reset/tekrar koşum:** R4. **Risk ve notlar:** Technical client appendix. **İlişkili özellikler:** 137, 145–146.

#### 139. OpenAI Responses compatibility
...
- **Reset/tekrar koşum:** R1. **Risk ve notlar:** Management API ile compatibility surface ayrıdır. **İlişkili özellikler:** 21, 49, 63.

#### 140. OpenAI Chat Completions compatibility
...
- **Reset/tekrar koşum:** R1. **Risk ve notlar:** Responses-only özellik parity'si varsayılmaz. **İlişkili özellikler:** 21, 49–50.

#### 141. OpenAI Conversations compatibility
...
- **Reset/tekrar koşum:** R1. **Risk ve notlar:** SQL store önerilir. **İlişkili özellikler:** 61–62, 139.
...
- **Reset/tekrar koşum:** R1. **Risk ve notlar:** Remote access/scope guard zorunludur. **İlişkili özellikler:** 95, 112, 143.
...
- **Reset/tekrar koşum:** R1. **Risk ve notlar:** Protocol appendix. **İlişkili özellikler:** 72, 77, 142.
...
- **Reset/tekrar koşum:** R1. **Risk ve notlar:** Console'da A2A screen yoktur. **İlişkili özellikler:** 95, 112, 142.

#### 145. Typed .NET client
...
- **Reset/tekrar koşum:** R4/R1. **Risk ve notlar:** Technical client appendix. **İlişkili özellikler:** 137–138, 146.

#### 146. Typed TypeScript client
...
- **Reset/tekrar koşum:** R4/R1. **Risk ve notlar:** Console da bu client'ı kullanır; bu tek başına public consumer proof değildir. **İlişkili özellikler:** 138, 145.
...
- **Reset/tekrar koşum:** R4. **Risk ve notlar:** Technical/CI appendix. **İlişkili özellikler:** 83, 118, 129.

#### 148. Embeddable chat widget
...
- **Reset/tekrar koşum:** Browser storage + R1. **Risk ve notlar:** “Panel ekranı” diye sunulmaz. **İlişkili özellikler:** 38, 111, 150.

#### 149. Voice WebSocket protocol
...
- **Reset/tekrar koşum:** R1. **Risk ve notlar:** Native client parity technical appendix'tedir. **İlişkili özellikler:** 29–30, 64.

### Dashboard ve kullanıcı arayüzü

#### 150. Embedded console
...
- **Reset/tekrar koşum:** Browser cache. **Risk ve notlar:** Source UI build'i showcase'e kopyalanmaz. **İlişkili özellikler:** 151–160, 180.

#### 151. Runtime base path
...
- **Reset/tekrar koşum:** Profile restart. **Risk ve notlar:** Proxy path rewrite prova edilir. **İlişkili özellikler:** 137, 150.
...
- **Reset/tekrar koşum:** Analytics seed. **Risk ve notlar:** Seed olmadan boş dashboard demo değeri düşürür. **İlişkili özellikler:** 127, 129, 133.

#### 153. Agent ve skill yönetimi
...
- **Reset/tekrar koşum:** R1. **Risk ve notlar:** Tool body ve registered script UI'da yazılmaz. **İlişkili özellikler:** 1–14, 41–42.
...
- **Reset/tekrar koşum:** Playground reset/R1. **Risk ve notlar:** Her capability aynı provider'da olmayabilir. **İlişkili özellikler:** 9, 29–30, 38, 49, 63–65.

#### 155. Session ve run inceleme
...
- **Reset/tekrar koşum:** R1. **Risk ve notlar:** Tek run'da tüm action'lar zorlanmaz. **İlişkili özellikler:** 51–65, 125.

#### 156. Workflow ve job yönetimi
...
- **Reset/tekrar koşum:** R1. **Risk ve notlar:** Restart/failure cases prerecorded olabilir. **İlişkili özellikler:** 66–82.

#### 157. Eval ve experiment yönetimi
...
- **Reset/tekrar koşum:** R1. **Risk ve notlar:** Seeded traffic gerekir. **İlişkili özellikler:** 83–90.

#### 158. Governance ekranları
...
- **Reset/tekrar koşum:** R2. **Risk ve notlar:** Raw keys/secrets gösterilmez. **İlişkili özellikler:** 35–46, 91–113.

#### 159. Catalog ve operations ekranları
...
- **Reset/tekrar koşum:** Health cache/profile. **Risk ve notlar:** Diagnostics sensitive olabilir. **İlişkili özellikler:** 31–33, 129, 132.

#### 160. Kullanılabilirlik seçenekleri
...
- **Reset/tekrar koşum:** Browser local storage. **Risk ve notlar:** Locale agent output dili değildir. **İlişkili özellikler:** 10, 150.

### Paketleme, DI ve genişletilebilirlik

#### 161. Modüler NuGet ailesi
...
- **Beklenen görünür sonuç:** Capability'ye göre farklı graph. **Kanıt ölçütü:** Packed-consumer tests. **Reset/tekrar koşum:** R4. **Risk ve notlar:** Teknik satın alma eki. **İlişkili özellikler:** 162–164.
...
- **Beklenen görünür sonuç:** OpenAI/PostgreSQL/MCP/workflow/API/UI hazırdır. **Kanıt ölçütü:** Packed sample. **Reset/tekrar koşum:** R4. **Risk ve notlar:** Her projede en küçük graph değildir. **İlişkili özellikler:** 161, 168.

#### 163. Target framework desteği
...
- **Beklenen görünür sonuç:** Üç TFM başarılı. **Kanıt ölçütü:** Packed-consumer CI. **Reset/tekrar koşum:** R4. **Risk ve notlar:** Canlı terminalde üç restore gereksizdir. **İlişkili özellikler:** 180–182.

#### 164. Package bazlı AOT contract'ı
...
- **Beklenen görünür sonuç:** AOT publish/smoke success. **Kanıt ölçütü:** Package metadata + tests. **Reset/tekrar koşum:** R4. **Risk ve notlar:** ASP.NET hosting package parity varsayılmaz. **İlişkili özellikler:** 32, 169.

#### 165. Consumer-first DI
...
- **Beklenen görünür sonuç:** Consumer implementation kazanır. **Kanıt ölçütü:** Service descriptor ve contract tests. **Reset/tekrar koşum:** R4. **Risk ve notlar:** UI demo değeri düşük. **İlişkili özellikler:** 121, 167.

#### 166. MAF type passthrough
...
- **Beklenen görünür sonuç:** Native MAF types work end-to-end. **Kanıt ölçütü:** Public API/reflection signature test. **Reset/tekrar koşum:** R4. **Risk ve notlar:** Architect appendix. **İlişkili özellikler:** 2, 31.

#### 167. Agent decorator seam
...
- **Beklenen görünür sonuç:** Decorated output/metadata ve normal recording. **Kanıt ölçütü:** Registration order test'i. **Reset/tekrar koşum:** R1/R4. **Risk ve notlar:** Panel action yoktur. **İlişkili özellikler:** 60, 124, 165.
...
- **Beklenen görünür sonuç:** Çalışan host/sample tool ve boş secret placeholders. **Kanıt ölçütü:** Packed template test. **Reset/tekrar koşum:** R4. **Risk ve notlar:** Ana showcase'i template sırasında üretme. **İlişkili özellikler:** 162, 180.

#### 169. Pre-release dependency isolation
...
- **Beklenen görünür sonuç:** Preview MAF hosting yalnız ASP.NET Core package graph'ındadır. **Kanıt ölçütü:** NuGet dependency tests. **Reset/tekrar koşum:** R4. **Risk ve notlar:** Son kullanıcı capability'si gibi sunma. **İlişkili özellikler:** 161–164.

### Test, kalite ve coding-agent desteği

#### 170. Fake model provider
...
- **Beklenen görünür sonuç:** Aynı output/usage/tool sequence. **Kanıt ölçütü:** No network ve stable assertions. **Reset/tekrar koşum:** R4/R1. **Risk ve notlar:** Gerçek provider kalitesini kanıtlamaz. **İlişkili özellikler:** 171–172.

#### 171. Integrated test host
...
- **Beklenen görünür sonuç:** Isolated passing test. **Kanıt ölçütü:** Real endpoint/catalog path. **Reset/tekrar koşum:** Test teardown. **Risk ve notlar:** Production persistence proof değildir. **İlişkili özellikler:** 170, 172.

#### 172. Framework-neutral assertions
...
- **Beklenen görünür sonuç:** Clear assertion message. **Kanıt ölçütü:** xUnit/NUnit/MSTest dependency yoktur. **Reset/tekrar koşum:** R4. **Risk ve notlar:** Technical testing appendix. **İlişkili özellikler:** 170–173.

#### 173. Extension contract suites
...
- **Beklenen görünür sonuç:** Passing implementation ve actionable failing contract. **Kanıt ölçütü:** Relevant contract coverage. **Reset/tekrar koşum:** R4. **Risk ve notlar:** Ana canlı akışa alınmaz. **İlişkili özellikler:** 4, 26, 73, 85, 121.

#### 174. Source generator diagnostics
...
- **Beklenen görünür sonuç:** Expected compile diagnostics. **Kanıt ölçütü:** Analyzer/generator tests. **Reset/tekrar koşum:** R4. **Risk ve notlar:** Ayrı failing project kullan. **İlişkili özellikler:** 32, 175.

#### 175. Usage analyzer diagnostics
...
- **Beklenen görünür sonuç:** Expected diagnostics; secret yalnız placeholder'dır. **Kanıt ölçütü:** Analyzer tests/baselines. **Reset/tekrar koşum:** R4. **Risk ve notlar:** Gerçek secret yazma. **İlişkili özellikler:** 107, 174.

#### 176. Generated agent map
...
- **Beklenen görünür sonuç:** Capability map; mevcut file ezilmez. **Kanıt ölçütü:** Build tests. **Reset/tekrar koşum:** R4. **Risk ve notlar:** Showcase runtime müşterisi için ikincil değer. **İlişkili özellikler:** 177–178.

#### 177. Local reference üretimi
...
- **Beklenen görünür sonuç:** Exact local references. **Kanıt ölçütü:** Restore graph ile version eşleşir. **Reset/tekrar koşum:** R4. **Risk ve notlar:** Runtime feature değildir. **İlişkili özellikler:** 176, 178.

#### 178. Web-agent doküman yüzeyi
...
- **Beklenen görünür sonuç:** Index ve full text. **Kanıt ölçütü:** Documentation build/checks. **Reset/tekrar koşum:** R4. **Risk ve notlar:** Product docs appendix. **İlişkili özellikler:** 176–177.

#### 179. Public API tracking
...
- **Beklenen görünür sonuç:** Build error. **Kanıt ölçütü:** Public API analyzer. **Reset/tekrar koşum:** R4. **Risk ve notlar:** Showcase repo'suna intentional break ekleme. **İlişkili özellikler:** 163, 182.

#### 180. Packed-consumer doğrulaması
...
- **Beklenen görünür sonuç:** Package/template/UI/generated tool tests pass. **Kanıt ölçütü:** `.nupkg` consumer run. **Reset/tekrar koşum:** R4. **Risk ve notlar:** Uzun ve release-internal. **İlişkili özellikler:** 150, 168, 181–182.

#### 181. Gerçek altyapı testleri
...
- **Beklenen görünür sonuç:** Provider/browser/client test status. **Kanıt ölçütü:** Real infrastructure gates. **Reset/tekrar koşum:** CI rerun/R4. **Risk ve notlar:** Canlı koşum uzun ve kırılgandır. **İlişkili özellikler:** 115–118, 138, 146.

#### 182. Build kalite kapıları
...
- **Beklenen görünür sonuç:** Zero-warning green report. **Kanıt ölçütü:** Repository closing gate. **Reset/tekrar koşum:** CI rerun. **Risk ve notlar:** Uzun canlı build sunumu bozabilir. **İlişkili özellikler:** 174–181.
...
- **Kapsanan özelliklerin tam listesi:** 1–20, ayrıca 49, 51–57, 150–155.
...
- **Kapsanan özelliklerin tam listesi:** 31–48, ayrıca 101, 107, 113, 134, 158–159.
...
- **Kapsanan özelliklerin tam listesi:** 28–30, 49–65, 125, 127, 149, 155.
...
- **Kapsanan özelliklerin tam listesi:** 66–71, ayrıca 35, 59, 156.
...
- **Kapsanan özelliklerin tam listesi:** 72–82. 79, 80 ve 82 yalnız prerecorded/technical evidence'dır.
...
- **Kapsanan özelliklerin tam listesi:** 83–90, ayrıca 58, 75, 157.
...
- **Kapsanan özelliklerin tam listesi:** 91–113, ayrıca 60, 96, 123, 158. 106, 108, 110–113 technical/security appendix'tedir.
...
- **Kapsanan özelliklerin tam listesi:** 17–27, 129–132.
...
- **Kapsanan özelliklerin tam listesi:** 124–136, ayrıca 51–60, 99, 152, 159. 124, 126, 131 ve 135 external technical evidence kullanır; 136 gösterilmez.
...
- **Kapsanan özelliklerin tam listesi:** 137–149, ayrıca 38, 49–50, 61, 95, 111–112.
...
- **Kapsanan özelliklerin tam listesi:** 108–110, 114–123, ayrıca 61–63, 132.
...
- **Kapsanan özelliklerin tam listesi:** 32, 119–121, 138, 145–147, 161–182.
...
- **Kapsanan özelliklerin tam listesi:** 150–160.
...
| 9 | C1 | Parameterized instructions | DP | A | Playground form | Model | Evet |
| 10 | C1 | Culture-aware instructions | DP | A | Editor/Playground/Diff | Model | Evet |
| 11 | C1 | Shared instructions | PD | A | Run output/fingerprint | Model | Kısa |
| 12 | C1 | Agent-to-agent çağrı grafiği | DP | A/C | Callable editor/Run tree | Model | Evet |
| 13 | C1 | Harness mode | DP | A | Editor/Run events | Model/fixture | Kısa |
| 14 | C1 | Context compaction | DP | A/C | Editor/Timeline/Trace | Model | Kısa |
| 15 | C1 | Response cache | PD | A/I | Trace/Compare/Dashboard | Redis | Kısa |
| 16 | C1 | Concurrent tool calls | DP | A/I | Editor/Trace waterfall | Model | Kısa |
| 17 | C1 | Model fallback zinciri | DP | H | Editor/Timeline | 2 providers/proxy | Evet |
| 18 | C1 | Provider concurrency limiti | PD | H | Runs/Trace + proxy metric | Proxy | Ek |
| 19 | C1 | Context preflight | PD | A | Playground rejection | Model catalog | Kısa |
| 20 | C1 | Compiled agent cache | G | A/L | Yalnız dolaylı | Benchmark | Hayır |
| 21 | C2 | OpenAI provider | PD | H | Models/Run | OpenAI | Evet |
| 22 | C2 | OpenAI-compatible provider | PD | H | Models/Run | Compatible endpoint | Kısa |
| 23 | C2 | Anthropic provider | PD | H | Models/Reasoning/Usage | Anthropic | Kısa |
| 24 | C2 | Google provider | PD | H | Models/Tenant/Run | Google | Kısa |
| 25 | C2 | Azure OpenAI provider | PD | H | Models/Diagnostics/Run | Azure | Ek |
| 26 | C2 | Custom model provider | PD | H/L | Models/Run | Custom code | Ek |
| 27 | C2 | Configuration tabanlı model catalog | PD | H/I | Models/Dashboard | Config | Evet |
| 28 | C2 | Image generation | PD | C | Playground/Session attachment | Image provider | Kısa |
| 29 | C2 | Speech tools | DP | C | Playground/Settings/Tools | ElevenLabs | Kısa |
| 30 | C2 | Live voice conversation | DP | C | Playground/Session | Speech + browser | Kısa |
| 31 | C3 | Tool kayıt seçenekleri | PD | B/L | Tools/Run | Code | Kısa |
| 32 | C3 | Build-time tool generation | TD | L | Build + Tools dolaylı | SDK | Ek |
| 33 | C3 | Tool governance metadata | PD | B | Tools badges | Code | Evet |
| 34 | C3 | Tool authorization | PD | B/G | Run decision | Auth context | Kısa |
| 35 | C3 | Tool approval akışı | DP | B | Approvals/Run/Jobs | Worker | Evet |
| 36 | C3 | Standing approval rules | DP | B | MCP remembered approvals | Yok | Evet |
| 37 | C3 | Tool timeout ve output sınırı | PD | B | Tool calls/Timeline | Fixture | Kısa |
| 38 | C3 | Client-side tools | DP | B/J | Playground/Widget/Tools | Browser | Evet |
| 39 | C3 | Custom content guards | PD | B | Run guard events | Custom code | Kısa |
| 40 | C3 | Pattern content guard | PD | B/G | Playground/Run | Config | Kısa |
| 41 | C3 | Skill catalog | DP | B | Skills/Agent/Playground | Yok | Evet |
| 42 | C3 | Skill script execution | PD | B | Skills grants/Run/Audit | Interpreter | Kısa |
| 43 | C3 | Remote MCP tool discovery | DP | B | MCP/Tools | MCP server | Evet |
| 44 | C3 | MCP authentication ve OAuth | DP | B | MCP Authorize | OAuth fixture | Kısa |
| 45 | C3 | MCP prompts | PD | B/J | API + Run output | MCP server | Ek |
| 46 | C3 | MCP resources | PD | B/J | MCP + Run output | MCP server | Kısa |
| 47 | C3 | Knowledge ingestion ve search | PD | B/K | Tool calls/Run | pgvector/embed | Kısa |
| 48 | C3 | Working memory | DP | B | Agent context/Run | Optional vector | Kısa |
| 49 | C4 | Streaming run | DP | A/C | Playground/live Run | Model | Evet |
| 50 | C4 | Non-streaming run | PD | J | HTTP + Run | Client | Kısa |
| 51 | C4 | Run recording | DP | C/I | Runs/Detail/Dashboard | SQL | Evet |
| 52 | C4 | Ordered event stream | DP | C | Run timeline/reload | Browser/SSE | Kısa |
| 53 | C4 | Run tree | DP | C | Run call tree | Model | Evet |
| 54 | C4 | Run budget | PD | C | Run failure/tree | Model/fake | Kısa |
| 55 | C4 | Run cancellation | DP | C/E | Run Cancel/tree | Slow fixture | Kısa |
| 56 | C4 | Run replay | DP | C | Run Replay | Recorded input | Evet |
| 57 | C4 | Run karşılaştırma | DP | A/C | Run Compare | Seeded runs | Evet |
| 58 | C4 | Run feedback ve score | DP | F/I | Run Feedback/Dashboard | Judge optional | Evet |
| 59 | C4 | Run error classification | DP | E/I | Failure/Clusters | Failure fixture | Evet |
| 60 | C4 | Run attribution | PD | G/I | Run badges/filters | Consumer context | Kısa |
| 61 | C4 | Durable sessions | DP | C/K | Sessions/Playground | SQL | Evet |
| 62 | C4 | Session branching | DP | C | Session Branch | SQL | Evet |
| 63 | C4 | Attachment yönetimi | DP | C/K | Playground/Session | Storage | Evet |
| 64 | C4 | Multimodal messages | DP | C | Playground/Run | Capable provider | Kısa |
| 65 | C4 | Document channel | PD | C | Attachment + proxy proof | Provider/proxy | Ek |
| 66 | C5 | Multi-agent workflows | DP | D | Workflow graph/run | Model | Evet |
| 67 | C5 | Workflow tanımları | DP | D | Workflow editor/detail | SQL | Evet |
| 68 | C5 | Workflow functions | PD | D | Graph/events | Code | Kısa |
| 69 | C5 | Durable checkpoints | PD | D/E | Pending/resumed run | SQL/restart | Kısa |
| 70 | C5 | Human-in-the-loop | DP | D | Pending request | Worker | Evet |
| 71 | C5 | Workflow node retry | PD | D | Live graph/timeline | Fail-once node | Kısa |
| 72 | C5 | Job queue | DP | E | Jobs/list/detail | Worker/SQL | Evet |
| 73 | C5 | Custom job handler | PD | E/L | Job detail | Custom code | Kısa |
| 74 | C5 | Agent ve workflow jobs | DP | E | Jobs/linked runs | Worker | Evet |
| 75 | C5 | Schedules | DP | E/F | Jobs/Schedules | Worker | Evet |
| 76 | C5 | Worker control | PD | E | Pending→running jobs | 2 processes | Ek |
| 77 | C5 | Async HTTP run | PD | E/J | HTTP 202 + Jobs/Runs | Client/worker | Kısa |
| 78 | C5 | Idempotent HTTP işlemleri | PD | E/J | HTTP + single Run | Client | Kısa |
| 79 | C5 | Singleton execution | G | E | Job dolaylı + lease | 2 workers | Hayır |
| 80 | C5 | Run reconciliation | G | E | Reconciled Run | Crash fixture | Hayır |
| 81 | C5 | Run continuation | PD | E | Continuation link | Crash fixture | Ek |
| 82 | C5 | Graceful drain | G | E | Run status dolaylı | Process control | Hayır |
| 83 | C6 | Eval suite ve case yönetimi | DP | F | Evals/Suite/Run | Worker/model | Evet |
| 84 | C6 | Built-in eval checks | DP | F | Eval results | Yok | Evet |
| 85 | C6 | Custom eval checks | PD | F/L | Eval results | Custom code | Kısa |
| 86 | C6 | Run judges | PD | F | Scores/Eval results | Judge model/code | Kısa |
| 87 | C6 | Run-to-case promotion | DP | F | Run Promote/Evals | Input recording | Evet |
| 88 | C6 | Online evaluation | DP | F/I | Dashboard/Jobs/Scores | Worker/judge | Kısa |
| 89 | C6 | A/B experiments | DP | F | Experiments/results | Seeded traffic | Evet |
| 90 | C6 | Canary rollback | DP | F | Experiment Canary | Worker/fake clock | Evet |
| 91 | C7 | Loopback varsayılanı | PD | G/J | Settings Access + remote red | Network profile | Kısa |
| 92 | C7 | Static bearer token | PD | G | Access Gate/Settings | Secret store | Kısa |
| 93 | C7 | ASP.NET Core authorization | PD | G | Login/Settings + 403 | Auth pipeline | Kısa |
| 94 | C7 | Role policy'leri | PD | G | Role-aware UI + 403 | Demo identities | Evet |
| 95 | C7 | Scoped API key'ler | DP | G/J | Settings/API keys | Client | Evet |
| 96 | C7 | Multi-tenancy | PD | G/K | Tüm tenant-scoped screens | Auth/SQL | Evet |
| 97 | C7 | Tenant BYOK | DP | G/H | Settings/Tenant providers | Provider secret | Evet |
| 98 | C7 | Tenant provider egress policy | DP | G/H | Settings/Agent validation | Providers | Evet |
| 99 | C7 | Usage quota | DP | G/I | Settings/Quota/Run red | Pricing/SQL | Evet |
| 100 | C7 | HTTP rate limiting | PD | G/J | HTTP 429 + run count | Client/fake clock | Ek |
| 101 | C7 | Audit trail | DP | G | Audit | SQL/auth | Evet |
| 102 | C7 | Tamper-evident audit chain | PD | G | Audit + verify API | SQL | Ek |
| 103 | C7 | Signed outbound webhooks | DP | G/E | Settings/Webhooks/Jobs | HTTPS receiver | Evet |
| 104 | C7 | Signed inbound triggers | DP | G/E | Triggers/Jobs/Runs | Signed client | Evet |
| 105 | C7 | Trigger replay ve flood koruması | PD | G | HTTP red + job count | Signed client | Ek |
| 106 | C7 | Outbound network guard | G | G | Failure dolaylı | Isolated network | Hayır |
| 107 | C7 | Secret reference sınırı | DP | G | Settings/Triggers/MCP | Config keys | Evet |
| 108 | C7 | At-rest content protection | TD | G/K | Normal panel read + DB proof | Key provider | Ek |
| 109 | C7 | Retention ve archive | DP | G/K | Settings/Retention | SQL/archive sink | Evet |
| 110 | C7 | Data subject hakları | TD | G/K | API + list change | Resolver | Ek |
| 111 | C7 | CORS allowlist | TD | J | Browser network/widget | 2 origins | Ek |
| 112 | C7 | External protocol guard | TD | G/J | Protocol matrix | MCP/A2A clients | Ek |
| 113 | C7 | Skill script güvenlik sınırı | G | B/G | Skills grants/Audit | Interpreter | Hayır |
| 114 | C8 | In-memory varsayılanlar | PD | K | Settings/footer | Yok | Kısa |
| 115 | C8 | PostgreSQL persistence | PD | K | Settings/Diagnostics/durable UI | PostgreSQL | Evet |
| 116 | C8 | SQL Server persistence | PD | K | Settings/Diagnostics | SQL Server | Ek |
| 117 | C8 | SQLite persistence | PD | K | Settings/Diagnostics | SQLite | Ek |
| 118 | C8 | Migration yönetimi | PD | K/L | CLI + Diagnostics | SQL/CLI | Evet |
| 119 | C8 | Consumer data source kullanımı | TD | K/L | Diagnostics dolaylı | Instrumented DB | Ek |
| 120 | C8 | Read contract view'ları | TD | K/I | SQL/BI + Dashboard | SQL client | Ek |
| 121 | C8 | Replaceable store'lar | TD | K/L | Diagnostics + DI test | Custom store | Ek |
| 122 | C8 | External attachment storage | PD | C/K | Playground/Session + object | Object store | Kısa |
| 123 | C8 | Tenant-isolated durable contracts | PD | G/K | Tüm domain lists + API | SQL/auth | Evet |
| 124 | C9 | OpenTelemetry traces | TD | I | External trace + waterfall | OTLP backend | Ek |
| 125 | C9 | Persisted trace sample | DP | I | Run Trace waterfall | SQL | Evet |
| 126 | C9 | Metrics | TD | I | Metrics backend + Dashboard | Exporter | Ek |
| 127 | C9 | Cost attribution | DP | I | Dashboard/Run tree | Priced providers | Evet |
| 128 | C9 | Cost recalculation | PD | I | API + Dashboard | Price config | Kısa |
| 129 | C9 | Provider health | DP | H/I | Models/Dashboard | Providers | Evet |
| 130 | C9 | Circuit breaker | PD | H | Run events + proxy count | Failure proxy | Kısa |
| 131 | C9 | ASP.NET Core health check | TD | H/I | Health endpoint/Diagnostics | Health pipeline | Ek |
| 132 | C9 | Diagnostics report | DP | H/I/K | Diagnostics | Admin/config | Evet |
| 133 | C9 | Run analytics | DP | I | Dashboard/Runs/Settings | Seeded history | Evet |
| 134 | C9 | Tool usage analytics | DP | B/I | Tools/Run tool calls | Seeded calls | Evet |
| 135 | C9 | Run event bridge | TD | I | Run timeline + bus | Custom sink/bus | Ek |
| 136 | C9 | Silent-gap diagnostics | G | I | Log only | Corrupt fixture | Hayır |
| 137 | C10 | Management HTTP API | PD | J | Console state + HTTP | Client/auth | Evet |
| 138 | C10 | OpenAPI metadata | TD | J/L | OpenAPI document | Tooling | Ek |
| 139 | C10 | OpenAI Responses compatibility | PD | J | Runs/Sessions | OpenAI client | Evet |
| 140 | C10 | OpenAI Chat Completions compatibility | PD | J | Runs | OpenAI client | Kısa |
| 141 | C10 | OpenAI Conversations compatibility | PD | J | Sessions | OpenAI client/SQL | Kısa |
| 142 | C10 | MCP server | PD | J | MCP client + Runs | MCP client/key | Evet |
| 143 | C10 | MCP task mode | PD | J/E | Client poll + Jobs/Runs | MCP client/worker | Kısa |
| 144 | C10 | A2A server | PD | J | A2A client + Runs | A2A client/key | Kısa |
| 145 | C10 | Typed .NET client | TD | J/L | Client + panel state | .NET sample | Ek |
| 146 | C10 | Typed TypeScript client | TD | J/L | Client + panel state | Node/browser | Ek |
| 147 | C10 | CLI | TD | L | Terminal + domain screens | CLI | Ek |
| 148 | C10 | Embeddable chat widget | PD | J | Widget + Runs/Sessions | Product origin | Evet |
| 149 | C10 | Voice WebSocket protocol | DP | C/J | Playground voice | Speech/browser | Kısa |
| 150 | C11 | Embedded console | DP | M | Tüm console | UI package | Evet |
| 151 | C11 | Runtime base path | DP | M | Settings/deep links | Proxy optional | Evet |
| 152 | C11 | Dashboard | DP | M/I | Dashboard | Seeded data | Evet |
| 153 | C11 | Agent ve skill yönetimi | DP | M/A/B | Agents/Skills | SQL | Evet |
| 154 | C11 | Playground | DP | M/A/C | Playground | Model/media | Evet |
| 155 | C11 | Session ve run inceleme | DP | M/C | Sessions/Runs details | SQL | Evet |
| 156 | C11 | Workflow ve job yönetimi | DP | M/D/E | Workflows/Jobs | Worker/SQL | Evet |
| 157 | C11 | Eval ve experiment yönetimi | DP | M/F | Evals/Experiments | Worker/model | Evet |
| 158 | C11 | Governance ekranları | DP | M/G | Approvals/MCP/Triggers/Audit/Settings | Auth/SQL | Evet |
| 159 | C11 | Catalog ve operations ekranları | DP | M/H/I | Tools/Models/Diagnostics/Settings | Providers | Evet |
| 160 | C11 | Kullanılabilirlik seçenekleri | DP | M | Shell/Settings/Palette | Browser | Evet |
| 161 | C12 | Modüler NuGet ailesi | TD | L | Package graph | NuGet | Ek |
| 162 | C12 | Meta package | TD | L | Restore + console dolaylı | NuGet | Ek |
| 163 | C12 | Target framework desteği | TD | L | Build matrix | SDK matrix | Ek |
| 164 | C12 | Package bazlı AOT contract'ı | TD | L | AOT report/smoke | Native AOT | Ek |
| 165 | C12 | Consumer-first DI | TD | L | DI test/Diagnostics | Custom code | Ek |
| 166 | C12 | MAF type passthrough | TD | L | Compile/signatures | MAF | Ek |
| 167 | C12 | Agent decorator seam | PD | L/I | Run metadata/trace | Custom code | Ek |
| 168 | C12 | Project template | TD | L | Template build/console | CLI/NuGet | Ek |
| 169 | C12 | Pre-release dependency isolation | G | L | Package graph | NuGet | Hayır |
| 170 | C13 | Fake model provider | TD | L | Test + deterministic run | Testing package | Ek |
| 171 | C13 | Integrated test host | TD | L | Test output | Testing package | Ek |
| 172 | C13 | Framework-neutral assertions | TD | L | Test output | Testing package | Ek |
| 173 | C13 | Extension contract suites | TD | L | Contract report | xUnit/infra | Ek |
| 174 | C13 | Source generator diagnostics | TD | L | Failing build | SDK | Ek |
| 175 | C13 | Usage analyzer diagnostics | TD | L | Failing build | SDK | Ek |
| 176 | C13 | Generated agent map | TD | L | Generated file | Build target | Ek |
| 177 | C13 | Local reference üretimi | TD | L | Generated reference | Restore/build | Ek |
| 178 | C13 | Web-agent doküman yüzeyi | TD | L | llms artifacts | Docs build | Ek |
| 179 | C13 | Public API tracking | G | L | Build gate | Repository | Hayır |
| 180 | C13 | Packed-consumer doğrulaması | G | L | CI evidence | Package pipeline | Hayır |
| 181 | C13 | Gerçek altyapı testleri | G | L | CI matrix/screenshots | Infra matrix | Hayır |
| 182 | C13 | Build kalite kapıları | G | L | CI gate report | Full repo | Hayır |
...
| --: | --- | --- | --- | --- | --- |
| 20 | Compiled agent cache | Internal optimization; güvenilir UI kanıtı yok. | Compile counter ve benchmark. | Performance appendix. | Benchmark isteniyor mu? |
| 79 | Singleton execution | İki process ve lease timing'i gerekir. | Multi-worker integration test ve lease query. | Reliability appendix. | Multi-node topology sunulacak mı? |
| 80 | Run reconciliation | Process kill ana ortamı kirletir. | Isolated crash test ve prerecorded run. | Reliability appendix. | Recorded failure kanıtı yeterli mi? |
| 82 | Graceful drain | Showcase host'unu kapatır. | SIGTERM integration test. | Deployment appendix. | Kubernetes senaryosu gerekli mi? |
| 106 | Outbound network guard | SSRF/private network denemesi güvenlik riski taşır. | Isolated DNS/socket integration tests. | Security appendix. | Pentest raporu paylaşılacak mı? |
| 113 | Skill script güvenlik sınırı | OS sandbox yoktur; untrusted script demosu yanlış güven verir. | Allowlist/grant/limit tests. | Security limitations. | Script özelliği ana showcase'te açık mı? |
| 136 | Silent-gap diagnostics | Bozuk event/span verisi üretmek gerekir. | Corruption fixture ve log assertion. | Observability appendix. | Operasyon log'u gösterilecek mi? |
| 169 | Pre-release dependency isolation | Package internal hygiene'dır. | Packed dependency graph assertion. | Package appendix. | Dependency due diligence kapsamda mı? |
| 179 | Public API tracking | Maintainer gate'idir. | Isolated baseline failure. | Release engineering appendix. | API stability kanıtı isteniyor mu? |
| 180 | Packed-consumer doğrulaması | Uzun release-pipeline testidir. | Son CI artifact/report. | Due-diligence pack. | CI evidence paylaşılacak mı? |
| 181 | Gerçek altyapı testleri | Canlı koşum uzun ve nondeterministic'tir. | CI matrix ve E2E screenshots. | Due-diligence pack. | Hangi infra raporu paylaşılabilir? |
| 182 | Build kalite kapıları | Ürün yeteneği değil repository gate'idir. | `kapi.py`/CI green report. | Release engineering appendix. | Full gate log'u gerekli mi? |

### Güvenlik ve failure dipnotları

- 102 audit-chain invalid örneği için canlı SQL mutation yapılmaz. Isolated snapshot
  veya test sonucu kullanılır.
- 108 at-rest protection için yalnız sentetik known plaintext kullanılır. Encryption
  key, raw ciphertext veya production schema dump'ı sunum kaydına alınmaz.
- 110 data-subject erase yalnız demo tenant ve sentetik subject üzerinde çalışır.
- 130 circuit breaker ve 17 fallback gerçek provider credential'ını bozarak değil,
  controlled proxy ile tetiklenir.
- 69, 80, 81 ve 82 process lifecycle senaryoları ayrı environment'ta prova edilir.
  Ana demo ortamında process kill yapılmaz.
- 174 ve 175 intentionally failing projects ayrı dizinde tutulur. Ana showcase
...
6. **Sunum tek uzun akış mı, modüler mi olmalı?** Öneri 45 dakikalık modüler ana
   akış ve seçilebilir eklerdir. 182 özelliği art arda göstermek etkisizdir.
...
2. **Kaynak özellik sayısı:** 182. Özet altındaki iki sayı satırı özellik olarak
   sayılmamıştır.
3. **Sınıf dağılımı:** 73 Doğrudan panel demosu, 65 Panel destekli demo, 32 Panel
   dışı teknik demo, 12 Gösterilmemeli, 0 Belirsiz.
4. **Değerlendirilmeden kalan:** Yok. Katalog ve matris 1–182 arasında kesintisizdir.
5. **Kod veya UI ile doğrulanamayan iddialar:** Kaynak envanterdeki paket destek
   ...
   source failure detail (4–5), shared-instruction resolved text (11), compiled
   cache (20), MCP prompt/resource viewer (45–46), document-channel badge (65),
   idempotency UI (78), singleton/reconciliation/drain UI (79–80, 82), audit-chain
   verify action'ı (102), data-subject UI (110), OTel/metrics/health/event-bridge UI
   (124, 126, 131, 135), circuit-breaker state ekranı (130), A2A ekranı (144) ve
   build/package/test özelliklerinin UI yüzeyi (161–182). Bunlar mevcut özellik
