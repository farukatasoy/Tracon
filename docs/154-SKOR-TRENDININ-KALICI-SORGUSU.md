...
- [ ] `GET /api/evaluation/scores/summary` sunucu yeniden başladıktan **sonra** aynı sonucu döner
- [ ] `GET /api/evaluation/online` davranışı **değişmemiştir** (canlı gösterge hâlâ sıfırlanır)
- [ ] Farklı `RunScoreKind`'lar aynı ortalamaya girmez; kova anahtarı `(name, kind)`'dır
- [ ] `Value = null` sayıma girer, ortalamaya girmez ve `noValueCount` ile raporlanır
- [ ] `Categorical` skorlar kategori sayımı döner, ortalama dönmez
- [ ] `created_at` indeksi üç sağlayıcıda da kuruldu; migration ikinci kez koşulunca hata vermiyor
- [ ] Yeni tablo **açılmadı**; "no durable counter store" kuralı korundu
- [ ] `EvalEndpoints.cs`'teki *"the 'run_scores' table can be queried directly"* cümlesi **kalktı**
- [ ] Dört doğrulama kapısı sıfır uyarı verir
- [ ] `samples/AgentPrism.Api` ile gerçek skor yazımı + yeniden başlatma sonrası özet alındı, çıktı belgeye yazıldı
- [ ] `secret` taraması boş döndü
- [ ] Manuel kabul case'leri `17-EVAL-VE-DENEYLER.md` ve `12-GOZLEMLENEBILIRLIK-MALIYET.md` içine eklendi
