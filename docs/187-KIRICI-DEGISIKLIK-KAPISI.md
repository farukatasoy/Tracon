...
C# public yüzeyi **değişmedi** (`PublicAPI.*.txt` diff'i yok). MSBuild özellik sözleşmesi
plandaki tabloyla aynıdır; ek: `TRACON0007` hedefi `TraconValidatePackageBaselineInputs`
(`BeforeTargets="GenerateNuspec"`).

`scripts/breaking_changes.py`:

```python
resolve_baseline(root) -> str                                        # BaselineError
compare_versions(left, right) -> int; version_is_above_baseline(version, baseline) -> bool
has_first_release_flag(root, project_id) -> bool
baseline_package_ids(root, library_ids) -> list[str]
stale_first_release_flags(root, baseline_tag) -> list[str]
restore_baselines(ids, version, work_dir, *, run=subprocess.run) -> Path   # RestoreError
baseline_source_violations(ids, version, cache) -> list[str]
pack_properties(cache, version, report_dir) -> list[str]             # üç -p
semaphore_path(root, project_id) -> Path                             # SemaphoreError
validation_not_run(root, ids, since) -> list[str]
type_of_doc_id(doc_id) -> str | None
read_breaking_changes(report_dir) -> BreakingChanges                 # types · dropped_frameworks · errors
release_notes_for(changelog, version) -> tuple[str | None, str | None]
untagged_cut_hint(changelog, baseline) -> str | None
unnamed_changes(changes, notes) -> list[str]
write_result(path, *, baseline, notes_heading, changes) -> Path
check(*, report_dir, baseline, version, changelog, result_path) -> int
main(argv) -> int    # --rapor-dizini --taban [--surum] [--changelog]
```

`scripts/kapi.py`: `BreakingChangeGate(baseline, report_dir, library_ids, pack_started)` ·
`_check_breaking_changes(root, gate, resolved_version) -> int` ·
`_finish_release_rehearsal(..., breaking_gate)` (yeni zorunlu parametre).
...
```
scripts/breaking_changes.py                (yeni, 603 satır)
scripts/breaking_changes_test.py           (yeni, 46 test)
scripts/testdata/breaking-changes/core-preview2.xml     (yeni, 187.0 adım 7)
scripts/testdata/breaking-changes/voice-net10-only.xml  (yeni, 187.0 adım 4)
scripts/kapi.py · kapi_test.py (+11 test, 1 güncellendi)
scripts/dokuman-bakim.py · dokuman_bakim_test.py (+1 test, 1 güncellendi)
src/Directory.Build.props · Directory.Build.targets · Directory.Build.props
tests/Tracon.Package.Tests/PackageBaselineWiringTests.cs (yeni, 9 fact)
CHANGELOG.md · README.md
docs-site/src/content/docs/reference/versioning.md
.agents/skills/faz-planlama/SKILL.md · resources/faz-plani-sablonu.md · faz-denetim/SKILL.md · faz-tamamlama/SKILL.md
docs/ADAYLAR.md (F-285…F-288) · KARARLAR.md (K-864, K-865, K-421 notu) · YAYIN-HAZIRLIK.md
docs/hafiza/paketleme-ve-dagitim.md (15.556 B) · yayin-ve-surumleme.md · kod-haritasi.md
docs/manuel-test/01-KURULUM-VE-PAKETLEME.md (MT-PKG-135…144) · 00-INDEKS.md · 33-DOKUMAN-KAPILARI.md
```

**Testler:** `breaking_changes_test.py` 46 (taban seçimi 10 · izole restore 5 · ilk yayın
bayrağı 3 · kanıt 4 · kırıcı liste 12 · not eşleşmesi 7 · `check`/CLI 6 · depo koruması 1) ·
`kapi_test.py` `YayinTestleri` +11 (sıra, restore hatası, rapor dizini benzersizliği,
istisnada temizlik, bayat bayrak, kırmızı kapıda promote yok + yeşil kontrol, eşit sürüm) ·
`PackageBaselineWiringTests` 9 (20 paketin gerçek MSBuild değerlendirmesi, `TRACON0007`,
kendi tabanına karşı iki ardışık gerçek pack). Python toplamı 539.
