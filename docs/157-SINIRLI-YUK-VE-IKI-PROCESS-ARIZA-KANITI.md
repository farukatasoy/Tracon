...
## Sonraki Faza Devir Notu/,0' docs/arsiv/fazlar/116-PERFORMANS-TAHSIS-KAPISI.md
   ```
   Bench projesinin sözleşmesini ve **neden yalnız tahsisin kapı olduğunu**
   oradan devralıyorsun. Bu faz o kararı değiştirmez, üstüne ekler.
4. Alan hafızası (bu faz iki alana dokunuyor):
...
|---|---|---|
| 🔴 Sağlayıcı zaman aşımı `Canceled` yazılıyor, uç `200` + boş gövde dönüyor, fallback denenmiyor | Manifest yazımı (`ProviderTimeoutTests`) | K-737 ile düzeltildi; iki seviyede regresyon testi eklendi |
| 🔴 `AgentPrism.WorkerHarness` `dotnet test AgentPrism.slnx` koşumunu ABORT ediyor | Kapanış kapısı | `IsTestProject=false` + `IsTestingPlatformApplication=false` AÇIKÇA yazıldı |
| 🟡 İlk `RetentionVolumeTests` kurgusu tablo kilidi altında da YEŞİL kalırdı | Kendi gözden geçirmesi | Test yeniden yazıldı: delete **commit edilmeden** açık tutulur, okumalar sınırlı token ile yapılır |
| 🟡 Kapı, izole koşumda da düşen SQL Server testleri raporladı | Kapanış kapısı | Paket TEK BAŞINA koşuldu: **703/703 yeşil**. `test-kosum-tuzaklari.md`'deki bilinen çekişme sınıfı; bu fazın değişiklikleri SQL Server'a dokunmuyor |
