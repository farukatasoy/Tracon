...
import { AgentPrismError } from '../src/error.js';
import { createAgentPrismClient } from '../src/client.js';

function client(fetch: (request: Request) => Promise<Response>) {
  return createAgentPrismClient({ baseUrl: 'https://example.test/agentprism', fetch });
}

describe('problem+json -> AgentPrismError (section 84.5)', () => {
  ...
  it('falls back to a status-line title when only title is present', async () => {
    ...
    expect(caught).toBeInstanceOf(AgentPrismError);
    expect((caught as AgentPrismError).title).toBe('Not found');
    expect((caught as AgentPrismError).detail).toBeNull();
  });
