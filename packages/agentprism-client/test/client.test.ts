...
describe('onUnauthorized', () => {
  it('is called on a 401 response, which still rejects with AgentPrismError', async () => {
    ...
    await expect(client.GET('/api/agents')).rejects.toBeInstanceOf(AgentPrismError);
...
describe('subsystem failure', () => {
  it('propagates a network failure as-is, not as an AgentPrismError', async () => {
    const client = createAgentPrismClient({
      baseUrl: 'https://example.test/agentprism',
      fetch: async () => {
