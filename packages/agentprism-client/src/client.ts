...
import { AgentPrismError } from './error.js';

/** Options for {@link createAgentPrismClient}. */
export interface AgentPrismClientOptions {
  /** Application root plus the prefix passed to `MapAgentPrism` (default `/agentprism`). */
  baseUrl: string;
...
/** A client typed against every AgentPrism management API path. */
export type AgentPrismClient = Client<paths>;
...
/**
 * Creates a typed client that throws {@link AgentPrismError} on a failed
 * response.
 *
...
export function createAgentPrismClient(options: AgentPrismClientOptions): AgentPrismClient {
  ...
  const middleware: Middleware = {
    ...
    async onResponse({ response }) {
      ...
      throw new AgentPrismError(response.status, title, detail);
    },
  };
