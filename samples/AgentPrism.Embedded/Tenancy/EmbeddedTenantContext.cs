...
/// <remarks>
/// <para>
/// This class replaces AgentPrism's built-in <c>SingleTenantContext</c>
...
/// </para>
/// <para>
/// Registered <strong>before</strong> <c>AddAgentPrism()</c>: every contract
/// here uses <c>TryAdd</c>, so a registration made first wins.
/// </para>
/// </remarks>
internal sealed class EmbeddedTenantContext(IHttpContextAccessor accessor, IOptions<AgentPrismOptions> options)
    : ITenantContext
{
    ...
    /// <inheritdoc />
    /// <remarks>
    /// Checks <see cref="AmbientTenantScope.Current"/> first: AgentPrism's own
    ...
    /// fallback below must never throw — the same safety property AgentPrism's
