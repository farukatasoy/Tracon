// Tracon embedding sample.
//
// samples/Tracon.Api is a GREEN-FIELD setup: it shows the two-line promise
// (AddTracon() + MapTracon()) working on its own. This sample shows the
// OTHER promise: an application that already has its own tenants, users,
// permissions, event bus, and object storage, embedding Tracon into that
...
// BEFORE AddTracon() so the host's own registration wins over the
// built-in default (every contract uses TryAdd). The seventh,
// IToolApprovalPresenter, is left on its built-in default on purpose:
// samples/Tracon.Api shows that one. Count claims here are gated by
...
// of Tracon's OWN HTTP run endpoints (mapped below), not every code path
// that can start a run.
//
// GET /tracon/api/diagnostics (enabled below) reports all six as
...
// Six of the seven embedding points. Order matters: AddTracon() below
...
// database. With one, the host's OWN schema (HostDbContext) and Tracon's
...
var postgreSqlConnectionString = builder.Configuration
    .GetSection(TraconPostgreSqlOptions.SectionName)[nameof(TraconPostgreSqlOptions.ConnectionString)];
...
if (hasPersistence)
{
    ...
    tracon.UsePostgreSql(o => o.DataSource = dataSource);
}
...
if (hasPersistence)
{
    // Tracon never disposes a data source it did not build (see
    // TraconPostgreSqlOptions.DataSource): ownership stays with whoever
    ...
    // independently of `tracon migrate` / AutoApplyMigrations.
    using var scope = app.Services.CreateScope();
    await scope.ServiceProvider.GetRequiredService<HostDbContext>().Database.EnsureCreatedAsync();
}

// The host's own endpoint, in the host's own style — not part of Tracon.
...
if (hasPersistence)
{
    ...
    // caller generates the run's identifier UP FRONT (TraconRunOptions.RunId)
    // so it can save a host-owned row that references the run without waiting
    // for — or copying — anything Tracon itself records for it.
...
// The single entry point for Tracon's own management API, OpenAI-compatible
...
app.MapTracon("/tracon", options =>
{
    ...
    // GET /tracon/api/diagnostics can show the bound embedding points.
