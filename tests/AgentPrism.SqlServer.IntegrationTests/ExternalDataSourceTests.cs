...
/// <summary>
/// <c>UseSqlServer(o =&gt; o.DataSource = ...)</c>: a data source AgentPrism did
/// not build and must never dispose (Phase 110).
/// </summary>
/// <remarks>
/// <c>Microsoft.Data.SqlClient</c> does not offer a <see cref="DbDataSource"/>
/// of its own (measured against 7.0.2); these tests use AgentPrism's own
...
public sealed class ExternalDataSourceTests(SqlServerFixture fixture)
{
    ...
    public async Task DataSource_and_ConnectionString_together_is_rejected()
    {
        ...
        services.AddAgentPrism().UseSqlServer(options =>
        {
        ...
        exception.Message.ShouldContain(nameof(AgentPrismSqlServerOptions.DataSource));
        exception.Message.ShouldContain(nameof(AgentPrismSqlServerOptions.ConnectionString));
    }
    ...
    public async Task DataSource_alone_does_not_require_a_connection_string()
    {
        ...
        provider.GetRequiredService<IOptions<AgentPrismSqlServerOptions>>().Value.ShouldNotBeNull();
