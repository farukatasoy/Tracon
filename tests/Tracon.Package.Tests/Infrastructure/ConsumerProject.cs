...
/// application that references the packed <c>Tracon</c> and
/// <c>Tracon.Testing</c> packages exactly the way an external consumer
...
/// <c>Tracon.Testing</c> directly, because that package is built in the
...
internal static class ConsumerProject
{
    ...
    public static async Task WriteAsync(string version, string directory)
    {
        ...
        await File.WriteAllTextAsync(Path.Combine(directory, "OrderTools.cs"), """
            ...
            /// <summary>
            /// The tool this smoke test exercises. Marked with [TraconTool] and
            ...
            {
                [TraconTool("order_status", "Returns the shipping status of an order.")]
