...
/// <summary>
/// The MSBuild gate (<c>TraconValidateCleanWorkingTree</c>,
...
public sealed class PackCleanlinessGateTests
{
    private static readonly string ProjectPath =
        Path.Combine(RepoPaths.Root, "src", "Tracon.Abstractions", "Tracon.Abstractions.csproj");

    private static readonly TimeSpan Timeout = TimeSpan.FromMinutes(5);

    [Fact]
    public async Task DirtyWorkingTreeStopsPackWithTracon0004()
    {
        ...
        result.Combined.ShouldContain("TRACON0004");
        ...
        result.Combined.ShouldContain(".tracon-pack-cleanliness-test-marker");
    }
    ...
    /// <summary>
    /// TraconSkipCleanWorkingTreeCheck is what lets `ReleaseArtifactFixture`
    ...
    public async Task SkipCleanWorkingTreeCheckBypassesTheGateOnADirtyTree()
    {
        ...
        try
        {
            var result = await ProcessRunner.RunAsync(
                "dotnet",
                $"pack \"{ProjectPath}\" -c Release -p:TraconSkipCleanWorkingTreeCheck=true",
                timeout: Timeout);
        ...
        {
            foreach (var nupkg in Directory.EnumerateFiles(RepoPaths.PackageReleaseDirectory, "Tracon.Abstractions.0.0.0-preview.0.*.nupkg"))
            {
                File.Delete(nupkg);
            }

            foreach (var snupkg in Directory.EnumerateFiles(RepoPaths.PackageReleaseDirectory, "Tracon.Abstractions.0.0.0-preview.0.*.snupkg"))
            {
                File.Delete(snupkg);
            }
        }
    ...
    [Fact]
    public async Task OverrideWithoutDirtyVersionStopsPackWithTracon0006()
    {
        using var marker = DirtMarker.Create();

        // CI is cleared explicitly: this fact asserts the NON-CI branch
        // (TRACON0006), and this very test suite may itself be running
        // inside a CI job that already sets CI=true ambiently - which would
        // otherwise make TRACON0005 fire first instead.
        var result = await ProcessRunner.RunAsync(
            "dotnet",
            $"pack \"{ProjectPath}\" -c Release -p:TraconAllowDirtyPack=true",
            environment: new Dictionary<string, string>(StringComparer.Ordinal) { ["CI"] = "" },
            timeout: Timeout);

        result.ExitCode.ShouldNotBe(0);
        result.Combined.ShouldContain("TRACON0006");
    }

    [Fact]
    public async Task OverrideInCiStopsPackWithTracon0005()
    {
        using var marker = DirtMarker.Create();

        var result = await ProcessRunner.RunAsync(
            "dotnet",
            $"pack \"{ProjectPath}\" -c Release -p:TraconAllowDirtyPack=true -p:MinVerVersionOverride=0.0.0-dirty.gate-test",
            environment: new Dictionary<string, string>(StringComparer.Ordinal) { ["CI"] = "true" },
            timeout: Timeout);

        result.ExitCode.ShouldNotBe(0);
        result.Combined.ShouldContain("TRACON0005");
    }
    ...
    public async Task OverrideWithDirtyVersionPacksSuccessfully()
    {
        ...
        try
        {
            // CI cleared for the same reason as OverrideWithoutDirtyVersionStopsPackWithTracon0006.
            var result = await ProcessRunner.RunAsync(
                "dotnet",
                $"pack \"{ProjectPath}\" -c Release -p:TraconAllowDirtyPack=true -p:MinVerVersionOverride=0.0.0-dirty.gate-test",
                environment: new Dictionary<string, string>(StringComparer.Ordinal) { ["CI"] = "" },
        ...
        {
            File.Delete(Path.Combine(RepoPaths.PackageReleaseDirectory, "Tracon.Abstractions.0.0.0-dirty.gate-test.nupkg"));
            File.Delete(Path.Combine(RepoPaths.PackageReleaseDirectory, "Tracon.Abstractions.0.0.0-dirty.gate-test.snupkg"));
        }
    }
}
