...
/// <summary>
/// <c>tracon eval</c> against a real, listening Tracon host
/// (docs/arsiv/fazlar/115-EVALIN-BASSIZ-KOSUCUSU.md): triggers a suite through
/// <c>Tracon.Client</c>, polls it to completion through the background job
...
public sealed class EvalCommandTests
{
    ...
    public async Task A_run_that_ends_Failed_exits_2_never_3()
    {
        ...
        // throws (TraconException, "No agent named ... exists") and the
    ...
    {
        var result = await CliRunner.RunAsync(
            "eval", "--url", "http://127.0.0.1:1/tracon/", "--suite", "any-suite");
    ...
    {
        var result = await CliRunner.RunAsync(
            "eval", "--url", "http://localhost:1/tracon/", "--suite", "any-suite", "--min-pass-rate", "1.5");
    ...
    {
        var result = await CliRunner.RunAsync(
            "eval", "--url", "http://localhost:1/tracon/", "--suite", "any-suite", "--max-failures", "-1");
