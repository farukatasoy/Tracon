...
#pragma warning restore MEAI001
    ...
    {
        // Tracon carries no built-in model list; decision K-032.
        using var provider = BuildProvider();
    ...
    public void Options_are_read_from_configuration()
    {
        ...
        var services = new ServiceCollection();
        services.AddTracon().UseOpenAI(configuration);
    ...
    {
        var services = new ServiceCollection();
        services.AddTracon().UseOpenAI(options => options.ApiKey = "   ");
    ...
    {
        var services = new ServiceCollection();
        services.AddTracon().UseOpenAI(TestData.ApiKey, options => options.Endpoint = new Uri("/v1", UriKind.Relative));
    ...
    {
        var services = new ServiceCollection();
        services.AddTracon().UseOpenAI(TestData.ApiKey, options => options.Timeout = TimeSpan.Zero);
    ...
    {
        var builder = new ServiceCollection().AddTracon();
    ...
    private static ServiceProvider BuildProvider(Action<OpenAIProviderOptions>? configure = null)
    {
        ...
        services.AddTracon().UseOpenAI(TestData.ApiKey, configure);
