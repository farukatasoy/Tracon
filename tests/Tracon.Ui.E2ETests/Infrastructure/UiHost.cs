...
/// <summary>
/// Brings up Tracon on a real Kestrel server.
/// </summary>
/// <remarks>
/// <para>
...
internal sealed class UiHost : IAsyncDisposable
{
    ...
    /// <summary>The path prefix Tracon is bound to.</summary>
        ...
        Action<WebApplication>? configureApp = null)
    {
        ...
        // Registered BEFORE AddTracon() so TryAddSingleton does not overwrite it
        // (same pattern as IRunAttributionContext in samples/Tracon.Api).
        ...
        // to the Tracon.Voice package. A single instance backs both.
    ...
    /// In a solution built with <c>-p:TraconFrontendEnabled=false</c> (fast
    /// inner loop) or in an environment without Node.js, <c>Tracon.UI</c>
    ...
    {
        var provider = app.Services.GetService<ITraconUiProvider>();
        ...
        throw new InvalidOperationException(
            ...
            "`dotnet build Tracon.slnx -c Release` (that is, WITHOUT " +
            "`-p:TraconFrontendEnabled=false`). Also, `find src -name \"* 2.*\" -not -path \"*/node_modules/*\"` " +
