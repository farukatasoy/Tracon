...
/// <summary>
/// <c>UseSqlite(o =&gt; o.DataSource = ...)</c>: a data source Tracon did not
/// build and must never dispose (Phase 110).
/// </summary>
/// <remarks>
/// <c>Microsoft.Data.Sqlite</c> does not offer a <see cref="DbDataSource"/> of
/// its own; these tests use Tracon's own <see cref="SqliteDataSource"/>
...
public sealed class ExternalDataSourceTests(SqliteFixture fixture)
{
    ...
    public async Task DataSource_and_ConnectionString_together_is_rejected()
    {
        ...
        services.AddTracon().UseSqlite(options =>
        {
        ...
        var exception = Should.Throw<OptionsValidationException>(
            () => provider.GetRequiredService<IOptions<TraconSqliteOptions>>().Value);

        exception.Message.ShouldContain(nameof(TraconSqliteOptions.DataSource));
        exception.Message.ShouldContain(nameof(TraconSqliteOptions.ConnectionString));
    }
    ...
    public async Task DataSource_alone_does_not_require_a_connection_string()
    {
        ...
        provider.GetRequiredService<IOptions<TraconSqliteOptions>>().Value.ShouldNotBeNull();
