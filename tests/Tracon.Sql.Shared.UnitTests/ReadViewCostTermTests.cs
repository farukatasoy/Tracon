...
public sealed class ReadViewCostTermTests
{
    ...
    public void PostgreSql_total_cost_sums_every_cost_addend()
        => AssertCostAddends(typeof(PostgresQueries).Assembly, "Tracon.PostgreSql.MigrationsViews.");

    [Fact]
    public void SqlServer_total_cost_sums_every_cost_addend()
        => AssertCostAddends(typeof(SqlServerQueries).Assembly, "Tracon.SqlServer.MigrationsViews.");

    [Fact]
    public void Sqlite_total_cost_sums_every_cost_addend()
        => AssertCostAddends(typeof(SqliteQueries).Assembly, "Tracon.Sqlite.MigrationsViews.");
