...
public sealed class ReadViewStatusNameTests
{
    ...
    public void PostgreSql_status_name_names_every_RunStatus_value()
        => AssertStatusNames(typeof(PostgresQueries).Assembly, "Tracon.PostgreSql.MigrationsViews.");

    [Fact]
    public void SqlServer_status_name_names_every_RunStatus_value()
        => AssertStatusNames(typeof(SqlServerQueries).Assembly, "Tracon.SqlServer.MigrationsViews.");

    [Fact]
    public void Sqlite_status_name_names_every_RunStatus_value()
        => AssertStatusNames(typeof(SqliteQueries).Assembly, "Tracon.Sqlite.MigrationsViews.");
