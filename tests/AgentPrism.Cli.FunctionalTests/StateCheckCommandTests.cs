...
/// <summary>
/// <c>agentprism state-check</c> against a real SQLite file, without an
...
public sealed class StateCheckCommandTests
{
    ...
    /// <summary>
    /// SQLite has one object namespace per database, so AgentPrism prefixes
    /// its tables instead of using a schema. This is
    /// <c>AgentPrismSqliteOptions.TablePrefix</c>'s default, which is what
    /// <c>agentprism migrate --provider sqlite</c> creates.
    /// </summary>
    private const string Tables = "agentprism_";
    ...
    public async Task An_unreachable_database_fails_with_one_line_and_no_stack_trace()
    {
        ...
        result.Combined.ShouldNotContain("   at AgentPrism.", Case.Sensitive);
    }
    ...
    private static async Task<string> MigratedDatabaseAsync()
    {
        var connectionString =
            $"Data Source={Path.Combine(Path.GetTempPath(), $"agentprism-state-check-{Guid.NewGuid():N}.db")}";
