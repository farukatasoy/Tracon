...
public sealed class MigrationRunnerTests(PostgresFixture fixture)
{
    ...
    private static int EmbeddedCoreMigrationCount { get; } = typeof(MigrationRunner).Assembly
        ...
        .Count(static name =>
            name.StartsWith("Tracon.PostgreSql.Migrations.", StringComparison.Ordinal)
    ...
    private static int EmbeddedKnowledgeMigrationCount { get; } = typeof(MigrationRunner).Assembly
        .GetManifestResourceNames()
        .Count(static name =>
            name.StartsWith("Tracon.PostgreSql.MigrationsKnowledge.", StringComparison.Ordinal)
            && name.EndsWith(".sql", StringComparison.Ordinal));
    ...
    {
        var options = Options.Create(new TraconPostgreSqlOptions
        {
    ...
    /// old uniqueness index no longer blocks a second name per author.
    /// </summary>
    [Fact]
    public async Task Populated_pre_152_run_scores_upgrade_without_data_loss()
    {
        ...
        var upgrade = migrations.Single(candidate =>
            string.Equals(candidate.Name, "0048_run_score_name_and_shape", StringComparison.Ordinal));

        await context.ExecuteAsync($"CREATE SCHEMA IF NOT EXISTS {schemaName};");
        ...
        await context.ExecuteAsync($"""
            ...
            VALUES
                ('{TraconId.NewId()}', 'test', '{runId}', NULL, 1, 1, 'good', 'human', 'alice', '2026-01-01T00:00:00Z'),
                ('{TraconId.NewId()}', 'test', '{runId}', NULL, 3, 70, 'ok', 'judge:quality', 'judge:quality', '2026-01-01T00:00:00Z');
            """);
        ...
        (await context.ScalarAsync<string>(
            $"SELECT name FROM {schemaName}.run_scores WHERE author = 'alice';")).ShouldBe("overall");

        (await context.ScalarAsync<string>(
            $"SELECT name FROM {schemaName}.run_scores WHERE author = 'judge:quality';")).ShouldBe("quality");

        await context.ExecuteAsync(
            $"UPDATE {schemaName}.run_scores SET value = 0.87 WHERE author = 'judge:quality';");

        (await context.ScalarAsync<double>(
            $"SELECT value FROM {schemaName}.run_scores WHERE author = 'judge:quality';")).ShouldBe(0.87);

        await context.ExecuteAsync($"""
            INSERT INTO {schemaName}.run_scores
                (id, tenant_id, run_id, message_id, kind, value, comment, source, author, created_at, name, text_value)
            VALUES
                ('{TraconId.NewId()}', 'test', '{runId}', NULL, 1, 0, NULL, 'human', 'alice', '2026-01-01T00:00:00Z', 'accuracy', NULL);
            """);

        (await context.ScalarAsync<long>(
            $"SELECT COUNT(*) FROM {schemaName}.run_scores WHERE author = 'alice';")).ShouldBe(2);
    }
