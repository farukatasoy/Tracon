...
public sealed class DiagnosticIntegrityTests
{
    ...
    /// - <c>'MapTracon()'</c> - and a nested one exists as well:
    ...
    public void Every_api_a_diagnostic_teaches_still_exists(string id)
    {
        ...
        missing.ShouldBeEmpty(
            ...
            "Rename it in the message, or add it to ExternalApis when Tracon does not own it.");
    }
    ...
    public void Every_DiagnosticInfo_routed_tool_diagnostic_is_wired_into_the_generators_dispatch_table()
    {
        ...
        var missing = Descriptors()
            .Where(descriptor => string.Equals(descriptor.Category, "Tracon.Tools", StringComparison.Ordinal))
    ...
    public void Every_provider_registration_TRC0102_recommends_still_exists()
    {
        ...
        foreach (var provider in TraconUsageAnalyzer.BuiltInProviders)
        {
    ...
    /// <summary>
    /// Every <c>Tracon.Usage</c> diagnostic (not <c>Tracon.Tools</c>,
    ...
    /// <c>TraconUsageDiagnostics=false</c> still sees the new diagnostic -
    ...
    public static TheoryData<string> UsageDiagnosticIds() => [.. Descriptors()
        .Where(descriptor => string.Equals(descriptor.Category, "Tracon.Usage", StringComparison.Ordinal))
        .Select(descriptor => descriptor.Id)];
    ...
    public void Every_usage_diagnostic_is_in_the_NoWarn_switch(string id)
    {
        ...
        targets.ShouldContain(
            id,
            Case.Sensitive,
            $"{id} does not appear in Tracon.Core.targets' NoWarn list; TraconUsageDiagnostics=false would not silence it.");
    }
    ...
    private static string CoreTargetsPath { get; } =
        Path.Combine(RepositoryRoot, "src", "Tracon.Core", "buildTransitive", "Tracon.Core.targets");
