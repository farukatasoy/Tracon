...
/// <summary>
/// Behaviour of <see cref="TraconUsageAnalyzer"/>. Every diagnostic is
...
public sealed class UsageAnalyzerTests
{
    ...
    public async Task TRC0101_reports_a_mapped_but_unregistered_control_plane()
    {
        var diagnostics = await AnalyzerTestHelper.RunAsync($$"""
            ...
            {
                public static void Configure(IEndpointRouteBuilder endpoints) => endpoints.MapTracon();
            }
            """);
    ...
    public async Task TRC0101_is_silent_when_the_registration_is_in_another_file()
    {
        var registration = $$"""
            ...
            {
                public static void Register(IServiceCollection services) => services.AddTracon();
            }
            """;
        ...
        var mapping = $$"""
            ...
            {
                public static void Map(IEndpointRouteBuilder endpoints) => endpoints.MapTracon();
            }
            """;
        ...
        var source = $$"""
            ...
            public static class Agents
            {
                public static void Register(IServiceCollection services)
                    => services.AddTracon().UseOpenAI("configured-elsewhere");
    ...
    public async Task TRC0302_is_silent_when_the_wrapper_serves_a_factory_agent()
    {
        var diagnostics = await AnalyzerTestHelper.RunAsync($$"""
            ...
            {
                public static void Register(IServiceCollection services)
                    => services.AddTracon().AddAgent("logged", provider => new LoggingAgent(null!));
            }
            """);
    ...
    public async Task TRC0401_is_silent_when_the_revisions_match()
    {
        var diagnostics = await AnalyzerTestHelper.RunAsync(
            "public static class Empty;",
            ("/repo/AGENTS.md", Marker("aaaaaaaa")),
            ("/packages/buildTransitive/Tracon.AgentMap.md", Marker("aaaaaaaa")));
    ...
    public async Task TRC0402_is_silent_when_the_instructions_name_the_local_reference()
    {
        var diagnostics = await AnalyzerTestHelper.RunWithPropertiesAsync(
            "public static class Empty;",
            AnalyzerTestHelper.LocalReferenceOn,
            ("/repo/AGENTS.md", "# House rules\n\nExact paths: see Tracon.LocalReference.md beside each project.\n"),
            ("/packages/buildTransitive/Tracon.AgentMap.md", Marker("bbbbbbbb")));
    ...
    public async Task TRC0402_is_silent_when_the_property_is_turned_off_on_its_own()
    {
        var diagnostics = await AnalyzerTestHelper.RunWithPropertiesAsync(
            "public static class Empty;",
            [("build_property.TraconWriteLocalReference", "false")],
            ("/repo/AGENTS.md", "# House rules\n\nRun the tests before you commit.\n"),
            ("/packages/buildTransitive/Tracon.AgentMap.md", Marker("bbbbbbbb")));
    ...
    /// <summary>
    /// Without the map among the additional files, Tracon is not referenced
    ...
    /// evaluated on its own account. Measured against Tracon's own code
    ...
    /// verdict. <see cref="TraconUsageAnalyzer"/>'s remarks on
    ...
    public async Task Correct_wiring_reports_nothing()
    {
        var source = $$"""
            ...
            {
                public static void Register(IServiceCollection services)
                    => services.AddTracon().UseOpenAI("configured-elsewhere").AddAgent(Support());
    ...
    [Fact]
    public async Task TRC0403_reports_a_gate_skill_written_from_an_older_map()
    {
        var diagnostics = await AnalyzerTestHelper.RunAsync(
            "public static class Empty;",
            ("/repo/.claude/skills/tracon/SKILL.md", GateSkill("aaaaaaaa")),
            ("/packages/buildTransitive/Tracon.AgentMap.md", Marker("bbbbbbbb")));

        var reported = diagnostics.ShouldHaveSingleItem();
        reported.Id.ShouldBe("TRC0403");
        reported.GetMessage(CultureInfo.InvariantCulture).ShouldContain("aaaaaaaa");
        reported.GetMessage(CultureInfo.InvariantCulture).ShouldContain("bbbbbbbb");
    }

    [Fact]
    public async Task TRC0403_is_silent_when_the_revisions_match()
    {
        var diagnostics = await AnalyzerTestHelper.RunAsync(
            "public static class Empty;",
            ("/repo/.claude/skills/tracon/SKILL.md", GateSkill("aaaaaaaa")),
    ...
    /// <summary>
    /// The name <c>SKILL.md</c> belongs to the harness, not to Tracon, so a
    /// consumer's own skill lands among the additional files too. Only the
    /// marker says a file came from the tool, and only such a file can be
    /// stale.
    /// </summary>
    [Fact]
    public async Task TRC0403_is_silent_for_a_skill_this_package_did_not_write()
    {
        var diagnostics = await AnalyzerTestHelper.RunAsync(
            "public static class Empty;",
    ...
    /// <summary>
    /// The marker cannot be the first line - a skill file opens with its front
    /// matter, and a harness that cannot parse that does not load the skill at
    /// all. This pins the reader to searching past the front matter, which is
    /// the one thing that would make the diagnostic silently stop firing.
    /// </summary>
    [Fact]
    public async Task TRC0403_finds_the_marker_after_the_front_matter()
    {
        var withExtraFrontMatter =
            "---\nname: tracon\ndescription: Read before writing Tracon code.\nversion: 2\nlicense: MIT\n---\n\n"
            + GateSkillMarker("aaaaaaaa")
            + "\n\n# Tracon gate\n";

        var diagnostics = await AnalyzerTestHelper.RunAsync(
            "public static class Empty;",
            ("/repo/.claude/skills/tracon/SKILL.md", withExtraFrontMatter),
            ("/packages/buildTransitive/Tracon.AgentMap.md", Marker("bbbbbbbb")));

        diagnostics.ShouldHaveSingleItem().Id.ShouldBe("TRC0403");
    }

    /// <summary>
    /// A skill without a map beside it has nothing to be compared against: the
    /// consumer references Tracon some other way, and a warning would name a
    /// revision the analyzer cannot see.
    /// </summary>
    [Fact]
    public async Task TRC0403_is_silent_without_a_map_to_compare_against()
    {
        var diagnostics = await AnalyzerTestHelper.RunAsync(
            "public static class Empty;",
            ("/repo/.claude/skills/tracon/SKILL.md", GateSkill("aaaaaaaa")));
    ...
    /// <summary>
    /// Both files at once. The gate skill's staleness is its own answer, and it
    /// must not swallow - or be swallowed by - the one about AGENTS.md.
    /// </summary>
    [Fact]
    public async Task TRC0403_and_TRC0401_are_reported_independently()
    {
        var diagnostics = await AnalyzerTestHelper.RunAsync(
            "public static class Empty;",
            ("/repo/AGENTS.md", Marker("aaaaaaaa")),
        ...
        diagnostics.Select(diagnostic => diagnostic.Id).Order(StringComparer.Ordinal)
            .ShouldBe(["TRC0401", "TRC0403"]);
    }

    private static string Marker(string revision)
        => $"<!-- Tracon agent map · revision: {revision} · generated by docs-site/scripts/build-agent-map.mjs -->\n# Tracon\n";

    private static string GateSkillMarker(string revision)
        => $"<!-- Tracon gate skill · revision: {revision} · written by `tracon agent-skill` -->";

    private static string GateSkill(string revision)
        => "---\nname: tracon\ndescription: Read before writing Tracon code.\n---\n\n"
           + GateSkillMarker(revision)
