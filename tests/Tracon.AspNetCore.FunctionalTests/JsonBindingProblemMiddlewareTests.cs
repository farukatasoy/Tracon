...
public sealed class JsonBindingProblemMiddlewareTests
{
    ...
    public async Task Unrecognized_enum_value_in_implicit_binding_also_returns_400()
    {
        ...
        using var response = await host.Client.PutAsJsonAsync(
            new Uri("/tracon/api/mcp-servers/stdio-test", UriKind.Relative),
    ...
    public async Task Text_sent_to_a_numeric_field_also_returns_400_on_the_retention_endpoint()
    {
        ...
        using var response = await host.Client.PutAsync(
            new Uri("/tracon/api/retention/runs", UriKind.Relative),
            content);
    ...
    /// <c>MapTracon</c>, has a broader catch and turns that exception
    ...
    /// </remarks>
    [Theory]
    [InlineData("/tracon/api/stats/timeseries?bucket=day")]
    [InlineData("/tracon/api/evaluation/scores/summary?bucket=day")]
