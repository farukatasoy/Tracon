...
{
    private const string SecretConfigurationKey = "Tracon:TriggerSecrets:Slack";
    ...
    public async Task Response_never_carries_the_secret_value()
    {
        ...
        using var listResponse = await host.Client.GetAsync(new Uri("/tracon/api/triggers", UriKind.Relative));
        var listBody = await listResponse.Content.ReadAsStringAsync();
        listBody.ShouldNotContain(Secret);

        using var auditResponse = await host.Client.GetAsync(new Uri("/tracon/api/audit", UriKind.Relative));
    ...
    public async Task Unknown_name_returns_404_on_get_and_delete()
    {
        ...
        (await host.Client.GetAsync(new Uri("/tracon/api/triggers/missing", UriKind.Relative)))
            .StatusCode.ShouldBe(HttpStatusCode.NotFound);
        (await host.Client.DeleteAsync(new Uri("/tracon/api/triggers/missing", UriKind.Relative)))
            .StatusCode.ShouldBe(HttpStatusCode.NotFound);
    }
    ...
    public async Task A_correctly_signed_request_is_accepted_and_queues_a_run()
    {
        ...
        using var runResponse = await host.Client.GetAsync(new Uri($"/tracon/api/runs/{accepted.RunId}", UriKind.Relative));
    ...
    public async Task An_unsigned_request_is_rejected()
    {
        ...
        using var request = new HttpRequestMessage(HttpMethod.Post, "/tracon/api/triggers/default/slack")
        {
    ...
    {
        await using var host = await TraconTestHost.StartAsync(
            configureTracon: builder => builder.Services.Configure<TraconInboundTriggerOptions>(
    ...
    public async Task A_workflow_target_is_queued_without_a_synchronous_run_id()
    {
        ...
        using var jobResponse = await host.Client.GetAsync(new Uri($"/tracon/api/jobs/{accepted.JobId}", UriKind.Relative));
        jobResponse.StatusCode.ShouldBe(HttpStatusCode.OK);
    }

    private static async Task<TraconTestHost> StartWithSecretAsync()
    ...
    private static Task<HttpResponseMessage> SendSignedAsync(
        TraconTestHost host, string tenantId, string name, string body)
    {
        var timestamp = DateTimeOffset.UtcNow;
    ...
    private static async Task<HttpResponseMessage> SendSignedAsync(
        ...
        string signature)
    {
        using var request = new HttpRequestMessage(HttpMethod.Post, $"/tracon/api/triggers/{tenantId}/{name}")
        {
            Content = new StringContent(body, Encoding.UTF8, "application/json"),
        };
