...
public sealed class SecurityTests
{
    ...
    public async Task Loopback_ip_passes()
    {
        ...
        using var request = new HttpRequestMessage(HttpMethod.Get, "/tracon/api/agents");
        request.Headers.Add(TraconTestHost.RemoteIpHeader, "127.0.0.1");
    ...
    public async Task IPv6_loopback_passes()
    {
        ...
        using var request = new HttpRequestMessage(HttpMethod.Get, "/tracon/api/agents");
        request.Headers.Add(TraconTestHost.RemoteIpHeader, "::1");
    ...
    public async Task IPv4_loopback_mapped_to_IPv6_passes()
    {
        ...
        using var request = new HttpRequestMessage(HttpMethod.Get, "/tracon/api/agents");
        request.Headers.Add(TraconTestHost.RemoteIpHeader, "::ffff:127.0.0.1");
    ...
    public async Task Shell_still_loads_on_a_non_loopback_request_HATA_S4_003()
    {
        ...
        await using var host = await TraconTestHost.StartAsync(
            configureServices: static services => services.AddSingleton<ITraconUiProvider, FakeUiProvider>());

        using var shellRequest = new HttpRequestMessage(HttpMethod.Get, "/tracon/");
        shellRequest.Headers.Add(TraconTestHost.RemoteIpHeader, "203.0.113.7");

        using var shellResponse = await host.Client.SendAsync(shellRequest);

        shellResponse.StatusCode.ShouldBe(HttpStatusCode.OK);

        using var dataRequest = new HttpRequestMessage(HttpMethod.Get, "/tracon/api/agents");
        dataRequest.Headers.Add(TraconTestHost.RemoteIpHeader, "203.0.113.7");
    ...
    {
        await using var host = await TraconTestHost.StartAsync(
            configureEndpoints: static options => options.RequireAuthorization("TraconAdmin"),
            configureServices: static services => TestAuthenticationHandler.Add(services)
                .AddAuthorizationBuilder()
                .AddPolicy("TraconAdmin", static policy => policy.RequireAssertion(static _ => false)));
    ...
    public async Task Succeeding_policy_allows_access()
    {
        await using var host = await TraconTestHost.StartAsync(
            ...
            configureServices: static services => TestAuthenticationHandler.Add(services)
                .AddAuthorizationBuilder()
                .AddPolicy("TraconAdmin", static policy => policy.RequireAssertion(static _ => true)));
    ...
    public async Task Meta_endpoint_is_open_from_a_remote_ip_too()
    {
        ...
        using var request = new HttpRequestMessage(HttpMethod.Get, "/tracon/api/meta");
        request.Headers.Add(TraconTestHost.RemoteIpHeader, "203.0.113.7");
