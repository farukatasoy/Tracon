...
public sealed class OnlineEvaluationEndpointTests
{
    ...
    public async Task Manual_scoring_skips_sampling_and_writes_a_score()
    {
        ...
        using var feedback = await host.Client.GetAsync(new Uri($"/tracon/api/runs/{runId}/feedback", UriKind.Relative));
        (await TraconTestHost.ReadJsonAsync(feedback)).GetArrayLength().ShouldBe(1);
    }
    ...
    public async Task Manual_scoring_a_nonexistent_run_returns_404()
    {
        ...
        using var response = await host.Client.PostAsync(JudgeUri(TraconId.NewId()), content: null);
    ...
    public async Task Summary_endpoint_returns_zero_samples_for_an_empty_window()
    {
        ...
        using var response = await host.Client.GetAsync(new Uri("/tracon/api/evaluation/online", UriKind.Relative));
    ...
    private static Uri JudgeUri(Guid runId) => new($"/tracon/api/runs/{runId}/judge", UriKind.Relative);

    private static async Task<Guid> SeedScorableRunAsync(TraconTestHost host)
    {
