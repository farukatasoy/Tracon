...
namespace Tracon.AspNetCore.FunctionalTests.Evals;

/// <summary>Tests for <c>GET /api/evaluation/scores/summary</c>.</summary>
public sealed class RunScoreSummaryEndpointTests
{
    private const string SummaryPath = "/tracon/api/evaluation/scores/summary";
    ...
    public async Task Written_scores_are_summarized_by_name()
    {
        ...
        await scores.UpsertAsync(Score(TraconId.NewId()) with { Author = "alice", Value = 1 });
        await scores.UpsertAsync(Score(TraconId.NewId()) with { Author = "bob", Value = 0 });
    ...
    public async Task Categorical_scores_report_categories_not_an_average()
    {
        ...
        await scores.UpsertAsync(Score(TraconId.NewId()) with
        {
            Name = "severity",
    ...
    public async Task Bucket_adds_a_trend_series()
    {
        ...
        await scores.UpsertAsync(Score(TraconId.NewId()) with { CreatedAt = now, Author = "alice" });
        await scores.UpsertAsync(Score(TraconId.NewId()) with { CreatedAt = now.AddDays(-2), Author = "bob" });
    ...
    public async Task A_bucketed_query_with_no_from_defaults_to_the_last_90_days()
    {
        ...
        var now = DateTimeOffset.UtcNow;
        await scores.UpsertAsync(Score(TraconId.NewId()) with { CreatedAt = now.AddDays(-1), Author = "alice" });
        await scores.UpsertAsync(Score(TraconId.NewId()) with { CreatedAt = now.AddDays(-120), Author = "bob" });
    ...
    public async Task Live_online_summary_still_resets_while_this_endpoint_does_not()
    {
        ...
        await scores.UpsertAsync(Score(TraconId.NewId()) with { Author = "alice" });
        ...
        using var online = await host.Client.GetAsync(new Uri("/tracon/api/evaluation/online", UriKind.Relative));
        (await TraconTestHost.ReadJsonAsync(online)).GetProperty("sampleCount").GetInt64().ShouldBe(0);

        using var summary = await host.Client.GetAsync(new Uri(SummaryPath, UriKind.Relative));
        (await TraconTestHost.ReadJsonAsync(summary)).GetProperty("byName").GetArrayLength().ShouldBe(1);
    }
    ...
    public async Task A_failing_store_is_not_softened_into_an_empty_200()
    {
        ...
        // as "no scores exist" instead of "the query failed". Tracon
        ...
        await using var host = await TraconTestHost.StartAsync(configureServices: static services =>
