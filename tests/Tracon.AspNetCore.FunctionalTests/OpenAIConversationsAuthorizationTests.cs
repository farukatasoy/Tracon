...
/// it worth a file of its own. Phase 148 gave these endpoints Tracon's own
...
public sealed class OpenAIConversationsAuthorizationTests
{
    ...
    private static Uri Items(string conversationId)
        => new($"/tracon/v1/conversations/{conversationId}/items", UriKind.Relative);
    ...
    public async Task Every_conversation_endpoint_is_unchanged_when_no_handler_is_registered()
    {
        ...
        using (var created = await host.Client.PostAsync(
            new Uri("/tracon/v1/conversations", UriKind.Relative), content: null))
        {
    ...
    public async Task The_conversations_surface_is_mapped_by_default()
    {
        ...
        new TraconEndpointOptions().MapOpenAIConversations.ShouldBeTrue();
    ...
    public async Task Turning_the_surface_off_removes_all_four_routes()
    {
        ...
        using var session = await host.Client.GetAsync(
            new Uri("/tracon/api/sessions/conv-1", UriKind.Relative));
    ...
    public async Task Turning_the_surface_off_leaves_the_other_OpenAI_routes_alone()
    {
        ...
        paths.ShouldContain(static path => string.Equals(path, "/tracon/v1/responses", StringComparison.Ordinal));
        paths.ShouldContain(static path => string.Equals(path, "/tracon/v1/chat/completions", StringComparison.Ordinal));
    }
    ...
    private static ISessionStore StoreOf(TraconTestHost host)
    ...
    private static async Task<string[]> PathsAsync(TraconTestHost host)
    {
    ...
    private static async Task<string[]> ConversationPathsAsync(TraconTestHost host)
