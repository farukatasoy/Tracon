...
{
    private static Task<TraconTestHost> StartAsync(bool allowPrivateNetworkTargets = false)
        => TraconTestHost.StartAsync(configureServices: services =>
            services.Configure<TraconEgressOptions>(
                options => options.AllowPrivateNetworkTargets = allowPrivateNetworkTargets));

    private static Uri McpServer(string name) => new($"/tracon/api/mcp-servers/{name}", UriKind.Relative);
    ...
    public async Task Mcp_endpoint_pointing_at_a_private_address_is_rejected(string endpoint)
    {
        ...
        body.ShouldContain(nameof(TraconEgressOptions.AllowPrivateNetworkTargets));
    }
    ...
    [InlineData("Tracon:Providers:OpenAI:ApiKey")]
    [InlineData("Tracon:Mcp:GithubToken")]
    public async Task Mcp_authorization_key_outside_the_allowed_prefix_is_rejected(string key)
    {
        ...
        body.ShouldContain(new TraconMcpSecurityOptions().AllowedConfigurationPrefix);
...
#pragma warning restore CS0618
    ...
    public async Task Another_tenants_definition_is_judged_by_the_same_rule()
    {
        ...
        request.Headers.Add("X-Tracon-Tenant", "acme");
    ...
    public async Task Tenant_provider_endpoint_pointing_at_a_private_address_is_rejected(string endpoint)
    {
        ...
        (await response.Content.ReadAsStringAsync())
            .ShouldContain(nameof(TraconEgressOptions.AllowPrivateNetworkTargets));
    }
    ...
    [Theory]
    [InlineData("ConnectionStrings:Default")]
    [InlineData("Tracon:Webhooks:Secrets:orders")]
    public async Task Webhook_secret_key_outside_the_allowed_prefix_is_rejected(string key)
    {
        await using var host = await StartAsync();

        using var response = await host.Client.PutAsJsonAsync(
            new Uri("/tracon/api/webhooks/orders", UriKind.Relative),
            new
            {
        ...
        body.ShouldContain(new TraconWebhookOptions().AllowedConfigurationPrefix);
    ...
    public async Task Guard_is_registered_and_rejects_a_private_target_at_connection_time()
    {
        ...
        exception.Message.ShouldContain(nameof(TraconEgressOptions.AllowPrivateNetworkTargets));
    }
