...
public sealed class StreamingTests
{
    ...
    public async Task Nonexistent_agent_returns_404_before_the_stream_starts()
    {
        ...
        using var response = await host.Client.PostAsJsonAsync(
            new Uri("/tracon/api/agents/no-such-agent/run", UriKind.Relative),
    ...
    public async Task A_consumer_written_Custom_event_carries_its_CustomType_over_the_wire()
    {
            ...
            builder => builder
                ...
                .AddTool(
                    AIFunctionFactory.Create(
                        async () =>
                        {
                            await TraconRunContext.Current!.Writer!.AppendAsync(new RunEventDraft(RunEventType.Custom)
                            {
        ...
        var runId = (await TraconTestHost.ReadJsonAsync(runs))[0].GetProperty("id").GetGuid();
        ...
        // Phase 145: the frame's OWN SSE "event:" name is the closed, Tracon-owned
    ...
    public async Task Nonexistent_runs_events_return_404()
    {
        ...
        using var response = await host.Client.GetAsync(
            new Uri($"/tracon/api/runs/{Guid.NewGuid()}/events", UriKind.Relative));

        response.StatusCode.ShouldBe(HttpStatusCode.NotFound);
        response.Content.Headers.ContentType?.MediaType.ShouldBe("application/problem+json");
    }

    private static Task<HttpResponseMessage> PostRunAsync(TraconTestHost host, AgentRunRequest request)
    ...
    private static async Task<Guid> RunAndGetRunIdAsync(TraconTestHost host)
    {
