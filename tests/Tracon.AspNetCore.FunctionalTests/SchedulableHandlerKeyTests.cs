...
{
    private static readonly Uri HandlerKeys = new("/tracon/api/schedules/handler-keys", UriKind.Relative);
    private static readonly Uri Schedule = new("/tracon/api/schedules/probe", UriKind.Relative);
    ...
    public async Task An_unlisted_key_is_rejected_and_no_schedule_is_created()
    {
        ...
        (await response.Content.ReadAsStringAsync(TestContext.Current.CancellationToken))
            .ShouldContain(nameof(TraconSchedulingOptions.HttpSchedulableHandlerKeys));
    ...
    public async Task A_built_in_key_is_accepted_by_default()
    {
        ...
        (await TraconTestHost.ReadJsonAsync(response)).GetProperty("handlerKey").GetString()
    ...
    public async Task The_key_list_endpoint_reports_exactly_what_the_save_endpoint_accepts()
    {
        ...
        response.StatusCode.ShouldBe(HttpStatusCode.OK);
        var keys = (await TraconTestHost.ReadJsonAsync(response))
