...
public sealed class EvalRunDiffTests
{
    ...
    public async Task Reversing_the_two_runs_turns_a_regression_into_a_fix()
    {
        ...
        {
            KindOf(await TraconTestHost.ReadJsonAsync(forward), caseId).ShouldBe("Regressed");
        }

        using var reversed = await host.Client.GetAsync(Diff(older, newer));
        KindOf(await TraconTestHost.ReadJsonAsync(reversed), caseId).ShouldBe("Fixed");
    }
    ...
    public async Task A_run_whose_details_retention_removed_answers_409_not_an_empty_diff()
    {
        ...
        var candidate = await CompletedRunAsync(store, suite.Id, (TraconId.NewId(), true));
    ...
    public async Task The_candidate_losing_its_details_answers_409_too()
    {
        ...
        var baseline = await CompletedRunAsync(store, suite.Id, (TraconId.NewId(), true));
        var trimmed = await TrimmedRunAsync(store, suite.Id, caseCount: 4);

        using var response = await host.Client.GetAsync(Diff(trimmed, baseline));

        response.StatusCode.ShouldBe(HttpStatusCode.Conflict);
        (await TraconTestHost.ReadJsonAsync(response)).GetProperty("detail").GetString()
    ...
    public async Task An_unknown_baseline_answers_404()
    {
        ...
        using var response = await host.Client.GetAsync(Diff(candidate, TraconId.NewId()));
    ...
    public async Task Another_tenants_run_answers_404()
    {
        ...
        var candidate = await CompletedRunAsync(store, mine.Id, (TraconId.NewId(), true));
        var foreign = await CompletedRunAsync(store, theirs.Id, (TraconId.NewId(), true), tenantId: "other-tenant");
    ...
    public async Task Paging_moves_the_window_but_never_the_counters()
    {
        ...
        var ids = Enumerable.Range(0, 12).Select(static _ => TraconId.NewId()).ToArray();
        ...
        using var response = await host.Client.GetAsync(
            new Uri($"/tracon/api/evals/runs/{candidate}/diff?baseline={baseline}&skip=10&take=50", UriKind.Relative));
    ...
    public async Task A_failing_store_surfaces_the_fault_rather_than_an_empty_diff()
    {
        ...
        var fault = await Should.ThrowAsync<InvalidOperationException>(async () =>
            await host.Client.GetAsync(Diff(TraconId.NewId(), TraconId.NewId())));

        fault.Message.ShouldBe("store is down");
    }

    private static Uri Diff(Guid candidate, Guid baseline)
        => new($"/tracon/api/evals/runs/{candidate}/diff?baseline={baseline}", UriKind.Relative);
