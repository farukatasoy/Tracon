...
public sealed class AttachmentRunTests
{
    ...
    public async Task Run_referencing_an_attachment_sends_the_actual_content_to_the_model()
    {
        ...
        var attachmentId = (await TraconTestHost.ReadJsonAsync(uploaded)).GetProperty("id").GetGuid();
    ...
    public async Task Another_tenants_attachment_cannot_be_used_in_a_run()
    {
        ...
        using var runRequest = new HttpRequestMessage(
            HttpMethod.Post,
            "/tracon/api/agents/kod-agent/run");
        runRequest.Headers.Add("X-Tracon-Tenant", "tenant-b");
    ...
    private static async Task<HttpResponseMessage> UploadAsync(TraconTestHost host, byte[] data)
    {
