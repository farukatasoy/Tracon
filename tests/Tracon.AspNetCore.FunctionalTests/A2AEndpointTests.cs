...
/// <summary>Phase 50: the A2A server that exposes Tracon agents.</summary>
public sealed class A2AEndpointTests
{
    ...
    public async Task An_agent_added_at_runtime_does_not_appear_in_A2A()
    {
        ...
        await using var host = await TraconTestHost.StartAsync(
            configureTracon: builder => builder.UseA2A(o => o.ExposedAgents.Add("db-agent")),
            configureAfterMap: app => app.MapTraconA2A());
        ...
        using var response = await host.Client.GetAsync(
            new Uri("/tracon/a2a/db-agent/.well-known/agent-card.json", UriKind.Relative));
    ...
    public async Task An_agent_carrying_a_tool_that_requires_approval_cannot_be_exposed_via_A2A()
    {
        ...
        // background inside A2AApprovalGuardFilter (see TraconA2AExtensions)
    ...
    [Fact]
    public async Task MapTraconA2A_does_not_crash_on_an_empty_sqlite_database()
    {
        // Regression: MapTraconA2A() used to read the catalog SYNCHRONOUSLY
        ...
        string text)
    {
        ...
        using var request = new HttpRequestMessage(HttpMethod.Post, $"/tracon/a2a/{agentName}")
        {
