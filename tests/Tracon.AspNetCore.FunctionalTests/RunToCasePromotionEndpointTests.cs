...
public sealed class RunToCasePromotionEndpointTests
{
    ...
    public async Task Promoting_the_same_run_a_second_time_returns_200_and_creates_no_second_case()
    {
        ...
        var firstCaseId = (await TraconTestHost.ReadJsonAsync(first)).GetProperty("id").GetGuid();

        using var second = await host.Client.PostAsync(PromoteUri(runId), content: null);
        second.StatusCode.ShouldBe(HttpStatusCode.OK);
        var secondCaseId = (await TraconTestHost.ReadJsonAsync(second)).GetProperty("id").GetGuid();

        secondCaseId.ShouldBe(firstCaseId);

        using var cases = await host.Client.GetAsync(CasesUri);
        (await TraconTestHost.ReadJsonAsync(cases)).GetArrayLength().ShouldBe(1);
    }
    ...
    public async Task Another_tenants_run_cannot_be_promoted_returns_the_SAME_404()
    {
        ...
        using var missing = await SendAsTenant(host, PromoteUri(TraconId.NewId()), "tenant-b");
    ...
    public async Task Nonexistent_suite_returns_404()
    {
        ...
        using var response = await host.Client.PostAsync(
            new Uri("/tracon/api/evals/no-such-thing/cases/from-run/" + runId, UriKind.Relative),
            content: null);

        response.StatusCode.ShouldBe(HttpStatusCode.NotFound);
    }

    private static readonly Uri CasesUri = new("/tracon/api/evals/customer-support-team/cases", UriKind.Relative);

    private static Uri PromoteUri(Guid runId)
        => new($"/tracon/api/evals/customer-support-team/cases/from-run/{runId}", UriKind.Relative);

    private static async Task SaveSuiteAsync(TraconTestHost host, string agentName)
    {
        using var response = await host.Client.PutAsJsonAsync(
            new Uri("/tracon/api/evals/customer-support-team", UriKind.Relative),
    ...
    private static async Task SaveSuiteAsAsync(TraconTestHost host, string agentName, string tenant)
    {
        var request = new HttpRequestMessage(
            HttpMethod.Put, new Uri("/tracon/api/evals/customer-support-team", UriKind.Relative));
    ...
    private static async Task<Guid> RunAsAsync(TraconTestHost host, string message, string sessionId, string tenant)
    {
        var request = new HttpRequestMessage(HttpMethod.Post, $"/tracon/api/agents/kod-agent/run");
        request.Headers.Add(TenantHeader, tenant);
        ...
        var runsRequest = new HttpRequestMessage(HttpMethod.Get, "/tracon/api/runs");
