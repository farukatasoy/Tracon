...
public sealed class SessionOwnershipTests
{
    ...
    public async Task Ownership_is_off_in_the_default_options()
    {
        ...
        // flag explicitly.
        var defaults = new TraconSessionOwnershipOptions();
        ...
        defaults.ManagementPolicy.ShouldBe(TraconPolicies.Operator);
    ...
    public async Task A_management_caller_sees_the_whole_tenant_including_unowned_rows()
    {
        ...
        // run: TraconPolicies.Operator also guards the run endpoint, so
        ...
        await using var host = await StartAsync(
            ...
            configureServices: static services => TestAuthenticationHandler.Add(services)
                ...
                .AddPolicy(TraconPolicies.Operator, static policy => policy.RequireAssertion(
    ...
    {
        // 🚨 The single worst way this could fail. Tracon's own role
    ...
    public async Task Opening_a_session_without_an_identity_is_refused_and_writes_no_row()
    {
        ...
        problem.GetProperty("errorType").GetString()
            .ShouldBe(TraconSessionOwnerRequiredException.SessionOwnerRequiredErrorType);
    ...
    public async Task Branching_keeps_the_source_sessions_owner()
    {
        ...
        using var response = await host.Client.PostAsJsonAsync(
            new Uri("/tracon/api/sessions/source/branch", UriKind.Relative),
    ...
    public async Task Opening_a_voice_socket_on_another_owners_session_is_refused()
    {
        ...
        var connect = async () => await socketClient.ConnectAsync(
            new Uri("http://localhost/tracon/api/voice/sessions/voice-a/stream"),
    ...
    public async Task An_unknown_session_still_opens_a_voice_socket()
    {
        ...
        using var socket = await socketClient.ConnectAsync(
            new Uri("http://localhost/tracon/api/voice/sessions/never-seen/stream", UriKind.Absolute),
            TestContext.Current.CancellationToken);
    ...
    public async Task Strict_mode_refuses_branching_an_unowned_session()
    {
        ...
        using var response = await host.Client.PostAsJsonAsync(
            new Uri("/tracon/api/sessions/legacy/branch", UriKind.Relative),
            new SessionBranchRequest { NewSessionId = "branched" });
    ...
    public async Task Strict_mode_refuses_a_voice_socket_on_an_unowned_session()
    {
        ...
        var connect = async () => await socketClient.ConnectAsync(
            new Uri("http://localhost/tracon/api/voice/sessions/legacy/stream"),
            TestContext.Current.CancellationToken);
    ...
    public async Task Strict_mode_is_bound_from_configuration()
    {
        ...
        // every deployment that configures Tracon through appsettings —
        // the exact defect class K-406 and K-253 both were.
        var configuration = new ConfigurationBuilder()
            .AddInMemoryCollection(new Dictionary<string, string?>(StringComparer.Ordinal)
            {
                ["Tracon:SessionOwnership:Enabled"] = "true",
                ["Tracon:SessionOwnership:RefuseUnownedSessions"] = "true",
            })
            .Build();
        ...
        var bound = provider.GetRequiredService<IOptions<TraconSessionOwnershipOptions>>().Value;
    ...
    private static ISessionStore StoreAsync(TraconTestHost host)
        => host.Services.GetRequiredService<ISessionStore>();

    private static async Task RunAsync(TraconTestHost host, string sessionId)
    {
        using var response = await host.Client.PostAsJsonAsync(
    ...
    private static Task<HttpResponseMessage> PostQueuedAsync(TraconTestHost host, string sessionId)
    {
        var request = new HttpRequestMessage(HttpMethod.Post, Run)
        {
    ...
    private static Task<HttpResponseMessage> PostIdempotentAsync(
        TraconTestHost host, string key, string sessionId)
    {
        var request = new HttpRequestMessage(HttpMethod.Post, Run)
        {
            Content = JsonContent.Create(new AgentRunRequest { Message = "hello", SessionId = sessionId }),
    ...
    private static Task<TraconTestHost> StartSqliteAsync(
    ...
    private static Task<TraconTestHost> StartVoiceAsync(
    ...
    private static void Configure(
        ...
        bool refuseUnownedSessions = false)
        => services.Configure<TraconSessionOwnershipOptions>(options =>
        {
    ...
    private sealed class FailingSessionStore : ISessionStore
    {
        public ValueTask SaveAsync(SessionRecord record, CancellationToken cancellationToken = default)
            => throw new TraconException("the session store is unavailable");
