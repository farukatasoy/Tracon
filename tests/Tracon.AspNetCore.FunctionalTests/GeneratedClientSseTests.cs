...
/// other operation uses — which threw <see cref="TraconApiException"/>
...
/// <c>TraconPublicApiTrackingEnabled=false</c> leaves for a generated
...
public sealed class GeneratedClientSseTests
{
    ...
    [Fact]
    public async Task TraconRunAgentAsync_reads_the_SSE_body_as_plain_text()
    {
        ...
        var body = await client.TraconRunAgentAsync("kod-agent", RunRequest());
    ...
    [Fact]
    public async Task TraconStreamRunEventsAsync_reads_the_SSE_body_as_plain_text()
    {
        await using var host = await TraconTestHost.StartAsync(
            static builder => builder.AddAgent(TestData.Definition()));
        ...
        await client.TraconRunAgentAsync("kod-agent", RunRequest());

        var runs = await client.TraconListRunsAsync();
        var runId = runs.Single().Id;

        var body = await client.TraconStreamRunEventsAsync(runId);
    ...
    public async Task OpenAI_responses_streams_frames_when_the_body_asks_for_a_stream()
    {
        ...
        await foreach (var frame in client.TraconOpenAIResponsesStreamAsync(
    ...
    public async Task OpenAI_chat_completions_streams_frames_ending_with_the_DONE_marker()
    {
        ...
        await foreach (var frame in client.TraconOpenAIChatCompletionsStreamAsync(
            Body(new
            {
                model = "kod-agent",
                stream = true,
    ...
    public async Task OpenAI_responses_still_returns_JSON_when_the_body_does_not_ask_for_a_stream()
    {
        ...
        var json = await client.TraconOpenAIResponsesAsync(
    ...
    public async Task The_JSON_method_names_the_streaming_sibling_when_the_body_asked_for_a_stream()
    {
        ...
        var exception = await Should.ThrowAsync<TraconApiException>(
            async () => await client.TraconOpenAIResponsesAsync(
                Body(new { model = "kod-agent", input = "hello", stream = true })));

        exception.Message.ShouldContain("text/event-stream");
        exception.Message.ShouldContain("TraconOpenAIResponsesStreamAsync");
    }
    ...
    public async Task The_streaming_method_names_the_JSON_sibling_when_the_body_did_not_ask_for_a_stream()
    {
        ...
        var exception = await Should.ThrowAsync<TraconApiException>(async () =>
        {
            await foreach (var _ in client.TraconOpenAIResponsesStreamAsync(
                Body(new { model = "kod-agent", input = "hello", stream = false })))
            {
        ...
        exception.Message.ShouldContain("TraconOpenAIResponsesAsync");
    ...
    public async Task An_uninitialized_JsonElement_body_is_named_rather_than_failing_inside_the_serializer()
    {
        ...
        var exception = await Should.ThrowAsync<ArgumentException>(
            async () => await client.TraconOpenAIResponsesAsync(default));
    ...
    public async Task A_pure_SSE_endpoint_streams_frames_through_its_sibling()
    {
        ...
        await foreach (var frame in client.TraconRunAgentStreamAsync("kod-agent", RunRequest()))
        {
            frames.Add(frame);
        }
    ...
    public async Task Breaking_out_early_stops_the_stream_without_faulting()
    {
        ...
        await foreach (var _ in client.TraconRunAgentStreamAsync("kod-agent", RunRequest()))
        {
            seen++;
            break;
        }
    ...
    public async Task WithCancellation_reaches_the_request_the_streaming_method_sends()
    {
        ...
        await Should.ThrowAsync<OperationCanceledException>(async () =>
        {
            await foreach (var _ in client
                .TraconRunAgentStreamAsync("kod-agent", RunRequest())
    ...
    private static TraconApiClient CreateClient(TraconTestHost host)
    {
        // The generated client's paths are relative to the MapTracon prefix
        // (docs/hafiza/nswag-istemci-uretimi.md), which TraconClientOptions.BaseAddress
        // normally carries; host.Client's own BaseAddress is bare, so it is set here instead.
        host.Client.BaseAddress = new Uri(host.Client.BaseAddress!, "tracon/");

        return new TraconApiClient(host.Client);
    }
    ...
    // caller writes.
    // Fully qualified: this file also sees the server-side Tracon.AgentRunRequest
    // through the project's global usings, and the bare name is ambiguous (CS0104).
    private static Tracon.Client.Generated.AgentRunRequest RunRequest() => new()
    {
        Message = "merhaba",
    };
}
