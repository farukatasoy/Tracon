...
/// <summary>
/// Verifies that Tracon's endpoints appear in the OpenAPI document.
/// </summary>
/// <remarks>
/// <c>Tracon.AspNetCore</c> deliberately does <strong>not depend</strong> on
...
{
    [Fact]
    public async Task Document_is_generated_and_contains_Tracon_endpoints()
    {
        ...
        var paths = (await TraconTestHost.ReadJsonAsync(response)).GetProperty("paths");

        paths.TryGetProperty("/tracon/api/meta", out _).ShouldBeTrue();
        paths.TryGetProperty("/tracon/api/agents", out _).ShouldBeTrue();
        paths.TryGetProperty("/tracon/api/agents/{name}", out _).ShouldBeTrue();
        paths.TryGetProperty("/tracon/api/runs/{runId}/events", out _).ShouldBeTrue();
        paths.TryGetProperty("/tracon/v1/responses", out _).ShouldBeTrue();
        paths.TryGetProperty("/tracon/v1/chat/completions", out _).ShouldBeTrue();
    }
    ...
    public async Task Document_carries_summary_and_tag_metadata()
    {
        ...
        var meta = (await TraconTestHost.ReadJsonAsync(response))
            .GetProperty("paths")
            .GetProperty("/tracon/api/meta")
            .GetProperty("get");

        meta.GetProperty("summary").GetString().ShouldNotBeNullOrWhiteSpace();
        meta.GetProperty("tags")[0].GetString().ShouldBe("Tracon");
        meta.GetProperty("operationId").GetString().ShouldBe("TraconMeta");
    }

    [Fact]
    public async Task Custom_prefix_also_applies_in_the_document()
    {
        await using var host = await TraconTestHost.StartAsync(prefix: "/management", withOpenApi: true);
    ...
    public async Task Document_carries_operation_descriptions_and_schema_documentation()
    {
        ...
        var document = await TraconTestHost.ReadJsonAsync(response);
        ...
        operations
            ...
            .ShouldBeEmpty("Every Tracon endpoint must carry a description; the documentation site publishes it.");
