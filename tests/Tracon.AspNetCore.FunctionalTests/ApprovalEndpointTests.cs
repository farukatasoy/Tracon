...
public sealed class ApprovalEndpointTests
{
    ...
    public async Task Queued_run_requests_approval_is_approved_from_the_console_and_completes()
    {
        ...
        var decided = await TraconTestHost.ReadJsonAsync(decideResponse);
        ...
        (await TraconTestHost.ReadJsonAsync(pendingAfter)).EnumerateArray().ShouldBeEmpty();
    }
    ...
    public async Task Reader_role_cannot_make_a_decision()
    {
        ...
        using var response = await host.Client.PostAsJsonAsync(
            new Uri($"/tracon/api/approvals/{Guid.NewGuid()}/decide", UriKind.Relative),
            new ApprovalDecisionRequest { Approved = true });
    ...
    public async Task Synchronous_non_streaming_run_closes_as_AwaitingApproval_when_it_requests_approval()
    {
        ...
        using var runResponse = await host.Client.GetAsync(new Uri($"/tracon/api/runs/{runId}", UriKind.Relative));
        (await TraconTestHost.ReadJsonAsync(runResponse)).GetProperty("status").GetString()
    ...
    public async Task Synchronous_streaming_run_closes_as_AwaitingApproval_when_it_requests_approval()
    {
        ...
        using var runningJobs = await host.Client.GetAsync(
            new Uri("/tracon/api/runs?sessionId=session-sync-streaming", UriKind.Relative));
        var run = (await TraconTestHost.ReadJsonAsync(runningJobs)).EnumerateArray().ShouldHaveSingleItem();
    ...
    public async Task Approval_row_exists_before_the_run_reports_AwaitingApproval()
    {
        ...
        (await TraconTestHost.ReadJsonAsync(pending)).EnumerateArray().ShouldHaveSingleItem();
    ...
    /// real store is internal to Tracon.Core, so this stands in for it; only
