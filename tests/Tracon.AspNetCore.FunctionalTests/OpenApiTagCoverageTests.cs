...
/// </summary>
/// <remarks>
/// The first tag is always <c>Tracon</c> — this lets a consumer whose own
/// endpoints mix into the same document separate Tracon endpoints by this tag.
...
{
    [Fact]
    public async Task Every_endpoint_has_at_least_two_tags_and_the_first_is_Tracon()
    {
        ...
        foreach (var (path, method, operation) in OpenApiTestHelpers.EnumerateOperations(document))
        {
            ...
            if (tags.Count < 2 || !string.Equals(tags[0], "Tracon", StringComparison.Ordinal))
            {
        ...
        invalid.ShouldBeEmpty(
            customMessage: "Endpoints with missing or wrongly ordered tags (expected: [\"Tracon\", <domain>]): " +
                           string.Join("; ", invalid));
    }
}
