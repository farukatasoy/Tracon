...
public sealed class ApiKeyScopeEnforcementTests
{
    ...
    public async Task PlatformRead_scoped_key_lists_schedules()
    {
        ...
        using var request = new HttpRequestMessage(HttpMethod.Get, "/tracon/api/schedules");
    ...
    public async Task PlatformRead_scope_is_not_enough_to_write_a_schedule()
    {
        ...
        using var request = new HttpRequestMessage(HttpMethod.Put, "/tracon/api/schedules/daily")
        {
            Content = JsonContent.Create(new { handlerKey = "tracon.agent-batch", targetName = "kod-agent", timeZone = "UTC", enabled = true }),
        };
        request.Headers.Authorization = new AuthenticationHeaderValue("Bearer", created.PlaintextKey);
    ...
    public async Task PlatformAdmin_scoped_key_writes_a_schedule()
    {
        ...
        using var request = new HttpRequestMessage(HttpMethod.Put, "/tracon/api/schedules/daily")
        {
            Content = JsonContent.Create(
                new { handlerKey = "tracon.agent-batch", targetName = "kod-agent", timeZone = "UTC", payload = new { }, enabled = true }),
        };
        request.Headers.Authorization = new AuthenticationHeaderValue("Bearer", created.PlaintextKey);
    ...
    public async Task AuditRead_scoped_key_reads_the_audit_log()
    {
        ...
        using var request = new HttpRequestMessage(HttpMethod.Get, "/tracon/api/audit");
        request.Headers.Authorization = new AuthenticationHeaderValue("Bearer", created.PlaintextKey);
    ...
    public async Task RunsRead_reads_stats_but_cannot_recalculate_costs()
    {
        ...
        using var read = new HttpRequestMessage(HttpMethod.Get, "/tracon/api/stats");
        ...
        using var write = new HttpRequestMessage(HttpMethod.Post, "/tracon/api/stats/recalculate-costs");
