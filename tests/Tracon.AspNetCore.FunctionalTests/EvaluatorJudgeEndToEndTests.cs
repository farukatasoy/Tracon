...
public sealed class EvaluatorJudgeEndToEndTests
{
    ...
    public async Task Two_evaluators_reporting_the_same_metric_both_survive()
    {
        ...
        await using var host = await TraconTestHost.StartAsync(
            configureTracon: static tracon => tracon
    ...
    public async Task A_bridged_metric_never_reaches_the_zero_to_hundred_online_average()
    {
        ...
        using var summary = await host.Client.GetAsync(new Uri("/tracon/api/evaluation/online", UriKind.Relative));
        var body = await TraconTestHost.ReadJsonAsync(summary);
    ...
    private static async Task<IReadOnlyList<JsonElement>> ReadScoresAsync(TraconTestHost host, Guid runId)
    {
