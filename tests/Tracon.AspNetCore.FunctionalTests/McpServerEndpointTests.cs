...
/// <summary>
/// Phase 50: the MCP server that exposes Tracon agents.
/// </summary>
public sealed class McpServerEndpointTests
{
    ...
    public async Task Empty_allowlist_returns_no_tools()
    {
        ...
        var (response, body) = await McpTestClient.SendAsync(host.Client, "/tracon/mcp", "tools/list");
    ...
    public async Task Allowlisted_agent_appears_as_a_tool()
    {
        ...
        var (_, body) = await McpTestClient.SendAsync(host.Client, "/tracon/mcp", "tools/list");

        var tools = Tools(body!.Value);
        tools.GetArrayLength().ShouldBe(1);
        tools[0].GetProperty("name").GetString().ShouldBe("tracon_kod-agent");
    }
    ...
    public async Task ToolsCall_runs_the_agent_and_produces_a_runs_row()
    {
        ...
        var (response, body) = await McpTestClient.SendAsync(
            ...
            "/tracon/mcp",
            "tools/call",
            new { name = "tracon_kod-agent", arguments = new { message = "hello" } });
    ...
    public async Task Agent_carrying_a_tool_that_requires_approval_cannot_be_exposed()
    {
        ...
        // background inside McpApprovalGuardFilter (see TraconMcpServerExtensions)
        ...
        var exception = await Should.ThrowAsync<InvalidOperationException>(
            () => McpTestClient.SendAsync(host.Client, "/tracon/mcp", "tools/list"));

        exception.Message.ShouldContain("approval", Case.Sensitive);
    }

    [Fact]
    public async Task MapTraconMcpServer_does_not_crash_on_an_empty_sqlite_database()
    {
        // Regression: MapTraconMcpServer() used to read the catalog
    ...
    public async Task Depth_limit_blocks_a_sub_call()
    {
        ...
        await using var host = await TraconTestHost.StartAsync(configureTracon: builder =>
        {
            var provider = new Tracon.Testing.FakeModelProvider("routing")
        ...
        var (_, body) = await McpTestClient.SendAsync(
            ...
            "/tracon/mcp",
            "tools/call",
            new { name = "tracon_router", arguments = new { message = "start" } });
    ...
    {
        await Should.ThrowAsync<InvalidOperationException>(() => TraconTestHost.StartAsync(
            configureTracon: builder => builder
                .AddAgent(TestData.Definition())
                .UseMcpServer(o => o.ExposedAgents.Add("kod-agent")),
    ...
    public async Task Request_with_the_correct_token_is_accepted()
    {
        ...
        var (response, _) = await McpTestClient.SendAsync(host.Client, "/tracon/mcp", "tools/list", token: "s3cr3t");
    ...
    {
        await using var host = await TraconTestHost.StartAsync(
            configureTracon: builder => builder.UseMcpServer(o => o.ExposedAgents.Add("db-agent")),
            configureAfterMap: app => app.MapTraconMcpServer());

        var before = Tools((await McpTestClient.SendAsync(host.Client, "/tracon/mcp", "tools/list")).Body!.Value);
        ...
        var after = Tools((await McpTestClient.SendAsync(host.Client, "/tracon/mcp", "tools/list")).Body!.Value);
        after.GetArrayLength().ShouldBe(1);
        after[0].GetProperty("name").GetString().ShouldBe("tracon_db-agent");
    }

    [Fact]
    public async Task Tenant_isolation_is_preserved()
    {
        const string TenantHeader = "X-Tracon-Tenant";
        ...
        var mine = Tools((await McpTestClient.SendAsync(
            host.Client, "/tracon/mcp", "tools/list", tenantHeader: "tenant-a")).Body!.Value);
        ...
        var theirs = Tools((await McpTestClient.SendAsync(
            host.Client, "/tracon/mcp", "tools/list", tenantHeader: "tenant-b")).Body!.Value);
