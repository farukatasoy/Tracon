...
/// <summary>Phase 117: the MCP Tasks extension, backed by Tracon's run store.</summary>
public sealed class McpTasksEndpointTests
{
    ...
    public async Task Discover_probe_negotiates_2026_07_28_even_when_the_first_response_is_slow()
    {
        ...
        await using var client = await McpTaskTestClient.ConnectAsync(host.Client, "/tracon/mcp");
    ...
    public async Task EnableTasks_false_still_answers_synchronously()
    {
        ...
        var outcome = await client.CallToolAsTaskAsync(
            new CallToolRequestParams { Name = "tracon_kod-agent", Arguments = Arguments("hello") });
    ...
    public async Task EnableTasks_false_still_resolves_the_real_tenant_when_tenancy_is_on()
    {
        ...
        await using var client = await McpTaskTestClient.ConnectAsync(host.Client, "/tracon/mcp", tenantHeader: "tenant-a");

        var outcome = await client.CallToolAsTaskAsync(
            new CallToolRequestParams { Name = "tracon_db-agent", Arguments = Arguments("hello") });
    ...
    public async Task Tasks_cancel_of_a_running_task_actually_cancels_the_in_flight_tool_call()
    {
        ...
        var outcome = await client.CallToolAsTaskAsync(
            new CallToolRequestParams { Name = "tracon_kod-agent", Arguments = Arguments("go") });
