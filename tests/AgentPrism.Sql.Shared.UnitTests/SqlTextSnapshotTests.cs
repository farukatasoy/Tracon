...
/// <c>AGENTPRISM_SQL_SNAPSHOT_REFRESH=1 dotnet test tests/AgentPrism.Sql.Shared.UnitTests -c Release</c>.
/// </para>
/// </remarks>
public sealed class SqlTextSnapshotTests
{
    private const string RefreshEnvVar = "AGENTPRISM_SQL_SNAPSHOT_REFRESH";

    /// <summary>The schema name every provider ships as its own default.</summary>
    private const string PostgresAndSqlServerSchema = "agentprism";

    /// <summary>The table prefix SQLite ships as its own default (K-193: no dot).</summary>
    private const string SqliteTablePrefix = "agentprism_";
    ...
    private static string BaselinePath(string dialectName) => Path.Combine(
        RepositoryRoot,
        "tests",
        "AgentPrism.Sql.Shared.UnitTests",
        "Baselines",
