...
/// </remarks>
public sealed class GoogleModelProviderContractTests : ModelProviderContract
{
    protected override ValueTask<IModelProvider> CreateProviderAsync()
        => new(new GoogleModelProvider(
            GoogleProviderNames.Google,
            new GoogleChatClientFactory(TestData.Options()),
            [new ModelDescriptor { Name = TestData.Model }]));
}

/// <summary>
/// The Google adapter honors a per-tenant credential (BYOK), so it also
/// owes the credential contract.
/// </summary>
public sealed class GoogleModelProviderCredentialContractTests : ModelProviderCredentialContract
{
    protected override ValueTask<IModelProvider> CreateProviderAsync()
    {
        var options = TestData.Options();

        return new(new GoogleModelProvider(
            GoogleProviderNames.Google,
    ...
    /// the Google GenAI SDK's <c>Client._apiClient.ApiClient.ApiKey</c>. The
    /// walk is generic rather than pinned to that path, so an SDK refactor
    /// that keeps the key reachable somewhere in the graph does not break
    /// this test.
    /// </remarks>
...
/// <summary>
/// The Google adapter reads provider settings (safety thresholds, thinking
/// budget), so it also owes the settings contract.
/// </summary>
public sealed class GoogleModelProviderSettingsContractTests : ModelProviderSettingsContract
{
    ...
    protected override KeyValuePair<string, JsonElement> SupportedSetting
        => new(GoogleProviderNames.ThinkingIncludeThoughtsSetting, True);
}
...
/// </summary>
public sealed class GoogleProviderContractCoverageTests
{
    [Fact]
