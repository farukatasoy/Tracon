...
/// (<c>TraconApiClient.Sse.cs</c>) precisely so they can be tested here:
...
/// </remarks>
public sealed class TraconApiClientSseTests
{
    [Fact]
    ...
    {
        // Tracon's SseWriter sends ": keep-alive\n\n" while a run is still
    ...
    public async Task Cancelling_mid_stream_stops_the_enumeration()
    {
        ...
        var act = async () =>
        {
            await foreach (var frame in TraconApiClient.ReadServerSentEventFramesAsync(response, cts.Token))
            {
                received.Add(frame);
    ...
    public async Task A_streaming_read_of_a_JSON_answer_is_refused_by_name()
    {
        ...
        var exception = await Should.ThrowAsync<TraconApiException>(
            async () => await TraconApiClient.EnsureContentTypeAsync(
    ...
    public async Task A_connection_that_drops_mid_stream_surfaces_the_error_rather_than_ending_quietly()
    {
        ...
        var act = async () =>
        {
            await foreach (var frame in TraconApiClient.ReadServerSentEventFramesAsync(response, default))
            {
                received.Add(frame);
            }
        };
    ...
    public void A_charset_parameter_does_not_hide_the_media_type()
    {
        ...
        TraconApiClient.IsServerSentEventStream(response).ShouldBeTrue();
    }
