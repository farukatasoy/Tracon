...
/// <summary>
/// Phase 137 — the SQL Server twin of the PostgreSQL mapping test.
/// </summary>
/// <remarks>
/// 🚨 SQL Server compiles a batch UP FRONT, so every statement that names the
/// column the same file just added has to go through <c>EXEC</c> (the K-475
/// class). A mistake there fails at run time only, against a real server —
/// which is what this test provides.
/// </remarks>
public sealed class JobHandlerKeyMigrationTests(SqlServerFixture fixture)
{
    ...
    public async Task All_nine_kinds_are_mapped_and_no_row_or_job_item_is_lost()
    {
        ...
        await context.ExecuteAsync(
            ...
            $"SELECT NEWID(), id, 0, 'input', 1 FROM {prefix}jobs;");

        var jobsBefore = await context.ScalarAsync<long>($"SELECT CAST(COUNT(*) AS bigint) FROM {prefix}jobs;");
        var itemsBefore = await context.ScalarAsync<long>($"SELECT CAST(COUNT(*) AS bigint) FROM {prefix}job_items;");
        ...
        (await context.ScalarAsync<long>($"SELECT CAST(COUNT(*) AS bigint) FROM {prefix}jobs;")).ShouldBe(jobsBefore);
        (await context.ScalarAsync<long>($"SELECT CAST(COUNT(*) AS bigint) FROM {prefix}job_items;")).ShouldBe(itemsBefore);
    ...
    private static ValueTask<int> SeedScheduleAsync(SqlServerTestContext context, string prefix, short kind)
    {
        ...
        return context.ExecuteAsync(
            ...
            $"VALUES (NEWID(), 'test', 'schedule-{value}', {value}, 'target', " +
            "'{}', SYSDATETIMEOFFSET(), SYSDATETIMEOFFSET());");
    }

    private static ValueTask<int> SeedJobAsync(SqlServerTestContext context, string prefix, short kind)
    {
        var value = kind.ToString(CultureInfo.InvariantCulture);
        ...
        return context.ExecuteAsync(
            ...
            $"VALUES (NEWID(), 'test', {value}, 'target-{value}', 3, " +
            "'{}', SYSDATETIMEOFFSET(), SYSDATETIMEOFFSET());");
    }
    ...
    private static async Task<bool> ColumnExistsAsync(
        ...
        => await context.ScalarAsync<int>(
            $"SELECT CASE WHEN COL_LENGTH(N'{prefix}{table}', N'{column}') IS NULL THEN 0 ELSE 1 END;") > 0;
}
