...
public sealed class MigrationRunnerTests(SqlServerFixture fixture)
{
    ...
    private static int EmbeddedMigrationCount { get; } = typeof(MigrationRunner).Assembly
        ...
        .Count(static name =>
            name.StartsWith("Tracon.SqlServer.Migrations.", StringComparison.Ordinal)
    ...
    public void Invalid_schema_name_is_rejected()
    {
        ...
        {
            Should.Throw<TraconException>(() => new SqlServerDialect(invalid));
        }
    }
    ...
    public void Migration_runner_reads_from_the_correct_assembly()
    {
        ...
        var dialect = new SqlServerDialect("tracon");

        dialect.MigrationResourcePrefix.ShouldBe("Tracon.SqlServer.Migrations.");
        MigrationDescriptor
    ...
    public async Task Populated_pre_152_run_scores_upgrade_without_data_loss()
    {
        ...
        await context.ExecuteAsync($"""
            ...
            VALUES
                ('{TraconId.NewId()}', N'test', '{runId}', NULL, 1, 1, N'good', N'human', N'alice', '2026-01-01T00:00:00Z'),
                ('{TraconId.NewId()}', N'test', '{runId}', NULL, 3, 70, N'ok', N'judge:quality', N'judge:quality', '2026-01-01T00:00:00Z');
            """);
        ...
        await context.ExecuteAsync($"""
            ...
            VALUES
                ('{TraconId.NewId()}', N'test', '{runId}', NULL, 1, 0, NULL, N'human', N'alice', '2026-01-01T00:00:00Z', N'accuracy', NULL);
            """);
