...
public sealed class ObservabilityTests
{
    ...
    public void Metric_names_stay_stable()
    {
        ...
        TraconDiagnostics.ActivitySourceName.ShouldBe("Tracon");
        TraconDiagnostics.MeterName.ShouldBe("Tracon");
        TraconDiagnostics.RunCounterName.ShouldBe("tracon.runs");
        TraconDiagnostics.RunDurationName.ShouldBe("tracon.run.duration");
        TraconDiagnostics.TokenCounterName.ShouldBe("tracon.tokens");
        TraconDiagnostics.ToolCounterName.ShouldBe("tracon.tool.invocations");
        TraconDiagnostics.ToolDurationName.ShouldBe("tracon.tool.duration");
    }
    ...
    public void Run_metrics_are_emitted()
    {
        ...
        collector.LongValues(TraconDiagnostics.RunCounterName).ShouldBe([1]);
        ...
        collector.DoubleValues(TraconDiagnostics.RunDurationName).ShouldBe([2.0]);

        collector.LongValues(TraconDiagnostics.TokenCounterName).ShouldBe([10, 4]);
    }
    ...
    public void Unknown_model_does_not_leave_tag_empty()
    {
        ...
        var modelTags = collector.Tags(TraconDiagnostics.TokenCounterName)
            .Select(static tags => Convert.ToString(
                tags.GetValueOrDefault(TraconDiagnostics.Tags.ModelId),
    ...
    public void Tool_metrics_do_not_write_histogram_without_duration()
    {
        ...
        collector.LongValues(TraconDiagnostics.ToolCounterName).ShouldBe([1]);
        collector.DoubleValues(TraconDiagnostics.ToolDurationName).ShouldBeEmpty();
    }
    ...
    public async Task No_listener_is_set_up_while_span_persistence_is_disabled()
    {
        ...
        (await collector.CompleteRunAsync("trace", "span", TraconId.NewId(), "default", RunStatus.Completed))
            .ShouldBeFalse();
    }
    ...
    public async Task Untracked_trace_leaves_no_buffer()
    {
        ...
        (await collector.CompleteRunAsync("unknown", "span", TraconId.NewId(), "default", RunStatus.Completed))
            .ShouldBeFalse();
    }
    ...
    public async Task Inner_spans_become_children_of_root_span()
    {
        ...
        var root = trace.Spans
            .Single(span => string.Equals(span.Name, TraconDiagnostics.RunActivityName, StringComparison.Ordinal));
    ...
    public async Task Error_message_is_withheld_while_sensitive_data_is_off()
    {
        ...
        using var source = new ActivitySource(TraconDiagnostics.ActivitySourceName);
    ...
    private static RunTraceCollector CreateCollector(
        ITraceStore store,
        Action<TraconObservabilityOptions> configure)
    {
    ...
    public async Task Two_runs_sharing_a_trace_id_do_not_share_a_buffer()
    {
        ...
        var source = new ActivitySource(TraconDiagnostics.ActivitySourceName);
        ...
        var firstRunId = TraconId.NewId();
        var secondRunId = TraconId.NewId();
    ...
    /// <summary>
    /// Agent that opens its own span from the Tracon source during a run.
    /// </summary>
    /// <remarks>
    ...
    private sealed class ActivityStartingAgent : Microsoft.Agents.AI.DelegatingAIAgent
    {
        ...
        private static readonly ActivitySource Source = new(TraconDiagnostics.ActivitySourceName);
    ...
    /// (<c>instrument.Meter.Name == "Tracon"</c>) also catches measurements from
