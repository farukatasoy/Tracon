...
/// a consumer can call <c>AddTracon()</c> on a bare service collection that no
...
public sealed class SilentGapWarningRegistrationTests
{
    ...
    private sealed class RecordingLoggerProvider : ILoggerProvider
    {
        ...
        private sealed class RecordingLogger(string category, ConcurrentQueue<string> entries) : ILogger
        {
            ...
            {
                if (!category.StartsWith(nameof(Tracon) + "." + nameof(SilentGapWarningService), StringComparison.Ordinal))
                {
                    return;
                }
