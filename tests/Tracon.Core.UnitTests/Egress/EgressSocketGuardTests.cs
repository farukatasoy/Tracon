...
public sealed class EgressSocketGuardTests
{
    ...
    private static EgressSocketGuard FromOptions(bool allowPrivateNetworkTargets)
        => new(new StaticMonitor(new TraconEgressOptions
        {
    ...
    public async Task Guarded_client_refuses_to_connect_to_a_private_address(string url)
    {
        ...
        Unwrap(exception).ShouldNotBeNull()
            .Message.ShouldContain(nameof(TraconEgressOptions.AllowPrivateNetworkTargets));
    }
    ...
    public void Null_arguments_are_rejected()
    {
        ...
        Should.Throw<ArgumentNullException>(
            () => new EgressSocketGuard((IOptionsMonitor<TraconEgressOptions>)null!));
    }

    /// <summary>Finds the guard's own exception inside HttpClient's wrapping, if it is there.</summary>
    private static TraconException? Unwrap(Exception? exception)
    {
        for (; exception is not null; exception = exception.InnerException)
        {
            if (exception is TraconException tracon)
            {
                return tracon;
            }
        }
    ...
    private sealed class StaticMonitor(TraconEgressOptions value) : IOptionsMonitor<TraconEgressOptions>
    {
        public TraconEgressOptions CurrentValue => value;

        public TraconEgressOptions Get(string? name) => value;

        public IDisposable? OnChange(Action<TraconEgressOptions, string?> listener) => null;
    }
}
