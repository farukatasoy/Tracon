...
public sealed class RunJudgeSetTests
{
    ...
    public void Invalid_names_are_rejected_at_startup(string name)
        => Should.Throw<TraconException>(() => new RunJudgeSet([new FixedJudge(name)]));

    [Fact]
    public void Names_are_unique_without_case_sensitivity()
        => Should.Throw<TraconException>(() => new RunJudgeSet([new FixedJudge("quality"), new FixedJudge("QUALITY")]));
