...
public sealed class FallbackChatClientTests
{
    ...
    public async Task Empty_fallbacks_preserves_todays_error_path()
    {
        var primary = new FakeModelProvider(
            new FakeChatClient(_ => throw new TraconProviderUnavailableException("circuit open") { ProviderName = "primary" }),
            name: "primary");

        using var chatClient = new ModelProviderRegistry([primary]).CreateChatClient(Binding(primary: "primary"));

        var exception = await Should.ThrowAsync<TraconProviderUnavailableException>(
    ...
    public async Task Open_circuit_on_primary_falls_over_to_the_fallback()
    {
        var primaryClient = new FakeChatClient(
            _ => throw new TraconProviderUnavailableException("circuit open") { ProviderName = "primary" });
    ...
    {
        // 🚨 Regression (found live, samples/Tracon.Api, a real OpenAI
        ...
        var primaryClient = new FakeChatClient(
            _ => throw new TraconProviderUnavailableException("circuit open"));
        var fallbackClient = new FakeChatClient(_ => new ChatResponse(new ChatMessage(ChatRole.Assistant, "fallback answer")));
    ...
    public async Task Content_filter_does_not_trigger_a_fallback_attempt()
    {
        ...
        await Should.ThrowAsync<TraconContentFilteredException>(
            () => chatClient.GetResponseAsync(Messages, cancellationToken: TestContext.Current.CancellationToken));
    ...
    {
        var primaryClient = new FakeChatClient(_ => throw new TraconProviderUnavailableException("circuit open"));
        var fallbackClient = new FakeChatClient(_ => new ChatResponse(new ChatMessage(ChatRole.Assistant, "fallback answer")));
    ...
    {
        var primary = new FakeModelProvider(
            FailingStreamClient(new TraconProviderUnavailableException("circuit open")),
            name: "primary");
