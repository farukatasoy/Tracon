...
/// <summary>
/// A consumer can call <c>AddTracon()</c> on a bare service collection that
/// no host backs. Nothing Tracon registers may then depend on a service only
/// a host provides.
/// </summary>
/// <remarks>
...
public sealed class NonPersistentStorageWarningRegistrationTests
{
    ...
    private sealed class RecordingLoggerProvider : ILoggerProvider
    {
        ...
        private sealed class RecordingLogger(string category, ConcurrentQueue<string> entries) : ILogger
        {
            ...
            {
                if (!category.StartsWith(nameof(Tracon) + "." + nameof(NonPersistentStorageWarningService), StringComparison.Ordinal))
                {
                    return;
                }
