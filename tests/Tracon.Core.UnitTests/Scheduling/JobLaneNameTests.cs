...
/// <summary>Tests for <see cref="JobLanes"/> and the lane checks in <see cref="TraconSchedulingOptionsValidator"/>.</summary>
public sealed class JobLaneNameTests
{
    private readonly TraconSchedulingOptionsValidator _validator = new();
    ...
    public void Validator_accepts_default_options()
        => _validator.Validate(null, new TraconSchedulingOptions()).Succeeded.ShouldBeTrue();

    [Fact]
    public void Validator_rejects_an_invalid_name_in_lanes()
    {
        var options = new TraconSchedulingOptions { Lanes = ["Media"] };
    ...
    {
        var options = new TraconSchedulingOptions();
    ...
    {
        var options = new TraconSchedulingOptions { MaxConcurrentJobs = 2 };
