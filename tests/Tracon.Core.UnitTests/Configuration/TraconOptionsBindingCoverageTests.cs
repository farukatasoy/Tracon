...
/// <summary>
/// <c>TraconOptions</c> is bound by hand (K-021, AOT). This structurally locks
...
/// field in the <c>TraconOptions</c> tree with a single configuration set and
...
/// </remarks>
public sealed class TraconOptionsBindingCoverageTests
{
    /// <summary>
    ...
    private static readonly HashSet<string> ExcludedPaths = new(StringComparer.Ordinal)
    {
        ...
        // TraconOptionsValidator floor is TruncatingAIFunction.MinimumEnvelopeBytes
    ...
    [Fact]
    public void Every_scalar_field_in_TraconOptions_tree_is_bound_from_configuration()
    {
        ...
        Collect(new TraconOptions(), TraconOptions.SectionName, [], configValues, expectations);
        ...
        var bound = provider.GetRequiredService<IOptions<TraconOptions>>().Value;
    ...
    public void DefaultMaxOutputBytes_binds_from_configuration()
    {
        var configuration = new ConfigurationBuilder()
            ...
            {
                [$"{TraconOptions.SectionName}:Tools:DefaultMaxOutputBytes"] = "500",
            })
            .Build();
    ...
    private static bool IsRecursableOptionType(Type type)
        => type.IsClass
            && string.Equals(type.Namespace, "Tracon", StringComparison.Ordinal)
            && type.Name.StartsWith("Tracon", StringComparison.Ordinal)
