...
/// </summary>
/// <remarks>
/// 🚨 The most important test comes first: <c>AddTracon()</c> alone
...
public sealed class ContentGuardRegistrationTests
{
    ...
    {
        using var provider = Build(services => services.AddTracon());
    ...
    {
        using var provider = Build(services => services.AddTracon().AddPatternContentGuard());
    ...
    public void When_the_configuration_section_is_present_the_built_in_guard_is_registered()
    {
        var configuration = new ConfigurationBuilder()
            ...
            {
                ["Tracon:ContentGuard:Pattern:MaskedPii"] = "Email,CreditCard",
                ["Tracon:ContentGuard:Pattern:DeniedTerms:0"] = "secret-project",
                ["Tracon:ContentGuard:BufferStreamingOutput"] = "false",
            })
            .Build();
    ...
    public void When_the_configuration_section_is_absent_no_guard_is_registered()
    {
        var configuration = new ConfigurationBuilder()
            .AddInMemoryCollection(new Dictionary<string, string?>(StringComparer.Ordinal)
            {
                ["Tracon:DefaultTenantId"] = "default",
            })
            .Build();
    ...
    public void The_registry_picks_up_the_guard_pipeline_from_DI()
    {
        ...
        using var provider = Build(services =>
        {
            services.AddTracon().AddPatternContentGuard();
