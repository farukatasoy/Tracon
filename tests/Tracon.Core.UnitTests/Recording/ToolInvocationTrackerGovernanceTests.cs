...
public sealed class ToolInvocationTrackerGovernanceTests
{
    ...
    {
        var tracker = new ToolInvocationTracker(TraconId.NewId(), measureDuration: false, TimeProvider.System);

        tracker.OnCall(new FunctionCallContent("call-1", "slow_tool", arguments: null), source: null, arguments: null);

        var timeoutException = new TraconToolTimeoutException("Tool 'slow_tool' did not complete within 1s.")
        {
            ToolName = "slow_tool",
