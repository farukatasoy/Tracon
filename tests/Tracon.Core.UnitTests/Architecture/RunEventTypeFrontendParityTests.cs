...
/// <c>src/Tracon.UI/frontend/src/lib/run-event.ts</c> (phase 141).
/// </summary>
/// <remarks>
/// <para>
/// An SSE frame carries no OpenAPI schema, so that union type cannot be
/// generated the way the rest of <c>@tracon/client</c> is — the file's own
...
public sealed class RunEventTypeFrontendParityTests
{
    ...
    private static string TypeScriptPath { get; } = Path.Combine(
        RepositoryRoot, "src", "Tracon.UI", "frontend", "src", "lib", "run-event.ts");
