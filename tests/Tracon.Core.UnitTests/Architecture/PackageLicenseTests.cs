...
/// <summary>
/// Enforces the licence matrix. Tracon ships under the PolyForm Small
...
public sealed class PackageLicenseTests
{
    ...
    {
        // @tracon/client and Tracon.Client are the same client generated
        ...
        // The expected SPDX id is DERIVED from what Tracon.Client carries, not
        ...
        var twinLicence = ReleaseGateLicences()["Tracon.Client"];
        var expectedSpdx = string.Equals(twinLicence, MitLicenseFile, StringComparison.Ordinal)
            ? "MIT"
            : "PolyForm-Small-Business-1.0.0";

        var manifest = File.ReadAllText(
            Path.Combine(RepositoryRoot, "packages", "tracon-client", "package.json"));

        manifest.ShouldContain(
            $"\"license\": \"{expectedSpdx}\"",
            customMessage: $"packages/tracon-client/package.json must declare '{expectedSpdx}', " +
                           "the same terms as its NuGet twin Tracon.Client.");

        var rootText = File.ReadAllText(Path.Combine(RepositoryRoot, twinLicence));
        var npmText = File.ReadAllText(
            Path.Combine(RepositoryRoot, "packages", "tracon-client", twinLicence));

        npmText.ShouldBe(
            rootText,
            customMessage: "packages/tracon-client/LICENSE.md has drifted from the root LICENSE.md. " +
    ...
    public void The_build_matrix_and_the_release_gate_agree_on_which_packages_are_MIT()
    {
        ...
        fromGate.ShouldBe(
            fromBuild,
            customMessage: "TraconMitLicensed in src/Directory.Build.props and PACKAGE_LICENSES in " +
    ...
    public void A_MIT_package_depends_on_no_PolyForm_package()
    {
        ...
        {
            foreach (var reference in TraconProjectReferences(package))
            {
    ...
    private static IReadOnlyList<string> PackableProjectIds()
    {
        ...
        // src/Tracon.Templates/.../Tracon.Starter.csproj, which is template
    ...
    private static readonly Regex GateEntryPattern = new(
        @"^\s*""(?<id>Tracon[^""]*)"":\s*(?<licence>POLYFORM_LICENSE_FILE|MIT_LICENSE_FILE),\s*$",
    ...
    private static HashSet<string> MitPackagesFromBuildProps()
    {
        ...
        start.ShouldBeGreaterThanOrEqualTo(0, "TraconMitLicensed was not found in src/Directory.Build.props.");

        var end = props.IndexOf("</TraconMitLicensed>", start, StringComparison.Ordinal);
        end.ShouldBeGreaterThan(start, "TraconMitLicensed in src/Directory.Build.props is not terminated.");
    ...
    private static IReadOnlyList<string> TraconProjectReferences(string package)
    {
    ...
    /// <summary>
    /// Finds the repository root by walking up to the Tracon.slnx file; the
    ...
    private static string FindRepositoryRoot()
    {
        ...
        directory.ShouldNotBeNull("Tracon.slnx was not found while walking up from the test output directory.");
