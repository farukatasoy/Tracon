...
/// <summary>
/// Proves at-rest content protection against a real SQLite database: the
/// stored bytes, not just the round trip through the store.
/// </summary>
/// <remarks>
/// Each test builds its own <see cref="SqlStoreContext"/> from an isolated,
/// migrated schema (<see cref="SqliteTestContext"/>) and swaps in a
/// content protector — <see cref="SqliteTestContext"/> itself always uses
...
public sealed class ContentProtectionTests(SqliteFixture fixture)
{
    ...
    {
        await using var baseContext = await SqliteTestContext.CreateAsync(fixture);
        ...
        var raw = await baseContext.ScalarAsync<string>($"SELECT state FROM {baseContext.TablePrefix}sessions WHERE id = 's1';");
        raw.ShouldNotBeNull();
    ...
    public async Task A_column_left_out_of_the_protected_set_stays_plaintext_while_another_column_is_encrypted()
    {
        ...
        var rawSession = await baseContext.ScalarAsync<string>($"SELECT state FROM {baseContext.TablePrefix}sessions WHERE id = 's1';");
        var rawRunInput = await baseContext.ScalarAsync<string>($"SELECT messages FROM {baseContext.TablePrefix}run_inputs WHERE run_id = '{runId.ToString().ToUpperInvariant()}';");
    ...
    public async Task Run_input_messages_are_encrypted_at_rest()
    {
        ...
        var raw = await baseContext.ScalarAsync<string>($"SELECT messages FROM {baseContext.TablePrefix}run_inputs WHERE run_id = '{runId.ToString().ToUpperInvariant()}';");
        raw.ShouldNotBeNull();
        raw.ShouldContain("$apEnc");
    ...
    public async Task Run_event_text_and_payload_are_encrypted_at_rest()
    {
        ...
        var rawText = await baseContext.ScalarAsync<string>($"SELECT text FROM {baseContext.TablePrefix}run_events WHERE run_id = '{runId.ToString().ToUpperInvariant()}';");
        var rawPayload = await baseContext.ScalarAsync<string>($"SELECT payload FROM {baseContext.TablePrefix}run_events WHERE run_id = '{runId.ToString().ToUpperInvariant()}';");
    ...
    public async Task Tool_arguments_and_result_are_encrypted_at_rest()
    {
        ...
        var rawArguments = await baseContext.ScalarAsync<string>($"SELECT arguments FROM {baseContext.TablePrefix}tool_invocations WHERE run_id = '{runId.ToString().ToUpperInvariant()}';");
        var rawResult = await baseContext.ScalarAsync<string>($"SELECT result FROM {baseContext.TablePrefix}tool_invocations WHERE run_id = '{runId.ToString().ToUpperInvariant()}';");
    ...
    public async Task Agent_file_content_is_encrypted_at_rest_and_search_still_finds_matches()
    {
        ...
        try
        {
            ...
            var raw = await baseContext.ScalarAsync<string>(
                $"SELECT content FROM {baseContext.TablePrefix}agent_files WHERE agent_name = 'cp-test-agent' AND path = '/notes.md';");
            raw.ShouldNotBeNull();
            raw.ShouldContain("$apEnc");
    ...
    public async Task Attachment_content_is_encrypted_at_rest_and_the_hash_still_matches_plaintext()
    {
        ...
        var rawBytes = await baseContext.ScalarAsync<byte[]>(
            $"SELECT content FROM {baseContext.TablePrefix}attachments WHERE id = '{descriptor.Id.ToString().ToUpperInvariant()}';");
    ...
    public async Task Reading_with_an_unconfigured_key_id_throws_naming_the_key_id()
    {
        ...
        (await Should.ThrowAsync<AgentPrismException>(async () => await reader.GetAsync("s1"))).Message.ShouldContain("k1");
    }
    ...
    public async Task A_row_written_while_protection_was_on_stays_readable_after_it_is_turned_off()
    {
        ...
        var rawWhileOn = await baseContext.ScalarAsync<string>($"SELECT state FROM {baseContext.TablePrefix}sessions WHERE id = 'turned-off';");
