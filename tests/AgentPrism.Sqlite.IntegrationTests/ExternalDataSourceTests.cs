...
/// <summary>
/// <c>UseSqlite(o =&gt; o.DataSource = ...)</c>: a data source AgentPrism did not
/// build and must never dispose (Phase 110).
/// </summary>
/// <remarks>
/// <c>Microsoft.Data.Sqlite</c> does not offer a <see cref="DbDataSource"/> of
/// its own; these tests use AgentPrism's own <see cref="SqliteDataSource"/>
/// adapter as the "consumer-supplied" instance, the same adapter shape a
/// consumer would have to write.
/// </remarks>
public sealed class ExternalDataSourceTests(SqliteFixture fixture)
{
    [Fact]
    public async Task DataSource_and_ConnectionString_together_is_rejected()
    {
        await using var external = new SqliteDataSource(fixture.ConnectionString);

        var services = new ServiceCollection();
        services.AddAgentPrism().UseSqlite(options =>
        {
            options.DataSource = external;
            options.ConnectionString = fixture.ConnectionString;
            options.TablePrefix = SqliteTestContext.NewTablePrefix();
        });
        ...
        exception.Message.ShouldContain(nameof(AgentPrismSqliteOptions.DataSource));
        exception.Message.ShouldContain(nameof(AgentPrismSqliteOptions.ConnectionString));
    }
    ...
    public async Task DataSource_alone_does_not_require_a_connection_string()
    {
        ...
        provider.GetRequiredService<IOptions<AgentPrismSqliteOptions>>().Value.ShouldNotBeNull();
    ...
    public async Task External_data_source_is_not_disposed_when_the_host_stops()
    {
        ...
        await using (var provider = services.BuildServiceProvider())
        {
            ...
            await using var checkCommand = external.CreateCommand(
                $"SELECT count(*) FROM {tablePrefix}__migrations;");
        ...
        // If AgentPrism had disposed `external`, this would throw
    ...
    /// <summary>
    /// Mirror of the above: AgentPrism's OWN data source is marked as OWNED, so
    ...
    /// that a disposed <see cref="SqliteDataSource"/> actually rejects a later
    /// command: unlike <c>NpgsqlDataSource</c>, it owns no unmanaged pool of
    /// its own to release (a new <c>SqliteConnection</c> is opened per
    /// operation), so its <see cref="DbDataSource.Dispose()"/> is a no-op and
    /// a post-dispose command legitimately keeps working.
    /// </summary>
    /// <summary>
    /// Phase 110, K-625: the data source AgentPrism builds is no longer a
    /// public DI service — see the matching test and remarks in
    /// <c>AgentPrism.PostgreSql.IntegrationTests.ServiceRegistrationTests</c>.
    /// </summary>
    [Fact]
    public void The_data_source_is_not_registered_as_a_public_DI_service()
    {
        ...
        provider.GetService<SqliteDataSource>().ShouldBeNull();
    }
