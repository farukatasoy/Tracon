...
/// </remarks>
public sealed class AzureOpenAIModelProviderContractTests : ModelProviderContract
{
    protected override ValueTask<IModelProvider> CreateProviderAsync()
        => new(new AzureOpenAIModelProvider(
...
/// <summary>
/// The Azure OpenAI adapter honors a per-tenant credential (BYOK), so it
/// also owes the credential contract.
/// </summary>
public sealed class AzureOpenAIModelProviderCredentialContractTests : ModelProviderCredentialContract
{
    protected override ValueTask<IModelProvider> CreateProviderAsync()
    {
        var options = TestData.Options();

        return new(new AzureOpenAIModelProvider(
            AzureOpenAIProviderNames.AzureOpenAI,
    ...
    /// the OpenAI SDK's <c>ApiKeyAuthenticationPolicy</c> (Azure OpenAI reuses
    /// the OpenAI client pipeline). The walk is generic rather than pinned to
...
/// </summary>
/// <remarks>
/// <see cref="ModelProviderSettingsContract"/> is exempt: the Azure OpenAI
/// adapter accepts NO <see cref="ModelBinding.ProviderSettings"/> keys at
/// all (<c>AzureOpenAIChatClientFactory</c>'s own remarks) — Azure's chat
/// extensions break at run time across OpenAI SDK versions, so this
/// provider never reads a settings key to validate.
/// </remarks>
public sealed class AzureOpenAIProviderContractCoverageTests
{
    [Fact]
