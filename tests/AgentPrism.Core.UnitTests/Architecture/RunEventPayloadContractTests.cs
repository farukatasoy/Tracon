...
/// <c>AGENTPRISM_RUN_EVENT_PAYLOAD_REFRESH=1 dotnet test tests/AgentPrism.Core.UnitTests -c Release</c>.
/// </para>
/// </remarks>
public sealed class RunEventPayloadContractTests
{
    ...
    public void Payload_claiming_event_types_are_pinned_by_a_test()
    {
        ...
        failures.ShouldBeEmpty(
            customMessage: $"The run-event payload baseline is stale.{Environment.NewLine}" +
                           ...
                           $"Refresh with {RefreshEnvVar}=1 dotnet test tests/AgentPrism.Core.UnitTests -c Release");
    }
    ...
    private static void WriteBaseline(HashSet<string> claiming)
    {
        ...
        var builder = new StringBuilder()
            ...
            .AppendLine($"#   {RefreshEnvVar}=1 dotnet test tests/AgentPrism.Core.UnitTests -c Release")
