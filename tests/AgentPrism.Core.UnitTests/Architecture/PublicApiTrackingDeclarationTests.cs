...
/// <c>AgentPrismPublicApiTrackingEnabled=false</c>. A third state - neither -
/// is an error.
/// </summary>
/// <remarks>
...
public sealed class PublicApiTrackingDeclarationTests
{
    ...
    /// <c>AgentPrismPublicApiTrackingEnabled=false</c>, same as any other project.
    /// </summary>
    [Fact]
    public void Every_src_project_declares_its_public_API_tracking_status()
    {
        ...
        foreach (var projectDirectory in Directory.EnumerateDirectories(Path.Combine(RepositoryRoot, "src")))
        {
            ...
            {
                // AgentPrism.Sql.Shared is a linked-source tree (K-176), not
            ...
            var opensOut = File.ReadAllText(csproj).Contains(
                "<AgentPrismPublicApiTrackingEnabled>false</AgentPrismPublicApiTrackingEnabled>",
            ...
            switch (hasShipped && hasUnshipped, opensOut)
            {
                case (true, true):
                    failures.Add(
                        ...
                        "AgentPrismPublicApiTrackingEnabled=false - pick one.");
                    break;
                case (false, false):
                    failures.Add(
                        $"{name}: neither carries both PublicAPI.Shipped.txt and " +
                        "PublicAPI.Unshipped.txt, nor sets AgentPrismPublicApiTrackingEnabled=false " +
