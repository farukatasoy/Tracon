...
/// it (<c>src/AgentPrism.Sql.Shared/Stores/</c>) live in two different
/// places on purpose — the enum is public API in <c>AgentPrism.Abstractions</c>,
...
public sealed class ProtectedColumnCoverageTests
{
    ...
    public void Every_protected_column_is_referenced_by_a_write_call_in_the_shared_store_layer()
    {
        ...
        missing.ShouldBeEmpty(
            ...
            $"src/AgentPrism.Sql.Shared/Stores/: {string.Join(", ", missing)}. Either wire a " +
    ...
    {
        var storesDirectory = Path.Combine(RepositoryRoot, "src", "AgentPrism.Sql.Shared", "Stores");
