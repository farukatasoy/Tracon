...
public sealed class ProviderConcurrencyLimiterTests
{
    ...
    {
        var limiter = new ProviderConcurrencyLimiter(new StaticOptionsMonitor<AgentPrismOptions>(new AgentPrismOptions()));
    ...
    private static ProviderConcurrencyLimiter LimiterWithLimit(int limit)
        => new(new StaticOptionsMonitor<AgentPrismOptions>(
            new AgentPrismOptions { ModelConcurrency = new AgentPrismModelConcurrencyOptions { MaxConcurrentCallsPerProvider = limit } }));
}
