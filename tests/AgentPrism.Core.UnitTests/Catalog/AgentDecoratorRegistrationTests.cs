...
/// distinction on <c>IAgentPrismBuilder.Services</c> is real, measured behavior.
/// </summary>
public sealed class AgentDecoratorRegistrationTests
{
    ...
    public void Instance_registration_preserves_multiple_configurations_and_identity()
    {
        ...
        var services = new ServiceCollection();
        services.AddAgentPrism().AddAgentDecorator(first).AddAgentDecorator(second);
        using var provider = services.BuildServiceProvider();
    ...
    /// <summary>
    /// Proves the "before" row of the table added to <c>IAgentPrismBuilder.Services</c>'s
    /// documentation: a multi-registration seam registered through the raw escape hatch BEFORE
    /// <c>AddAgentPrism()</c> joins the list alongside the built-in decorators, it does not
    /// replace them.
    /// </summary>
    [Fact]
    public void An_escape_hatch_registration_made_before_AddAgentPrism_joins_the_built_ins()
    {
        var services = new ServiceCollection();
    ...
    /// <c>AddAgentPrism()</c> does not remove or shadow the built-in decorators - both run. This
    /// is the "real behavior difference from the single-instance case" the documentation calls out.
    /// </summary>
    [Fact]
    public void An_escape_hatch_registration_made_after_AddAgentPrism_also_joins_the_built_ins()
    {
        var services = new ServiceCollection();
        services.AddAgentPrism();
        services.AddSingleton<IAgentDecorator>(new NamedDecorator("custom", order: 5));
