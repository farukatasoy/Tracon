...
namespace AgentPrism.Core.UnitTests.Webhooks;
...
/// entry named <c>X-AgentPrism-Signature</c> used to add a SECOND signature
...
{
    [Theory]
    [InlineData("X-AgentPrism-Signature")]
    [InlineData("x-agentprism-signature")]
    [InlineData("X-AgentPrism-Timestamp")]
    [InlineData("X-AgentPrism-Event")]
    [InlineData("X-AgentPrism-Delivery")]
    ...
    [InlineData("X-AgentPrism-Custom")]
    [InlineData(null)]
    [InlineData("")]
    ...
    public async Task Extra_header_carrying_a_reserved_name_is_not_sent()
    {
        ...
        {
            ["X-AgentPrism-Signature"] = "attacker-supplied",
            ["X-Tenant"] = "acme",
        });
    ...
    public async Task Out_of_prefix_secret_key_drops_the_delivery_instead_of_throwing()
    {
        ...
        var handler = new WebhookDeliveryJobHandler(
            store,
            client,
            new StaticOptionsMonitor<AgentPrismWebhookOptions>(options),
