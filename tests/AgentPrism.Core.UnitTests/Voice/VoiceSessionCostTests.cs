...
public sealed class VoiceSessionCostTests
{
    ...
    {
        var pricing = new VoiceDurationPricing(Options(new AgentPrismOptions()));
    ...
    {
        // AgentPrism does not see a live session's media. When the provider reports
    ...
    private static AgentPrismOptions WithLivePrice(decimal perMinute)
    {
    ...
    private static StaticOptionsMonitor Options(AgentPrismOptions options) => new(options);

    private sealed class StaticOptionsMonitor(AgentPrismOptions value) : IOptionsMonitor<AgentPrismOptions>
    {
        public AgentPrismOptions CurrentValue => value;

        public AgentPrismOptions Get(string? name) => value;

        public IDisposable? OnChange(Action<AgentPrismOptions, string?> listener) => null;
    }
}
