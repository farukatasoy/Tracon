...
{
    private const string Prefix = "AgentPrism:McpSecrets:";
    ...
    public void Prefix_comparison_is_case_sensitive()
        => Should.Throw<AgentPrismException>(() =>
            ConfigurationKeyGuard.RequirePrefix("agentprism:mcpsecrets:Token", Prefix, "authorizationConfigurationKey"));
    ...
    public void Empty_allowed_prefix_is_a_programming_error(string prefix)
        => Should.Throw<ArgumentException>(() =>
            ConfigurationKeyGuard.RequirePrefix("AgentPrism:McpSecrets:Token", prefix, "field"));
    ...
    public void Shipped_default_prefixes_are_distinct_and_terminated()
    {
        var defaults = new[]
        {
            new AgentPrismInboundTriggerOptions().AllowedConfigurationPrefix,
            new AgentPrismTenantProviderOptions().AllowedConfigurationPrefix,
            new AgentPrismMcpSecurityOptions().AllowedConfigurationPrefix,
            new AgentPrismWebhookOptions().AllowedConfigurationPrefix,
        };
        ...
        {
            // Without the trailing separator, "AgentPrism:McpSecrets" would also
            // match "AgentPrism:McpSecretsSomethingElse:Key".
            prefix.ShouldEndWith(":");
            prefix.ShouldStartWith("AgentPrism:");
        }
    }
}
