...
public sealed class RunRecordingAgentOutcomeMatrixTests
{
    ...
    public async Task Failed_run_reaches_the_same_outcome_on_both_paths(bool streaming)
    {
        ...
        await Should.ThrowAsync<AgentPrismContentFilteredException>(() => Harness.RunAsync(agent, streaming));
    ...
    private sealed class Harness : IDisposable
    {
        ...
        public async Task<string?> RunSpanStatusTagAsync(Guid runId)
        {
            ...
            var root = trace.Spans.Single(
                span => string.Equals(span.Name, AgentPrismDiagnostics.RunActivityName, StringComparison.Ordinal));

            return root.Attributes.GetValueOrDefault(AgentPrismDiagnostics.Tags.Status);
        }
