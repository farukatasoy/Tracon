...
public sealed class RunBudgetAccountingTests
{
    ...
    public async Task Two_turn_tool_loop_is_recorded_exactly_once_no_cap()
    {
        ...
        await agent.RunAsync("go", options: new AgentPrismRunOptions { Budget = budget });
    ...
    public async Task Third_turn_is_cut_off_before_it_reaches_the_provider()
    {
        ...
        var exception = await Should.ThrowAsync<AgentPrismRunBudgetExceededException>(
            () => agent.RunAsync("go", options: new AgentPrismRunOptions { Budget = budget }));

        exception.ErrorType.ShouldBe(AgentPrismRunBudgetExceededException.RunBudgetExceededErrorType);
        ...
        run.Error!.Type.ShouldBe(AgentPrismRunBudgetExceededException.RunBudgetExceededErrorType);
    ...
    public async Task Streaming_run_is_cut_off_at_the_same_turn_boundary()
    {
        ...
        var exception = await Should.ThrowAsync<AgentPrismRunBudgetExceededException>(async () =>
        {
            await foreach (var _ in agent.RunStreamingAsync("go", options: new AgentPrismRunOptions { Budget = budget }))
            {
    ...
    public async Task Cost_cap_cuts_the_run_off_once_the_resolver_reports_enough_spend()
    {
        ...
        await Should.ThrowAsync<AgentPrismRunBudgetExceededException>(
            () => agent.RunAsync("go", options: new AgentPrismRunOptions { Budget = budget }));
    ...
    public async Task Two_different_agents_sharing_one_tree_budget_are_cut_off_by_each_others_spend()
    {
        ...
        // through the ambient AgentPrismRunContext actually see and are cut
