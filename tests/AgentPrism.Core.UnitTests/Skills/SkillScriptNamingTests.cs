...
/// <c>/etc/cron.d/agentprism</c> was written outside the scratch directory,
...
{
    [Theory]
    [InlineData("/etc/cron.d/agentprism")]
    [InlineData("../../escape")]
    [InlineData("..")]
    [InlineData(".")]
    ...
    public void A_name_that_carries_a_path_is_rejected(string scriptName)
    {
        ...
        Should.Throw<AgentPrismException>(() => SkillScriptNaming.RequireSafeFileName(scriptName));
    }
    ...
    public void A_rejected_name_never_leaves_the_directory_it_is_combined_with()
    {
        ...
        var scratch = Path.Combine(Path.GetTempPath(), "agentprism-scratch");
