...
public sealed class ToolWrapperChainTests
{
    ...
    private static AIFunctionDeclaration Compose(
        AgentPrismToolRegistration registration,
    ...
    {
        var codeRegistration = new AgentPrismToolRegistration(TestData.Tool("code_tool"));
        var mcpRegistration = new AgentPrismToolRegistration(TestData.Tool("mcp_tool"), source: "github-mcp");
    ...
    {
        var codeRegistration = new AgentPrismToolRegistration(TestData.Tool("code_tool"), requiresApproval: true);
        var mcpRegistration = new AgentPrismToolRegistration(TestData.Tool("mcp_tool"), requiresApproval: true, source: "github-mcp");
    ...
    {
        var registration = new AgentPrismToolRegistration(TestData.Tool("plain_tool"));
    ...
    {
        var registration = new AgentPrismToolRegistration(TestData.Tool("validated_tool"));
    ...
    public void A_client_side_declaration_passes_through_unwrapped()
    {
        ...
        var registration = new AgentPrismToolRegistration(declaration);
    ...
    public void A_client_side_declaration_requiring_approval_throws()
    {
        var schema = System.Text.Json.JsonSerializer.Deserialize<System.Text.Json.JsonElement>(
            """{"type":"object","properties":{}}""");
        var declaration = AIFunctionFactory.CreateDeclaration("client_tool", "desc", schema, returnJsonSchema: null);
        var registration = new AgentPrismToolRegistration(declaration, requiresApproval: true);
