...
/// <summary>
/// Phase 133.2: the lane tag is the one value AgentPrism does not control, so
...
public sealed class JobLaneCardinalityTests
{
    ...
    {
        // AgentPrismMetrics can be constructed with no options at all; the
        // guard must not fall back to "unbounded".
        using var factory = new TestMeterFactory();
        using var collector = new LaneCollector(factory.Meter);
        using var metrics = new AgentPrismMetrics(factory);
    ...
    /// <summary>Records job measurements against a real <see cref="AgentPrismMetrics"/> and reads back the lane tags.</summary>
    private sealed class MetricsHarness : IDisposable
    {
        ...
        public MetricsHarness(int? maxLanes)
        {
            ...
            _metrics = new AgentPrismMetrics(_factory, new StaticOptionsMonitor<AgentPrismOptions>(options));
        }
    ...
    /// <summary>Captures the lane tag of every <c>agentprism.job.executions</c> measurement, in order.</summary>
        ...
        public LaneCollector(Meter meter)
        {
            _listener.InstrumentPublished = (instrument, listener) =>
            {
                if (ReferenceEquals(instrument.Meter, meter)
                    && string.Equals(instrument.Name, AgentPrismDiagnostics.JobCounterName, StringComparison.Ordinal))
                {
            ...
            _listener.SetMeasurementEventCallback<long>((_, _, tags, _) =>
            {
                ...
                {
                    if (string.Equals(tag.Key, AgentPrismDiagnostics.Tags.Lane, StringComparison.Ordinal)
