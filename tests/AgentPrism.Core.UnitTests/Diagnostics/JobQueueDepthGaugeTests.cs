...
/// <summary>
/// Phase 133.4: the <c>agentprism.job.queue.depth</c> gauge is off by default,
...
public sealed class JobQueueDepthGaugeTests
{
    ...
    public async Task The_gauge_carries_no_tenant_tag()
    {
        ...
        single.Tags.ShouldNotContainKey(AgentPrismDiagnostics.Tags.TenantId);
    ...
    public void A_failing_depth_query_is_swallowed_and_the_gauge_stays_readable()
    {
        ...
        Should.NotThrow(() => collector.Trigger(AgentPrismDiagnostics.JobQueueDepthGaugeName))
            .ShouldBeEmpty();
    }
    ...
    public async Task The_gauge_applies_the_same_lane_cardinality_guard_as_the_counter()
    {
        ...
        var options = new StaticOptionsMonitor<AgentPrismOptions>(new AgentPrismOptions
        {
    ...
    private static long Value(
        ...
        JobStatus status)
        => measurements
            .Where(m => Equals(m.Tags.GetValueOrDefault(AgentPrismDiagnostics.Tags.Lane), lane)
                && Equals(m.Tags.GetValueOrDefault(AgentPrismDiagnostics.Tags.JobStatus), status.ToString()))
    ...
    private sealed class CountingJobStore : DelegatingJobStore
    {
            ...
            CancellationToken cancellationToken = default)
        {
            ...
            if (FailNextRefresh)
            {
                throw new AgentPrismException("simulated store outage");
            }
