...
/// <see cref="AgentPrismMetrics"/>.
/// </summary>
/// <remarks>
/// 🚨 This is the phase's silent-failure risk. Adding an OPTIONAL
/// <c>AgentPrismMetrics?</c> parameter to <c>JobWorkerBackgroundService</c>
...
public sealed class JobMetricsRegistrationTests
{
    ...
    public void The_worker_resolved_via_DI_actually_receives_the_metrics_instance()
    {
        ...
        var metrics = typeof(JobWorkerBackgroundService)
            ...
            .Where(field => field.FieldType == typeof(AgentPrismMetrics))
            .Select(field => field.GetValue(worker))
            .SingleOrDefault();

        metrics.ShouldNotBeNull("the worker was resolved with a null AgentPrismMetrics; job metrics would be silently dropped");
        metrics.ShouldBeSameAs(provider.GetRequiredService<AgentPrismMetrics>());
    }
    ...
    public void The_metrics_instance_resolved_via_DI_sees_the_configured_lane_limit()
    {
        var configuration = new ConfigurationBuilder()
            ...
            {
                ["AgentPrism:Observability:MaxJobLaneCardinality"] = "7",
            })
            .Build();
        ...
        var metrics = provider.GetRequiredService<AgentPrismMetrics>();

        var field = typeof(AgentPrismMetrics).GetField("_options", BindingFlags.NonPublic | BindingFlags.Instance)
            ?? throw new InvalidOperationException("AgentPrismMetrics._options not found — did the field name change?");

        var monitor = (IOptionsMonitor<AgentPrismOptions>?)field.GetValue(metrics);
    ...
    [Theory]
    [InlineData("AgentPrism:Observability:EnableJobQueueDepthGauge", "true")]
    [InlineData("AgentPrism:Observability:JobQueueDepthRefreshInterval", "00:01:15")]
    [InlineData("AgentPrism:Observability:MaxJobLaneCardinality", "9")]
    public void Every_new_observability_setting_is_actually_bound(string key, string value)
    {
        ...
        var observability = provider.GetRequiredService<IOptionsMonitor<AgentPrismOptions>>().CurrentValue.Observability;
        ...
        var actual = key.Split(':')[^1] switch
        {
            ...
            nameof(AgentPrismObservabilityOptions.JobQueueDepthRefreshInterval) =>
