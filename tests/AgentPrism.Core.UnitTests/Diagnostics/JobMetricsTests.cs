...
/// <summary>
/// Phase 133: what <c>agentprism.job.executions</c> and
/// <c>agentprism.job.duration</c> count, and what they deliberately do not.
/// </summary>
public sealed class JobMetricsTests
{
    ...
    public async Task The_duration_histogram_carries_no_tenant_tag()
    {
        ...
        measurement.Tags.ShouldNotContainKey(AgentPrismDiagnostics.Tags.TenantId);
        measurement.Tags[AgentPrismDiagnostics.Tags.Lane].ShouldBe(JobLanes.Default);
    }
    ...
    public async Task A_release_for_retry_is_not_counted_only_the_final_failure_is()
    {
        ...
        var measurement = harness.Counter.ShouldHaveSingleItem();
        measurement.Value.ShouldBe(1);
        measurement.Tags[AgentPrismDiagnostics.Tags.JobStatus].ShouldBe(nameof(JobStatus.Failed));
    ...
    public async Task A_job_with_no_registered_handler_is_counted_as_failed()
    {
        ...
        harness.Counter.ShouldHaveSingleItem()
            .Tags[AgentPrismDiagnostics.Tags.JobStatus].ShouldBe(nameof(JobStatus.Failed));
    }
    ...
    public async Task A_job_cancelled_mid_flight_is_counted_as_cancelled()
    {
        ...
        harness.Counter.ShouldHaveSingleItem()
            .Tags[AgentPrismDiagnostics.Tags.JobStatus].ShouldBe(nameof(JobStatus.Cancelled));
    }
    ...
    public async Task A_throwing_metric_listener_does_not_stop_the_job_from_completing()
    {
        ...
        using var worker = new JobWorkerBackgroundService(
            ...
            metrics: new AgentPrismMetrics(factory));
    ...
    private sealed class WorkerHarness : IDisposable
    {
            ...
            ManualTimeProvider? clock = null)
        {
            ...
            var schedulingOptions = new AgentPrismSchedulingOptions
            {
                MaxConcurrentJobs = 1,
                PollInterval = TimeSpan.FromMilliseconds(5),
            ...
            _worker = new JobWorkerBackgroundService(
                ...
                timeProvider: clock,
                logger: NullLogger<JobWorkerBackgroundService>.Instance,
                metrics: new AgentPrismMetrics(_meterFactory));
        }

        public InMemoryJobStore Jobs { get; }

        public List<Measured> Counter => _collector.Named(AgentPrismDiagnostics.JobCounterName);

        public List<Measured> Histogram => _collector.Named(AgentPrismDiagnostics.JobDurationName);
    ...
    private sealed class NotDrainingState : IAgentPrismDrainState
    {
