...
public sealed class InboundTriggerSecretResolverTests
{
    ...
    public void Name_outside_the_allowed_prefix_is_rejected()
    {
        ...
        exception.Message.ShouldContain("AgentPrism:TriggerSecrets:");
    }
    ...
    public void Name_under_the_prefix_is_accepted()
        => Should.NotThrow(() => CreateResolver().ValidatePrefix("AgentPrism:TriggerSecrets:Slack"));
    ...
    public void A_custom_prefix_is_honored()
    {
        var resolver = CreateResolver(
            ...
            options: new AgentPrismInboundTriggerOptions { AllowedConfigurationPrefix = "Custom:Prefix:" });

        Should.Throw<AgentPrismException>(() => resolver.ValidatePrefix("AgentPrism:TriggerSecrets:Slack"));
    ...
    {
        var resolver = CreateResolver(BuildConfiguration(("AgentPrism:TriggerSecrets:Slack", "whsec_test")));

        resolver.Resolve(Trigger("AgentPrism:TriggerSecrets:Slack")).ShouldBe("whsec_test");
    }

    [Fact]
    public void Resolve_returns_null_when_the_key_has_no_value()
        => CreateResolver().Resolve(Trigger("AgentPrism:TriggerSecrets:Slack")).ShouldBeNull();
    ...
    public void Resolve_rejects_an_out_of_prefix_trigger_even_if_it_was_saved_before_the_prefix_changed()
    {
        ...
        Should.Throw<AgentPrismException>(() => resolver.Resolve(Trigger("Legacy:Key")));
    }
    ...
    public void No_configuration_registered_resolves_to_null()
    {
        ...
        resolver.Resolve(Trigger("AgentPrism:TriggerSecrets:Slack")).ShouldBeNull();
    }
