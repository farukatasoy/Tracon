...
public sealed class DocumentChannelTests
{
    ...
    public async Task Document_reaches_the_model_as_its_own_message_wrapped_in_a_delimiter()
    {
        ...
        documentText.ShouldStartWith("-----BEGIN AGENTPRISM DOCUMENT-----");
    ...
    public async Task A_delimiter_inside_the_document_content_is_escaped_and_cannot_forge_the_boundary()
    {
        ...
        var firstIndex = documentText.IndexOf("-----END AGENTPRISM DOCUMENT-----", StringComparison.Ordinal);
        var lastIndex = documentText.LastIndexOf("-----END AGENTPRISM DOCUMENT-----", StringComparison.Ordinal);

        firstIndex.ShouldBe(lastIndex);
        documentText.ShouldEndWith("-----END AGENTPRISM DOCUMENT-----");
    }
    ...
    private static Task<Infrastructure.AgentPrismTestHost> StartAsync(FakeModelProvider provider)
        => Infrastructure.AgentPrismTestHost.StartAsync(configureAgentPrism: builder => builder
    ...
    private static async Task<HttpResponseMessage> PostBufferedAsync(Infrastructure.AgentPrismTestHost host, AgentRunRequest body)
    {
