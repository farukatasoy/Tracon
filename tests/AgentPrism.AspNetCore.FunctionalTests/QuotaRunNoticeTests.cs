...
public sealed class QuotaRunNoticeTests
{
    ...
    public async Task Enabled_notice_arrives_as_a_custom_frame_before_done()
    {
        ...
        payload.ShouldContain("\"customType\":\"agentprism.quota.threshold\"", Case.Sensitive);
    ...
    private static Task<AgentPrismTestHost> StartAsync(bool publishToRunStream, IReadOnlyList<int>? thresholdPercents = null)
        => AgentPrismTestHost.StartAsync(
            configureAgentPrism: static builder => builder.AddAgent(TestData.Definition()),
            configureServices: services => services.Configure<AgentPrismQuotaOptions>(options =>
            {
    ...
    private static Task<HttpResponseMessage> PostRunAsync(AgentPrismTestHost host)
    ...
    private static async Task CreateQuotaAsync(AgentPrismTestHost host, long maxRuns)
    {
    ...
    private static async Task<Guid> LatestRunIdAsync(AgentPrismTestHost host)
    {
