...
public sealed class RequiredBindingStartupTests
{
        ...
        string builtInDefault)
    {
        var exception = await Should.ThrowAsync<InvalidOperationException>(
            async () => await AgentPrismTestHost.StartAsync(
                configureAgentPrism: builder => Declare(builder, contract)));
    ...
    {
        // The two points AgentPrism registers NOTHING for. A type comparison
    ...
    [Fact]
    public async Task A_binding_registered_before_AddAgentPrism_starts_the_host()
    {
        await using var host = await AgentPrismTestHost.StartAsync(
            configureServices: static services =>
                services.AddSingleton<IRunAuthorizationHandler, StubRunAuthorization>(),
            configureAgentPrism: static builder => builder.RequireCustomBinding<IRunAuthorizationHandler>());
    ...
    [Fact]
    public async Task A_TryAdd_registration_made_after_AddAgentPrism_still_stops_the_host()
    {
        // The consumer's reported scenario: a module that registers its handler
        // with TryAdd runs after AddAgentPrism, the built-in default is already
    ...
    [Fact]
    public async Task An_Add_registration_made_after_AddAgentPrism_wins_and_the_host_starts()
    {
        // Measured, and the opposite of what the phase plan assumed: the
        // container resolves the LAST descriptor, so a plain Add after
        // AddAgentPrism does bind the consumer's type. The gate reports what
    ...
    {
        // No WebApplication and no MapAgentPrism: binding an extension point is
    ...
    public async Task A_contract_that_is_not_an_extension_point_stops_the_host()
    {
        var exception = await Should.ThrowAsync<InvalidOperationException>(
            async () => await AgentPrismTestHost.StartAsync(
                configureAgentPrism: static builder => builder.RequireCustomBinding<IRunStore>()));

        exception.Message.ShouldContain("not an AgentPrism extension point");
    ...
    private static void Declare(IAgentPrismBuilder builder, string contract)
    {
    ...
    private static IHost BuildHostWithoutHttp(
        ...
        Action<IAgentPrismBuilder> configureAgentPrism,
        bool validate)
    {
        ...
        agentPrism.AddModelProvider(new AgentPrism.Testing.FakeModelProvider("echo").EchoesUserMessage());
        configureAgentPrism(agentPrism);
