...
public sealed class LiveVoiceLifecycleTests
{
    ...
    {
        // 🚨 The append is the one path on which AgentPrism's own text reaches the
    ...
    public async Task A_delegation_over_the_ceiling_opens_NO_run_and_is_told_the_session_is_busy()
    {
        ...
        // how many agent runs AgentPrism starts, which is a denial-of-service axis
    ...
    public async Task Closing_a_session_cancels_a_delegation_that_is_still_running()
    {
        ...
        using var close = await host.Client.DeleteAsync(
            $"/agentprism/api/voice/live/sessions/{voiceSessionId:D}",
            cancellation.Token);
    ...
    public async Task A_session_nothing_ever_connected_to_stays_pending()
    {
        ...
        var body = await AgentPrismTestHost.ReadJsonAsync(status);
        ...
        string sessionId)
    {
        using var response = await host.Client.PostAsJsonAsync(
            "/agentprism/api/voice/live/sessions",
        ...
        return (await AgentPrismTestHost.ReadJsonAsync(response)).GetProperty("voiceSessionId").GetGuid();
    }

    private static async Task CloseAsync(AgentPrismTestHost host, Guid voiceSessionId)
    {
        using var close = await host.Client.DeleteAsync(
            $"/agentprism/api/voice/live/sessions/{voiceSessionId:D}",
            TestContext.Current.CancellationToken);
    ...
    private static async Task<IReadOnlyList<string>> HistoryOfAsync(AgentPrismTestHost host, string sessionId)
    {
