...
public sealed class RunContinuationTests
{
    ...
    public async Task MaxAttempts_stops_a_second_continuation_in_the_same_chain()
    {
        ...
        var ancestorId = AgentPrismId.NewId();
    ...
    private static async Task<AgentPrismTestHost> StartHostAsync(Action<AgentPrismRunContinuationOptions> configureContinuation)
    ...
    private static async ValueTask<Guid> SeedCrashedRunAsync(AgentPrismTestHost host)
    {
    ...
    private static async Task<Guid> WaitForContinuationAsync(AgentPrismTestHost host, Guid sourceRunId)
    {
        var runs = host.Services.GetRequiredService<IRunStore>();
...
internal static class ContinuationProbeTools
{
    ...
    [AgentPrismTool("delete_order", "Deletes an order.", Effect = ToolEffect.Destructive)]
    ...
    /// </summary>
    [AgentPrismTool("idempotent_delete_order", "Deletes an order (idempotent).", Effect = ToolEffect.Destructive, SafeToRepeat = true)]
