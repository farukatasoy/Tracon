...
public sealed class SessionConflictEndpointTests
{
    ...
    private static Task<AgentPrismTestHost> StartAsync()
        => AgentPrismTestHost.StartAsync(static builder =>
        {
    ...
    /// </summary>
    private static async Task<HttpResponseMessage> PostRunAsync(AgentPrismTestHost host)
    {
    ...
    private static async Task<HttpResponseMessage> PostResponsesAsync(AgentPrismTestHost host)
