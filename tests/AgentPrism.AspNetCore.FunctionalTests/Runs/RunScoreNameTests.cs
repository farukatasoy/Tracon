...
namespace AgentPrism.AspNetCore.FunctionalTests.Runs;
...
public sealed class RunScoreNameTests
{
    ...
    private static async Task PostAsync(AgentPrismTestHost host, Guid runId, object body)
    {
        using var response = await host.Client.PostAsJsonAsync(FeedbackUri(runId), body);
    ...
    private static async Task<System.Text.Json.JsonElement> ListAsync(AgentPrismTestHost host, Guid runId)
    {
