...
public sealed class HarnessLoopTests
{
    ...
    [Fact]
    public async Task An_unreachable_criterion_stops_at_the_AgentPrism_ceiling_instead_of_running_on()
    {
        // No MaxIterations, and a marker the model never writes. Without an
        // AgentPrism-side ceiling this is an unbounded model bill.
        var loop = new LoopSettings
        {
    ...
    public async Task A_streaming_loop_keeps_one_unbroken_span_tree()
    {
        ...
        var root = trace.Spans.SingleOrDefault(static span =>
            string.Equals(span.Name, AgentPrismDiagnostics.RunActivityName, StringComparison.Ordinal))
            .ShouldNotBeNull();
    ...
    public async Task An_exhausted_tree_budget_stops_the_loop_rather_than_letting_it_open_another_iteration()
    {
        ...
        var options = new AgentPrismRunOptions { Budget = new AgentRunBudget { MaxTotalTokens = 1 } };

        await Should.ThrowAsync<AgentPrismRunBudgetExceededException>(async () => await agent.RunAsync(
    ...
    private static async Task<Microsoft.Agents.AI.AgentResponse> RunAsync(AgentPrismTestHost host)
    {
        var agent = await ResolveAsync(host);
        var session = await agent.CreateSessionAsync();
    ...
    private static async Task<AIAgent> ResolveAsync(AgentPrismTestHost host)
    {
    ...
    private static async Task<RunRecord> RootRunAsync(AgentPrismTestHost host)
    {
    ...
    private static async Task<List<RunEvent>> ReadRootEventsAsync(AgentPrismTestHost host)
    {
        var runs = host.Services.GetRequiredService<IRunStore>();
        var root = await RootRunAsync(host);
