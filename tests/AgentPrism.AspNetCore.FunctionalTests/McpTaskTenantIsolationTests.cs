...
/// <summary>
/// Phase 117: AgentPrism's <c>IMcpTaskStore</c> implementation carries no tenant
...
public sealed class McpTaskTenantIsolationTests
{
    ...
    public async Task Another_tenants_task_id_is_not_found()
    {
        ...
        await using var clientA = await McpTaskTestClient.ConnectAsync(host.Client, "/agentprism/mcp", tenantHeader: "tenant-a");

        var outcome = await clientA.CallToolAsTaskAsync(
            new CallToolRequestParams { Name = "agentprism_db-agent", Arguments = new Dictionary<string, JsonElement>(StringComparer.Ordinal) });
        ...
        await using var clientB = await McpTaskTestClient.ConnectAsync(host.Client, "/agentprism/mcp", tenantHeader: "tenant-b");
    ...
    public async Task Another_tenant_can_interfere_with_cancellation_but_never_reads_the_result()
    {
        ...
        // IMcpTaskStore.SetCancelledAsync's tenant-aware check. AgentPrism has
        ...
        // cancelled. What AgentPrism's own boundary (IRunStore's ambient-tenant
