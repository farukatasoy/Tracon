...
public sealed class AgentParameterEndpointTests
{
    ...
    private static readonly Uri EstimateUri = new($"/agentprism/api/agents/{AgentName}/estimate", UriKind.Relative);
    ...
    public async Task Run_missing_a_required_parameter_does_not_start_and_names_it()
    {
        ...
        var body = await Infrastructure.AgentPrismTestHost.ReadJsonAsync(response);
    ...
    {
        var provider = Provider();
        await using var host = await Infrastructure.AgentPrismTestHost.StartAsync(
            configureAgentPrism: builder => builder.AddModelProvider(provider),
    ...
    public async Task An_agent_with_no_parameter_schema_is_unaffected()
    {
        ...
        using var response = await host.Client.PostAsJsonAsync(
            new Uri("/agentprism/api/agents/plain-agent/run", UriKind.Relative),
    ...
    private static async Task CreateParameterizedAgentAsync(Infrastructure.AgentPrismTestHost host)
    {
        using var response = await host.Client.PostAsJsonAsync(
            Agents,
            new AgentDefinitionRequest
    ...
    private static Task<Infrastructure.AgentPrismTestHost> StartAsync(FakeModelProvider provider)
        => Infrastructure.AgentPrismTestHost.StartAsync(configureAgentPrism: builder => builder.AddModelProvider(provider));

    /// <summary>Enters the non-streaming branch so a 400/200 status code is directly observable.</summary>
    private static async Task<HttpResponseMessage> PostBufferedAsync(Infrastructure.AgentPrismTestHost host, Uri uri, AgentRunRequest body)
    {
