...
public sealed class RunAttributionEndpointTests
{
    ...
    public async Task The_statistics_endpoint_breaks_down_by_user()
    {
        ...
        var stats = await host.Client.GetFromJsonAsync<RunStatistics>(
            new Uri("/agentprism/api/stats", UriKind.Relative));
    ...
    private static async Task<int> CountAsync(AgentPrismTestHost host, string query)
    {
    ...
    private static Task<AgentPrismTestHost> StartAsync(IRunAttributionContext? attribution)
