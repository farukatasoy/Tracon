...
/// through <c>ModelProviderRegistry</c>, not <see cref="AgentPrismResponseCachingChatClient"/>
...
public sealed class ResponseCachePipelineTests
{
    ...
    private static async Task<string> WaitForStatusAsync(AgentPrismTestHost host, Guid runId, string expected, string? tenant = null)
    {
    ...
    public async Task Second_identical_run_is_a_cache_hit_and_the_tool_still_runs()
    {
        ...
        var firstRunId = (await AgentPrismTestHost.ReadJsonAsync(first)).GetProperty("runId").GetGuid();
        ...
        var secondRunId = (await AgentPrismTestHost.ReadJsonAsync(second)).GetProperty("runId").GetGuid();
    ...
    public async Task Second_identical_run_shows_null_usage_through_the_run_record()
    {
        ...
        // locks the behavior at the AgentPrismResponseCachingChatClient level,
        ...
        using var firstRecord = await host.Client.GetAsync(new Uri($"/agentprism/api/runs/{firstRunId}", UriKind.Relative));
        var firstUsage = (await AgentPrismTestHost.ReadJsonAsync(firstRecord)).GetProperty("usage");
        ...
        using var secondRecord = await host.Client.GetAsync(new Uri($"/agentprism/api/runs/{secondRunId}", UriKind.Relative));
        var secondUsage = (await AgentPrismTestHost.ReadJsonAsync(secondRecord)).GetProperty("usage");
    ...
    public async Task Enabling_response_cache_without_a_registered_store_fails_validation_naming_the_missing_registration()
    {
        ...
        // (MultiProviderTests) exercises - and catches AgentPrismException
        ...
        using var response = await host.Client.PostAsJsonAsync(
            new Uri("/agentprism/api/agents/validate", UriKind.Relative),
            new
            {
    ...
    public async Task Response_cache_never_leaks_across_tenants()
    {
        ...
        async Task<Guid> RunAsAsync(string tenant, string sessionId)
        {
            ...
            return (await AgentPrismTestHost.ReadJsonAsync(response)).GetProperty("runId").GetGuid();
        }
