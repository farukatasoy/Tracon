...
public sealed class RunAuthorizationEndpointTests
{
    ...
    public async Task Bare_setup_reports_the_run_authorization_handler_as_built_in_default()
    {
        ...
        var body = await AgentPrismTestHost.ReadJsonAsync(
            await host.Client.GetAsync(new Uri("/agentprism/api/diagnostics", UriKind.Relative)));
    ...
    public async Task Denied_session_read_returns_404_not_403()
    {
        ...
        using var response = await host.Client.GetAsync(
            new Uri($"/agentprism/api/sessions/{sessionId}", UriKind.Relative));

        response.StatusCode.ShouldBe(HttpStatusCode.NotFound);

        var deniedProblem = await AgentPrismTestHost.ReadJsonAsync(response);

        using var missing = await host.Client.GetAsync(
            new Uri("/agentprism/api/sessions/does-not-exist-at-all", UriKind.Relative));
        var missingProblem = await AgentPrismTestHost.ReadJsonAsync(missing);
    ...
    public async Task Denied_session_branch_returns_404()
    {
        ...
        using var response = await host.Client.PostAsJsonAsync(
            new Uri($"/agentprism/api/sessions/{sessionId}/branch", UriKind.Relative),
    ...
    public async Task Workflow_run_is_covered()
    {
        ...
        using var response = await host.Client.PostAsJsonAsync(
            "/agentprism/api/workflows/chain/run", new WorkflowRunHttpRequest { Message = "hello" });
    ...
    {
        const string secretKey = "AgentPrism:TriggerSecrets:RunAuthTest";
        const string secret = "whsec_test";
        const string body = """{"event":{"text":"hello"}}""";

        var handler = new ConfigurableRunAuthorizationHandler { OnRun = static _ => RunAuthorizationResult.Deny("no") };

        await using var host = await AgentPrismTestHost.StartAsync(configureServices: services =>
        {
        ...
        using var request = new HttpRequestMessage(
            HttpMethod.Post, "/agentprism/api/triggers/default/hook")
        {
    ...
    public async Task Allowed_user_id_reaches_the_tool_via_scope()
    {
        ...
        await using var host = await AgentPrismTestHost.StartAsync(
            builder => builder
                ...
                .AddTool((Func<string>)(() =>
                {
                    seenByTool = AgentPrismRunContext.Current?.UserId;
        ...
        using var response = await host.Client.PostAsJsonAsync(
            new Uri("/agentprism/api/agents/scope-agent/run", UriKind.Relative),
    ...
    /// </summary>
    private static async Task<(AgentPrismTestHost Host, ConfigurableRunAuthorizationHandler Handler, string SessionId)> StartWithSessionAsync()
    {
        var handler = new ConfigurableRunAuthorizationHandler();
