...
/// <summary>
/// <see cref="AgentPrismAgentGraphOptions.MaxDuration"/> (Phase 128, T-7): the
...
public sealed class RunDeadlineTests
{
    ...
    private static Uri RunUri(Guid runId) => new($"/agentprism/api/runs/{runId}", UriKind.Relative);
    ...
    public async Task Deadline_is_enforced_on_the_queued_durable_run_path_too()
    {
        ...
        await using var host = await AgentPrismTestHost.StartAsync(
            ...
            configureServices: services =>
            {
                services.Configure<AgentPrismOptions>(o => o.AgentGraph.MaxDuration = TimeSpan.FromMilliseconds(200));
    ...
    {
        // The cutoff (AgentPrismRunBudgetExceededException -> QuotaExceeded)
