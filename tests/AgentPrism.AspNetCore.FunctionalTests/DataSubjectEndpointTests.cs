...
{
    private static readonly Uri ExportUri = new("/agentprism/api/data-subjects/subject-1/export", UriKind.Relative);
    private static readonly Uri EraseUri = new("/agentprism/api/data-subjects/subject-1", UriKind.Relative);
    ...
    public async Task Unknown_subjectId_resolves_to_an_empty_scope_and_erases_nothing()
    {
        ...
        using var response = await host.Client.DeleteAsync(
            new Uri("/agentprism/api/data-subjects/unknown-subject?dryRun=false", UriKind.Relative));
    ...
    public async Task Erase_with_dryRun_false_actually_erases()
    {
        ...
        using var response = await host.Client.DeleteAsync(
            new Uri("/agentprism/api/data-subjects/subject-1?dryRun=false", UriKind.Relative));
    ...
    public async Task Erase_writes_an_audit_entry_carrying_the_subject_id()
    {
        ...
        using var audit = await host.Client.GetAsync(
            new Uri("/agentprism/api/audit?entity=data-subject:subject-1", UriKind.Relative));
        var entries = await AgentPrismTestHost.ReadJsonAsync(audit);
