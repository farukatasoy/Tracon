...
/// in <c>AgentPrism.Core.UnitTests.Compilation.StructuredResponseValidatingAgentTests</c>.
/// </summary>
public sealed class StructuredResponseRepairEndpointTests
{
    ...
    public async Task The_trees_token_budget_still_applies_to_a_repair_turn_and_stops_it_from_looping()
    {
        ...
        await using var host = await StartAsync(
            ...
            configureServices: services =>
                services.Configure<AgentPrismOptions>(o => o.AgentGraph.MaxTotalTokens = 45));
    ...
    public async Task The_trees_deadline_still_applies_to_a_repair_turn_and_stops_it_from_looping()
    {
        ...
        await using var host = await AgentPrismTestHost.StartAsync(
            ...
            configureServices: services =>
            {
                ...
                services.Configure<AgentPrismStructuredResponseOptions>(options =>
                {
                    options.Enabled = true;
                    options.MaxRepairAttempts = 2;
                });
                services.Configure<AgentPrismOptions>(o => o.AgentGraph.MaxDuration = TimeSpan.FromSeconds(5));
            });
    ...
    public async Task A_repair_turn_also_goes_through_the_fallback_chain()
    {
        ...
        await using var host = await AgentPrismTestHost.StartAsync(
            ...
            configureServices: services => services.Configure<AgentPrismStructuredResponseOptions>(options =>
            {
                options.Enabled = true;
    ...
    public async Task Cancellation_during_a_repair_turn_cancels_the_run_not_a_validation_failure()
    {
        ...
        using var cancelled = await host.Client.PostAsync(
            new Uri($"/agentprism/api/runs/{runId}/cancel", UriKind.Relative),
            content: null,
    ...
    /// </summary>
    private static async Task<List<SseFrame>> SseFramesAsync(AgentPrismTestHost host, Guid runId)
    {
    ...
    private static async Task<RunRecord> WaitForTerminalStatusAsync(AgentPrismTestHost host, Guid runId)
    {
        ...
        while (DateTime.UtcNow < deadline)
        {
            var run = await host.Client.GetFromJsonAsync<RunRecord>(
                new Uri($"/agentprism/api/runs/{runId}", UriKind.Relative), TestContext.Current.CancellationToken);
    ...
    /// </summary>
    private static async Task<HttpResponseMessage> PostBufferedAsync(AgentPrismTestHost host, AgentRunRequest? body = null)
    {
        using var request = new HttpRequestMessage(HttpMethod.Post, Run)
        {
    ...
    /// <summary>
    /// A model provider that always throws <see cref="AgentPrismProviderUnavailableException"/> —
    ...
    private sealed class AlwaysUnavailableModelProvider(string name) : IModelProvider
    {
        ...
        private sealed class Client(string providerName) : IChatClient
        {
            public Task<ChatResponse> GetResponseAsync(
                IEnumerable<ChatMessage> messages, ChatOptions? options = null, CancellationToken cancellationToken = default)
                => throw new AgentPrismProviderUnavailableException("simulated outage") { ProviderName = providerName };
