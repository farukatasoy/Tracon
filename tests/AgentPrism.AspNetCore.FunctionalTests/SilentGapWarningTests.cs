...
public sealed class SilentGapWarningTests
{
    ...
    public async Task Production_with_no_content_guard_warns_exactly_once()
    {
        ...
        matches[0].ShouldStartWith($"{LogLevel.Warning} {nameof(AgentPrism)}.{nameof(SilentGapWarningService)}");
    }
    ...
    public async Task A_registered_content_guard_stays_quiet_in_Production()
    {
        await using var host = await AgentPrismTestHost.StartAsync(
            configureAgentPrism: builder => builder.AddPatternContentGuard(),
    ...
    {
        await using var host = await AgentPrismTestHost.StartAsync(
            configureServices: services => services.Configure<AgentPrismRetentionOptions>(options => options.Enabled = true),
            environment: Environments.Production);
