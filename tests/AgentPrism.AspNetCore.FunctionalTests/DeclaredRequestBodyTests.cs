...
public sealed class DeclaredRequestBodyTests
{
    ...
    {
        ["POST /agentprism/api/runs/{runId}/cancel"] = "Action route: the path names the target; no body.",
        ["POST /agentprism/api/jobs/{id}/cancel"] = "Action route: the path names the target; no body.",
        ["POST /agentprism/api/runs/{runId}/judge"] = "Action route: the judges are registered; no body.",
        ["POST /agentprism/api/experiments/{name}/start"] = "Action route: a state transition; no body.",
        ["POST /agentprism/api/experiments/{name}/stop"] = "Action route: a state transition; no body.",
        ["POST /agentprism/api/stats/recalculate-costs"] = "Action route: recomputes for the whole tenant; no body.",
        ["POST /agentprism/api/webhooks/{name}/test"] = "Action route: sends a probe to the registered target; no body.",
        ["POST /agentprism/api/retention/run"] = "Action route: runs the configured policy; no body.",
        ["POST /agentprism/api/mcp-servers/{name}/oauth/start"] = "Action route: starts authorization; no body.",
        ["POST /agentprism/v1/conversations"] = "Reserves an identifier only; writes nothing and reads no body.",
        ...
        ["POST /agentprism/api/triggers/{tenantId}/{name}"] = "Schemaless raw payload; HMAC over the raw body.",
    };
    ...
    private static string OpenApiPath { get; } = Path.Combine(
        RepositoryRoot, "docs", "openapi", "agentprism.json");
