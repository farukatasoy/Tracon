...
public sealed class AgentSourceCustomOriginTests
{
    ...
    public async Task Detail_reports_Custom_origin_and_is_not_editable()
    {
        await using var host = await AgentPrismTestHost.StartAsync(
            configureAgentPrism: builder => builder.AddAgentSource(new StubCustomSource()));

        using var response = await host.Client.GetAsync(new Uri($"/agentprism/api/agents/{AgentName}", UriKind.Relative));
    ...
    public async Task Update_is_rejected_with_409_naming_the_source()
    {
        ...
        using var response = await host.Client.PutAsJsonAsync(
            new Uri($"/agentprism/api/agents/{AgentName}", UriKind.Relative),
    ...
    public async Task Delete_is_rejected_with_409()
    {
        ...
        using var response = await host.Client.DeleteAsync(new Uri($"/agentprism/api/agents/{AgentName}", UriKind.Relative));
