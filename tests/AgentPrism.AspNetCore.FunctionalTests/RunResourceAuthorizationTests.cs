...
public sealed class RunResourceAuthorizationTests
{
    ...
    public async Task Every_resource_endpoint_is_unchanged_when_no_handler_is_registered()
    {
        ...
        await AssertStatusAsync(host, HttpMethod.Get, "/agentprism/api/runs", HttpStatusCode.OK);
        await AssertStatusAsync(host, HttpMethod.Get, $"/agentprism/api/runs/{runId}", HttpStatusCode.OK);
        await AssertStatusAsync(host, HttpMethod.Get, $"/agentprism/api/runs/{runId}/tree", HttpStatusCode.OK);
        await AssertStatusAsync(host, HttpMethod.Get, $"/agentprism/api/runs/{runId}/events", HttpStatusCode.OK);
        await AssertStatusAsync(host, HttpMethod.Get, $"/agentprism/api/runs/{runId}/input", HttpStatusCode.OK);
        await AssertStatusAsync(host, HttpMethod.Get, $"/agentprism/api/runs/{runId}/trace", HttpStatusCode.OK);
        await AssertStatusAsync(host, HttpMethod.Get, $"/agentprism/api/runs/{runId}/tools", HttpStatusCode.OK);
        await AssertStatusAsync(host, HttpMethod.Get, $"/agentprism/api/runs/{runId}/feedback", HttpStatusCode.OK);
        await AssertStatusAsync(host, HttpMethod.Get, $"/agentprism/api/runs/{runId}/compare/{runId}", HttpStatusCode.OK);
        await AssertStatusAsync(host, HttpMethod.Get, "/agentprism/api/attachments", HttpStatusCode.OK);
        await AssertStatusAsync(host, HttpMethod.Get, $"/agentprism/api/attachments/{attachmentId}", HttpStatusCode.OK);
        await AssertStatusAsync(host, HttpMethod.Get, "/agentprism/api/approvals/pending", HttpStatusCode.OK);
        await AssertStatusAsync(host, HttpMethod.Get, $"/agentprism/api/approvals/{approvalId}", HttpStatusCode.OK);

        // A completed run cannot be canceled: 409, not 404 - proof the gate did
        // not short-circuit ahead of the status check.
        await AssertStatusAsync(host, HttpMethod.Post, $"/agentprism/api/runs/{runId}/cancel", HttpStatusCode.Conflict);
        ...
        await AssertStatusAsync(host, HttpMethod.Delete, $"/agentprism/api/attachments/{attachmentId}", HttpStatusCode.NoContent);
        ...
        using (var replayed = await host.Client.PostAsJsonAsync(
            new Uri($"/agentprism/api/runs/{runId}/replay", UriKind.Relative), new { toolMode = "NoTools" }))
        {
        ...
        var score = (await AgentPrismTestHost.ReadJsonAsync(
        ...
        await AssertStatusAsync(
            host, HttpMethod.Delete, $"/agentprism/api/runs/{runId}/feedback/{score}", HttpStatusCode.NoContent);
    }
    ...
    public async Task Denied_run_read_is_indistinguishable_from_a_missing_run()
    {
        ...
        await AssertLooksMissingAsync(host, HttpMethod.Get, $"/agentprism/api/runs/{runId}", runId);
    }
    ...
    public async Task Denied_run_tree_returns_404()
    {
        ...
        await AssertLooksMissingAsync(host, HttpMethod.Get, $"/agentprism/api/runs/{runId}/tree", runId);
    }
    ...
    public async Task Denied_run_input_returns_404()
    {
        ...
        await AssertLooksMissingAsync(host, HttpMethod.Get, $"/agentprism/api/runs/{runId}/input", runId);
    }
    ...
    public async Task Denied_run_trace_returns_404()
    {
        ...
        using var denied = await host.Client.GetAsync(
            new Uri($"/agentprism/api/runs/{runId}/trace", UriKind.Relative));
    ...
    public async Task Denied_run_tool_invocations_returns_404()
    {
        ...
        await AssertLooksMissingAsync(host, HttpMethod.Get, $"/agentprism/api/runs/{runId}/tools", runId);
    }
    ...
    public async Task Denied_run_feedback_read_returns_404()
    {
        ...
        await AssertLooksMissingAsync(host, HttpMethod.Get, $"/agentprism/api/runs/{runId}/feedback", runId);
    }
    ...
    public async Task Denied_run_list_returns_403_and_is_never_filtered()
    {
        ...
        using var response = await host.Client.GetAsync(new Uri("/agentprism/api/runs", UriKind.Relative));
    ...
    public async Task Denied_attachment_list_returns_403()
    {
        ...
        await AssertStatusAsync(host, HttpMethod.Get, "/agentprism/api/attachments", HttpStatusCode.Forbidden);
    }
    ...
    public async Task Denied_approval_list_returns_403()
    {
        ...
        await AssertStatusAsync(host, HttpMethod.Get, "/agentprism/api/approvals/pending", HttpStatusCode.Forbidden);
    }
    ...
    public async Task Compare_asks_the_gate_for_each_side_and_either_denial_refuses_it()
    {
        ...
        using var response = await host.Client.GetAsync(
            new Uri($"/agentprism/api/runs/{left}/compare/{right}", UriKind.Relative));
    ...
    public async Task Denied_feedback_delete_returns_404()
    {
        ...
        using var response = await host.Client.DeleteAsync(
            new Uri($"/agentprism/api/runs/{runId}/feedback/{saved.Id}", UriKind.Relative));
    ...
    public async Task Denied_attachment_download_returns_404()
    {
        ...
        using var missing = await host.Client.GetAsync(
            new Uri($"/agentprism/api/attachments/{Missing}", UriKind.Relative));
    ...
    public async Task Denied_approval_read_returns_404()
    {
        ...
        using var missing = await host.Client.GetAsync(
            new Uri($"/agentprism/api/approvals/{Missing}", UriKind.Relative));
    ...
    public async Task Denied_replay_returns_403_and_opens_no_run_row()
    {
        ...
        using var response = await host.Client.PostAsJsonAsync(
            new Uri($"/agentprism/api/runs/{runId}/replay", UriKind.Relative),
    ...
    public async Task Feedback_delete_refuses_a_score_that_belongs_to_a_different_run()
    {
        ...
        using var response = await host.Client.DeleteAsync(
            new Uri($"/agentprism/api/runs/{ownRun}/feedback/{othersScore.Id}", UriKind.Relative));
    ...
    public async Task Denied_judge_returns_404_and_writes_no_score()
    {
        ...
        using var response = await host.Client.PostAsync(
            new Uri($"/agentprism/api/runs/{runId}/judge", UriKind.Relative), content: null);
    ...
    public async Task Denied_workflow_resume_returns_403_and_opens_no_run_row()
    {
        ...
        using var response = await host.Client.PostAsJsonAsync(
            new Uri($"/agentprism/api/workflows/runs/{runId}/resume", UriKind.Relative),
    ...
    public async Task Denied_workflow_respond_returns_403()
    {
        ...
        using var response = await host.Client.PostAsJsonAsync(
            new Uri($"/agentprism/api/workflows/runs/{runId}/respond", UriKind.Relative),
    ...
    public async Task Denied_workflow_checkpoints_and_requests_return_404()
    {
        ...
        // reading the run.
        await AssertStatusAsync(
            host, HttpMethod.Get, $"/agentprism/api/workflows/runs/{runId}/checkpoints", HttpStatusCode.NotFound);
        await AssertStatusAsync(
            host, HttpMethod.Get, $"/agentprism/api/workflows/runs/{runId}/requests", HttpStatusCode.NotFound);
    }
    ...
    public async Task Workflow_continuation_is_unchanged_when_no_handler_is_registered()
    {
        ...
        await AssertStatusAsync(
            host, HttpMethod.Get, $"/agentprism/api/workflows/runs/{runId}/checkpoints", HttpStatusCode.OK);
        await AssertStatusAsync(
            host, HttpMethod.Get, $"/agentprism/api/workflows/runs/{runId}/requests", HttpStatusCode.OK);
    ...
    public async Task A_failing_run_store_does_not_turn_into_an_allow()
    {
        ...
        await AssertStatusAsync(host, HttpMethod.Get, $"/agentprism/api/runs/{Missing}/input", HttpStatusCode.NotFound);
        await AssertStatusAsync(host, HttpMethod.Get, $"/agentprism/api/runs/{Missing}/trace", HttpStatusCode.NotFound);
    ...
    public async Task Throwing_handler_denies_every_resource_fail_closed()
    {
        ...
        await AssertStatusAsync(host, HttpMethod.Get, $"/agentprism/api/runs/{runId}", HttpStatusCode.NotFound);
        await AssertStatusAsync(host, HttpMethod.Get, $"/agentprism/api/runs/{runId}/tools", HttpStatusCode.NotFound);
        await AssertStatusAsync(host, HttpMethod.Get, "/agentprism/api/runs", HttpStatusCode.Forbidden);
        await AssertStatusAsync(host, HttpMethod.Get, $"/agentprism/api/attachments/{attachmentId}", HttpStatusCode.NotFound);
        await AssertStatusAsync(host, HttpMethod.Get, "/agentprism/api/approvals/pending", HttpStatusCode.Forbidden);
    }
    ...
    public async Task A_missing_run_is_answered_without_asking_the_handler()
    {
        ...
        await AssertStatusAsync(host, HttpMethod.Get, $"/agentprism/api/runs/{Missing}", HttpStatusCode.NotFound);
    ...
    public async Task Another_tenants_run_never_reaches_the_handler()
    {
        ...
        await AssertStatusAsync(host, HttpMethod.Get, $"/agentprism/api/runs/{foreign}", HttpStatusCode.NotFound);
    ...
    private static async Task<string> MissingTraceBodyAsync(AgentPrismTestHost host)
    {
        using var response = await host.Client.GetAsync(
            new Uri($"/agentprism/api/runs/{Missing}/trace", UriKind.Relative));

        return await ReadBodyAsync(response, Missing);
    }

    private static async Task<HttpResponseMessage> UploadAsync(AgentPrismTestHost host)
    {
    ...
    /// </summary>
    private static async Task<(AgentPrismTestHost Host, ConfigurableRunAuthorizationHandler Handler, Guid RunId)> StartWithRunAsync()
    {
        var handler = new ConfigurableRunAuthorizationHandler();
    ...
    /// </remarks>
    private static async Task<Guid> SeedRunAsync(AgentPrismTestHost host, string? tenantId = null)
    {
    ...
    private static async Task<Guid> SeedAttachmentAsync(AgentPrismTestHost host)
    {
        var store = host.Services.GetRequiredService<IAttachmentStore>();
    ...
    private static async Task<Guid> SeedApprovalAsync(AgentPrismTestHost host, Guid runId)
    {
    ...
    private static Task<AgentPrismTestHost> StartAsync(ConfigurableRunAuthorizationHandler? handler)
        => AgentPrismTestHost.StartAsync(
            ...
            configureServices: services =>
            {
                ...
                {
                    // Registered BEFORE AgentPrism's own TryAdd, the way a consumer
    ...
    private static Task<AgentPrismTestHost> StartWorkflowAsync(ConfigurableRunAuthorizationHandler? handler)
        => AgentPrismTestHost.StartAsync(
            static builder => builder
    ...
    private static async Task<Guid> RunWorkflowAsync(AgentPrismTestHost host)
    {
        using (var saved = await host.Client.PutAsJsonAsync(
            new Uri("/agentprism/api/workflows/chain", UriKind.Relative),
        ...
        using var response = await host.Client.PostAsJsonAsync(
            new Uri("/agentprism/api/workflows/chain/run", UriKind.Relative),
