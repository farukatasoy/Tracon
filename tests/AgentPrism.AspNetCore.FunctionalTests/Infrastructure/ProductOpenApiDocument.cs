...
/// <remarks>
/// <para>
/// AgentPrism.AspNetCore deliberately does NOT depend on
...
internal static class ProductOpenApiDocument
{
    ...
    /// <remarks>
    /// The prefix is chosen by the consumer in <c>MapAgentPrism(prefix)</c>; the
    /// snapshot has to pick one, and this is the default the samples and the
    /// template use.
    /// </remarks>
    public const string Prefix = "/agentprism";
    ...
    public static void Configure(OpenApiOptions options)
    {
        options.AddDocumentTransformer(static (document, _, _) =>
        {
            document.Info = new OpenApiInfo
            {
                ...
                Version = "v1",
                Description =
                    "The control plane AgentPrism maps into an ASP.NET Core application. Every " +
                    "path below is relative to the prefix passed to MapAgentPrism, which is " +
                    "'" + Prefix + "' in this document. The AgentPrism packages do not generate " +
            ...
            document.Components.SecuritySchemes = new Dictionary<string, IOpenApiSecurityScheme>(StringComparer.Ordinal)
            {
                ["bearer"] = new OpenApiSecurityScheme
                {
                    ...
                    Scheme = "bearer",
                    Description =
                        "The token configured through AgentPrismEndpointOptions, or an API key " +
