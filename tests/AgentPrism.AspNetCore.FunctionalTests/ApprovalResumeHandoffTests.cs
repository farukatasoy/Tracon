...
// K-269: AgentPrism.Testing.AgentPrismTestHost and this project's own host
...
public sealed class ApprovalResumeHandoffTests
{
    ...
    public async Task A_decision_whose_handoff_failed_is_completed_by_repeating_it()
    {
        ...
        using var runs = await host.Client.GetAsync(
            new Uri("/agentprism/api/runs?sessionId=handoff-session", UriKind.Relative));

        var resumed = (await AgentPrismTestHost.ReadJsonAsync(runs))
            .EnumerateArray()
    ...
    private sealed class FailFirstResumeEnqueue
    {
        ...
        /// <c>configureAgentPrism</c>, so AgentPrism's own
