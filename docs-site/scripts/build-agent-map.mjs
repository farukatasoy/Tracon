...
//
//   src/Tracon.Core/buildTransitive/Tracon.AgentMap.md  shipped in the nupkg
...
export const outputs = {
  agentMap: join(repositoryRoot, 'src/Tracon.Core/buildTransitive/Tracon.AgentMap.md'),
...
/** The single line TRC0401 reads. Both artifacts carry the same revision. */
export function marker(revision) {
  return `<!-- Tracon agent map · revision: ${revision} · generated by docs-site/scripts/build-agent-map.mjs -->`;
}

/** Reads the revision out of a generated file, or null when the file is not one. */
export function readRevision(text) {
  return /^<!-- Tracon agent map · revision: ([0-9a-f]{8}) ·/m.exec(text)?.[1] ?? null;
}
...
function readPackages() {
  ...
  return readdirSync(sourceRoot, { withFileTypes: true })
    .filter((entry) => entry.isDirectory() && entry.name.startsWith('Tracon'))
...
function renderMap({ lead, sections }, packages, fullTextKilobytes) {
  ...
  lines.push('');
  lines.push('```csharp');
  lines.push('builder.Services.AddTracon();  // required: catalog, runs, in-memory stores');
  lines.push('app.MapTracon();               // optional: management HTTP API and console');
  lines.push('```');
  lines.push('');
  ...
  lines.push('');
  lines.push('- Exact local paths for the version you have: Tracon.LocalReference.md, beside each project that references Tracon');
