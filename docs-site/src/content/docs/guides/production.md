...
| Buffered | 32 | 5 472 | 1060 ms | 1108 ms | 29.5 |
| Buffered | 64 | 10 944 | 1055 ms | 1120 ms | 59.1 |
| Streaming (SSE) | 1 | 153 | 1185 ms | 1226 ms | 0.83 |
| Streaming | 8 | 1 192 | 1215 ms | 1277 ms | 6.44 |
| Streaming | 32 | 5 226 | 1104 ms | 1189 ms | 28.1 |
| Streaming | 64 | 10 718 | 1074 ms | 1136 ms | 57.4 |
| Queued (`Prefer: respond-async`) | 1 | 140 | 1309 ms | 1330 ms | 0.75 |
| Queued | 8 | 1 104 | 1318 ms | 1336 ms | 5.93 |
| Queued | 32 | 1 392 | 4457 ms | 4494 ms | 6.75 |
| Queued | 64 | 1 488 | 8773 ms | 8946 ms | 6.31 |
...
about 1% at every step, so those rows are not repeated here. **The tails did
not match**: buffered at concurrency 1 reached a p99 of 1721 ms against the
full database versus 1073 ms against the empty one, and queued at concurrency
64 reached 9376 ms versus 8946 ms. Existing data moved the outliers, not the
median.
...
|---|---|---|---|---|
| 1 | 180 of 180 | 0 | 180 | 0.05–0.11 s |
| 4 | 720 of 720 | 0 | 720 | 0.08–0.10 s |
| 8 | 1 440 of 1 440 | 0 | 1 440 | 5.7 s |
| 16 | 1 672 of 2 880 | 1 208 | 1 668 | 16.5 s |
...
|---|---|---|---|
| Buffered | 10.5 | ~8.4 KiB (1 clean repeat) | `run_events` (6 rows) |
| Queued | 10.6 | ~10.3 KiB (1 clean repeat) | `run_events` (6 rows), plus one `jobs` row |
| Streaming | 27.6 | ~14.9 KiB (3 clean repeats) | `run_events` (24 rows) |

Only the streaming figure rests on three clean windows; the other two come from
a single window each and are marked as such rather than presented as averages.
The row counts all come from three.
...
projection of the streaming figure to a million runs is roughly 15 GB before
