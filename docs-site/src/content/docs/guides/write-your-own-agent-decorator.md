...
AgentPrism uses for run recording, telemetry, and the tool-approval gate. Use
...
first and ends up **innermost** — closest to the agent's own calls. AgentPrism's
...
`AgentPrismAgentSourceException` (the same shape a broken `IAgentSource`
...
`IAgentPrismBuilder.Services` is the escape hatch for anything AgentPrism does
not model directly. Every AgentPrism service is registered with `TryAdd`, so
...
| Seam shape | Examples | Registered **before** `AddAgentPrism()` | Registered **after** `AddAgentPrism()` |
|---|---|---|---|
| Single-instance | `ITenantContext`, `IProviderRetryClassifier`, `IRunErrorClassifier` | Your registration wins outright; only one registration remains | Also wins for a direct resolve, but AgentPrism's own registration is not removed — it stays behind as a second, unused entry |
...
// whether it runs before or after AddAgentPrism().
...
- [Reference: IAgentDecorator](/api/agentprism.iagentdecorator/) — full API contract
