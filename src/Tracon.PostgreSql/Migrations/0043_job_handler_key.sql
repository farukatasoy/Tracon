...
-- identity; there is no second column. Tracon's own keys live in the
-- reserved `tracon.` namespace, a consumer picks its own prefix.
--
...
UPDATE {schema}.jobs SET handler_key = CASE kind
    WHEN 0 THEN 'tracon.agent-batch'
    WHEN 1 THEN 'tracon.workflow'
    WHEN 2 THEN 'tracon.eval'
    WHEN 3 THEN 'tracon.webhook-delivery'
    WHEN 4 THEN 'tracon.retention'
    WHEN 5 THEN 'tracon.agent-run'
    WHEN 6 THEN 'tracon.online-eval'
    WHEN 7 THEN 'tracon.approval-resume'
    WHEN 8 THEN 'tracon.run-continuation'
END
