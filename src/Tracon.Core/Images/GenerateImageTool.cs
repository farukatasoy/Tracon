...
// MEAI 10.9.0 marks its image-generation surface experimental. Tracon uses
...
internal sealed class GenerateImageTool : AIFunction
{
        ...
        CancellationToken cancellationToken)
    {
        ...
        var options = Resolve<IOptions<TraconImageOptions>>().Value;
        ...
        if (count > options.MaxImagesPerRequest)
        {
            throw new TraconException(
                ...
                "Request fewer images or raise the `Tracon:Images:MaxImagesPerRequest` setting.");
        }
        ...
        {
            throw new TraconException("The image provider returned no image content.");
        }
        ...
        // its identifier. Reporting itself is isolated by TraconToolUsage.
        ...
        var scope = TraconRunContext.Current
            ?? throw new TraconException("Image generation requires an active Tracon run scope.");
        var tenantId = scope.TenantId
            ?? throw new TraconException("Image generation requires a tenant-bound Tracon run scope.");
    ...
    private static int? OptionalCount(AIFunctionArguments arguments, string name)
    {
        ...
        var valid = raw switch
        {
            ...
            _ => throw new TraconException($"The '{name}' argument must be a positive integer."),
        };

        if (valid < 1)
        {
            throw new TraconException($"The '{name}' argument must be a positive integer.");
        }
    ...
    private static (Size? Size, string? Key) OptionalSize(AIFunctionArguments arguments, string name)
    {
        ...
        if (separator < 1 || separator == value.Length - 1 ||
            ...
            width < 1 || height < 1)
        {
            throw new TraconException($"The '{name}' argument must use the WIDTHxHEIGHT form, for example '1024x1024'.");
        }
