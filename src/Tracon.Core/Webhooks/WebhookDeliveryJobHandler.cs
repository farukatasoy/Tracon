...
internal sealed class WebhookDeliveryJobHandler(
    ...
    IOptionsMonitor<TraconEgressOptions>? egressOptions = null) : IJobHandler
{
    ...
    public async ValueTask ExecuteAsync(JobContext context, CancellationToken cancellationToken = default)
    {
        ...
        if (context.Items.Count == 0 || !Guid.TryParse(context.Items[0].Input, out var deliveryId))
        {
            ...
            throw new TraconException("The webhook delivery job does not carry a valid delivery id.");
        }

        var delivery = await store.GetDeliveryAsync(deliveryId, cancellationToken).ConfigureAwait(false)
            ?? throw new TraconException($"No webhook delivery record was found: {deliveryId}.");
        ...
        var verdict = await WebhookUrlValidator
            .ValidateResolvedAsync(
                subscription.Url,
                options,
                egressOptions?.CurrentValue ?? new TraconEgressOptions(),
                cancellationToken)
            .ConfigureAwait(false);
        ...
        if (isFinalAttempt)
        {
            ...
            throw new TraconException($"Webhook delivery failed after {attempt} attempts: {outcome.Error}");
        }
    ...
    private string? ResolveSecret(WebhookSubscription subscription, TraconWebhookOptions options)
    {
