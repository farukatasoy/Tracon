...
/// <c>AddTracon()</c> on a bare service collection that no host ever backs;
/// <see cref="IHostEnvironment"/> is registered by the host, not by Tracon.
    ...
    ILogger<NonPersistentStorageWarningService> logger) : IHostedService
{
    ...
    public Task StartAsync(CancellationToken cancellationToken)
    {
        ...
        logger.LogWarning(
            "Tracon is running in the Production environment with storage that is not " +
