...
internal static class ContentProtectionEnvelope
{
        ...
        out byte[] tag)
    {
        ...
        using (document)
        {
            ...
            {
                throw new TraconException("A protected value's envelope carries no key id.");
            }
            ...
            {
                throw new TraconException($"A protected value's envelope (key id '{parsedKeyId}') is missing its nonce or ciphertext.");
            }
            ...
            {
                throw new TraconException($"A protected value's envelope (key id '{parsedKeyId}') has a ciphertext shorter than the authentication tag.");
            }
    ...
    /// <exception cref="TraconException"><paramref name="keyId"/> is too long to fit the header's one-byte length.</exception>
        ...
        out byte[] tag)
    {
        ...
        {
            throw new TraconException($"Unsupported content protection binary envelope version {version}.");
        }
        ...
        {
            throw new TraconException("A protected binary value is truncated.");
        }
