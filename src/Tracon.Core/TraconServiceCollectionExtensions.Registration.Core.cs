...
public static partial class TraconServiceCollectionExtensions
{
    private static void RegisterCoreInfrastructure(IServiceCollection services)
    {
        ...
        // Run and session authorization (phase 139, F-185): Tracon draws
        ...
        // Registered here, ahead of every IHostedService Tracon itself
        // registers, so a host whose module order left a required binding on the
        // built-in default fails before Tracon's own background work starts.
        // A host that registers its own hosted service before AddTracon()
        ...
        services.TryAddSingleton(static provider => new ContentGuardPipeline(
            ...
            provider.GetRequiredService<IOptionsMonitor<TraconContentGuardOptions>>(),
        ...
        // taxonomy is Tracon's opinion; thanks to TryAddSingleton, the
        ...
        services.TryAddSingleton<IModelProviderRegistry>(static provider => new ModelProviderRegistry(
            ...
            provider.GetService<TraconMetrics>(),
        ...
        services.TryAddSingleton(static provider => new TraconDiagnosticsCollector(
            provider.GetServices<IModelProvider>(),
...
#pragma warning disable MAAI001 // AgentFileStore — the rationale is the same as in AgentDefinitionCompiler.
        services.TryAddSingleton(static provider => new AgentDefinitionCompiler(
            ...
            // Tracon.PostgreSql fills this in with PostgresChatHistoryProvider.
            ...
            provider.GetRequiredService<ITenantContext>(),
            provider.GetRequiredService<IOptions<TraconOptions>>().Value.UtilityModel,
            ...
            // compilation error. Tracon.Mcp's UseMcp() call registers this.
            ...
            // Tracon.PostgreSql's UsePostgreSql() call registers this.
            ...
            provider.GetRequiredService<IOptions<TraconKnowledgeOptions>>().Value.MaxResults,
            ...
            provider.GetRequiredService<IOptions<TraconOptions>>().Value.AgentGraph,
            provider.GetService<TimeProvider>(),
            ...
            provider.GetServices<TraconLoopEvaluatorRegistration>(),
...
#pragma warning restore MAAI001
        ...
        // throws an overt TraconException.
        services.TryAddSingleton(static provider => new KnowledgeIngestionService(
            provider.GetRequiredService<ITenantContext>(),
            provider.GetRequiredService<IOptions<TraconKnowledgeOptions>>(),
            provider.GetService<IVectorSearchStore>(),
            provider.GetService<Microsoft.Extensions.AI.IEmbeddingGenerator<string, Microsoft.Extensions.AI.Embedding<float>>>()));
        ...
        services.TryAddSingleton(static provider => new AgentDefinitionValidator(
            ...
            // When not registered, Tracon.Mcp is not in use; missing tool
    ...
    private static void RegisterCatalogAssembly(IServiceCollection services)
    {
        ...
        services.TryAddSingleton(static provider => new TraconMetrics(
            provider.GetService<System.Diagnostics.Metrics.IMeterFactory>(),
            provider.GetService<IOptionsMonitor<TraconOptions>>()));
        ...
        services.TryAddSingleton(static provider => new AgentSessionManager(
            ...
            provider.GetService<Microsoft.Extensions.Options.IOptionsMonitor<TraconSessionOwnershipOptions>>()));
        ...
        // is registered by the host, and AddTracon() is also valid on a bare
        ...
        services.TryAddEnumerable(ServiceDescriptor.Singleton<IHostedService, SilentGapWarningService>(
            static provider => new SilentGapWarningService(
                provider.GetService<IHostEnvironment>(),
                provider.GetServices<IContentGuard>(),
                provider.GetRequiredService<IOptions<TraconRetentionOptions>>(),
        ...
        // TraconDrainOptions.Enabled is off (the default) IsDraining
        // never becomes true and StopAsync returns immediately (K1).
        services.TryAddSingleton<TraconDrainService>();
        services.TryAddSingleton<ITraconDrainState>(
            static provider => provider.GetRequiredService<TraconDrainService>());
        services.TryAddEnumerable(ServiceDescriptor.Singleton<IHostedService, TraconDrainService>(
            static provider => provider.GetRequiredService<TraconDrainService>()));
