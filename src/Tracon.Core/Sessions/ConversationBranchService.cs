...
/// (<see cref="TraconSessionStateKeys.ChatHistory"/>); the only way to
...
internal sealed class ConversationBranchService
{
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        {
            // 🚨 ex.Message describes Tracon's OWN previously-stored session state
        ...
        if (!session.StateBag.TryGetValue<ChatHistoryState>(
                TraconSessionStateKeys.ChatHistory,
                out var state,
                TraconCoreJsonContext.Default.Options) ||
            state is null ||
        ...
        // conversation identity and the Tracon identity stamp change.
