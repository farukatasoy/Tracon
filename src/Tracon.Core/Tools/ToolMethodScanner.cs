...
/// <summary>
/// Finds methods marked with <see cref="TraconToolAttribute"/> on a type and
...
internal static class ToolMethodScanner
{
    ...
    public static List<TraconToolRegistration> Scan(Type type)
    {
        ...
        var registrations = new List<TraconToolRegistration>();

        foreach (var method in type.GetMethods(Flags))
        {
            var attribute = method.GetCustomAttribute<TraconToolAttribute>(inherit: false);
        ...
        if (registrations.Count == 0)
        {
            throw new TraconException(
                $"Type '{type.FullName}' has no methods marked with [TraconTool]. " +
    ...
    [RequiresUnreferencedCode("Tool scanning uses reflection; method metadata can be removed from trimmed applications.")]
    [RequiresDynamicCode("Tool scanning can require run-time code generation.")]
    private static AIFunction CreateFunction(Type type, MethodInfo method, TraconToolAttribute attribute)
    {
        if (method.IsGenericMethodDefinition)
        {
            throw new TraconException(
                $"Method '{type.FullName}.{method.Name}' is marked with [TraconTool] but is generic. " +
