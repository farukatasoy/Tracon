...
/// storage warning: <c>AddTracon()</c> is valid on a bare service
/// collection no host backs, and <see cref="IHostEnvironment"/> is registered
/// by the host, not by Tracon. Without it the environment is unknown, so
...
internal sealed class SilentGapWarningService(
    ...
    IOptions<TraconRetentionOptions> retentionOptions,
    ILogger<SilentGapWarningService> logger) : IHostedService
{
    ...
    private void WarnIfNoContentGuard()
    {
        ...
        logger.LogWarning(
            "Tracon is running in the Production environment with no IContentGuard registered: " +
    ...
    private void WarnIfRetentionDisabled()
    {
        ...
        catch (Exception exception)
        {
            logger.LogDebug(
                exception,
                "The retention configuration check could not read TraconRetentionOptions. No warning is reported.");
        ...
        logger.LogWarning(
            "Tracon is running in the Production environment with data retention disabled " +
            "(Tracon:Retention:Enabled is false): rows accumulate indefinitely unless an " +
