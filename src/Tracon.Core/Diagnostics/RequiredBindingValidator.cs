...
/// required custom binding still resolves to Tracon's built-in default.
/// </summary>
/// <remarks>
/// <para>
    ...
    IServiceScopeFactory scopeFactory) : IHostedService
{
    ...
    private static void Validate(IServiceProvider provider, ExtensionPoint point)
    {
        ...
        {
            // A collection point has no built-in default TYPE; Tracon
        ...
        {
            throw new InvalidOperationException(
                $"{contract} was declared as a required custom binding, but Tracon's built-in default " +
                $"{builtIn.Name} is what resolved. Register your own {contract} on IServiceCollection BEFORE " +
                $"the AddTracon() call. Tracon registers {contract} with TryAdd, so a TryAdd " +
                $"registration made after AddTracon() is dropped and the built-in default stays bound.");
        }
    }
    ...
    private static InvalidOperationException Missing(string contract)
        => new(
            ...
            $"Register your own {contract} on IServiceCollection before the AddTracon() call.");
}
