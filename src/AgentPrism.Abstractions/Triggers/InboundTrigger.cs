...
/// <remarks>
/// <para>
/// A trigger is identified on the wire by <see cref="TenantId"/> +
/// <see cref="Name"/> (<c>POST /api/triggers/{tenantId}/{name}</c>). The
/// caller authenticates with an HMAC signature over the request body, the
/// same <c>WebhookSigner</c> contract Phase 21's outbound webhooks use, in
/// the reverse direction.
/// </para>
/// <para>
/// 🚨 <see cref="SigningSecretConfigurationName"/> is the configuration
/// KEY'S NAME, never the secret's value (K-059). The value is resolved from
