...
/// with <c>IAgentPrismBuilder.AddLoopEvaluator(kind, evaluator)</c> and is
...
/// built, with <c>AgentPrismCompilationException</c>; it is never ignored.
...
public sealed record LoopCriterion
{
    ...
    /// </summary>
    /// <remarks>
    /// AgentPrism does not configure the harness mode provider today, so a
