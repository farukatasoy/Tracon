...
// (AgentPrismApiClientSseTests), and a `dotnet nswag run` regeneration must
...
// AgentPrism.Client.UnitTests internals access, so they are unit-testable
...
public partial class AgentPrismApiClient
{
        ...
        CancellationToken cancellationToken)
    {
        ...
        throw new AgentPrismApiException(
    ...
    /// and <c>/v1/chat/completions</c> is OpenAI's schema, not AgentPrism's, and
    /// a typed wrapper here would go stale the moment OpenAI adds an event.
    /// </para>
    /// <para>
    /// Comment-only blocks are skipped. AgentPrism sends <c>: keep-alive</c>
