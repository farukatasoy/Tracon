...
{
    /// <summary>Registers <see cref="AgentPrismApiClient"/> as a singleton.</summary>
    ...
    /// </exception>
    /// <exception cref="ArgumentException"><see cref="AgentPrismClientOptions.BaseAddress"/> was not set.</exception>
    /// <remarks>
        ...
        Action<AgentPrismClientOptions> configure)
    {
        ...
        var options = new AgentPrismClientOptions();
        configure(options);

        if (options.BaseAddress is null)
        {
            throw new ArgumentException(
                $"{nameof(AgentPrismClientOptions.BaseAddress)} must be set to the application root " +
                "plus the MapAgentPrism prefix, for example https://example.com/agentprism/.",
                nameof(configure));
        }
        ...
        services.TryAddSingleton(_ =>
        {
            ...
            return new AgentPrismApiClient(httpClient);
        });
