...
internal sealed class McpConnection : IAsyncDisposable
{
    ...
    private TraconMcpOptions _options;
    ...
    public IReadOnlyList<TraconToolRegistration> Tools { get; private set; }
        ...
        CancellationToken cancellationToken)
    {
        ...
        {
            // A remote server crashing does not stop Tracon: that server's
    ...
    private static bool ShouldSkipConnection(McpServerDefinition server, TraconMcpOptions options, ILogger logger)
    {
        ...
        {
            logger.LogWarning(
                "MCP server '{ServerName}' skipped: OAuth is enabled but Tracon:Mcp:OAuthCallbackBaseUri " +
    ...
    private TraconToolRegistration CreateReadResourceTool()
    {
    ...
    private List<TraconToolRegistration> Project(
        IList<McpClientTool> discovered,
        TraconMcpOptions options,
        ILogger logger)
    {
        var registrations = new List<TraconToolRegistration>(discovered.Count);
        ...
        foreach (var tool in discovered)
        {
            if (registrations.Count >= options.MaxToolsPerServer)
            {
                logger.LogWarning(
                    ...
                    "The limit is changed with Tracon:Mcp:MaxToolsPerServer.",
                    _serverName,
