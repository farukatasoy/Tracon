...
/// <summary>
/// Reads model provider health over HTTP, through <see cref="AgentPrismApiClient"/>.
/// </summary>
internal static class HealthCommand
{
    ...
    public static async Task<int> RunAsync(IReadOnlyList<string> args, CancellationToken cancellationToken)
    {
        ...
        await using (provider.ConfigureAwait(false))
        {
            ...
            try
            {
                var health = await client.AgentPrismModelsHealthAsync(refresh: null, timeoutSource.Token).ConfigureAwait(false);
                Print(health, asJson);
                return 0;
            }
    ...
    private static void Print(ICollection<AgentPrism.Client.Generated.ModelProviderHealth> health, bool asJson)
    {
        if (asJson)
        {
