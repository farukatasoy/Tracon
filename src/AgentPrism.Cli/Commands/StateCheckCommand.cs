...
internal static class StateCheckCommand
{
    ...
    private static readonly JsonSerializerOptions PrettyJson = new()
    {
        ...
        // AgentPrism serializes over the wire is a string; this one matches.
    ...
    public static async Task<int> RunAsync(IReadOnlyList<string> args, CancellationToken cancellationToken)
    {
        ...
        var connectionString = CliArgs.RequireOption(args, "--connection", "AGENTPRISM_CONNECTION");
        ...
        await using (services.ConfigureAwait(false))
        {
            ...
            catch (AgentPrismException ex)
            {
                ...
                // AgentPrismException a preflight was ever expected to meet -
                ...
                // stack trace; AgentPrism's own messages never carry the
    ...
    private static void Print(StatePreflightReport report, bool asJson)
    {
        ...
        if (report.HasUnreadableGeneration)
        {
            Console.Error.WriteLine(
                ...
                "Upgrade the AgentPrism packages before starting this build against this database.");
        }
