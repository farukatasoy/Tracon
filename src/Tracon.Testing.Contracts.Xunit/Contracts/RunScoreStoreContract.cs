...
public abstract class RunScoreStoreContract : TenantIsolationContract<IRunScoreStore>
{
    ...
    public async Task An_illegal_name_is_REJECTED(string name)
        => await Should.ThrowAsync<ArgumentException>(
            async () => await Store.UpsertAsync(Score(TraconId.NewId()) with { Name = name }));
    ...
    public async Task Deleting_a_nonexistent_score_returns_false()
        => (await Store.DeleteAsync(Tenant, TraconId.NewId())).ShouldBeFalse();
    ...
    public async Task Another_tenants_score_never_enters_the_summary()
    {
        ...
        await Store.UpsertAsync(SummaryScore(tenant, TraconId.NewId()));
        await Store.UpsertAsync(SummaryScore("other-" + tenant, TraconId.NewId()));
    ...
    public async Task Different_kinds_of_the_same_name_are_not_averaged_together()
    {
        ...
        await Store.UpsertAsync(SummaryScore(tenant, TraconId.NewId()) with
        {
            Name = "quality",
    ...
    public async Task A_null_value_is_counted_but_does_not_enter_the_average()
    {
        ...
        await Store.UpsertAsync(SummaryScore(tenant, TraconId.NewId()) with { Value = 1, Author = "alice" });
        await Store.UpsertAsync(SummaryScore(tenant, TraconId.NewId()) with { Value = null, Author = "bob" });
    ...
    public async Task Author_breakdown_excludes_scores_with_no_author()
    {
        ...
        await Store.UpsertAsync(SummaryScore(tenant, TraconId.NewId()) with { Author = "alice" });
        await Store.UpsertAsync(SummaryScore(tenant, TraconId.NewId()) with { Author = null });
    ...
    public async Task An_empty_string_Author_is_excluded_from_the_author_breakdown_like_no_author()
    {
        ...
        await Store.UpsertAsync(SummaryScore(tenant, TraconId.NewId()) with { Author = "alice" });
        await Store.UpsertAsync(SummaryScore(tenant, TraconId.NewId()) with { Author = string.Empty });
    ...
    public async Task Scores_are_grouped_into_time_buckets_and_empty_buckets_are_omitted()
    {
        ...
        await Store.UpsertAsync(SummaryScore(tenant, TraconId.NewId()) with { CreatedAt = dayOne, Author = "alice" });
        await Store.UpsertAsync(SummaryScore(tenant, TraconId.NewId()) with { CreatedAt = dayOne.AddHours(2), Author = "bob" });
        await Store.UpsertAsync(SummaryScore(tenant, TraconId.NewId()) with { CreatedAt = dayTwo, Author = "carol" });
    ...
    public async Task Hour_buckets_group_scores_within_the_same_hour()
    {
        ...
        await Store.UpsertAsync(SummaryScore(tenant, TraconId.NewId()) with { CreatedAt = hourOne, Author = "alice" });
        await Store.UpsertAsync(SummaryScore(tenant, TraconId.NewId()) with { CreatedAt = hourOne.AddMinutes(45), Author = "bob" });
        await Store.UpsertAsync(SummaryScore(tenant, TraconId.NewId()) with { CreatedAt = hourTwo, Author = "carol" });
    ...
    public async Task Week_buckets_start_on_Monday_and_group_the_whole_week()
    {
        ...
        await Store.UpsertAsync(SummaryScore(tenant, TraconId.NewId()) with { CreatedAt = weekOneMonday, Author = "alice" });
        await Store.UpsertAsync(SummaryScore(tenant, TraconId.NewId()) with { CreatedAt = weekOneThursday, Author = "bob" });
        await Store.UpsertAsync(SummaryScore(tenant, TraconId.NewId()) with { CreatedAt = weekThreeMonday, Author = "carol" });
    ...
    public async Task Summary_can_be_read_while_a_score_is_being_written()
    {
        ...
        await Task.WhenAll(
            ...
            Store.UpsertAsync(SummaryScore(tenant, TraconId.NewId()) with { Author = "bob" }).AsTask());

        var summary = await Store.SummarizeAsync(new RunScoreQuery { TenantId = tenant });
        summary.ByName.ShouldHaveSingleItem().Count.ShouldBe(2);
    }

    private static string UniqueTenant() => $"summary-{TraconId.NewId():N}";
