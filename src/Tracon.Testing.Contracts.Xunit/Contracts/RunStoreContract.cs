...
public abstract class RunStoreContract : TenantIsolationContract<IRunStore>
{
    ...
    [Fact]
    public async Task StartRunAsync_return_value_carries_the_coalesced_attribution()
    {
        // The RETURN VALUE, not a follow-up GetRunAsync: a caller that reads
        // only what StartRunAsync hands back must see the same attribution
        // a query would report immediately after.
        var runId = TraconId.NewId();
        ...
        var second = await Store.StartRunAsync(TestData.Run(runId));

        second.UserId.ShouldBe("user-1");
        second.Labels.ShouldNotBeNull();
    ...
    public async Task Reusing_a_sequence_number_is_rejected()
    {
        ...
        // A duplicate Sequence is a caller error, not a legitimate retry: it
        // must be rejected, not silently ignored (which would hide the
        // duplicate) and not silently accepted a second time (which would
        // leave a gap the append-only stream never explains).
        await Should.ThrowAsync<TraconException>(
            async () => await Store.AppendEventAsync(TestData.Event(runId, 0)));
    ...
    public async Task Event_cannot_be_appended_to_a_nonexistent_run()
        => await Should.ThrowAsync<TraconException>(
            async () => await Store.AppendEventAsync(TestData.Event(TraconId.NewId(), 0)));
    ...
    public async Task Nonexistent_run_returns_empty_events()
    {
        ...
        await foreach (var runEvent in Store.ReadEventsAsync(TraconId.NewId()))
        {
            events.Add(runEvent);
        }
    ...
    public async Task ContinuedFromRunId_round_trips()
    {
        ...
        var continuationRunId = TraconId.NewId();
    ...
    {
        await Store.StartRunAsync(TestData.Run(TraconId.NewId()) with { UserId = "ada" });
        await Store.StartRunAsync(TestData.Run(TraconId.NewId()) with { UserId = "grace" });
        await Store.StartRunAsync(TestData.Run(TraconId.NewId()));
    ...
    public async Task Query_filters_by_a_label_key_and_value_together()
    {
        ...
        // return team=billing.
        await Store.StartRunAsync(TestData.Run(TraconId.NewId()) with
        {
            Labels = new Dictionary<string, string>(StringComparer.Ordinal) { ["team"] = "payments" },
        });
    ...
    {
        var withBreakdown = TraconId.NewId();
        var withoutBreakdown = TraconId.NewId();
    ...
    public async Task Every_cost_total_includes_the_cache_charge()
    {
        ...
        var experimentId = TraconId.NewId();
        var runId = TraconId.NewId();
    ...
    {
        await Store.StartRunAsync(TestData.Run(TraconId.NewId(), "alpha"));
        await Store.StartRunAsync(TestData.Run(TraconId.NewId(), "beta"));
        await Store.StartRunAsync(TestData.Run(TraconId.NewId(), "alpha"));
    ...
    public async Task Query_sorts_newest_first()
    {
        ...
        await Store.StartRunAsync(TestData.Run(TraconId.NewId()) with { StartedAt = now.AddMinutes(-10) });
        await Store.StartRunAsync(TestData.Run(TraconId.NewId()) with { StartedAt = now });
        await Store.StartRunAsync(TestData.Run(TraconId.NewId()) with { StartedAt = now.AddMinutes(-5) });
    ...
    public async Task Query_applies_paging()
    {
        ...
        for (var i = 0; i < 5; i++)
        {
            await Store.StartRunAsync(TestData.Run(TraconId.NewId()) with { StartedAt = now.AddMinutes(-i) });
        }
    ...
    public async Task Query_filters_by_start_time()
    {
        ...
        await Store.StartRunAsync(TestData.Run(TraconId.NewId()) with { StartedAt = now.AddMinutes(-30) });
        await Store.StartRunAsync(TestData.Run(TraconId.NewId()) with { StartedAt = now });
    ...
    public async Task Nonexistent_run_returns_null()
        => (await Store.GetRunAsync(TraconId.NewId())).ShouldBeNull();
    ...
    public async Task Summary_rows_with_no_error_class_appear_in_the_unknown_bucket()
    {
        ...
        await Store.CompleteRunAsync(new RunCompletion
        {
            ...
            Status = RunStatus.Failed,
            CompletedAt = DateTimeOffset.UtcNow,
            Error = new RunError { Type = "Tracon.TraconCompilationException", Message = "legacy record" },
        });
    ...
    public async Task Summary_filters_by_start_time()
    {
        ...
        await Store.StartRunAsync(TestData.Run(TraconId.NewId(), "alpha") with { StartedAt = now.AddHours(-2) });
        await Store.StartRunAsync(TestData.Run(TraconId.NewId(), "alpha") with { StartedAt = now });
    ...
    public async Task Experiment_results_are_summarized_per_arm()
    {
        ...
        await Store.StartRunAsync(TestData.Run(TraconId.NewId(), "alpha") with
        {
    ...
    public async Task Workflow_run_lives_in_the_same_table()
    {
        ...
        // mechanism.
        var workflowRunId = TraconId.NewId();
        var agentRunId = TraconId.NewId();
    ...
    public async Task List_returns_only_root_runs_by_default()
    {
        ...
        await Store.StartRunAsync(TestData.Run(rootId));
        await Store.StartRunAsync(TestData.Run(TraconId.NewId(), "researcher") with
        {
            ParentRunId = rootId,
            RootRunId = rootId,
            Depth = 1,
    ...
    public async Task Query_session_filter_with_includeChildren_also_returns_child_runs()
    {
        ...
        var rootId = TraconId.NewId();
        var childId = TraconId.NewId();
        var yabanciRootId = TraconId.NewId();
    ...
    public async Task Tree_query_returns_the_root_and_all_its_descendants()
    {
        var rootId = TraconId.NewId();
        var childId = TraconId.NewId();
        var grandChildId = TraconId.NewId();
        var yabanciId = TraconId.NewId();
    ...
    public async Task Time_series_fills_empty_buckets()
    {
        ...
        await Store.StartRunAsync(TestData.Run(TraconId.NewId()) with { StartedAt = from.AddMinutes(5) });
        await Store.StartRunAsync(TestData.Run(TraconId.NewId()) with { StartedAt = from.AddHours(2).AddMinutes(5) });
    ...
    public async Task Time_series_does_not_exclude_eval_runs()
    {
        ...
        await Store.StartRunAsync(TestData.Run(TraconId.NewId()) with { StartedAt = from.AddMinutes(5), Kind = RunKind.Eval });
    ...
    public async Task CompleteRunAsync_does_not_close_with_the_wrong_expected_tenant()
    {
        ...
        await Should.ThrowAsync<TraconException>(async () => await Store.CompleteRunAsync(new RunCompletion
        {
            RunId = runId,
            Status = RunStatus.Completed,
            CompletedAt = DateTimeOffset.UtcNow,
    ...
    public async Task TouchHeartbeatAsync_silently_skips_a_nonexistent_or_non_Running_id()
    {
        ...
        await Store.TouchHeartbeatAsync([runId, TraconId.NewId()], DateTimeOffset.UtcNow);
    ...
    public async Task ClaimOrphanedRunsAsync_reports_ContinuedFromRunId_on_the_claimed_record()
    {
        ...
        // continuation chain has already run once (TraconRunContinuationOptions.MaxAttempts).
        AmbientTenant.TenantId = TenantA;
