    ...
    ILogger<ToolRegistrationValidationService> logger) : IHostedService
{
    public Task StartAsync(CancellationToken cancellationToken)
    {
        ...
        if (registry is not IVerifiedToolRegistry)
        {
            ...
            {
                throw new AgentPrismException("IToolRegistry was replaced. This removes the Authorizing, Timeout, ApprovalRequired, and Truncating wrappers. Use AddTool APIs, or explicitly set Tools.AllowUnverifiedToolRegistry to true.");
            }
        ...
        foreach (var registration in registrations)
        {
            ...
            {
                throw new AgentPrismException($"Tool '{registration.Function.Name}' has a timeout that must be greater than zero.");
            }

            if (registration.MaxOutputBytes is { } maxOutputBytes && maxOutputBytes < TruncatingAIFunction.MinimumEnvelopeBytes)
            {
                throw new AgentPrismException($"Tool '{registration.Function.Name}' has MaxOutputBytes {maxOutputBytes}, which is below {TruncatingAIFunction.MinimumEnvelopeBytes}.");
            }
        }
