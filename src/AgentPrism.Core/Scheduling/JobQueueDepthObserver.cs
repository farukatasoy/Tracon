...
/// <summary>
/// Cached data source for the <c>agentprism.job.queue.depth</c> observable gauge.
/// </summary>
/// <remarks>
/// <para>
...
/// <see cref="AgentPrismObservabilityOptions.JobQueueDepthRefreshInterval"/> on
...
/// </para>
/// <para>
/// As long as <see cref="AgentPrismObservabilityOptions.EnableJobQueueDepthGauge"/>
...
/// through the same <see cref="AgentPrismMetrics"/> instance, so a lane is named
...
internal sealed class JobQueueDepthObserver : IHostedService, IDisposable
{
        ...
        AgentPrismMetrics? metrics = null)
    {
        ...
        _meter.CreateObservableGauge(
            AgentPrismDiagnostics.JobQueueDepthGaugeName,
            ObserveDepth,
            unit: "{job}",
    ...
    private IEnumerable<Measurement<long>> ObserveDepth()
        => Snapshot().Select(depth => new Measurement<long>(
            ...
            new KeyValuePair<string, object?>(
                AgentPrismDiagnostics.Tags.Lane,
                _metrics?.ResolveLaneTag(depth.Lane) ?? depth.Lane),
            new KeyValuePair<string, object?>(AgentPrismDiagnostics.Tags.JobStatus, depth.Status.ToString())));
