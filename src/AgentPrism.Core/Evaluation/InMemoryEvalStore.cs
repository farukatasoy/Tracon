...
public sealed class InMemoryEvalStore : IEvalStore
{
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        for (var seq = 0; seq < cases.Count; seq++)
        {
            ...
            {
                Id = candidate.Id == Guid.Empty ? AgentPrismId.NewId() : candidate.Id,
                SuiteId = suiteId,
                Seq = seq,
            });
        }
    ...
    public ValueTask<EvalRun> CreateRunAsync(EvalRun run, CancellationToken cancellationToken = default)
    {
        ...
        var record = run with { Id = run.Id == Guid.Empty ? AgentPrismId.NewId() : run.Id };
    ...
    public ValueTask RecordCaseResultAsync(EvalCaseResult result, CancellationToken cancellationToken = default)
    {
        ...
        var record = result.Id == Guid.Empty ? result with { Id = AgentPrismId.NewId() } : result;
    ...
    private static EvalSuite Create(EvalSuite suite)
    {
        ...
        return suite with
        {
            Id = suite.Id == Guid.Empty ? AgentPrismId.NewId(now) : suite.Id,
            CreatedAt = now,
            UpdatedAt = now,
        };
    }
