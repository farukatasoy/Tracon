...
public static class AgentPrismOnlineEvaluationBuilderExtensions
{
    ...
    /// AgentPrism deliberately does not reference: add it yourself when you
    ...
    /// <c>AgentPrism:OnlineEvaluation:Enabled</c> and its sample rate are still
    /// the gate.
    /// </para>
    /// <example>
    /// <code>
    ...
    /// <summary>
    /// Registers a Microsoft.Extensions.AI <see cref="IEvaluator"/> as an
    /// AgentPrism run judge and configures the shared judge model.
    /// </summary>
    /// <param name="builder">Configuration chain.</param>
    /// <param name="name">The judge name, matching <c>[A-Za-z0-9._-]{1,64}</c>.</param>
    /// <param name="evaluator">The evaluator to grade sampled runs with.</param>
