...
/// <remarks>
/// <para>
/// A key's raw material is never held in <see cref="AgentPrismContentProtectionOptions"/>:
/// <see cref="AgentPrismContentProtectionOptions.Keys"/> maps a key id to the
...
/// elsewhere in AgentPrism, so the settings object stays safe to log or inspect.
/// </para>
/// <para>
/// Key rotation is lazy: writes always use <see cref="AgentPrismContentProtectionOptions.ActiveKeyId"/>;
...
public sealed class AesGcmContentProtector : IContentProtector
{
    ...
    /// <see cref="AgentPrismContentProtectionOptions.ActiveKeyId"/> is not set, or its key
    ...
    private (string KeyId, byte[] Key) ResolveActiveKey()
    {
        ...
        if (string.IsNullOrWhiteSpace(keyId))
        {
            throw new AgentPrismException(
                $"Content protection is enabled but {nameof(AgentPrismContentProtectionOptions)}." +
    ...
    private byte[] LoadKey(string keyId)
    {
        if (!_options.CurrentValue.Keys.TryGetValue(keyId, out var configurationKeyName))
        {
            throw new AgentPrismException(
                ...
                $"{nameof(AgentPrismContentProtectionOptions)}.{nameof(AgentPrismContentProtectionOptions.Keys)}, " +
