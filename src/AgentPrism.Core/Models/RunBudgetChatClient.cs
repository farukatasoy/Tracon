...
/// <remarks>
/// <para>
/// Sits in the same ring position as <see cref="AgentPrismResponseCachingChatClient"/>
...
/// </para>
/// <para>
/// <strong>Reads <see cref="AgentPrismRunContext"/>, never writes it.</strong>
    ...
    : DelegatingChatClient(inner)
{
    ...
    {
        var budget = AgentPrismRunContext.Current?.Budget;

        if (budget is not null && budget.IsExhausted)
        {
            throw new AgentPrismRunBudgetExceededException(budget.DescribeModelCallExhaustion());
        }
