...
/// <remarks>
/// <para>
/// AgentPrism reaches the network from three surfaces — webhook delivery, MCP
...
public static class EgressAddressValidator
{
    ...
    private static string Describe(IPAddress address)
        => $"The target resolves to a private network address ({address}); set " +
           $"'{AgentPrismEgressOptions.SectionName}:{nameof(AgentPrismEgressOptions.AllowPrivateNetworkTargets)}' " +
           "to true to allow it.";
