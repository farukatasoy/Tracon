...
/// <summary>
/// The seven embedding points a host may bind, and what "AgentPrism's built-in
/// default" means for each one.
/// </summary>
/// <remarks>
/// <para>
/// Single-sourced on purpose. Two callers ask the same question:
/// <see cref="AgentPrismDiagnosticsCollector"/> reports WHICH implementation is
...
/// </para>
/// </remarks>
internal static class AgentPrismExtensionPoints
{
    ...
    /// <summary>
    /// The type AgentPrism binds for <paramref name="contract"/> when the host
    /// binds nothing.
    /// </summary>
    /// <param name="contract">One of the seven contracts.</param>
    /// <returns>
    /// The built-in default type, or <see langword="null"/> when AgentPrism
    ...
    public static Type? BuiltInDefaultOf(Type contract)
    {
        var point = Find(contract)
            ?? throw new ArgumentOutOfRangeException(nameof(contract), contract, "Not an AgentPrism extension point.");
...
/// The type AgentPrism binds when the host binds nothing, or
/// <see langword="null"/> when AgentPrism registers NOTHING for this point. For
