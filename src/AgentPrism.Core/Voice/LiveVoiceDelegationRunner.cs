...
/// <summary>Turns one delegation from a live voice session into an AgentPrism run.</summary>
/// <remarks>
/// <para>
...
internal sealed class LiveVoiceDelegationRunner
{
        ...
        CancellationToken cancellationToken)
    {
        ...
        try
        {
            ...
            var updates = _agent.RunStreamingAsync(
                ...
                _session,
                new AgentPrismRunOptions { RunId = runId, SessionId = _sessionId },
                cancellationToken);
        ...
        catch (Exception exception) when (exception is AgentPrismException or InvalidOperationException or HttpRequestException or TimeoutException)
        {
        ...
        CancellationToken cancellationToken)
    {
        ...
        foreach (var piece in budget.Take(inspected))
        {
            ...
            catch (Exception exception) when (exception is AgentPrismException or InvalidOperationException or HttpRequestException)
            {
