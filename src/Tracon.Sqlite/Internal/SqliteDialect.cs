...
///     A sidecar file lock (<c>&lt;database&gt;.&lt;prefix&gt;.tracon-migration-lock</c>)
...
///     table prefix is the equivalent that separates Tracon installations, and the lock
...
internal sealed class SqliteDialect : SqlDialect, IDisposable
{
    ...
    private const string LockFileSuffix = ".tracon-migration-lock";
    ...
    /// <inheritdoc />
    public override string MigrationResourcePrefix => "Tracon.Sqlite.Migrations.";
    ...
    public override IReadOnlyDictionary<string, string> OptionalMigrationResourcePrefixes { get; } =
        ...
        {
            ["views"] = "Tracon.Sqlite.MigrationsViews.",
        };
        ...
        while (true)
        {
            ...
            catch (IOException ex)
            {
                throw new TraconException(
                    $"Could not acquire the Tracon migration lock: '{lockPath}' is in use by " +
        ...
        => AddTyped(
            ...
            values is null
                ...
                : JsonSerializer.Serialize(
                    values.ToArray(),
                    TraconJsonContext.Default.StringArray));

    /// <inheritdoc />
    /// <remarks>
    /// Serialized through <see cref="TraconJsonContext.StringArray"/> as
