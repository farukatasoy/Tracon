...
SQLite persistence layer for [Tracon](https://tracon.dev).

For single-file setups: demos, embedded/edge scenarios, and testing with real SQL behavior
beyond in-memory stores. Tables carry a configurable **prefix** (default `tracon_`) so
...
builder.AddTracon()
       .UseSqlite(options =>
       {
           options.ConnectionString = "Data Source=tracon.db";
           options.TablePrefix = "tracon_";
...
`DbDataSource` for the same database file and you want Tracon using that same
instance:
...
`Tracon.PostgreSql`'s `https://tracon.dev/guides/ef-core/` for
the pattern.
...
(`<database-file>.tracon-migration-lock`). The lock is skipped for `:memory:` databases
...
`Tracon.PostgreSql` or `Tracon.SqlServer`.
...
typically affects publish behavior. Use `Tracon.PostgreSql` for setups that require AOT.
