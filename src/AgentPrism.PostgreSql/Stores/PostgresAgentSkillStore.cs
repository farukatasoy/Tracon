        ...
        CancellationToken cancellationToken = default)
    {
        ...
        await using (connection.ConfigureAwait(false))
        {
            ...
            await using (transaction.ConfigureAwait(false))
            {
                ...
                var deleteScripts = new NpgsqlCommand(_sql.DeleteAgentSkillScripts, connection, transaction)
                {
                    CommandTimeout = _commandTimeout,
                };
                deleteScripts.Parameters.AddWithValue("skill_id", saved.Id);
                await NpgsqlHelpers.ExecuteAsync(deleteScripts, cancellationToken).ConfigureAwait(false);

                foreach (var script in skill.Scripts)
                {
                    var insertScript = new NpgsqlCommand(_sql.InsertAgentSkillScript, connection, transaction)
                    {
                        CommandTimeout = _commandTimeout,
                    };
                    insertScript.Parameters.AddWithValue("id", AgentPrismId.NewId());
                    insertScript.Parameters.AddWithValue("skill_id", saved.Id);
                    insertScript.Parameters.AddWithValue("name", script.Name);
                    insertScript.Parameters.AddWithValue("description", (object?)script.Description ?? DBNull.Value);
                    ...
                    insertScript.Parameters.AddWithValue(
                        "extension",
                        script.Extension.TrimStart('.').ToLowerInvariant());
                    insertScript.Parameters.AddWithValue("content", script.Content);
                    insertScript.Parameters.Add(new NpgsqlParameter("parameters_schema", NpgsqlDbType.Jsonb)
                    {
                        Value = (object?)script.ParametersSchema ?? DBNull.Value,
                    });
                    insertScript.Parameters.AddWithValue("created_at", now.UtcDateTime);
                    await NpgsqlHelpers.ExecuteAsync(insertScript, cancellationToken).ConfigureAwait(false);
                }
        ...
        CancellationToken cancellationToken)
    {
        ...
        return await NpgsqlHelpers.ReadListAsync(
            ...
            static reader => new AgentSkillScriptDefinition
            {
                ...
                Content = reader.GetString(3),
                ParametersSchema = NpgsqlHelpers.GetNullableString(reader, 4),
            },
            cancellationToken).ConfigureAwait(false);
    }
