...
/// <summary>In-memory Tracon host.</summary>
/// <remarks>
/// <para>
...
public sealed class TraconTestHost : IAsyncDisposable
{
    ...
    private TraconTestHost(WebApplication app, HttpClient client, string prefix)
    {
        _app = app;
        _prefix = prefix;
        Client = client;
    ...
    public static async ValueTask<TraconTestHost> StartAsync(
        Action<TraconTestHostOptions>? configure = null,
        CancellationToken cancellationToken = default)
    {
        var options = new TraconTestHostOptions();
        ...
        tracon.AddModelProvider(options.ModelProvider);
        options.ConfigureTracon?.Invoke(tracon);

        var app = builder.Build();

        app.MapTracon(options.Prefix, options.ConfigureEndpoints);

        await app.StartAsync(cancellationToken).ConfigureAwait(false);

        var client = app.GetTestClient();

        return new TraconTestHost(app, client, '/' + options.Prefix.Trim('/'));
    }
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        var record = await runs.GetRunAsync(id, cancellationToken).ConfigureAwait(false)
            ?? throw new TraconAssertionException($"No run record with ID '{id}' was found in the store.");
