...
internal sealed class SqlChatHistoryProvider : ChatHistoryProvider
{
    ...
    /// sessions. The key is shared with <see cref="TraconSessionStateKeys.ChatHistory"/>:
    /// conversation branching reads the same state and writes it into the new session.
    /// </remarks>
    public const string SessionStateKey = TraconSessionStateKeys.ChatHistory;
            ...
            storeInputResponseMessageFilter: null)
    {
        ...
        _sessionState = new ProviderSessionState<ChatHistoryState>(
            stateInitializer: static _ => new ChatHistoryState { ConversationId = TraconId.NewId() },
            stateKey: SessionStateKey,
            jsonSerializerOptions: TraconJsonContext.Default.Options);
    }
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        {
            if (JsonSerializer.Deserialize(json, TraconJsonContext.Default.ChatMessage) is { } message)
            {
                history.Add(message);
            }
        }
        ...
        await using (connection.ConfigureAwait(false))
        {
            ...
            await using (transaction.ConfigureAwait(false))
            {
                ...
                foreach (var message in messages)
                {
                    ...
                    DbHelpers.Add(insert, "id", TraconId.NewId());
                    ...
                    Dialect.AddJson(
                        ...
                        ProtectedValue.Write(
                            ...
                            JsonSerializer.Serialize(message, TraconJsonContext.Default.ChatMessage)));
