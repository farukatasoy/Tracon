...
internal sealed class SqlExperimentStore : IExperimentStore
{
    ...
    public async ValueTask<Experiment> SaveAsync(Experiment experiment, CancellationToken cancellationToken = default)
    {
        ...
        var variantsJson = JsonSerializer.Serialize(experiment.Variants, TraconJsonContext.Default.IReadOnlyListExperimentVariant);
        ...
        if (result is null)
        {
            ...
            throw existing is null
                ? new TraconException($"Failed to save experiment '{experiment.Name}'.")
                : new TraconException(
        ...
        return await GetAsync(experiment.TenantId, experiment.Name, cancellationToken).ConfigureAwait(false)
            ?? throw new TraconException($"Experiment '{experiment.Name}' was saved but could not be read back.");
    }
    ...
    public async ValueTask<bool> DeleteAsync(string tenantId, string name, CancellationToken cancellationToken = default)
    {
        ...
        {
            throw new TraconException($"Experiment '{name}' cannot be deleted while running; stop it first.");
        }
    ...
    public async ValueTask<Experiment> StartAsync(string tenantId, string name, CancellationToken cancellationToken = default)
    {
        ...
        catch (DbException ex) when (Dialect.IsUniqueViolation(ex))
        {
            var experiment = await GetAsync(tenantId, name, cancellationToken).ConfigureAwait(false)
                ?? throw new TraconException($"No experiment named '{name}' was found.", ex);
        ...
        return await GetAsync(tenantId, name, cancellationToken).ConfigureAwait(false)
            ?? throw new TraconException($"Experiment '{name}' was started but could not be read back.");
    }
    ...
    public async ValueTask<Experiment> StopAsync(string tenantId, string name, CancellationToken cancellationToken = default)
    {
        ...
        if (result is null)
        {
            ...
            throw existing is null
                ? new TraconException($"No experiment named '{name}' was found.")
                : new TraconException($"Experiment '{name}' is not running.");
        }

        return await GetAsync(tenantId, name, cancellationToken).ConfigureAwait(false)
            ?? throw new TraconException($"Experiment '{name}' was stopped but could not be read back.");
    }
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        _ = await DbHelpers.ExecuteScalarAsync(command, cancellationToken).ConfigureAwait(false)
            ?? throw new TraconException($"No experiment named '{name}' was found.");

        return await GetAsync(tenantId, name, cancellationToken).ConfigureAwait(false)
            ?? throw new TraconException($"Experiment '{name}''s canary policy was updated but could not be read back.");
    }
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        return await GetAsync(tenantId, name, cancellationToken).ConfigureAwait(false)
            ?? throw new TraconException($"Experiment '{name}''s canary weight was updated but could not be read back.");
    }
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        return await GetAsync(tenantId, name, cancellationToken).ConfigureAwait(false)
            ?? throw new TraconException($"Experiment '{name}' was rolled back but could not be read back.");
    }

    private async ValueTask<TraconException> BuildStartFailureAsync(string tenantId, string name, CancellationToken cancellationToken)
    {
        var existing = await GetAsync(tenantId, name, cancellationToken).ConfigureAwait(false);
    ...
    private static Experiment ReadExperiment(DbDataReader reader)
        => new()
        {
            ...
            Variants = JsonSerializer.Deserialize(reader.GetString(4), TraconJsonContext.Default.IReadOnlyListExperimentVariant)
                ?? [],
            ...
            Canary = DbHelpers.GetNullableString(reader, 10) is { } canaryJson
                ? JsonSerializer.Deserialize(canaryJson, TraconJsonContext.Default.CanaryPolicy)
                : null,
