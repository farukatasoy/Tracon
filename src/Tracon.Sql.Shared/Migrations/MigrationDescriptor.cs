...
internal sealed record MigrationDescriptor(int Id, string Name, string Sql, string Checksum)
{
    ...
    /// The embedded resource name prefix. Example: <c>Tracon.PostgreSql.Migrations.</c>.
    ...
    private static string ReadResource(Assembly assembly, string resourceName)
    {
        using var stream = assembly.GetManifestResourceStream(resourceName)
            ?? throw new TraconException($"Could not read embedded migration resource: '{resourceName}'.");
