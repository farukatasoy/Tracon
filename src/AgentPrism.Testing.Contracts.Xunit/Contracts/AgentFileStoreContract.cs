...
/// establishes -- AgentPrism's own stores read it from
/// <c>AgentPrismRunContext</c> (<c>AgentPrism.Core</c>), which this package
/// deliberately does not reference (it depends on
/// <c>AgentPrism.Abstractions</c> only).
/// </para>
/// <para>
...
public abstract class AgentFileStoreContract : TenantIsolationContract<AgentFileStore>
{
    ...
    /// agent name from. AgentPrism's own SQL-backed implementations read
    /// <c>AgentPrismRunContext.Current</c> (<c>AgentPrism.Core</c>); a
    /// derived contract in a project that already references
    /// <c>AgentPrism.Core</c> sets that up here.
    /// </summary>
