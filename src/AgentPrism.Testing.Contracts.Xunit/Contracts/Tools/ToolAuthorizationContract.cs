...
/// the handler throws, the call must still end up denied. AgentPrism tests
...
public abstract class ToolAuthorizationContract : IAsyncLifetime
{
    ...
    {
        // AgentPrism's own wrapper (AuthorizingAIFunction) invokes the tool
        // body only when IsAllowed is true, and is tested by AgentPrism
