...
export const testMeta: TraconMetaResponse = {
  version: 'test',
  prefix: '/tracon',
...
/** One SSE frame in the shape `readSse` (`@tracon/client`) expects. */
 ...
 *
 * `openapi-fetch` reads `globalThis.fetch` once, when `createTraconClient`
...
export function installPersistentFetchStub(): void {
  const handler = vi.fn(async (input: RequestInfo | URL, init?: RequestInit): Promise<Response> => {
    ...
    // Strips a leading UI prefix (e.g. '/tracon/') the same way the real
