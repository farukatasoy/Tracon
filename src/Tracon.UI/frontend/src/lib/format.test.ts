...
describe('toRelativePath', () => {
  it('treats the base with and without a trailing slash as the root', () => {
    ...
    expect(toRelativePath('/tracon', '/tracon/')).toBe('');
    expect(toRelativePath('/tracon/', '/tracon/')).toBe('');
  });

  it('strips the base from nested paths', () => {
    expect(toRelativePath('/tracon/runs/019fc0', '/tracon/')).toBe('runs/019fc0');
  ...
  it('ignores a trailing slash on a nested path', () => {
    expect(toRelativePath('/tracon/tools/', '/tracon/')).toBe('tools');
  });
});
