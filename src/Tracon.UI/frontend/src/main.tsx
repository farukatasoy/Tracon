...
import { App } from './app';
import { TraconError } from './lib/api';
  ...
  defaultOptions: {
    queries: {
      ...
      retry: (failureCount, error) => {
        ...
        if (error instanceof TraconError && error.status >= 400 && error.status < 500) {
          return false;
        }
...
if (container === null) {
  throw new Error('Tracon UI: #root element is missing from the shell.');
}
