...
internal sealed class SqlServerDialect : SqlDialect
{
    ...
    /// <remarks>
    /// The value is specific to Tracon and <strong>must not change</strong>.
    /// The lock is SCOPED TO THE SCHEMA: <c>SchemaName</c> exists so
    /// two independent Tracon deployments can share one database, and the
    ...
    /// transaction).
    /// </remarks>
    private const string MigrationLockResourcePrefix = "Tracon.Migrations:";
    ...
    public override IReadOnlyDictionary<string, string> OptionalMigrationResourcePrefixes { get; } =
        ...
        {
            ["views"] = "Tracon.SqlServer.MigrationsViews.",
        };
        ...
        await using (command.ConfigureAwait(false))
        {
            ...
            if (result.Value is int code && code < 0)
            {
                throw new TraconException(
                    $"Could not acquire the Tracon migration lock (sp_getapplock returned {code}). " +
    ...
    public override void AddUuidArray(DbCommand command, string name, IReadOnlyList<Guid>? values)
        => AddTyped(
            ...
            values is null
                ...
                : JsonSerializer.Serialize(
                    values.ToArray(),
                    TraconJsonContext.Default.GuidArray));
