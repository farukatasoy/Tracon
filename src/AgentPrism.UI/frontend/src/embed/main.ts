 ...
 * @example
 * ```html
 * <script src="https://api.example.com/agentprism/embed/embed.js"
 ...
 *   window.AgentPrismEmbed.registerTool('read_page_title', () => document.title);
 * </script>
 * ```
 */
function main(): void {
  ...
  const prefix = dataset.prefix ?? '/agentprism';
  const agent = dataset.agent;
  const apiKey = dataset.apiKey;

  if (!agent || !apiKey) {
    console.error("AgentPrism embed: the script tag needs 'data-agent' and 'data-api-key' attributes.");
