import { AgentPrismError, createAgentPrismClient } from '@agentprism/client';
 ...
 *
 * `apiBase` already carries the application root plus the `MapAgentPrism`
 ...
 */
export const client = createAgentPrismClient({
  baseUrl: apiBase,
  token: getToken,
...
/**
 * Unwraps a successful `@agentprism/client` response.
 *
 * Safe: `client`'s own middleware throws `AgentPrismError` before a call can
...
async function toError(response: Response): Promise<AgentPrismError> {
  ...
  return new AgentPrismError(response.status, title, detail);
}
