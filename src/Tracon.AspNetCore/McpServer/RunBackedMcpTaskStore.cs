...
/// Tracon's existing <see cref="IRunStore"/>: an MCP task id IS a run id.
/// </summary>
/// <remarks>
/// <para>
...
/// real, built <see cref="IServiceProvider"/> in <c>MapTraconMcpServer</c> —
...
/// <see cref="InputResponseReceived"/> is never raised. Tracon's
...
#pragma warning restore CS0067
    ...
    public async Task<McpTaskInfo> CreateTaskAsync(CancellationToken cancellationToken = default)
    {
        var provisioning = McpTaskRunAmbient.Current ?? throw new InvalidOperationException(
            "MCP task creation requires Tracon's task-provisioning filter, which UseMcpServer() " +
    ...
    private static TraconMcpServerOptions CurrentOptions(IServiceProvider services)
        => services.GetRequiredService<IOptionsMonitor<TraconMcpServerOptions>>().CurrentValue;

    private IServiceProvider RequireServices()
        => _services ?? throw new InvalidOperationException(
            "RunBackedMcpTaskStore is not attached to a service provider. Call MapTraconMcpServer() " +
